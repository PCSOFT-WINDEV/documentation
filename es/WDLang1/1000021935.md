


## &lt;Tabla de vista de árbol&gt;.ListChild (Función)



<a name="XUse"></a>
<a name="Use"></a>
<a name="description"></a>
Enumera los "hijos" de un nodo y los elementos "hijo" de estos "hijos" en un control Tabla TreeView. Se llama Procedure de WLanguage siempre que se encuentra un elemento "infantil".


<a name="Example1"></a>
<a name="sample_code"></a>

## Ejemplo


```wl
// List the "children" of "Desserts" node in the "TVT_RecipeTV" TreeView Table control
// The "ExpandAll" procedure is called for each "child" element found in the "Desserts" node
nNbDesserts is int 
nNbDesserts = TVT_RecipeTV.ListChild("Recipes" + TAB + "Desserts", "ExpandAll")
```

<a name="XSYNTAX"></a>

## Sintaxis
<a name="SYNTAX1"></a>

### Enumerar los hijos de una rama identificada por su ruta

`<Result> = <TreeView Table control>.ListItem(<Element name> , <WLanguage procedure> [, <Custom parameter>])`
---

**`<Result>: (Integro)`**

Número de elementos listados.

**`<TreeView Table control>: (Nombre del control)`**

Nombre del control Tabla TreeView a manipular.

**`<Element name>: (Cadena de caracteres)`**

Nombre del elemento a utilizar. Este parámetro tiene el siguiente formato:

```txt
"<Root name>" + TAB + ["<Name of 1st node>" + TAB + ...
["<Name of 2nd node>" + TAB + [...]]]"<Element name>"
```
Se produce un error de WLanguage si este parámetro no corresponde a un elemento existente.

**`<WLanguage procedure>: (Nombre de Procedure)`**

Nombre de WLanguage Procedure que se llamará siempre que se encuentre un elemento "infantil".

**`<Custom parameter>: (Entero opcional)`**

Parámetro pasado a la Procedure &lt;Procedure nombre&gt;.


<a name="SYNTAX2"></a>

### Listado de los hijos de una rama identificada por su índice

`<Result> = <TreeView Table control>.ListItem(<Row number> , <WLanguage procedure> [, <Custom parameter>])`
---

**`<Result>: (Integro)`**

Número de elementos listados.

**`<TreeView Table control>: (Nombre del control)`**

Nombre del control Tabla TreeView a manipular.

**`<Row number>: (Integro)`**

Número de la fila correspondiente a la sucursal a utilizar.

**`<WLanguage procedure>: (Nombre de Procedure)`**

Nombre de WLanguage Procedure que se llamará siempre que se encuentre un elemento "infantil".

**`<Custom parameter>: (Entero opcional)`**

El parámetro pasó a &lt;WLanguage Procedure&gt;.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Observaciones


### Procedimiento WLanguage
<a name="procedimiento_wlanguage_ELTPARAGRAPHE000226"></a>

El &lt;WLanguage Procedure&gt; se llama siempre que se encuentra un elemento "infantil".. Este Procedure es un WLanguage Procedure. 

Esta Procedure debe ser declarada de la siguiente manera:


```txt
PROCEDURE <WLanguage procedure> (<TreeView Table control>, <Child branch>,.. 
	<Element>, <Level>, <Optional parameter>)
```
Los diferentes parámetros son los siguientes: 

- &lt;control Tabla TreeView&gt; (cadena de caracteres): 
	Nombre del control Tabla TreeView a manipular.

- &lt;Rama infantil&gt; (cadena de caracteres): 
	ruta completa de "child" encontrado (sin el nombre) o una cadena vacía ("") si es la raíz de control Tabla TreeView. Este parámetro siempre termina con el separador "TAB".... Este parámetro se inicializará y se llenará siempre que se llame a la Procedure. 

- &lt;Elemento&gt; (cadena de caracteres): 
	Nombre del elemento encontrado. Este parámetro se inicializará y se llenará cada vez que se llame a la Procedure.

- &lt;Nivel&gt; (entero): 
	Nivel del elemento encontrado en relación con el elemento de búsqueda inicial:
	1. Hijo directo
	2. Nieto
	3.  ...

- &lt;Parámetro opcional&gt; (entero): 
	Valor pasado en el parámetro &lt;Parámetro personalizado&gt; de la función **&lt;Tabla de vista de árbol&gt;.ListChild**. Si no se especifica el &lt;Parámetro personalizado&gt;, &lt;Parámetro personalizado&gt; corresponde a la constante *NULL*.




Observación: Esta Procedure puede ser una [procedimiento interno](../Motscles/1514075.md).


### Interrupción de &lt;Tabla de vista de árbol&gt;.ListChild
<a name="interrupcion_docparampagetitleshort_ELTPARAGRAPHE000270"></a>

La función **&lt;Tabla de vista de árbol&gt;.ListChild** no se puede interrumpir si no se han navegado todos los elementos "hijo".

Para forzar la interrupción de la navegación realizada por **&lt;Tabla de vista de árbol&gt;.ListChild**, utilice la siguiente línea de código en &lt;WLanguage Procedure&gt;:


```wl
RESULT False
```
En este caso, la función **&lt;Tabla de vista de árbol&gt;.ListChild** devuelve el número de elementos encontrados hasta que se llame a "RESULT False".

<a name="XComponent"></a>

## Componente:
wd280obj.dll
