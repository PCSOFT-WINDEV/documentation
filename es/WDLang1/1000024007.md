


## &lt;Imagen&gt;.Font (Función)



<a name="XUse"></a>
<a name="Use"></a>
<a name="description"></a>
Declara la fuente que utilizará la función [&lt;Imagen&gt;.DrawText](../WDLang1/1000024017.md) en un control Imagen.



![Linux](https://doc.pcsoft.fr/ext/images/us/LX.png) **Atención**: Se requiere una configuración específica para usar esta función en Linux. Para obtener más información, consulte [Los dibujos](../WDLang1/3029035.md). 
<a name="Example1"></a>
<a name="sample_code"></a>

## Ejemplo


```wl
// Declare the character font:
// Arial, size 12, bold
IMG_Drawing.Font("Arial", 12, iBold)
```

<a name="XSYNTAX"></a>
<a name="SYNTAX1"></a>

## Sintaxis

### Utilizar una fuente específica para los dibujos

`<Image control>.Font(<Font> [, <Font size> [, <Font attributes> [, <Text inclination>]]])`
---

**`<Image control>: (Nombre del control)`**

Nombre del control Imagen a manipular.

**`<Font>: (Cadena de caracteres)`**

Nombre de la fuente utilizada al dibujar el texto con [&lt;Imagen&gt;.DrawText](../WDLang1/1000024017.md). Este parámetro no debe corresponder a una familia de fuentes. La lista de fuentes instaladas en el equipo actual es devuelta por la función [FontList](../WDLang1/3060003.md). 

![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Si no se encuentra el nombre de la fuente, Windows busca una fuente con un nombre similar. Para ello, se debe haber utilizado la función [&lt;Imagen&gt;.StartDrawing](../WDLang1/1000024326.md).

**`<Font size>: (Entero opcional)`**

Tamaño de la fuente (en pixeles). El valor predeterminado es 12.

**`<Font attributes>: (Constante o combinación de constantes opcional)`**

Atributos de fuente a utilizar:


|   |   |
| --- | --- |
| *iBold* | Pone el texto en negrita. |
| *iItalic* | Pone el texto en cursiva. |
| *iNormal*<br>(Valor predeterminado) | Normaliza el texto |
| *iStrikeOut* | Tacha el texto. <br> |
| *iUnderlined* | Subraya el texto. <br> |



**`<Text inclination>: (Entero opcional)`**

Indica la inclinación del texto (en grados). El valor predeterminado es 0.


<a name="SYNTAX2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Código de Usuario (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png) 
### Utilizar una fuente creada mediante programación

`<Image control>.Font(<Font used>)`
---

**`<Image control>: (Nombre del control)`**

Nombre del control Imagen a manipular.

**`<Font used>: (Fuente)`**

Fuente a utilizar para los dibujos. Este parámetro corresponde a una variable de tipo [Font](../Motscles/1514045.md). Las características de la fuente se definieron mediante la función [FontCreate](../WDLang1/3060002.md) o las [propiedades de gestión de fuentes](../Proprietes/2515045.md).



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Observaciones
<a name="NOTE0_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![Código de Usuario (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png) 

### Fuentes
<a name="fuentes_ELTPARAGRAPHE000606"></a>

La función [FontList](../WDLang1/3060003.md) permite obtener la lista de fuentes válidas (TrueType, PostScript) instaladas en el equipo actual.

Para obtener un buen resultado al inclinar el texto, la &lt;Fuente&gt; utilizada debe ser de tipo "True Type".
<a name="NOTE0_3"></a>
<a name="NOTE0_4"></a>
<a name="NOTE0_5"></a>

<a name="XComponent"></a>

## Clasificación Lógica de negocio / UI:
Código neutro
## Componente:
wd280pnt.dll
