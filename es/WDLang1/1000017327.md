


## &lt;Modificar (Función)



<a name="XUse"></a>
<a name="Use"></a>
<a name="description"></a>
Modifica un elemento de una Caja de Listas, ListView o control Combo Box poblada programáticamente.

**Observación**: **&lt;Modificar** se puede utilizar en los controles de cuadro de lista de selección única y selección múltiple.









<a name="Example1"></a>
<a name="sample_code"></a>

## Ejemplo


```wl
// Modify the element found at subscript 4 in "LIST_CustomerList"
LIST_CustomerList.Modify("Smith", 4)
```


<a name="Example2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Código de Usuario (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png) 
```wl
// Modify the image found at subscript 4 in "LSV_Diving"
LSV_Diving.Modify("Coral Reef", "C:\MyImages\CoralReef.JPG", 4)
```

<a name="XSYNTAX"></a>
<a name="SYNTAX1"></a>

## Sintaxis

`<List Box control>.Modify(<Element> [, <Image> [, <Element index>]])`
---

**`<List Box control>: (Nombre del control)`**



**`<Element>: (Cadena de caracteres)`**

Elemento a modificar en el Cuadro de Lista especificado (o Combo Box) control.
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) **Para un control ListView ,** este parámetro corresponde al título Image .

**`<Image>: (Cadena de caracteres opcional)`**

Nombre y ruta de la imagen que se añadirá a un control ListView. Este parámetro se tiene en cuenta sólo para los controles ListView visualizados en modo ListView.. 
Si no se especifica este parámetro, se utilizará la imagen por defecto (definida en la pestaña "Detalles" de la ventana description de control).
![Universal Windows 10 App](https://doc.pcsoft.fr/ext/images/us/UNIVERSALAPP.png) Este parámetro no está disponible.

**`<Element index>: (Entero opcional)`**

Índice del elemento a modificar.

Si este parámetro:

- no se especifica o si es set a -1, el elemento actual se modifica. Si no hay ningún elemento, la función **&lt;Modificar** no tiene ningún efecto.

- es igual a 0, se modifica el primer elemento.

- es mayor que el número de elementos que se encuentran en el Cuadro de Lista (o Combo Box) control, se produce un error de WLanguage. El número de elementos de un control de cuadro de lista (o Combo Box) es devuelto por [&lt;List Caja&gt;.Cuenta](../WDLang1/1000017334.md).






<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Observaciones


### Multiselección control List Box
<a name="multiseleccion_control_list_box_ELTPARAGRAPHE000288"></a>

Si se selecciona uno o más elementos en el control List Box, la función **&lt;Modificar** no deselecciona ningún elemento.
<a name="NOTE0_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![Código de Usuario (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png) 

### Elemento modificado
<a name="elemento_modificado_ELTPARAGRAPHE000302"></a>

Cuando se modifica un elemento que contiene un valor Invisible (especificado por [gStoredValue](../WDLang1/1410088363.md)), se debe reutilizar [gStoredValue](../WDLang1/1410088363.md). De lo contrario, este valor de Invisible se perderá durante la modificación.

Por ejemplo:

- añadiendo "MiElemento": 
	
	```wl
	COMBO_MyCombo.Add("MyElement" + gStoredValue("1"))
	```


- modificando "MiElemento":
	
	```wl
	COMBO_MyCombo.Modify("MyNewElement" + gStoredValue("1"))
	```




<a name="NOTE0_3"></a>
<a name="NOTE0_4"></a>


### Varios
<a name="varios_ELTPARAGRAPHE000371"></a>

- Para modificar un elemento en una control Tabla, utilice [&lt;Tabla&gt;.Modify](../WDLang1/1000024273.md).

- **&lt;Modificar** se puede usar en una columna " Combo Box " tabla .




<a name="XComponent"></a>

## Componente:
wd280obj.dll
