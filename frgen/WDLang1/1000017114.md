...
![Linux](https://doc.pcsoft.fr/ext/images/us/LX.png) Les services sont également appelés daemons.
<a name="NOTE1_2"></a>
...
- Un service ne peut pas avoir d'interaction directe avec l'utilisateur connecté. L'ouverture d'une fenêtre avec [Ouvre](../WDLang1/3038035.md) est possible (par exemple pour créer un traitement graphique sur des images), mais sa fermeture doit être systématique une fois le traitement effectué. De façon générale, il ne doit pas y avoir d'appel de fonction bloquante ([Info](../WDLang1/3021011.md), [Dialogue](../WDLang1/3021015.md), [Saisie](../WDLang1/3021016.md), etc.).

- Un service qui utilise une base de données HFSQL et plus généralement les fonctions H\* pour l'accès à une base de données doit gérer les conflits (doublons, intégrité, modification, etc.) grâce à la fonction [HSurErreur](../WDLang4/3044017.md). Dans le cas contraire, le service pourra être bloqué en cas de conflit.

- ![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png) Un service peut fonctionner sous l'identité du système d'exploitation lui-même.

- ![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png) Un service n'est pas soumis à l'UAC ou à la virtualisation (à partir de Windows Vista).
...
<a name="creation_service_avec_windev_ELTTEXTE000369"></a>
...
<a name="developpement_service_ELTTEXTE000393"></a>


### Programmation d'un service
<a name="programmation_service_ELTPARAGRAPHE000098"></a>Le projet créé dispose de deux événements supplémentaires pour gérer le service :
...
- Pour éditer les événements spécifiques d'un service : sous le volet "Accueil", dans le groupe "Général", déroulez ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=ico_code.gif) et sélectionnez "Code du projet".
...
<a name="generation_service_ELTTEXTE000417"></a>
...
- du service Windows 32 bits ou 64 bits correspond à l'assistant de création de l'exécutable Windows. Pour plus de détails, consultez [Création de l'exécutable](../Editeurs/2025002.md). 

- du service Linux 64 bits correspond à l'assistant de création de l'exécutable Linux. Pour plus de détails, consultez [Création de l'exécutable Linux](../Editeurs/9000114.md). 
...
<a name="creation_installation_service_windows_32_64_bits_ELTTEXTE000441"></a>


### Créer l'installation d'un service Windows 32 ou 64 bits
<a name="creer_installation_service_windows_32_64_bits_ELTPARAGRAPHE000225"></a>Pour créer l'installation d'un service Windows 32 ou 64 bits : 
