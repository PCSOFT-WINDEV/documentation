...
Si aucun de ces paramètres n'est spécifié, une colonne vide est ajoutée dans le champ Table. Pour un champ table liée aux données, les valeurs par défaut définies dans la table liée aux données sont prises en compte.
...
Si aucun de ces paramètres n'est spécifié, une colonne vide est ajoutée dans le champ Table. Pour un champ Table liée aux données, les valeurs par défaut définies dans le fichier lié sont prises en compte.
...
<a name="position_colonne_inseree_ELTPARAGRAPHE000233"></a>
...
<a name="limites_nombre_maximum_colonnes_affichees_ELTPARAGRAPHE000251"></a>
...
<a name="champ_table_liee_aux_donnees_modifications_dans_table_liee_ELTPARAGRAPHE000263"></a>
...
Si la saisie en cascade n'est pas activée, la table liée aux données liée au champ Table liée aux données n'est pas modifiée. Pour écrire l'enregistrement dans la table, exécutez la fonction [&lt;Table&gt;.Enregistre](../WDLang1/1000024249.md) après la fonction **&lt;Table&gt;.InsèreColonne**.
...
### Champ Table liée aux données: Insertion de plusieurs colonnes
<a name="champ_table_liee_aux_donnees_insertion_plusieurs_colonnes_ELTPARAGRAPHE000287"></a>

A chaque appel de la fonction **&lt;Table&gt;.InsèreColonne** dans un champ Table liée aux données, une colonne virtuelle est créée dans le champ Table liée aux données. Les informations seront enregistrées dans la table uniquement lors de l'appel de la fonction [&lt;Table&gt;.Enregistre](../WDLang1/1000024249.md).

Si la fonction **&lt;Table&gt;.InsèreColonne** est appelée successivement dans le même traitement sans être suivie de la fonction [&lt;Table&gt;.Enregistre](../WDLang1/1000024249.md), seul le PREMIER appel est effectif (les appels suivants sont ignorés). Une seule colonne est ajoutée dans le champ Table liée aux données.
<a name="NOTE0_6"></a>


### Insertion d'une image
<a name="insertion_une_image_ELTPARAGRAPHE000313"></a>
...
<a name="champ_table_saisie_ELTPARAGRAPHE000332"></a>
...
[TableInsèreColonne (Fonction)](../WDLang1/1000017178.md)
