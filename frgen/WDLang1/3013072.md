...
Le développement en architecture "3-tiers" est simplifié. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=3tiers.gif)
...
<a name="comment_seffectue_mise_oeuvre_ELTTEXTE000204"></a>
...
**1. Vous choisissez les sources de données (tables ou requêtes que vous désirez manipuler en 3-Tiers).**
Vous créez sous l'éditeur de schémas des données et l'éditeur de requêtes les différentes tables et requêtes nécessaires.
...
Pour cela, sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Génération" et sélectionnez "Générer le diagramme UML correspondant au schéma des données".

Il est également possible, dans l'éditeur d'UML de créer un diagramme de classes correspondant au schéma des données:
...
- Dans l'assistant, sélectionnez "Diagrammes de classes (retro-schéma des données, 3 Tiers, ...)" et passez à l'étape suivante.
...
Il est également possible de modifier le schéma des données, et de synchroniser le diagramme UML avec le schéma des données.
...
- **Pour la couche "Application":**
	Des fonctions WLangage spécifiques peuvent être utilisées: [TableVersMémoire](../WDLang1/3013071.md) et [MémoireVersTable](../WDLang1/3013070.md). Ces fonctions permettent de remplir les membres de la classe à partir des rubriques des tables correspondantes, et inversement.
...
### Fonctions WLangage
<a name="fonctions_wlangage_ELTPARAGRAPHE000128"></a>
...
| [MémoireVersTable](../WDLang1/3013070.md) | Initialise automatiquement la valeur mémoire des rubriques de l'enregistrement en cours d'un fichier avec la valeur des membres d'une structure ou d'une classe. |
...
| [TableVersMémoire](../WDLang1/3013071.md) | Initialise automatiquement les valeurs des membres d'une structure ou d'une classe avec les valeurs des rubriques de l'enregistrement en cours d'un fichier. |
