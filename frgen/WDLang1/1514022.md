


## Pile (Type de variable)

***En anglais : Stack ***
				



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Une variable de type **Pile** est un type structuré qui permet de regrouper une série d'éléments du même type. Les éléments sont ajoutés à la fin de la pile et sont récupérés du plus récent au plus ancien.

Par exemple, si les éléments sont ajoutés dans l'ordre suivant : 1, 2, 3, ils seront récupérés dans l'ordre suivant : 3, 2, 1.




<a name="Exemple1"></a>
<a name="sample_code"></a>

## Exemple


```wl
MaPile est une Pile d'entiers

// On empile les valeurs 1, 2 et 3
Empile(MaPile, 1)
Empile(MaPile, 2)
Empile(MaPile, 3)

// Affichage de la trace : 3, 2, 1
x est un entier
TANTQUE Dépile(MaPile, x)
	Trace(x)
FIN
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe

### Déclarer et initialiser une pile

`<Nom de la pile> est [une] Pile de <Type des éléments de la pile>`
---

**`<Nom de la pile> : ()`**

Nom de la variable Pile à déclarer.

**`<Type des éléments de la pile> : ()`**

Type des éléments constituant la pile. 
Tous les types de variables peuvent être utilisés y compris les tableaux, les tableaux associatifs, les files, les piles et les listes. Par exemple : 

```txt
<variable> est une pile de tableaux d'entiers

<variable> est une pile de tableaux fixes de 5 entiers

<variable> est une pile de tableaux associatifs d'entiers

<variable> est une pile de files d'entiers

<variable> est une pile de piles d'entiers

<variable> est une pile de listes d'entiers
```




<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques


### Propriétés WLangage utilisables avec le type Pile
<a name="proprietes_wlangage_utilisables_avec_type_pile_ELTPARAGRAPHE000071"></a>Les propriétés suivantes peuvent être utilisées pour manipuler une variable de type **Pile**. 

| Nom de la propriété | Effet |
| --- | --- |
| [AjoutTerminé](../Proprietes/1000018388.md) | Renvoie et modifie le mode d'ajout des éléments.<br>Cette propriété est utilisée pour la gestion du multithread.<br><br>![WEBDEV - Code Navigateur](https://doc.pcsoft.fr/ext/images/us/WBN.png) Cette propriété n'est pas disponible. |
| [NbThreadAttente](../Proprietes/1000018387.md) | Renvoie le nombre de threads en attente.<br>Cette propriété est utilisée pour la gestion du multithread.<br><br>![WEBDEV - Code Navigateur](https://doc.pcsoft.fr/ext/images/us/WBN.png) Cette propriété n'est pas disponible. |
| [Occurrence](../Proprietes/2510097.md) | Renvoie le nombre d'occurrences de la pile. |
| [Vide](../Proprietes/2510126.md) | <br><br>- <u><u><u><u>Vrai</u></u></u></u> si la pile est vide,<br><br>- <u><u><u><u>Faux</u></u></u></u> dans le cas contraire.<br><br><br> |




<a name="NOTE0_2"></a>


### Fonctions WLangage de gestion des piles
<a name="fonctions_wlangage_gestion_des_piles_ELTPARAGRAPHE000125"></a>Les fonctions suivantes peuvent être utilisées pour manipuler une variable de type **Pile**. 



|   |   |
| --- | --- |
| [Dépile](../WDLang1/1000017372.md) | Dépile un élément d'une pile. |
| [Désérialise](../WDLang1/3013066.md) | Désérialise un buffer ou une chaîne de caractères contenant les données d'une classe, d'une structure, d'un tableau (y compris un tableau associatif), d'une file, d'une pile, une liste ou une variable d'un type avancé et leurs sous-éléments. |
| [Empile](../WDLang1/1000017366.md) | Empile un élément dans une pile. |
| [PileInfo](../WDLang1/1000019546.md) | Récupère les caractéristiques d'une pile : types des éléments et nombre d'éléments. |
| [Sérialise](../WDLang1/3013065.md) | Transforme les éléments suivants dans un format spécifique :<br><br>- une structure (et ses sous-éléments),<br><br>- une classe (et ses sous-éléments),<br><br>- un tableau (y compris les tableaux associatifs),<br><br>- une file,<br><br>- une pile, <br><br>- une liste. <br><br><br> |
| [SupprimeTout](../WDLang1/1000017357.md) | Supprime tous les éléments : <br><br>- d'un tableau WLangage à une dimension ou deux dimensions. <br><br>- d'un tableau associatif. <br><br>- de la propriété tableau d'un type avancé (tableau des événements de gglAgenda, ...). <br><br>- d'une file WLangage.<br><br>- d'une pile WLangage. <br><br>- d'une liste WLangage.<br><br><br> |




<a name="NOTE0_3"></a>


### Les piles et le multithread
<a name="les_piles_multithread_ELTPARAGRAPHE000133"></a>La gestion du multithread est prise en compte lors de l'ajout et de la suppression d'un élément (fonctions [Empile](../WDLang1/1000017366.md) et [Dépile](../WDLang1/1000017372.md)).

Il est également possible d'utiliser les propriétés lors d'une gestion multithread mais le résultat n'est pas permanent. Par exemple :

```wl
SI MaPile.Occurrence > 0 ALORS
	// Quand le thread arrive ici, la pile peut être vide
FIN
```


![WEBDEV - Code Navigateur](https://doc.pcsoft.fr/ext/images/us/WBN.png) La gestion du multithread n'est pas disponible.
<a name="NOTE0_4"></a>


### Parcours des piles
<a name="parcours_des_piles_ELTPARAGRAPHE000153"></a>Il est possible d'utiliser la syntaxe POUR TOUT pour parcourir les piles. Les éléments sont parcourus dans l'ordre de dépilage qui est également l'ordre inverse de l'ordre d'empilage.

La syntaxe utilisée correspond à celle utilisée pour les tableaux :

```txt
POUR TOUT [ELEMENT] <Variable> [, <Compteur> [, <Compteur>]] DE <Pile>
	...
FIN
```
Les éléments peuvent être modifiés pendant le parcours. En cas de modification de la pile pendant un parcours, les éléments parcourus seront ceux présents lors de l'initialisation du parcours :

- si des éléments sont empilés après l'initialisation du parcours, ils ne seront pas parcourus.

- si des éléments sont dépilés après l'initialisation du parcours, ils seront quand même parcourus.



<a name="NOTE0_5"></a>


### Cas particuliers
<a name="cas_particuliers_ELTPARAGRAPHE000167"></a>

- Sous le débogueur, le contenu de la pile est affiché dans l'ordre de dépilement.

- Une pile peut être utilisée pour typer un paramètre de procédure.

- Une pile peut être copiée avec l'opérateur '='. Il est également possible de copier une instance de classe ou de structure contenant une pile.

- Une pile peut être initialisée par une liste d'éléments dans l'ordre d'empilage.





## Voir aussi

[File (Type de variable)](../WDLang1/1514021.md)

[Liste (Type de variable)](../WDLang1/1514023.md)

[Fonctions de gestion des files, des piles et des listes](../WDLang1/1000018307.md)

[Types avancés](../WDLang1/1514041.md)

