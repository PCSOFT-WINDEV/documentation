
## ChampEnCours (Fonction)

***En anglais : ControlCurrent***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Renvoie le nom du champ en cours de saisie.






<a name="Exemple1"></a>
<a name="sample_code"></a>

## Exemple


```wl
// Identification du champ en cours de saisie
ResChamp = ChampEnCours()
```



<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe

`<Résultat> = ChampEnCours()`
---

**`<Résultat> : (Chaîne de caractères)`**



- Nom du champ en cours de saisie, 

- Chaîne vide ("") si aucun champ n'est en cours de saisie (aucun champ n'a le focus). 


![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) Si le champ en cours de saisie est un champ différent d'un champ de saisie, &lt;Résultat&gt; vaut chaîne vide (""). 

Si le champ en cours de saisie n'appartient pas à la fenêtre qui possède le code, la chaîne renvoyée est de la forme "&lt;Nom de la fenêtre&gt;.&lt;Nom du champ&gt;".
Si le champ correspond à une cellule de table, &lt;Résultat&gt; contient le nom de la colonne.
Si le champ appartient à un superchamp, &lt;Résultat&gt; est de la forme &lt;Nom du superchamp&gt;.&lt;Nom du champ&gt;. 
**Attention** : La casse du nom du champ (majuscules/minuscules) correspond à celle utilisée lors de la création du champ sous l'éditeur.

![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Le cas des champs appartenant aux superchamps n'est pas traité dans cette version.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques
Pour connaître le champ ouvrant un menu contextuel, utilisez la fonction [ChampContextuel](../WDLang1/3025012.md). 



<a name="XComposante"></a>

## Composante:
wd310obj.dll
## Voir aussi

[ChampContextuel (Fonction)](../WDLang1/3025012.md)

[ChampPremier (Fonction)](../WDLang1/3025022.md)

[ChampSuivant (Fonction)](../WDLang1/3025020.md)

[ChampPrécédent (Fonction)](../WDLang1/3025033.md)

[Fonctions de gestion des champs](../WDLang1/3025007.md)

