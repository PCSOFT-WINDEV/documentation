...
- Définir le type utilisé pour la connexion (HFSQL Client/Serveur, OLE DB, ...). Dans ce cas, si la connexion est actuellement ouverte, elle sera fermée, puis ré-ouverte avec les nouveaux paramètres lors d'un premier accès à une table utilisant cette connexion.
...
## Remarques
La propriété [CxProviderOLEDB](../Proprietes/2512033.md) permet de définir et de connaître le type d'accès sur une table manipulée par une connexion. Cette propriété est conservée par compatibilité. Il est conseillé d'utiliser la propriété **Provider**.
