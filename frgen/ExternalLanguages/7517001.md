...
- Si vous migrez une application réalisée avec WINDEV 5.5, consultez [Migration d'une application Visual Basic réalisée avec WINDEV 5.5 vers WINDEV 2026](../LangageExt/7517002.md).
...
## Mise en place
<a name="mise_place_ELTTEXTE000234"></a>




### 1. Inclusion des fichiers de l'interface Visual Basic de WINDEV
<a name="1_inclusion_des_fichiers_interface_visual_basic_windev_ELTPARAGRAPHE000031"></a>

Si vous utilisez une base de données dans votre application, la première étape consiste à générer votre schéma des données. Dans la description de ce schéma des données, précisez le langage de programmation utilisé ainsi que le répertoire qui contiendra les fichiers source.

Grâce à la génération, un squelette de programme (\*.bas) est généré. Cette opération créera de plus la table .gbl (voir plus bas) nécessaire à l'utilisation de vos tables.

Voici une liste des fichiers à inclure dans un projet Visual Basic pour faire appel à l'interface Visual Basic de WINDEV:
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=VisualBasic.gif)


Les fichiers .glb et .bas décrivant la structure des tables seront générés automatiquement par WINDEV lors de la génération du schéma des données.
<a name="NOTE2_2"></a>


### 2. Inclusion des déclarations HFSQL
<a name="2_inclusion_des_declarations_hfsql_ELTPARAGRAPHE000046"></a>

Les fichiers .gbl et .bas correspondant à la description des fichiers du schéma des données doivent être ajoutés au projet Visual Basic. Les déclarations des tables sont incluses dans le fichier d'extension .gbl (ce fichier remplace les .wdr créés avec la version précédente de WINDEV).
...
<a name="declaration_initialisation_ELTTEXTE000264"></a>




### 1. Déclaration du contexte HFSQL et des buffers de travail de chaque fichier
<a name="1_declaration_contexte_hfsql_des_buffers_travail_chaque_fichier_ELTPARAGRAPHE000058"></a>

Si l'application doit gérer des tables, il est nécessaire de déclarer un contexte de travail HFSQL, ainsi qu'un buffer par table.

Ces déclarations sont effectuées par les lignes suivantes dans le projet WD31 Langage Externe.bas:


```txt
' Gestion du contexte des 3 tables HFSQL
...
<a name="2_initialisation_des_dll_windev_ELTPARAGRAPHE000069"></a>
...
<a name="3_initialisation_hfsql_ELTPARAGRAPHE000079"></a>
...
<a name="4_chargement_bibliotheque_windev_wdl_ELTPARAGRAPHE000086"></a>

La bibliothèque WINDEV d'extension .WDL contient tous les éléments du projet (fenêtres, états, classes, requêtes, schéma des données, ...). Il faut donc la charger en mémoire pour faire appel aux composants qu'elle contient.
...
### 5. Ouverture du schéma des données du projet et association des buffers avec les tables
<a name="5_ouverture_schema_des_donnees_projet_association_des_buffers_avec_les_tables_ELTPARAGRAPHE000097"></a>

L'ouverture du schéma des données permet de faire appel aux tables. Une fois ce schéma des données ouvert avec la fonction **HOuvreSchémaDesDonnées**, il ne reste plus qu'à associer les buffers de travail avec les fichiers décrits dans ce schéma des données:


```txt
'ouverture du schéma des données
If Houvreanalyse(gCtx, sNomAnalyse, sMotDePasse, "", "", "") = 0 Then
	MsgBox ("Erreur d'ouverture du schéma des données " + sNomAnalyse)
	Exit Sub
End If
...
<a name="execution_codes_windev_depuis_langage_externe_ELTTEXTE000312"></a>




### 1. Code d'initalisation du projet WINDEV
<a name="1_code_initalisation_projet_windev_ELTPARAGRAPHE000109"></a>
...
<a name="2_appel_code_wlangage_ELTPARAGRAPHE000117"></a>
...
<a name="3_recuperation_des_evenements_declenches_dans_les_fenetres_windev_ELTPARAGRAPHE000135"></a>
...
<a name="mettre_fin_application_ELTTEXTE000348"></a>
