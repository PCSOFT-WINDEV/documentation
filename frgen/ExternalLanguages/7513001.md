...
## Mise en place
<a name="mise_place_ELTTEXTE000253"></a>


### 1. Inclusion des fichiers de l'interface C de WINDEV
<a name="1_inclusion_des_fichiers_interface_windev_ELTPARAGRAPHE000027"></a>
...
Grâce à la génération, un squelette de programme est généré dans le langage externe choisi (par exemple le C). Cette opération créera de plus la table .h (voir plus bas) nécessaire à l'utilisation de vos tables.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=mode2.gif)
...
Ces lignes permettent de récupérer les déclarations minimales nécessaires pour l'utilisation de l'interface externe. Il est à noter que les fichiers .h décrivant la structure des fichiers seront générés automatiquement par WINDEV lors de la génération du schéma des données.
<a name="NOTE2_2"></a>
...
Une inclusion (#include) doit être ajoutée pour chaque table déclarée dans le schéma des données du projet WINDEV. Les déclarations des tables sont incluses dans le fichier d'extension .h (ce fichier remplace les .wdr créés avec la version précédente de WINDEV).
...
<a name="declaration_initialisation_ELTTEXTE000283"></a>


### 1. Déclaration du contexte HFSQL et des buffers de travail de chaque fichier: 
<a name="1_declaration_contexte_hfsql_des_buffers_travail_chaque_fichier_ELTPARAGRAPHE000059"></a>
...
```txt
// Ouverture du schéma des données (WDD contenu dans la WDL)
if (!gclHF.HOuvreAnalyse(szAnalyse,szMotDePasse))
{ 
// Erreur d'ouverture du schéma des données
}
...
<a name="execution_codes_windev_depuis_langage_externe_ELTTEXTE000307"></a>


### 1. Code d'initalisation du projet WINDEV
<a name="1_code_initalisation_projet_windev_ELTPARAGRAPHE000106"></a>

Ce code permet de démarrer le projet WINDEV en déclarant par exemple les variables globales. Son exécution depuis le langage externe est réalisée par la fonction [**nWDInitProjet**](../LangageExt/7513014.md). Par exemple:
...
<a name="2_appel_code_wlangage_ELTPARAGRAPHE000118"></a>
...
<a name="3_recuperation_des_evenements_declenches_dans_les_fenetres_windev_ELTPARAGRAPHE000144"></a>
...
<a name="mettre_fin_application_ELTTEXTE000343"></a>
