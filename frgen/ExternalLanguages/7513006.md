...
Ce paragraphe présente uniquement les différentes manipulations à réaliser pour migrer une application WINDEV 5.5 utilisant le langage C en WINDEV 2026.

Plusieurs cas sont à considérer:

- gestion des objets en langage C

- gestion des tables HFSQL en langage C ou C++
...
<a name="migrer_une_application_windev_55_realisant_une_gestion_des_objets_langage_ELTPARAGRAPHE000038"></a>

Pour migrer une application WINDEV 5.5 réalisant une gestion des objets en langage C: 

1. Migrez votre projet WINDEV 5.5 vers WINDEV 7.5 puis ouvrez votre application avec WINDEV 2026.
...
## Gestion des tables en langage C ou C++
<a name="gestion_des_tables_langage_ELTTEXTE000264"></a>
Pour migrer une application WINDEV 5.5 réalisant une gestion des tables en langage C ou C++:

1. Migrez votre projet WINDEV 5.5 vers WINDEV 7.5 puis ouvrez-le avec WINDEV 2026.

2. Générez le squelette de votre application en version 2026: 

	- ouvrez l'éditeur de schémas des données (bouton ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Ico_Analyse.gif) parmi les boutons d'accès rapide). 

	- sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Génération" et sélectionnez "Génération avancée".
...
6. Copiez les fichiers suivants à partir du sous-répertoire "External Languages\\FR\\C" de WINDEV dans le répertoire de votre application.
	...
	- le fichier &lt;Nom du schéma des données&gt;. h généré par le schéma des données WINDEV (point 2). Ce fichier est présent dans le répertoire du schéma des données du projet WINDEV.

	- le fichier &lt;Nom du schéma des données&gt;.cpp généré par le schéma des données WINDEV (point 2). Ce fichier est présent dans le répertoire du schéma des données du projet WINDEV.
...
8. Dans le source C original:
	...
	- Copiez les déclarations de structure présentes dans le squelette &lt;Schéma des données&gt;.cpp en les renommant. Ces déclarations doivent être copiées juste après la ligne ***#include"&lt;Analyse&gt;.h"***.
	...
	- Si votre code utilise la fonction **HOuvreSchémaDesDonnées**, il est nécessaire de préciser l'extension du fichier du schéma des données (.WDD) dans le premier paramètre de la fonction.
...
- Copiez les lignes de code suivantes présentes dans le squelette &lt;Schéma des données&gt;.cpp en les renommant. Ces lignes doivent être copiées juste après la ligne ***bInitHF***. 
