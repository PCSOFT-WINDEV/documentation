...
- définir et ouvrir une connexion. Il est ensuite nécessaire d'affecter cette connexion à une table avec la fonction [HChangeConnexion](../WDLang4/3044150.md).
...
- définir et ouvrir une connexion en utilisant une chaîne de connexion. Cette syntaxe peut être utilisée avec certains providers OLE DB ayant des chaînes de connexion ADO spécifique.
	![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) Cette option n'est pas disponible.
	![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Accès par JDBC: Cette option n'est pas disponible.
...
## Exemple
<a class="notetitle" target="_blank" href="$DOC$=1000003044107&name=houvreconnexion_fonction">Voir des exemples supplémentaires</a>
...
- soit au nom de la connexion définie sous l'éditeur de schémas des données,
...
Nom de la nouvelle connexion à définir et à ouvrir. Ce nom ne doit pas être déjà utilisé par une autre connexion (connexion définie dans le schéma des données, ou définie avec la fonction **HOuvreConnexion** ou [HDécritConnexion](../WDLang4/3044205.md)).
...
![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/us/HFCS.png) Ce paramètre correspond au nom du serveur. Il est possible de préciser le numéro de port utilisé par le serveur HFSQL. Par défaut, le numéro de port utilisé est 4900. Si le numéro de port du serveur HFSQL est différent, utilisez la notation suivante: 
...
Il est possible d'indiquer une adresse IPv6 en écrivant l'adresse IPv6 entre des crochets. Dans ce cas, le numéro de port doit être indiqué après le crochet fermant. Par exemple: [0154:2145:2155::5554]:4900. ![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Accès par JDBC: 
...
![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/us/AN.png) DB2: Ce paramètre n'est pas nécessaire.
...
![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png) Attention: Pour utiliser une connexion OLE DB il est nécessaire: 
...
|   |   |
| --- | --- |
| *hOLecture* | La base de données est accessible uniquement en lecture. Seule la lecture des enregistrements est possible. Il n'est pas possible d'en ajouter ou d'en supprimer.<br><br>![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/us/HFCS.png) Cette constante est ignorée. |
...
![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/us/AN.png) Les mots-clés reconnus par OLE DB et les Connecteurs Natifs sont présentés dans la page [Informations optionnelles de connexion](../WDLang4/9000124.md). 
...
| CRYPTAGE | Active le cryptage des informations transmises entre le Client et le Serveur. Ce mot-clé peut prendre une de ces valeurs:<br><br>- RAPIDE<br><br>- RC5_16 pour utiliser un cryptage RC5 à 16 boucles.<br><br>- AES128 pour utiliser un cryptage AES 128 bits avec un mode d'opération OFB.<br><br>- AES192 pour utiliser un cryptage AES 192 bits avec un mode d'opération OFB.<br><br>- AES256 pour utiliser un cryptage AES 256 bits avec un mode d'opération OFB.<br><br><br> |


Ce paramètre peut être récupéré et modifié grâce à la propriété [InfosEtendues](../Proprietes/2512041.md).![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Base de données HFSQL accédée à l'aide d'un serveur RMI: Ce paramètre correspond à l'adresse du serveur RMI et au port utilisé. Le format à utiliser est le suivant: "RMI=&lt;Adresse du serveur&gt;:&lt;Numéro du port&gt;". Par exemple: "RMI=192.168.52.148:1099".![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Accès par JDBC: Ce paramètre est obligatoire. Il doit correspondre au nom complet du driver JDBC à utiliser. Ce nom est sensible à la casse.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) Ce paramètre n'est pas disponible.
...
Nom de la nouvelle connexion à définir et à ouvrir. Ce nom ne doit pas être déjà utilisé par une connexion (connexion définie dans le schéma des données, ou définie avec la fonction **HOuvreConnexion** ou [HDécritConnexion](../WDLang4/3044205.md)).
...
<a name="syntaxe_1_ouvrir_une_connexion_predefinie_gestion_automatique_des_connexions_ELTPARAGRAPHE000730"></a>
...
- Si une connexion est ouverte avec la fonction **HOuvreConnexion**, cette connexion reste ouverte même si aucune table ne l'utilise.

- Si l'application effectue de nombreuses ouvertures et fermetures de tables successives, la gestion automatique des connexions va provoquer de nombreuses ouvertures/fermetures des connexions, entraînant d'importants ralentissements. Il est dans ce cas conseillé d'utiliser la fonction **HOuvreConnexion** pour forcer la conservation de la connexion ouverte.
...
<a name="syntaxe_2_utiliser_une_chaine_caracteres_pour_provider_ole_ELTPARAGRAPHE000747"></a>
...
1. Dans l'éditeur de schémas des données, créez une connexion par exemple sur HFSQL Client/Serveur. 
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=HouvreConnexion%20-%20HC%20N%B0001.gif)

<a name="NOTE0_3"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/us/ER.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/us/AN.png) 

### Syntaxe 2 et 3: En cas d'échec, la description de la connexion n'est pas conservée
<a name="syntaxe_2_3_cas_echec_description_connexion_nest_pas_conservee_ELTPARAGRAPHE000764"></a>
...
<a name="pseudoconnexion_une_base_donnees_hfsql_classic_ELTPARAGRAPHE000777"></a>

Des tables OLE DB ou natives sont décrites sous l'Éditeur de schémas des données. Pour manipuler ces descriptions avec une base de données HFSQL Classic, il suffit de spécifier la constante ***hAccèsHF7*** comme nom de provider.

Les tables au format HFSQL Classic correspondant à la description des tables OLE DB ou natifs seront créées si nécessaire dans le répertoire défini par le paramètre &lt;Source des données&gt;.
<a name="NOTE0_5"></a>


### Différences entre les fonctions HOuvreConnexion et HDécritConnexion
<a name="differences_entre_les_fonctions_docparampagetitleshort_hdecritconnexion_ELTPARAGRAPHE000786"></a>
...
Cette solution peut être très utile lorsqu'une application manipule à la fois des tables HFSQL et des tables Oracle. Par exemple, si l'application utilise une seule table Oracle, dans un traitement bien spécifique, la connexion à la base de données Oracle ne sera faite qu'en cas de besoin.
...
<a name="fichier_udl_fichier_description_connexion_ole_ELTPARAGRAPHE000824"></a>
...
<a name="connecteur_natif_oracle_gestion_authentification_externe_ELTPARAGRAPHE000843"></a>
...
### Limitations
<a name="limitations_ELTPARAGRAPHE000857"></a>

- ![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Widget Android](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) Seule la constante *hAccèsNatifSQLite* est disponible pour Android. 
