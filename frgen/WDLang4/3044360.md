...
En effet, lorsque la façon d'accéder aux données évolue (suite à un changement de structure des tables par exemple), il suffit de modifier la procédure stockée sur le serveur sans effectuer de modification dans les applications déployées qui utilisent cette procédure.
...
Les procédures stockées peuvent utiliser des requêtes SQL ou des requêtes créées avec l'éditeur de requêtes. **La création des procédures stockées et des requêtes utilisées par les procédures stockées est effectuée dans l'éditeur de schémas des données**. 
...
### La création de procédures stockées est effectuée dans le schéma des données.
<a name="creation_procedures_stockees_est_effectuee_dans_schema_des_donnees_ELTPARAGRAPHE000039"></a>

Pour créer une procédure stockée:

1. Créez si nécessaire la collection de procédures stockées dans le schéma des données:

	- soit cliquez sur ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=ico_nouveau.gif) parmi les boutons d'accès rapide. 
...
- soit par le volet "Schéma des données": sélectionnez le dossier "Procédures stockées". Dans le menu contextuel, sélectionnez l'option "Nouvelle collection de procédures". La collection de procédures stockées est automatiquement créée. Par défaut, cette collection de procédures a pour nom "Collection_&lt;Nom schéma des données&gt;".

- soit dans le ruban de l'éditeur de schémas des données:  sous le volet "Schéma des données", dans le groupe "Création", déroulez "Nouveau" et sélectionnez "Collection de procédures stockées". La collection de procédures stockées est automatiquement créée. Par défaut, cette collection de procédures a pour nom "Collection_&lt;Nom schéma des données&gt;".

2. Cette collection est enregistrée physiquement dans le sous-répertoire "Code" du répertoire du schéma des données.
...
<a name="importer_une_collection_procedures_existante_ELTPARAGRAPHE000071"></a>
...
1. Ouvrez si nécessaire l'éditeur de schémas des données. 

2. Dans le volet "Schéma des données", sélectionnez le dossier "Procédures stockées".
...
4. Les opérations effectuées sont les suivantes:
	...
	- ***Si la collection de procédures appartient au projet en cours***: la collection de procédures du projet est automatiquement recopiée dans le répertoire "Code" du schéma des données. Le nom logique de cette procédure est automatiquement renommé. 
			Attention: Les deux collections de procédures sont dissociées: une modification effectuée dans la collection de procédures du projet ne sera pas reportée dans la collection de procédures du schéma des données et inversement.

	- ***Si la collection de procédures n'appartient pas au projet en cours***: la collection de procédures est automatiquement recopiée dans le répertoire "Code" du schéma des données, et définie comme procédure stockée.
...
<a name="utiliser_une_requete_dans_une_procedure_stockee_ELTPARAGRAPHE000090"></a>

Vous pouvez utiliser une requête créée avec l'éditeur de requêtes dans une procédure stockée. Pour cela, la création de la requête doit avoir été effectuée depuis l'éditeur de schémas des données.

Pour créer une requête dans l'éditeur de schémas des données:
...
- Méthode 1: 
	...
	1. Cliquez sur ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=ico_nouveau.gif) parmi les boutons d'accès rapide.
		...
		- L'assistant de création d'une requête est automatiquement lancé. La requête est automatiquement créée. La requête est enregistrée physiquement dans le sous-répertoire "Code" du répertoire du schéma des données.




2. La requête peut être utilisée dans une des procédures stockées du schéma des données.

- Méthode 2: 

	1. Affichez l'éditeur de schémas des données.

	2. Affichez le volet "Schéma des données" et sélectionnez l'option "Nouvelle requête" du menu contextuel. L'assistant de création d'une requête est automatiquement lancé. La requête est automatiquement créée. La requête est enregistrée physiquement dans le sous-répertoire "Code" du répertoire du schéma des données.

	3. La requête peut être utilisée dans une des procédures stockées du schéma des données.
...
2. Dans le volet "Schéma des données", sélectionnez le dossier "Requêtes".
...
4. Les opérations effectuées sont les suivantes:
	...
	- ***Si la requête appartient au projet en cours***: la requête du projet est automatiquement recopiée dans le répertoire "Code" du schéma des données, et relocalisée: une seule requête est utilisée par le projet et le schéma des données. Ainsi une modification effectuée dans la requête du projet sera automatiquement reportée dans la requête du schéma des données et inversement.

	- ***Si la requête n'appartient pas au projet en cours***: la requête est automatiquement recopiée dans le répertoire "Code" du schéma des données, et définie comme requête liée au schéma des données.
...
<a name="code_wlangage_utilisable_dans_les_procedures_stockees_ELTPARAGRAPHE000124"></a>
...
- Pour savoir si une fonction est utilisable ou non dans une procédure stockée, il suffit de consulter l'aide de cette fonction: si l'image ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=proc_stock.gif) est affichée dans la barre de titre de la fonction, la fonction est utilisable dans le code des procédures stockées.
...
- Pour manipuler des tables, il faut utiliser la fonction [HDéclareExterne](../WDLang4/3044204.md). 
	Par exemple: 
	...
	SI PAS HTableExiste(CODEPOSTAUX) ALORS
		// Fichier non connu, il faut le déclarer
		// Attention, pour déclarer une nouvelle table dans une procédure stockée HFSQL, 
		// il faut:
		...
		// La table sera recherchée dans la base en cours. 
		// Il est possible d'indiquer un sous-répertoire de la base de données. 
		// - donner le nom complet de la table, extension comprise (.FIC)
...
<a name="procedures_stockees_compilables_64_bits_ELTPARAGRAPHE000186"></a>

Pour utiliser un exécutable 32 bits avec des procédures stockées s'exécutant sur un serveur 64 bits: 

1. Affichez la fenêtre de description du schéma des données. 

	- Ouvrez si nécessaire l'éditeur de schémas des données. 

	- Affichez le menu contextuel du schéma des données (clic droit sur le graphe du schéma des données) et sélectionnez l'option "Description du schéma des données". 
...
<a name="tester_une_procedure_stockee_ELTPARAGRAPHE000201"></a>

Pour tester une procédure stockée:

1. Sélectionnez la procédure stockée à tester dans le volet du schéma des données.
...
### Limite
<a name="limite_ELTPARAGRAPHE000247"></a>
...
### Présentation
<a name="presentation_ELTPARAGRAPHE000256"></a>
...
- **pour test**: les procédures stockées et/ou les requêtes sont déployées sur le serveur choisi, mais le schéma des données n'est pas mis à jour. Cette manipulation est conseillée par exemple si le schéma des données est très important et demande une génération relativement longue. 
...
- **réellement**: le schéma des données doit être re-généré et la modification automatique des tables doit être effectuée lors de l'installation de la version cliente.
...
### Déployer pour test
<a name="deployer_pour_test_ELTPARAGRAPHE000268"></a>

Pour tester rapidement les modifications effectuées dans une procédure stockée:

1. Dans l'éditeur de schémas des données, sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Synchronisation" et sélectionnez "Mettre à jour les éléments stockés (HFSQL Client/Serveur)".
...
2. Dans la fenêtre qui s'affiche, sélectionnez:
	...
	- la connexion concernée par la mise à jour. Il est également possible de saisir directement les caractéristiques de la connexion. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=proc_stock_Test.gif&type=thumb)
...
**Remarque**: Pour déployer pour tests, vous pouvez également générer le schéma des données et accepter la modification automatique des données.
<a name="NOTE3_3"></a>


### Déployer en réel
<a name="deployer_reel_ELTPARAGRAPHE000299"></a>
...
1. Générer le schéma des données contenant les procédures stockées (option "Génération du schéma des données" du menu contextuel du schéma des données). Une modification des tables peut être effectuée lors de cette étape.

2. Créer la version Client en demandant une modification automatique des tables: sous le volet "Projet", dans le groupe "Génération", déroulez "Procédure d'installation" et sélectionnez "Créer la procédure d'installation". 
	Remarque: il suffit de modifier le code des procédures stockées ou les requêtes du schéma des données pour qu'une modification automatique des tables soit proposée.
...
<a name="mise_jour_chaud_des_procedures_stockees_sur_serveur_hfsql_ELTPARAGRAPHE000346"></a>
...
<a name="execution_une_procedure_stockee_depuis_une_requete_sql_ELTPARAGRAPHE000393"></a>
...
<a name="mecanisme_des_souserreurs_ELTPARAGRAPHE000419"></a>
...
<a name="codes_erreur_pouvant_etre_rencontres_dans_les_procedures_stockees_ELTPARAGRAPHE000467"></a>
...
Pour que cette collection de procédures puisse fonctionner, il est nécessaire de sélectionner l'option "Activer les erreurs de compilation pour la compatibilité 64 bits" (option "Description du schéma des données" du menu contextuel du schéma des données, onglet "Compilation").
