...
`<Résultat> = HRécupèreEnregistrement([<Table> [, <Séparateur Rubrique> [, <Séparateur Tableau> [, <Format>]]]])`
---
...
Nom de la table (vue ou requête) manipulée. Si ce nom n'est pas spécifié, la fonction **HRécupèreEnregistrement** va manipuler la dernière table utilisée par la dernière fonction de gestion HFSQL (fonction commençant par la lettre H).
