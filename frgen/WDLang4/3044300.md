


## HRplProcédureFiltre (Fonction)

***En anglais : HRplFilterProcedure***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Définit la procédure WLangage qui sera appelée à chaque opération de réplication pour une table spécifique. Cette procédure est exécutée par les fonctions [HCréeRéplicaTransportable](../WDLang4/3044209.md) et [HSynchroniseRéplica](../WDLang4/3044014.md).

**Remarque: Cette fonction est utilisable uniquement lors de la réplication universelle.**


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Exemple


```wl
// La procédure valide les messages. 
// MessageValidation sera appelée pour chaque message répliqué
HRplProcédureFiltre(Messages, "MessageValidation")
// Préparation envoi des messages pour Paris (à MPL)
HCréeRéplicaTransportable("M:\BAL\BAL.RPM", "Paris", "c:\temp\MsgMPL.zip")
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe

`<Résultat> = HRplProcédureFiltre(<Nom du fichier> , <Nom de la procédure>)`
---

**`<Résultat> : (Booléen)`**



- <u><u><u><u>Vrai</u></u></u></u> si l'opération a été réalisée, 

- <u><u><u><u>Faux</u></u></u></u> en cas de problème (par exemple si les tables et les rubriques spécifiées n'existent pas). La fonction [HErreur](../WDLang4/3044088.md) permet d'identifier l'erreur. 
	Attention: &lt;Résultat&gt; vaut <u><u><u><u>Vrai</u></u></u></u> même si les fichiers/tables indiqués ne sont pas répliqués.




**`<Nom du fichier> : (Chaîne de caractères)`**

Nom de la table ou de la table sur lequel la procédure doit être exécutée.

**`<Nom de la procédure> : (Chaîne de caractères correspondant à une procédure WLangage)`**

Nom d'une procédure WLangage existant dans l'application WINDEV ou dans le site WEBDEV. Vous devez créer cette procédure dans votre application. Cette procédure n'attend aucun paramètre. Cette procédure est appelée avant chaque opération réalisée sur le fichier destination. Plusieurs variables sont utilisables dans cette procédure. Si cette procédure renvoie <u><u><u><u>Faux</u></u></u></u>, l'opération n'est pas effectuée. Cette procédure permet par exemple:

- de filtrer les enregistrements à ne pas répliquer.

- de gérer les cas de conflits, en réglant les valeurs avant qu'une erreur de doublons n'apparaisse.  






<a name="NOTE0"></a>
<a name="XComposante"></a>

## Composante:
wd310rpl.dll
## Voir aussi

[Fonctions de gestion HFSQL](../WDLang4/3044156.md)

[HRplDéclareLiaison (Fonction)](../WDLang4/3044299.md)

[Réplication universelle](../WDLang4/9000032.md)

[Fonctions de gestion de la réplication](../WDLang4/1000017222.md)

