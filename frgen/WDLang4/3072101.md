...
1. Création du projet et du schéma des données: importation de la structure des tables.
...
## Projet et schéma des données
<a name="projet_schema_des_donnees_ELTTEXTE000298"></a>


### Import des DDS (Reverse engineering)
<a name="import_des_dds_reverse_engineering_ELTPARAGRAPHE000029"></a>
...
WINDEV récupère automatiquement la structure des tables et crée le référentiel des données utiles au développement. Les champs AS/400 sont convertis dans le format Windows. Les données restent bien entendu au format AS/400.

Chacun des fichiers logiques devient une clé pour WINDEV (clé unique ou clé multiple). WINDEV déduit les relations entre les tables à partir de la définition des clés. Les contraintes d'intégrité définies sur l'AS/400 sont récupérées.

Pour importer les DDS: 

1. Sous le volet "Schéma des données", dans le groupe "Création", déroulez "Importer" et sélectionnez "Importer des descriptions de fichiers/tables". L'assistant se lance. Une connexion AS/400 est automatiquement créée et associée aux fichiers importés dans le schéma des données. Cette connexion devra être utilisée pour manipuler la table AS/400 par programmation.
...
**Remarque**: Lors de l'importation des fichiers AS/400 dans le schéma des données, des informations étendues sont automatiquement récupérées.
<a name="NOTE2_2"></a>


### Export des DDS
<a name="export_des_dds_ELTPARAGRAPHE000066"></a>

Si vous souhaitez créer de nouvelles tables sur l'AS/400:

1. Définissez les nouvelles tables sous l'éditeur de schémas des données.

2. Exportez les DDS sur l'AS/400: sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Synchronisation" et sélectionnez "Synchroniser un AS400 - iSeries (DDS et données)".

3. Dans l'assistant qui se lance, sélectionnez les structures de tables à exporter. Pour chacune des tables sélectionnées, les DDS sont créés et compilés sur l'AS/400 dans la bibliothèque de votre choix (\*LIBL). Les champs PC sont convertis dans le format AS/400.
...
Après l'import des tables dans le schéma des données, l'application WINDEV ou WEBDEV peut être réalisée. Le RAD (Rapid Application Development) peut également être utilisé pour créer en quelques minutes une application complète, directement utilisable.
...
<a name="programmation_acces_aux_donnees_ELTPARAGRAPHE000106"></a>

**1. Ouverture de la connexion**

- Pour ouvrir la connexion à la base de données AS/400, utilisez la fonction [HOuvreConnexion](../WDLang4/3044107.md), en précisant le nom de la connexion à ouvrir. Ce nom a été défini lors de l'importation de la structure des tables dans le schéma des données.
...
**Remarque**: Si vous tentez de lire directement la table (fonction [HLitPremier](../WDLang4/3044051.md), ...), la connexion associée à la description de la table dans le schéma des données sera automatiquement ouverte.
...
Le parcours du résultat est réalisé avec toutes les fonctions de lecture classiques: [HLitPremier](../WDLang4/3044051.md), [HLitSuivant](../WDLang4/3044037.md), [HLitRecherche](../WDLang4/3044050.md), etc.
...
<a name="programmation_specifique_ELTPARAGRAPHE000165"></a>
...
<a name="creation_executable_ELTPARAGRAPHE000206"></a>
...
<a name="deploiement_application_ELTPARAGRAPHE000225"></a>
...
[Connecteur Natif AS/400](../WDLang4/3072100.md)
