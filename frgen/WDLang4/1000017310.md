...
<a name="preambule_contexte_fiable_securise_pour_machine_ELTTEXTE000413"></a>
...
## Sécurité HFSQL
<a name="securite_hfsql_ELTTEXTE000437"></a>


### Installation du serveur HFSQL
<a name="installation_serveur_hfsql_ELTPARAGRAPHE000077"></a>
...
### Sauvegardes
<a name="sauvegardes_ELTPARAGRAPHE000108"></a>
...
<a name="utilisateurs_droits_ELTPARAGRAPHE000130"></a>
...
### Procédures stockées
<a name="procedures_stockees_ELTPARAGRAPHE000144"></a>
...
### Cryptage
<a name="cryptage_ELTPARAGRAPHE000155"></a>
...
- **Cryptage des tables**: 
	En plus du compte HFSQL qui requiert un utilisateur et un mot de passe pour se connecter, chaque table peut également être protégée par un cryptage et un mot de passe. L'algorithme de cryptage (128 bits, RC5 12 boucles, RC5 16 boucles, etc.) est défini dans le schéma des données WINDEV/WEBDEV par le développeur. Le mot de passe peut être défini par le développeur ou par une option d'une application cliente afin d'être défini par un administrateur de la base. 
	Un cryptage avec mot de passe fort est conseillé pour les tables contenant des données sensibles. Lors de la définition du mot cryptage dans le schéma des données, il faut le définir non seulement sur la table (.fic) mais aussi sur l'index (.ndx, .ftx) et sur les mémos. 
	**Pour une sécurité maximale du cryptage, activez l'option "Activer la sécurité renforcée "**. Cette option permet de crypter les données sans stocker la clé de cryptage dans la table. Avec cette option et sans le mot de passe de la table, il est impossible de récupérer le contenu de la table.
	Ces options sont définies sous l'éditeur de schémas des données, dans l'onglet "Détail" de la fenêtre de description des tables. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0001.gif)


- **Cryptage applicatif**: 
	Pour les rubriques contenant des données d'une très haute sensibilité (numéro de carte bancaire par exemple), il est conseillé d'ajouter un cryptage/décryptage applicatif. Un cryptage applicatif rend les données illisibles sans l'applicatif correspondant, même en ayant un compte utilisateur de la base et le mot de passe de la table.
...
<a name="tables_plus_2_ELTPARAGRAPHE000185"></a>

Si certaines tables risquent d'avoir une taille qui va dépasser 2 Go, le serveur HFSQL doit être installé sur un système d'exploitation et un système de fichiers qui permettent de gérer ce type de table (comme NTFS). 

De plus, l'option de gestion des tables de plus de 2 Go doit être activée dans le schéma des données WINDEV/WEBDEV correspondant aux bases de données (onglet "Compatibilité" de la fenêtre de description des tables): <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0002.gif&type=thumb)


Dans le cas d'une table journalée, le journal prend la même option que la table associée. Il faut donc activer cette option sur une table même si seul son journal risque de dépasser 2 Go.
...
3. Cliquez sur l'option "Système": <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0011%201.gif&type=thumb)
...
### Tracabilité
<a name="tracabilite_ELTPARAGRAPHE000210"></a>

Afin de pouvoir vérifier les opérations qui ont été réalisées sur la base de données, il faut mettre en place un mécanisme de traçabilité sur les tables sensibles. 

Cette opération peut être effectuée en activant la **journalisation sur ces tables**. La journalisation peut-être définie au niveau de la rubrique : il est donc possible de ne pas inclure certaines rubriques dans le journal pour des raisons de performances (taille du journal) ou de sécurité (éviter la duplication de données très sensibles).

Pour activer le journalisation sur les tables:

1. Sous l'éditeur de schémas des données, affichez la fenêtre de description des tables (option "Description de la table" du menu contextuel d'une table). 

2. Dans l'onglet "Journalisation", spécifiez le type de journalisation à réaliser: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0003.gif)
...
Pour activer cette option dans le Centre de Contrôle HFSQL, sélectionnez l'onglet "Configuration" de la description du serveur: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0009.gif&type=thumb)
...
<a name="securisation_des_applications_ELTPARAGRAPHE000260"></a>
...
<a name="rgpd_reglement_general_sur_protection_des_donnees_ELTPARAGRAPHE000280"></a>
...
<a name="impact_securite_sur_les_performances_ELTTEXTE000521"></a>
...
<a name="configuration_poste_ELTPARAGRAPHE000309"></a>
...
[RGPD: un audit de vos données](../WDLang4/1410086744.md)

[Sécurisé (Propriété)](../WDLang4/2512082.md)
