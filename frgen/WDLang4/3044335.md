...
- [Principe : gestion des transactions sur des tables HFSQL et cas particuliers](#NOTE3_1).
...
<a name="questce_quune_transaction_ELTTEXTE000214"></a>
Une transaction est un ensemble d'opérations indissociables : soit toutes les opérations de la transaction sont réalisées, soit aucune opération n'est réalisée. La gestion des transactions est le meilleur moyen d'assurer la cohérence d'un ensemble d'écritures indissociables sur des tables HFSQL.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=transaction.gif)

<a name="NOTE2_2"></a>


### Utiliser les transactions selon ses besoins
<a name="utiliser_les_transactions_selon_ses_besoins_ELTPARAGRAPHE000052"></a>
...
- **Réaliser des écritures fiables dans des tables (domaine bancaire, comptable, ...).**
	Les transactions permettent d'assurer la sécurité d'un ensemble d'opérations. Dans ce cas, il est nécessaire de regrouper toutes les opérations d'écriture dans votre code afin de les exécuter dans une transaction. Le code exécuté pendant la transaction ne doit pas afficher de fenêtres, réaliser de parcours de tables, ... Ce code doit uniquement comprendre les opérations de modification des tables. 
	Chaque opération réalisée pendant la transaction est enregistrée dans un fichier de transaction. L'enregistrement manipulé est bloqué en écriture jusqu'à la validation ou l'annulation de la transaction.
...
## Principe
<a name="principe_ELTTEXTE000244"></a>


### Principe des transactions sur des tables HFSQL
<a name="principe_des_transactions_sur_des_tables_hfsql_ELTPARAGRAPHE000072"></a>
...
- ![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) Les transactions ne sont pas disponibles sur des tables Hyper File 5.5.
...
- **Panne de courant ou arrêt brusque de l'application** (arrêt de l'application par le gestionnaire de tâches par exemple) : 
	...
	- ![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png) Par l'outil [WDTRANS](../WDTrans/3524005.md) (ou [WDOptimizer](../WDOptimiseur/3519002.md)). 

	- ![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/us/HFCS.png) Par le Centre de Contrôle HFSQL.
...
![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/us/HFCS.png) Cas particuliers spécifiques à HFSQL Client/Serveur :

- Si le Client est perdu, la transaction est annulée.

- Le redémarrage du serveur annule les transactions en cours. Attention : la transaction n'est pas annulée si la transaction porte sur une table avec mot de passe sécurisé. La transaction sera annulée à la prochaine ouverture de la table.
...
[Transactions Client/Serveur: Modes d'isolation disponibles](../WDLang4/1000017316.md)

[Transactions: Manipuler les transactions par programmation](../WDLang4/3044336.md)
