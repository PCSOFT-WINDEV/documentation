...
<a name="comment_indexer_partiellement_les_memos_texte_ELTTEXTE000194"></a>
L'indexation des mémos texte peut être réalisée:

- directement sous l'éditeur de schémas des données (pour les tables décrites dans le schéma des données)

- en WLangage pour les tables créées par programmation.
...
### Mise en place de l'indexation des mémos texte sous l'éditeur de schémas des données
<a name="mise_place_indexation_des_memos_texte_sous_editeur_schemas_des_donnees_ELTPARAGRAPHE000046"></a>

Pour mettre en place l'indexation des mémos texte:

1. Sous l'éditeur de schémas des données, affichez la description des rubriques d'une table contenant un mémo texte.

2. Sélectionnez la rubrique associée à un mémo texte.

3. Dans la partie droite de l'écran, il est possible de spécifier si le mémo doit être une clé ou non: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Indexation%20des%20m%E9mos%20-%20HC%20N%B0001.gif)
...
6. Générez le schéma des données. Si vous avez déjà utilisé des tables, une modification automatique des tables est réalisée.
...
<a name="mise_place_indexation_des_memos_texte_par_programmation_ELTPARAGRAPHE000066"></a>

Pour mettre en place l'indexation des mémos texte lors de la description d'une table par programmation, il suffit d'utiliser propriétés WLangage disponibles sur les rubriques pour indiquer les caractéristiques de la clé de type mémo texte.
