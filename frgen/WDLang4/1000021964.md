...
`<Résultat> = HExporteJSON(<Table> [, <Fichier JSON> [, <Rubriques à exporter>]] [, <Options> [, <Jauge>]])`
---
...
Nom de la table, de la vue ou de la requête manipulée. Si ce nom n'est pas spécifié, la fonction **HExporteJSON** va manipuler la dernière table utilisée par la dernière fonction de gestion HFSQL (fonction commençant par la lettre H).
...
Si ce paramètre correspond à une chaîne vide (""), toutes les rubriques de la table (sauf rubriques de type clé composée, binaire, mémo binaire et mot de passe) sont exportées.
...
<a name="exporter_contenu_une_requete_ELTPARAGRAPHE000120"></a>

Pour exporter le contenu d'une requête, il est nécessaire d'initialiser la requête (fonction [HExécuteRequête](../WDLang4/3044080.md)) avant d'utiliser la fonction **HExporteJSON**.
<a name="NOTE0_2"></a>


### Exporter un seul enregistrement
<a name="exporter_seul_enregistrement_ELTPARAGRAPHE000134"></a>

Pour exporter un seul enregistrement vers une chaîne JSON, utilisez la fonction [HEnregistrementVersJSON](../WDLang4/1000021909.md).
<a name="NOTE0_3"></a>


### Rubriques tableau
<a name="rubriques_tableau_ELTPARAGRAPHE000146"></a>Si une des rubriques exportées est une rubrique de type Tableau, chaque élément de la rubrique tableau est exportée dans un nouveau niveau:
...
[HExporteChaîneJSON (Fonction)](../WDLang4/1410088487.md)
