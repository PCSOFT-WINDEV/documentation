...
Le provider est disponible en lecture et en écriture. Une application écrite dans un langage tiers peut lire et écrire dans des tables HFSQL.
...
## Installation
<a name="installation_ELTTEXTE000394"></a>
...
Par défaut, le provider OLE DB est installé dans le répertoire "C:\\Program Files\\Fichiers communs\\PC SOFT\\31.0\\OLEDB".
...
| Data Source | Nom (ou adresse IP) du serveur HFSQL et port de connexion.<br>Le port peut ne pas être spécifié si le serveur utilise le port par défaut (4900).<br>Le nom et le numéro de port doivent être séparés par ":" (deux points).<br><br>Exemple: monserveur.masociete.fr:4910 | Chemin complet du schéma des données (fichier .WDD) de la base.<br>Ce paramètre est optionnel. |
| Initial Catalog | Nom de la base de données. | Chemin complet du répertoire contenant les tables. |
| User ID | Nom d'utilisateur pour se connecter au serveur HFSQL. | Ce paramètre est ignoré. |
| Password | Mot de passe pour se connecter au serveur HFSQL. | Mot de passe du schéma des données (fichier .WDD) de la base.<br>Ce paramètre est optionnel. |
...
| --- | --- |
| **Propriété** | **Valeur** |
| Password | Mot de passe des tables HFSQL sous la forme:<br><br><pre><code><NomFichier>:<MotDePasse></code></pre><br>**Remarques**:<br><br>- &lt;NomFichier&gt; représente le nom logique de la table.<br><br>- &lt;NomFichier&gt; peut être remplacé par le caractère "\*" (étoile) pour indiquer que le mot de passe s'applique à toutes les tables.<br><br>- Cette propriété peut être répétée pour indiquer les mots de passe de plusieurs tables.<br>	Exemple: Password=MonFichier1:MotDePasse1;Password=MonFichier2:MotDePasse2<br><br><br> |
...
### Utilisation du schéma des données dans une connexion à une base de données HFSQL Classic
<a name="utilisation_schema_des_donnees_dans_une_connexion_une_base_donnees_hfsql_classic_ELTPARAGRAPHE000266"></a>
...
- Si le chemin du fichier WDD est indiqué dans le paramètre "Data Source" de la chaîne de connexion:
	...
	- Seuls les fichiers décrits dans ce schéma des données seront pris en compte par la connexion.

	- Les liaisons et les règles d'intégrité décrites dans le schéma des données sont prises en compte automatiquement.
...
### Remarques
<a name="remarques_ELTPARAGRAPHE000282"></a>

- Lors d'un accès par le provider OLE DB pour HFSQL, les fenêtres d'assistance automatique de HFSQL sont désactivées.

- Les rubriques de type "Durée" dans les tables HFSQL sont retournées sous forme d'entiers sur 8 octets par le provider OLE DB. L'unité est la milliseconde.
...
<a name="exemples_chaines_connexion_ole_ELTPARAGRAPHE000299"></a>

- Connexion à une base HFSQL Classic sans préciser le schéma des données:
...
- Connexion à une base HFSQL Classic en précisant le schéma des données:
...
- Connexion à une base HFSQL Client/Serveur avec des fichiers protégés par des mots de passe et utilisant l'alphabet russe:
	...
	**Remarque**: Dans cet exemple, toutes les tables de la base de données sont protégées par le mot de passe "secret1" sauf la table "Table2" qui est protégée par le mot de passe "secret2".
...
<a name="arborescence_des_donnees_ELTPARAGRAPHE000319"></a>

L'emplacement désigné dans la chaîne de connexion par l'élément **Initial Catalog** peut contenir des tables placées dans des sous-dossiers. Ces tables sont automatiquement déclarées par le provider OLE DB pour HFSQL, en remplaçant les "\\" de l'arborescence par le caractère "_" (underscore).
Par exemple, s'il existe une table "&lt;Initial Catalog&gt;\\Archive\\Produit.fic", elle pourra être interrogée en utilisant "Archive_Produit" comme nom de table.


## Voir aussi

[Driver ODBC pour HFSQL Classic et pour HFSQL Client/Serveur (Windows)](../WDLang4/3044179.md)

[Exemples d'utilisation du provider OLE DB pour HFSQL](../WDLang4/9000064.md)
