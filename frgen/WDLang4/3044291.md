...
Liste les utilisateurs actuellement connectés à une ou plusieurs tables manipulées par une connexion Client/Serveur.
...
`<Résultat> = HListeUtilisateurConnecté(<Nom de la connexion> [, <Base de données> [, <Nom physique de la table>]])`
---
...
Nom de la connexion utilisée. Cette connexion a été définie sous l'éditeur de schémas des données ou grâce aux fonctions [HDécritConnexion](../WDLang4/3044205.md) ou [HOuvreConnexion](../WDLang4/3044107.md).
...
Si ce paramètre ainsi que le paramètre &lt;Nom physique du fichier de données&gt; ne sont pas précisés, la fonction **HListeUtilisateurConnecté** listera les utilisateurs connectés au serveur.

**`<Nom physique de la table> : (Chaîne de caractères optionnelle)`**

Nom de la table pour laquelle on veut connaître les utilisateurs actuellement connectés. 
