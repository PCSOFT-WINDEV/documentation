...
- Par défaut, les caractéristiques de la connexion sont définies sous l'éditeur de schémas des données, pour une ou plusieurs tables.
...
## Exemple
<a class="notetitle" target="_blank" href="$DOC$=1000003044263&name=hconnecte_fonction">Voir des exemples supplémentaires</a>
...
Nom de la table (table) à ouvrir, et pour laquelle la connexion doit être redéfinie. Si aucun autre paramètre n'est précisé, la table est ré-initialisée avec les paramètres par défaut définis dans le schéma des données.
...
- Si ce paramètre est égal à "\*" et qu'aucun autre paramètre n'est spécifié, toutes les tables de tous les Connecteurs Natifs définis dans le schéma des données sont ré-initialisées avec les paramètres par défaut définis dans le schéma des données.
...
Nom de l'utilisateur utilisé pour l'ouverture de la table.
...
|   |   |
| --- | --- |
| *hHCréationSiInexistant*<br>(Valeur par défaut) | Vérifie la connexion grâce à l'ouverture ou la création d'une table (si la table n'existe pas). Cette opération est possible uniquement en accès de type *hOLectureEcriture*. |
| *hHOuvre* | Vérifie la connexion grâce à une ouverture de table. Cette valeur est utilisée par défaut lors d'un accès à une base de type *hOLecture*. |
...
![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png) "initial catalog = dbo"![ODBC](https://doc.pcsoft.fr/ext/images/us/ODBC.png) "database = dbo"![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/us/AN.png) MySQL: "initial catalog = dbo"
...
<a name="parametrage_connexion_ELTPARAGRAPHE000266"></a>

- La description de la table doit être connue au moment de l'appel de la fonction **HConnecte** (la table doit avoir été décrite dans le schéma des données, ou bien avec les fonctions [HDéclare](../WDLang4/3044203.md) ou [HDécritTable](../WDLang4/3044206.md)).

- La fonction **HConnecte** ouvre la connexion et la table. La table est créée si elle n'existe pas. La fonction **HConnecte** équivaut pour cela à la fonction [HCréeSiInexistant](../WDLang4/3044254.md).

- Lors de la redéfinition de la connexion, seuls les paramètres spécifiés sont pris en compte. La valeur des paramètres non spécifiés avec la fonction **HConnecte** correspond à la valeur définie sous l'éditeur de schémas des données.

- Si seul le &lt;Nom de la table&gt; est spécifié, la fonction **HConnecte** annule la redéfinition de la connexion pour la ou les tables spécifiées. Les paramètres définis dans l'éditeur de schémas des données seront pris en compte.

- Lorsque le paramètre &lt;Nom de la table&gt; est égal à "\*", le changement s'applique à toutes les tables des Connecteurs Natifs définies dans le schéma des données. Dans ce cas, seuls les paramètres &lt;Utilisateur&gt;, &lt;Mot de passe&gt;, &lt;Base de données&gt; et &lt;Accès&gt; sont pris en compte.
