...
Pour que l'application puisse manipuler les tables présentes sur le serveur HFSQL, il est nécessaire de définir dans le projet la connexion qui sera utilisée par l'application. Cette connexion peut être définie:
...
Les tables accédées par un serveur HFSQL ne sont accessibles que via ce serveur. Ils ne peuvent pas êtres accédés directement par un autre application notamment à l'aide du moteur HFSQL Classic. Il est nécessaire de se connecter au serveur pour accéder à ces tables.
...
## Définition des connexions depuis l'éditeur de schémas des données
<a name="definition_des_connexions_depuis_editeur_schemas_des_donnees_ELTTEXTE000302"></a>
Pour définir une connexion depuis l'éditeur de schémas des données:

1. Sous le volet "Schéma des données", dans le groupe "Connexion", cliquez sur "Nouvelle connexion". L'assistant de définition d'une nouvelle connexion se lance.
...
4. Terminez l'assistant.
	Une boîte de dialogue s'affiche et propose d'associer des tables à cette nouvelle connexion. Acceptez.

5. Sélectionnez les tables à associer et validez. Il est possible de copier directement les tables existantes sur le serveur HFSQL. Cette opération peut être réalisée si nécessaire ultérieurement.

6. Affichez la liste des connexions: sous le volet "Schéma des données", dans le groupe "Connexion" cliquez sur "Connexions".
	...
	- Les différents onglets permettent de visualiser et de modifier les caractéristiques des connexions: 
		...
		- Onglet "Avancé": Numéro de port de la connexion, options de compression et de cryptage des données de la connexion. 
						Si les tables utilisant la connexion sont cryptées, il est conseillé de crypter également la connexion. 

		- Onglet "Utilisée par": Liste des tables utilisant la connexion. 
...
En exécution, la connexion associée à la table manipulée sera automatiquement ouverte lors du premier accès à cette table.

Les tables associées à une connexion de type "HFSQL Client/Serveur" se transforment automatiquement en tables de type "HFSQL Client/Serveur".

Pour les reconnaître, une indication "HFSQL/CS" apparaît au bas de la description graphique de ces fichiers données. De plus, la couleur associée à cette zone devient orangée: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=PictoHFSQLCS.gif)

<a name="NOTE2_2"></a>


### Crypter une connexion
<a name="crypter_une_connexion_ELTPARAGRAPHE000088"></a>

Pour crypter une connexion: 

1. Affichez la description de la connexion: sous le volet "Schéma des données", dans le groupe "Connexion" cliquez sur "Connexions".
...
Remarque: Si les tables utilisant la connexion sont cryptées, il est conseillé de crypter également la connexion. 
...
<a name="definition_des_connexions_par_programmation_ELTTEXTE000332"></a>
...
<a name="definir_une_connexion_aide_des_fonctions_hdecritconnexion_houvreconnexion_ELTPARAGRAPHE000134"></a>
...
3. Associez la connexion aux différentes tables grâce à la fonction [HChangeConnexion](../WDLang4/3044150.md).
	Par exemple: 
	...
	ou, pour associer toutes les tables du schéma des données à la connexion: 
...
<a name="definir_une_connexion_aide_type_connexion_ELTPARAGRAPHE000188"></a>

Pour définir une connexion à l'aide du type [Connexion](../WDLang4/1514073.md): 
...
<a name="permettre_une_application_windev_utiliser_choix_une_base_hfsql_clientserveur_hfsql_classic_ELTTEXTE000370"></a>
Le WLangage dispose de deux fonctions très simples permettant d'utiliser une base quelconque choisie lors de l'exécution. Et cela, indépendamment du type des tables spécifié dans le schéma des données.
...
Voici le code à utiliser, par exemple au début du projet, avant toute utilisation des tables:
