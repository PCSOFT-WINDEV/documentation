...
- le filtre est créé: la fonction **HFiltreCommencePar** renvoie la clé de parcours optimale à utiliser pour réaliser le parcours de la table, de la vue ou de la requête.
...
`<Résultat> = HFiltreCommencePar(<Table> , <Rubrique clé> , <Filtre pour la composante 1 de la clé> [, <Filtre pour la composante 2 de la clé> [... [, <Filtre pour la composante N de la clé>]]])`
---
...
<a name="gestion_automatique_des_cles_composees_ELTPARAGRAPHE000108"></a>
...
<a name="filtre_parcours_filtre_ELTPARAGRAPHE000180"></a>

Après l'exécution de la fonction **HFiltreCommencePar**, le parcours de la table doit obligatoirement être effectué sur la rubrique renvoyée par la fonction **HFiltreCommencePar**. Si une autre rubrique est utilisée pour le parcours de la table, le filtre ne sera pas pris en compte.
...
<a name="activationdesactivation_filtre_ELTPARAGRAPHE000227"></a>
...
- A un même instant, il ne peut exister qu'un seul filtre sur une table (une requête ou une vue). Si la fonction **HFiltreCommencePar** est utilisée plusieurs fois, seul le dernier filtre sera pris en compte: le(s) filtre(s) précédent(s) seront supprimés. De même si plusieurs filtres sont utilisés (fonctions HFiltre\*), seul le dernier filtre sera pris en compte.
...
### Tables non HFSQL
<a name="tables_non_hfsql_ELTPARAGRAPHE000252"></a>
