...
`<Résultat> = HExporteXML(<Table> [, <Chemin du fichier XML> [, <Rubriques à exporter>]] [, <Options> [, <Jauge>]])`
---
...
Nom de la table, de la vue ou de la requête manipulée. Si ce nom n'est pas spécifié, la fonction **HExporteXML** va manipuler la dernière table utilisée par la dernière fonction de gestion HFSQL (fonction commençant par la lettre H).

**`<Chemin du fichier XML> : (Chaîne de caractères optionnelle)`**

Chemin complet du fichier XML à générer. 
Si ce paramètre n'est pas spécifié, une table &lt;Table&gt;.XML sera générée dans le répertoire courant.
...
<a name="exporter_contenu_une_requete_ELTPARAGRAPHE000136"></a>

Pour exporter le contenu d'une requête, il est nécessaire d'initialiser la requête (fonction [HExécuteRequête](../WDLang4/3044080.md)) avant d'utiliser la fonction **HExporteXML**.
<a name="NOTE0_2"></a>


### Exporter un seul enregistrement
<a name="exporter_seul_enregistrement_ELTPARAGRAPHE000150"></a>

Pour exporter un seul enregistrement vers une chaîne XML, utilisez la fonction [HEnregistrementVersXML](../WDLang4/3044093.md).
<a name="NOTE0_3"></a>


### Rubriques tableau
<a name="rubriques_tableau_ELTPARAGRAPHE000162"></a>Si une des rubriques exportées est une rubrique de type tableau, chaque élément de la rubrique tableau est exporté dans un nouveau niveau:
...
### Divers
<a name="divers_ELTPARAGRAPHE000175"></a>
