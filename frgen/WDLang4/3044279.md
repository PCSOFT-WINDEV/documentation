...
<a name="replication_journalee_par_reseau_ELTTEXTE000247"></a>


### Qu'est-ce que c'est?
<a name="questce_que_cest_ELTPARAGRAPHE000023"></a>
...
<a name="les_actions_possibles_ELTPARAGRAPHE000034"></a>
...
<a name="comment_repliquer_par_reseau_par_programmation_ELTPARAGRAPHE000047"></a>
...
2. Création du réplica abonné sur le poste abonné (fonction [HCréeRéplicaAbonné](../WDLang4/3044253.md)). Le poste abonné doit avoir accès en lecture et écriture sur le disque où est installée l'application maître (répertoire des tables).
...
**Attention**: Ce paragraphe suppose que les opérations nécessaires ont été réalisées dans le schéma des données de l'application avant installation de l'application sur les différents postes. Pour plus de détails sur les opérations à effectuer, consultez [Réplication journalée: Modification du schéma des données](../WDLang4/3044246.md).
<a name="NOTE2_4"></a>


### Contraintes et conseils
<a name="contraintes_conseils_ELTPARAGRAPHE000077"></a>
...
- Pour effectuer la réplication journalée, il est possible d'utiliser pour identifier le chemin des tables à répliquer:
	...
	- soit une lettre de disque. Dans ce cas, le poste qui lance l'opération de réplication doit utiliser la même lettre de disque que le poste avec lequel il réplique. Par exemple, si la réplication est lancée depuis le poste abonné:
		...
		- sur le poste maître, le répertoire des tables est sur X,

		- le poste abonné doit associer à la lettre X le disque contenant les tables du poste maître.
...
- Pour gérer la réplication journalée dans une application, la table des opérations du journal (table JournalOperation.fic) doit être identique pour toutes les tables en réplication dans un même schéma des données.

- Il est conseillé d'enregistrer les tables nécessaires à la réplication journalée (fichiers \*.rpm et \*.rpl) dans un sous-répertoire RPL des tables de l'application.
...
- Si la journalisation est utilisée pour conserver une sauvegarde des tables, il est conseillé de placer ces tables sur un disque différent de celui des tables (pour plus de sécurité).
...
<a name="replication_journalee_par_support_transportable_ELTTEXTE000289"></a>


### Qu'est-ce que c'est?
<a name="questce_que_cest_ELTPARAGRAPHE000098"></a>
...
### Les actions possibles
<a name="les_actions_possibles_ELTPARAGRAPHE000107"></a>
...
### Contraintes et conseils
<a name="contraintes_conseils_ELTPARAGRAPHE000117"></a>

- Pour gérer la réplication journalée dans une application, la table des opérations du journal (table JournalOperation.fic) doit être identique pour toutes les tables en réplication dans un même schéma des données.

- Il est conseillé d'enregistrer les fichiers nécessaires à la réplication (fichiers \*.rpm et \*.rpl) dans un sous-répertoire RPL des tables de l'application.
...
### Par programmation
<a name="par_programmation_ELTPARAGRAPHE000127"></a>
...
**Attention**: Ce paragraphe suppose que les opérations nécessaires ont été réalisées dans le schéma des données de l'application avant installation de l'application sur les différents postes. Pour plus de détails sur les opérations à effectuer, consultez [Réplication avec WINDEV](../WDLang4/3044256.md).
...
<a name="fonctions_wlangage_permettant_gerer_replication_journalee_ELTTEXTE000331"></a>


### Fonctions de gestion de la réplication journalée
<a name="fonctions_gestion_replication_journalee_ELTPARAGRAPHE000173"></a>Les fonctions WLangage permettant de gérer la réplication journalée sont les suivantes:
