...
## Présentation
<a name="presentation_ELTTEXTE001150"></a>
...
Cette page présente les principales erreurs de gestion des tables HFSQL pouvant survenir. Vous trouverez pour chaque erreur :
...
- Les codes d'erreur spécifiques à WINDEV/WEBDEV 2026 (de 70150 à 73001).
...
**Remarque** : La fonction [HErreurInfo](../WDLang4/3044071.md) renvoie une information détaillée sur la dernière erreur survenue lors de la gestion des tables HFSQL.

**Attention : Les erreurs HFSQL peuvent également apparaître lors de l'utilisation d'une table liée aux données**. Dans ce cas, quelle que soit l'erreur HFSQL, l'erreur **10085** apparaît. Cette erreur signifie que le champ n'a pas pu s'initialiser car il a rencontré une erreur HFSQL.
...
<a name="codes_erreur_compatibles_avec_windev_55_ELTTEXTE001174"></a>
**70001: H.TROUVE=FAUX**
...
Il est retourné par exemple après la fonction [HLit](../WDLang4/3044047.md) si le pointeur est positionné en dehors du fichier ou après la fonction [HLitSuivant](../WDLang4/3044037.md) quand il n'y a plus rien à lire.

**70003: FICHIER INEXISTANT**

Le fichier spécifié n'existe pas. Il faut créer le fichier avec la fonction [HCrée](../WDLang4/3044255.md) (ou [HCréeSiInexistant](../WDLang4/3044254.md)).
...
Cette erreur intervient après la fonction [HAjoute](../WDLang4/3044147.md) ou la fonction [HModifie](../WDLang4/3044042.md). Elle indique que la valeur d'une clé déclarée unique de l'enregistrement existe déjà dans le fichier. La fonction [HErreurDoublon](../WDLang4/3044077.md) n'a pas été testé, WINDEV le détecte et le signale par cette erreur.
...
Cette erreur intervient après la fonction [HSupprime](../WDLang4/3044018.md), [TableSupprime](../WDLang1/3074032.md), [HAjoute](../WDLang4/3044147.md), [HModifie](../WDLang4/3044042.md), [TableEnregistre](../WDLang1/3074008.md), [HEcrit](../WDLang4/3044092.md), ... Elle indique qu'il existe dans un fichier relié des enregistrements reliés à l'enregistrement supprimé. La fonction [HErreurIntegrité](../WDLang4/3044086.md) n'a pas été testé, WINDEV le détecte et vous signale cette erreur.
...
La description du fichier physique est différente de la description du fichier dans le schéma des données ou décrit dynamiquement (fonction [HDécritTable](../WDLang4/3044206.md)).

Il faut synchroniser les tables sous l'Éditeur de schémas des données de WINDEV : sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Synchronisation" et sélectionnez Synchronisation de la Structure et des Données Déployées (SDD, Modif Auto)".

**70018: FICHIER INCONNU**

Cette erreur intervient lorsque le nom de la table passé en paramètre à une fonction de gestion de table n'est pas connu par le moteur HFSQL (table du schéma des données, table décrite dynamiquement, requête, alias, vue, ...).

**70019: RECHERCHE NON AMORCÉE POUR CETTE CLÉ**

Une fonction [HLitSuivant](../WDLang4/3044037.md) ou [HLitPrécédent](../WDLang4/3044048.md) est demandée alors qu'aucun enregistrement n'est pointé pour la clé (avec les fonctions HLitRecherche\*, [HLitPremier](../WDLang4/3044051.md), [HLitDernier](../WDLang4/3044140.md)).

Remarque : cette erreur peut arriver si le fichier a été fermé après l'amorçage de la recherche. La fermeture peut être causée par un simple appel de la fonction [HFerme](../WDLang4/3044073.md), mais aussi par une fonction qui ferme implicitement le fichier.

**70021: L'INDEX NE CORRESPOND PAS AUX DONNÉES**

Le fichier des données et le fichier d'index ne sont plus en phase : il faut ré-indexer avec la fonction [HReindexe](../WDLang4/3044133.md).

Cause probable : la modification automatique a modifié les tables mais n'a pas pu ré-indexer.

**70022: MOT DE PASSE DE FICHIER INCORRECT**

Le mot de passe utilisé pour la table n'est pas le bon. La fonction [HErreurMotDePasse](../WDLang4/3044082.md) n'a pas été testée.
...
Le fichier journal est absent ou bien le fichier journal ne correspond pas à la table. Utilisez WDLog pour corriger le problème.
...
La fonction suivante n'est pas autorisée pendant une transaction (après la fonction [HTransactionDébut](../WDLang4/3044002.md)), il faut donc l'enlever du programme grâce à la fonction [HTransactionDébut](../WDLang4/3044002.md).
...
- Soit la fonction [HTransactionAnnule](../WDLang4/3044001.md).

- Soit les fonctions [HTransactionDébut](../WDLang4/3044002.md) et [HTransactionFin](../WDLang4/3044032.md) dans le code d'initialisation de votre projet (après l'initialisation et l'ouverture de fichiers). Cas typique: une transaction est débutée, la transaction échoue, le programme est relancé sans avoir annulé la transaction.
...
Solution : Ré-indexer le fichier avec [WDOptimizer](../WDOptimiseur/3519002.md) (ou la fonction [HRéindexe](../WDLang4/3044133.md)).
...
- Problème de transfert des enregistrements de la vue vers le fichier d'origine (fonction [HVueVersTable](../WDLang4/3044021.md) par exemple).
...
Solution : Ré-indexer la table avec [WDOptimizer](../WDOptimiseur/3519002.md) (ou avec la fonction [HRéindexe](../WDLang4/3044133.md)).

**70055: SCHÉMA DES DONNÉES INCOMPATIBLE OU CORROMPU**

La version du schéma des données (fichier .WDD) n'est pas reconnue par le moteur HFSQL.

Solution : Faites une nouvelle génération du schéma des données sous l'éditeur de schémas des données : sous le volet "Schéma des données", dans le groupe "Schéma des données", déroulez "Génération" et cliquez sur "Génération".

**70100: BLOCAGE (FICHIER OU ENREGISTREMENT)**

La fonction [HErreurBlocage](../WDLang4/3044099.md) renvoie <u><u><u><u>Vrai</u></u></u></u>. Le programme tente d'accéder à une table ou un enregistrement bloqué par un autre poste ou un autre programme.
...
**71038: MOT DE PASSE DU SCHÉMA DES DONNÉES INCORRECT**

Le mot de passe utilisé pour ouvrir le schéma des données n'est pas le bon (fonction [HOuvreSchémaDesDonnées](../WDLang4/3044106.md) ou fonction [HInitHF55](../WDLang4/3044010.md)).
...
<a name="codes_erreur_windev_ELTTEXTE001198"></a>
...
Vous n'avez pas les droits suffisants pour accéder à la table (mauvais mot de passe pour accéder au réseau, accès table physique impossible, table en lecture seule, ...).

Vérifiez que la table n'est pas protégée en écriture, que vous avez les droits réseau suffisants pour accéder à la table (réseau).

**70180: ERREUR DESCRIPTION DYNAMIQUE**

Deux possibilités :

- Un paramètre obligatoire n'a pas été précisé lors de la description de la table (il manque le nom logique de la table par exemple).




- Une incohérence apparaît dans la description dynamique de la table (un mémo clé par exemple).
...
L'enregistrement spécifié est en dehors de la table. Cette erreur peut survenir lors de l'utilisation des fonctions [HRaye](../WDLang4/3044121.md), [HSupprime](../WDLang4/3044018.md), [HBloqueNumEnr](../WDLang4/3044152.md), [HDébloqueNumEnr](../WDLang4/3044194.md).
...
Vous tentez d'utiliser une table configurée pour un système acceptant les tables de plus de 2 Go. Or le système actuel n'accepte ce type de fichier.

Modifiez la description du fichier dans le schéma des données. Régénérez le schéma des données. Une modification automatique des tables sera effectuée.

**70202: RUBRIQUE INCONNUE DANS LE FICHIER**

Vous tentez d'utiliser une rubrique qui n'existe pas dans la description de la table.
...
Vous utilisez une fonction HFSQL qui manipule l'enregistrement en cours. Or il n'y a aucun enregistrement en cours actuellement dans le fichier. Utilisez une fonction de positionnement (HLit\* par exemple) pour sélectionner un enregistrement dans la table ou précisez le numéro de l'enregistrement dans la fonction.

**70206: TENTATIVE DE RAYURE DUN ENREGISTREMENT SUPPRIME**

Un enregistrement supprimé ne peut pas être rayé. Les données de l'enregistrement sont invalides. Pour ré-activer un enregistrement supprimé, utilisez la fonction [HEcrit](../WDLang4/3044092.md) ou la fonction [HModifie](../WDLang4/3044042.md).

**70207: DESCRIPTION DÉJÀ EXISTANTE**

Vous tentez de décrire une table, une requête, une vue, ... Le nom utilisé est déjà connu par le moteur HFSQL. Utilisez un autre nom.

**70208: ERREUR D'INITIALISATION DE LA REQUÊTE**

Une erreur a été rencontrée dans le schéma des données de la requête. Le message d'erreur détaille exactement le problème.

**70209: ERREUR D'INITIALISATION DU FILTRE**

Une erreur a été rencontrée dans le schéma des données de la condition du filtre. Le message d'erreur détaille exactement le problème.
...
Vous avez attaché un fichier supérieur à 4 Go à une rubrique de type mémo binaire (fonction [HAttacheMémo](../WDLang4/3044173.md)). Les tailles de fichier supérieures à 4 Go ne sont pas gérées.

**70217: IMPOSSIBLE D'EXTRAIRE LE MÉMO**

Vous utilisez la fonction [HExtraitMémo](../WDLang4/3044072.md). Aucun nom de fichier n'est défini dans l'entête du mémo. Vous devez préciser un nom de fichier dans lequel le mémo doit être extrait.

**70218: ERREUR DE LECTURE DU FICHIER ATTACHÉ**

Le fichier attaché au mémo (fonction [HAttacheMémo](../WDLang4/3044173.md)) ne peut pas être lu. Vérifiez la validité du fichier attaché (fichier en cours d'utilisation par exemple).
...
Le fichier mémo ne correspond pas à la table.
...
Vous tentez de créer un alias sur une requête créée avec la fonction [HExécuteRequêteSQL](../WDLang4/3044084.md). Vous devez ré-exécuter la fonction [HExécuteRequêteSQL](../WDLang4/3044084.md) en remplaçant le nom de la requête par le nom de l'alias souhaité.
...
Le schéma des données n'est pas propriétaire de la table manipulée par la modification automatique (la table n'a pas été créée par ce schéma des données). Cette opération est impossible.

**70226: MODIFICATION AUTOMATIQUE IMPOSSIBLE**

La table manipulée n'est pas décrite dans le schéma des données (la table n'a pas été créée par ce schéma des données). Cette opération est impossible.

**70227: MODIFICATION D'UN FICHIER PLUS RÉCENT QUE LE SCHÉMA DES DONNÉES**

La version du schéma des données correspondant à la table manipulée par la modification automatique est plus récente que le schéma des données en cours. Cette opération est impossible.

**70231: LE .FIC A ATTEINT SA TAILLE MAXIMALE**

La table a atteint sa taille maximale définie en fonction de son paramétrage dans le schéma des données ou lors de la description dynamique. Pour plus de détails, consultez les limites HFSQL.

**70232: PAS ASSEZ DE MÉMOIRE POUR LA RE-INDEXATION**

Vous n'avez pas assez de mémoire vive pour ré-indexer la table. Réalisez cette opération sur un autre poste.
...
**70235: PAS DE SCHÉMA DES DONNÉES 5.5 ATTACHÉ AU PROJET**

Vous utilisez une fonction accédant à la DLL Hyper File 5.5. Pour utiliser cette fonction, vous devez attacher un schéma des données au format WINDEV 5.5 à votre projet. Utilisez la fonction [HInitHF55](../WDLang4/3044010.md).

**70236: PAS DE SCHÉMA DES DONNÉES EN COURS**

Cette opération nécessite un schéma des données ouvert. Ce n'est pas le cas. Utilisez la fonction [HOuvreSchémaDesDonnées](../WDLang4/3044106.md) ou attachez un schéma des données à votre projet :

1. Sous le volet "Projet", dans le groupe "Projet", cliquez sur "Description". 

2. Sélectionnez l'onglet "Schéma des données".
...
Vous utilisez une fonction SQLxxxx. Vous devez définir et ouvrir une connexion grâce à la fonction [SQLConnecte](../WDLang4/3072005.md).

**70238: ANNULATION DE DÉCLARATION IMPOSSIBLE**

Vous tentez d'annuler une déclaration dynamique (fonction [HAnnuleDéclaration](../WDLang4/3044174.md)). La table correspondante est encore en cours d'utilisation. Fermez le fichier (fonction [HFerme](../WDLang4/3044073.md)) avant d'annuler la déclaration.
...
La structure de la table (.FIC) n'est pas reconnue par le moteur HFSQL.

**70301: FICHIER .FIC CORROMPU**

Vous manipulez une table illisible. Utilisez une sauvegarde.

**70302 : FICHIER D'INDEX INTROUVABLE**

Le fichier d'index n'a pas été trouvé. Ré-indexez la table.

**70303 : EN-TETE DU FICHIER INVALIDE**

L'entête du fichier est invalide. Le chaînage des enregistrements supprimés est incorrect. La table est corrompue. Ré-indexez la table ou utilisez une sauvegarde.

**70305 : GUIDAnalyse DIFFERENT**

Le fichier a été créé depuis un schéma des données différent..



**70306: MODIFICATION D'UN FICHIER PLUS RÉCENT QUE LE SCHÉMA DES DONNÉES**
...
**70405: OPÉRATION INTERDITE SUR UN FICHIER DU SCHÉMA DES DONNÉES**

Cette opération ne peut pas être effectuée sur une table décrite dans le schéma des données en cours. Consultez la documentation correspondant à l'opération effectuée.
...
Vous tentez de récupérer la valeur d'une propriété sur une table. Cette opération est impossible. Consultez la documentation correspondante.

**70551: VALEUR DE LA PROPRIÉTÉ D'UN FICHIER NON MODIFIABLE**

Vous tentez de modifier la valeur d'une propriété sur une table. Cette opération est impossible. Consultez la documentation correspondante.
...
Vous tentez d'importer un fichier texte. Une erreur est survenue lors de l'ajout des enregistrements dans la table (.FIC).

Le message d'erreur détaille exactement le problème.

**70602: HIMPORTEHF55 : PERTE DE DONNEES POSSIBLE**

Vous tentez d'importer une table au format Hyper File 5.5. Une rubrique source et sa rubrique destination correspondante sont de type différent. La transformation de type peut entraîner une perte de données. Il est possible d'éviter cette erreur grâce à une constante permettant de forcer l'importation. Consultez la documentation correspondante.

**70603: HIMPORTEHF55 : PAS DE RUBRIQUE DESTINATION**

Vous tentez d'importer une table au format Hyper File 5.5. Une rubrique source du fichier 5.5 n'a pas de destination dans le fichier HFSQL Classic. Il est possible d'éviter cette erreur grâce à une constante permettant de forcer l'importation. Consultez la documentation correspondante.
...
Le moteur HFSQL a détecté un conflit lors de la modification des données. Pour plus de détails sur les conflits de modification, consultez [Gestion assistée des erreurs HFSQL](../WDLang4/3044188.md).

**70710 : VALEUR DE RUBRIQUE OBLIGATOIRE**

La valeur d'une rubrique est obligatoire pour pouvoir écrire l'enregistrement. La rubrique n'a pas été renseignée (cas des bases de données manipulées par OLE DB). Pour plus de détails, consultez [Gestion assistée des erreurs HFSQL](../WDLang4/3044188.md).

**70720 : UNE REINDEXATION EST EN COURS SUR LE FICHIER**

Une opération est effectuée sur le fichier alors que celui-ci est en cours de ré-indexation. L'opération doit être retentée à la fin de la ré-indexation. Pour savoir si une ré-indexation est en cours, utilisez la fonction [HRéindexationEnCours](../WDLang4/3044131.md).

Pour plus de détails, consultez [Gestion assistée des erreurs HFSQL](../WDLang4/3044188.md).

**70800: ERREUR DE BLOCAGE**

Une erreur de blocage est intervenue sur la fonction précédente et n'a pas été testée (fonction [HErreurBlocage](../WDLang4/3044099.md)).

**70801: ERREUR D'INTÉGRITÉ**

Une erreur d'intégrité est intervenue sur la fonction précédente et n'a pas été testée (fonction [HErreurIntégrité](../WDLang4/3044086.md)).

**70802: ERREUR DE DOUBLONS**

Une erreur de doublons est intervenue sur la fonction précédente et n'a pas été testée (fonction [HErreurDoublon](../WDLang4/3044077.md)).

**70803: ERREUR DE MOT DE PASSE**

Une erreur de mot de passe est intervenue sur la fonction précédente et n'a pas été testée (fonction [HErreurMotDePasse](../WDLang4/3044082.md)).
...
Équivalent à l'erreur 24 des tables HFSQL.
...
La description du fichier renvoyée par Hyper File 5.5 est différente de la description définie dans le schéma des données au format HFSQL Classic. Vérifiez les deux descriptions.
...
La fonction [HSupprimeTable](../WDLang4/3044294.md) tente de supprimer un fichier qui n'existe pas sur le serveur.
...
1. Sous le volet "Schéma des données", dans le groupe "Élément courant", cliquez sur "Description". 
...
<a name="codes_erreurs_gestion_des_transactions_ELTTEXTE001222"></a>
