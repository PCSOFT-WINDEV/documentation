...
<a name="questce_quune_vue_ELTTEXTE000145"></a>
Une vue HFSQL correspond à une "image mémoire" de tout ou partie d'une table HFSQL.

Une vue est stockée en mémoire, ce qui la rend insensible aux modifications effectuées sur la table associée.

Une fois créée, une vue se manipule comme une table HFSQL.
...
<a name="fonctions_gestion_des_vues_ELTTEXTE000169"></a>


### Les fonctions du WLangage
<a name="les_fonctions_wlangage_ELTPARAGRAPHE000038"></a>Les fonctions suivantes permettent de manipuler les vues HFSQL:
...
## Utilisation
<a name="utilisation_ELTTEXTE000251"></a>


### Manipuler une vue
<a name="manipuler_une_vue_ELTPARAGRAPHE000048"></a>

Pour manipuler une vue sur une table HFSQL, les opérations à effectuer sont les suivantes:
...
2. La vue est automatiquement exécutée. Les enregistrements de la vue peuvent être manipulés, parcourus, ... avec les fonctions de gestion HFSQL. Le nom de la table spécifié dans les fonctions correspondra au nom de la vue.
...
3. Opérations spécifiques aux vues possibles:
	...
	- **Modifier l'ordre de tri des enregistrements de la vue**. Par défaut, les enregistrements d'une vue sont triés dans le même ordre que celui utilisé dans la table HFSQL correspondante. Cependant, il est possible de modifier cet ordre grâce à la fonction [HTrieVue](../WDLang4/3044114.md). Si le tri est appliqué sur plusieurs rubriques de la vue, le tri est appliqué d'abord selon la première rubrique de tri, puis selon la seconde, etc.

	- **Reporter les manipulations effectuées sur les enregistrements de la vue** (ajouts, modifications, suppression, ...) sur le fichier de base de la vue (fonction [HVueVersTable](../WDLang4/3044021.md)). Attention: cette opération ne pourra être effectuée que si les enregistrements ont été bloqués dans la table lors de la création de la vue.
