...
- Si un état a été créé par le développeur sur ce champ Tableur, cet état est imprimé. Pour plus de détails, consultez [Champ Tableur dans un état](../WDChamp/1000033009.md). 
...
- En programmation : 
	...
	- la fonction [FAAExécute](../WDLang1/1000022099.md) utilisée avec la constante *faaImprimerTableur* lance l'impression du champ Tableur spécifié.

	- la fonction [FAADésactive](../WDLang1/1000022018.md) utilisée avec la constante *faaImprimerTableur* supprime l'option "Imprimer" pour le champ Tableur spécifié.

	- la fonction [FAAChangeLibellé](../WDLang1/1000022100.md) utilisée avec la constante *faaImprimerTableur* permet de modifier le libellé de l'option "Imprimer" pour le champ Tableur spécifié.
...
<a name="impression_etat_cree_par_developpeur_ELTPARAGRAPHE000049"></a>Si le développeur a créé un état sur le champ Tableur, l'option "Imprimer" permettra d'imprimer directement cet état. Aucune programmation spécifique est nécessaire. 
...
<a name="impression_directe_contenu_champ_tableur_ELTPARAGRAPHE000059"></a>Si le champ Tableur comporte plusieurs pages, une fenêtre propose à l'utilisateur :

- d'imprimer en mode portait ou en mode paysage.

- les options de mise à l'échelle du document. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=FAA_Imprimer_Tableur%20-%20HC%20N%B0001.gif)
...
<a name="personnaliser_fenetre_parametrage_impression_champ_tableur_ELTPARAGRAPHE000081"></a>WINDEV offre la possibilité d'intégrer la fenêtre de configuration de l'impression du champ Tableur utilisée par la FAA dans votre application. Il est ainsi possible de personnaliser par exemple :
