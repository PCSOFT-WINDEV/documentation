...
<a name="alignement_des_elements_grace_caractere_tab_ELTPARAGRAPHE000029"></a>Pour aligner chaque élément d'un champ Liste ou Combo, la solution la plus simple est d' **utiliser le caractère de contrôle TAB**. 


|   |   |
| --- | --- |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Liste2.gif) | <br><pre><code>// Le premier est complété pour réserver la taille<br>ListeAjoute(LISTE_LISTE2, Complète("MALTESE", 8) + TAB + "Corto")<br>ListeAjoute(LISTE_LISTE2, "PICSOU" + TAB + "Jean")<br>ListeAjoute(LISTE_LISTE2, "ANA" + TAB + "Yoko")</code></pre><br> |
...
**Important** : Le premier appel à la fonction [ListeAjoute](../WDLang1/3049004.md) fixe les positions des tabulations pour la totalité du champ Liste et pour les autres champs Liste de la fenêtre. Ainsi, dès le premier [ListeAjoute](../WDLang1/3049004.md), il faut penser à réserver une "place suffisante" pour les autres éléments du champ. Par exemple, si vous savez que le plus grand élément affiché dans le champ contiendra 20 lettres, vous pouvez fixer la taille grâce à la fonction [Complète](../WDLang1/3024010.md).

<a name="NOTE2_2"></a>


### Cadrage des éléments à droite ou à gauche
<a name="cadrage_des_elements_droite_gauche_ELTPARAGRAPHE000065"></a>Par défaut, le texte des éléments est cadré à gauche. En effet, l'utilisation du caractère TAB permet d'aligner le texte des éléments sur la gauche.
...
|   |   |
| --- | --- |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Liste3.gif) | <br><pre><code>// Le premier est complété pour réserver la taille<br>ListeAjoute(LISTE_LISTE3, EOT + Complète("MALTESE", 8) + ...<br>	TAB + EOT + "Corto")<br>ListeAjoute(LISTE_LISTE3, EOT + "PICSOU" + TAB + EOT + "Jean")<br>ListeAjoute(LISTE_LISTE3, EOT + "ANA" + TAB + EOT + "Yoko")</code></pre><br> |
...
<a name="recuperation_des_elements_champ_liste_ELTPARAGRAPHE000088"></a>Lors de la récupération d'un élément d'un champ Liste, la ligne entière est retournée, y compris les éventuels caractères EOT et TAB.
...
<a name="alignement_avec_des_espaces_methode_utilisable_uniquement_avec_des_polices_non_proportionnelles_ELTPARAGRAPHE000103"></a>Dans les champs Liste comportant plusieurs éléments sur une même ligne, l'utilisation de polices de largeur variable rend souvent difficile l'alignement des éléments.
...
|   |   |
| --- | --- |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Liste01.gif) | <br><pre><code>ListeAjoute(LISTE_LISTE1, Complète("MALTESE", 8) + "Corto")<br>ListeAjoute(LISTE_LISTE1, Complète("PICSOU", 8) + "Jean")<br>ListeAjoute(LISTE_LISTE1, Complète("ANA", 8) + "Yoko")</code></pre><br> |
...
### Alignement des numériques
<a name="alignement_des_numeriques_ELTPARAGRAPHE000136"></a>L'alignement des numériques est réalisé de la même façon que l'alignement du texte (voir paragraphe précédent). Il est cependant possible d'effectuer également un alignement des nombres sur le séparateur décimal.
<a name="NOTE3_2"></a>


### Alignement des nombres sur le séparateur décimal
<a name="alignement_des_nombres_sur_separateur_decimal_ELTPARAGRAPHE000145"></a>Pour aligner les nombres sur le séparateur décimal, il faut combiner l'utilisation des caractères TAB et EOT :
...
Comme nous l'avons déjà précisé, le premier appel à [ListeAjoute](../WDLang1/3049004.md) détermine la largeur des éléments. Ainsi, il ne faut pas oublier de réserver la largeur des éléments lors de l'ajout de la première ligne dans le champ.


|   |   |
| --- | --- |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Liste4.gif) | <br><pre><code>ListeAjoute(LISTE_LISTE4, Complète("FRANCE", 7) + TAB + EOT + ...<br>	"  30" + TAB + ",00")<br>ListeAjoute(LISTE_LISTE4, "CANADA" + TAB + EOT + "10" + TAB + ",00")<br>ListeAjoute(LISTE_LISTE4, "SUISSE" + TAB + EOT + "246")</code></pre><br> |
...
<a name="recuperation_contenu_champ_liste_ELTPARAGRAPHE000172"></a>Lors de la récupération d'un élément d'un champ Liste, la ligne entière est retournée, y compris les éventuels caractères EOT et TAB.
...
```
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Liste6.gif)
