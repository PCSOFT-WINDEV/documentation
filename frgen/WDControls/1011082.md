...
Après avoir créé l'état, l'impression de l'état nécessite une programmation en WLangage. Pour plus détails, consultez le paragraphe [Imprimer un état sur un champ Table hiérarchique](#NOTE4_1).[**Le logiciel Etats & Requêtes**](../Presentation/3088004.md) ne permet pas de créer des états sur un champ Table hiérarchique. Cependant, il est possible de personnaliser des états sur un champ Table hiérarchique créés sous l'éditeur d'états intégré (couleur, polices, taille, ...).
...
9. Saisissez le nom et le titre de l'état (nom du fichier ".WDE" correspondant à l'état). Ce nom permettra d'identifier l'état dans vos programmes.
	![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) Il est également possible d'indiquer si l'état peut être modifié par l'utilisateur final sous le logiciel "Etats et Requêtes". 
...
<a name="caracteristiques_etat_sur_champ_table_hierarchique_ELTTEXTE000356"></a>
...
- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) **Lignes imprimées** : Comme pour le champ Table, il est possible d'indiquer si seules les lignes sélectionnées ou l'ensemble des lignes du champ doivent être imprimées.
...
<a name="imprimer_etat_sur_champ_table_hierarchique_ELTTEXTE000386"></a>
...
### Exemple WEBDEV
<a name="exemple_webdev_ELTPARAGRAPHE000236"></a>L'ensemble des informations à imprimer est contenu dans le champ Table hiérarchique "TABLEH_TableClient" présent dans la page "PAGE_Client".
