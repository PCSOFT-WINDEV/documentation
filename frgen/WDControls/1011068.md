...
<a name="creer_etat_via_assistant_ELTTEXTE000445"></a>
<a name="Creer_etat"></a>
Pour créer un état:
...
2. Spécifiez dans les différentes étapes de l'assistant:
	...
	- la source de données de l'état (table, requête, etc.). Pour plus de détails, consultez [Source de données d'un état](../WDChamp/1011055.md).
...
<a name="les_differentes_etapes_assistant_ELTTEXTE000469"></a>


### Les différents types d'états disponibles
<a name="les_differents_types_etats_disponibles_ELTPARAGRAPHE000090"></a>
...
<a name="les_sources_donnees_disponibles_ELTPARAGRAPHE000131"></a>
...
- **Table existante**: Les données à imprimer proviennent d'une table décrite dans le schéma des données du projet. Ce table peut être de n'importe quel format (HFSQL Classic, Oracle, etc.).
	**Logiciel "Etats et Requêtes"**: 
Les tables et les rubriques de tables pouvant être utilisées dans les états créés avec le logiciel Etats & Requêtes sont définies par le développeur sous l'éditeur de schémas des données. Pour plus de détails sur la préparation nécessaire à l'utilisation d'une application WINDEV avec le logiciel Etats & Requêtes, consultez [Diffuser le logiciel Etats & Requêtes](../Presentation/3088002.md).
...
Le logiciel Etats & Requêtes ne permet pas de créer des états sur une zone mémoire. Cependant, il est possible de personnaliser les états sur zone mémoire créés avec l'éditeur d'états intégré (couleur, polices, taille, etc.).
	...
	- **Vue HFSQL**: Les données à imprimer proviennent d'une vue HFSQL sur une table HFSQL Classic.
			**Logiciel "Etats et Requêtes"**: 
...
### Ruptures et tris
<a name="ruptures_tris_ELTPARAGRAPHE000216"></a>
...
- Si l'état est basé sur une table, les rubriques de parcours de la table sont automatiquement proposées en rubrique de tri.
...
- pour lister les clients et les sociétés  par ville, il suffira de trier les clients par ville puis par nom de la société et de faire une rupture sur la ville.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=EtatRupture.gif)
...
<a name="realiser_des_calculs_dans_etat_ELTPARAGRAPHE000291"></a>
...
Si le projet est associé à un thème (ou à un modèle), il est possible de sélectionner ce même thème (ou modèle) pour vos états. Vos états imprimés auront le même style que votre application. Ce thème (ou modèle) pourra être modifié après la création de l'état.

A partir de la version 28 Update 3, il est possible de sélectionner le thème "Aucun". Dans ce cas, aucun thème ne sera appliqué à l'état en cours de création.
...
<a name="format_impression_etat_ELTPARAGRAPHE000318"></a>
...
<a name="enregistrement_etat_ELTPARAGRAPHE000340"></a>
...
- ![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/us/WBS.png) lors du déploiement du site.
