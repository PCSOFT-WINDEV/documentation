...
Dans un état sur une source de données programmée, la lecture des enregistrements à afficher dans l'état nécessite une programmation complète en WLangage. La source de données peut être de n'importe quel type (table HFSQL, requête, fichier texte, table d'une base de données tierce, ...).
...
[Le logiciel Etats & Requêtes](../Presentation/3088004.md) ne permet pas de créer des états sur une source de données programmée. Cependant, il est possible de personnaliser des états sur une source de données programmée créés sous l'éditeur d'états intégré (couleur, polices, taille, ...).
...
<a name="creer_etat_sur_une_source_donnees_programmee_ELTTEXTE000276"></a>


### Présentation
<a name="presentation_ELTPARAGRAPHE000045"></a>
...
### Créer un état sur une source de données programmée
<a name="creer_etat_sur_une_source_donnees_programmee_ELTPARAGRAPHE000055"></a>
...
6. Sélectionnez le thème de l'état si nécessaire.
...
<a name="programmer_lecture_des_enregistrements_ELTPARAGRAPHE000121"></a>

La programmation nécessaire à la lecture des enregistrements est illustrée par l'exemple suivant:**Exemple**: 
 
L'ensemble des informations à imprimer est contenu dans la table HFSQL "Client". Les données sont affichées dans des champs rubriques.

**Pour programmer la lecture des enregistrements**, la méthode est la suivante:

1. Lecture du premier enregistrement de la table "Client" dans l'événement "Ouverture" de l'état" (fonction [HLitPremier](../WDLang4/3044051.md)).
	
	```wl
	// Lecture du premier enregistrement de la table "Client" 
	HLitPremier(Client, NumClient)
	```


2. L'événement "Lecture des données" de l'état permet de gérer la fin de la table. Selon le cas, les valeurs suivantes sont renvoyées:

	- <u><u><u><u>Vrai</u></u></u></u>: fin de la table non atteinte (étape 3 exécutée).

	- <u><u><u><u>Faux</u></u></u></u>: fin de la table atteinte (étape 3 non exécutée).
			
		```wl
		// Fin de la table "Client" atteinte?
...
3. L'événement "Avant impression" du bloc Corps permet de gérer les enregistrements de la table:
...
<a name="ordre_execution_des_differents_evenements_wlangage_ELTPARAGRAPHE000171"></a>
...
| **2.** Lecture des données de l'état | Gestion de la fin de la table. |
