...
<a name="evenements_geres_par_defaut_ELTTEXTE000402"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) **WINDEV** et **WINDEV Mobile** gèrent par défaut les événements suivants (ordre d'apparition sous l'éditeur de code) :
...
| Sortie du champ Table | Exécuté lors du changement de champ (touche TAB, changement de champ par la souris, etc.). |
...
| Sortie d'une ligne du champ Table | Exécuté lors du changement de ligne ou lors du changement de champ (touche TAB, changement de champ par la souris, etc.) à partir d'une ligne. Le champ Table doit être en saisie. <br><br>Dans ce code, la ligne en cours correspond à la ligne précédemment sélectionnée.<br><br>Attention : <br><br>- **Champ Table avec colonne interrupteur** :  la seule modification de l'interrupteur n'exécute pas le code de sortie de ligne. En effet, lors d'un clic sur une colonne interrupteur, WINDEV ne considère pas que l'utilisateur est entré en saisie dans la ligne . Les événements "Entrée en saisie d'une ligne" et "Sortie d'une ligne" ne sont pas exécutés. Pour gérer ce cas, il est conseillé d'utiliser le code de modification de la colonne. <br><br>- Champ Table en affichage : ce code n'est pas exécuté au changement de ligne dans un champ Table en affichage. <br><br><br><br><br>![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) Dans ce code, la ligne courante correspond à la nouvelle ligne sélectionnée. |
| Affichage d'une ligne du champ Table | Exécuté lorsque une nouvelle ligne doit être affichée dans le champ Table : ré-affichage du champ Table, scrolling ou déplacement dans le champ Table, etc.<br><br>Cet événement est également exécuté lors de l'affectation d'une colonne. Si plusieurs colonnes sont affectées par programmation, le code d'affichage sera exécuté plusieurs fois. <br>**Conseil** : Si vous devez modifier plusieurs colonnes, utilisez la fonction [TableModifieLigne](../WDLang1/3074028.md) et non l'affectation directe.<br><br>![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) Cet événement n'est pas disponible dans les champs Table fichier pour des raisons de performances. |
...
| Evénement | Condition d'exécution |
| --- | --- |
| Initialisation (Code serveur) | Exécuté à l'ouverture de la page.<br><br>**Cas particulier : Champ Table fichier rempli automatiquement** : <br>Lors de l'exécution de ce code, le champ Table n'est pas encore rempli. Il est possible de forcer le remplissage du champ dans ce code avec :<br><br>- soit la fonction [TableAffiche](../WDLang1/3074003.md),<br><br>- soit les propriétés de gestion des champs Table fichier : [FichierParcouru](../Proprietes/2510040.md), [RubriqueParcourue](../Proprietes/2510111.md) ou [Filtre](../Proprietes/2510042.md).<br><br><br><br><br>Il est également possible de filtrer les enregistrements avec la fonction [HFiltre](../WDLang4/3044100.md). |
| Fin d'initialisation (Code serveur) | **Code présent uniquement pour un champ Table fichier rempli automatiquement** :<br><br>Exécuté après le remplissage du champ Table. Ce code permet par exemple de sélectionner un élément dans le champ Table, d'ajouter ou de modifier un élément. |
| Sélection d'une ligne (Code serveur) | Code exécuté lorsque l'internaute sélectionne une ligne du champ Table. |
| Sélection d'une ligne (Code navigateur) | **Champ Table en mode "Serveur + AJAX"**<br>Exécuté lorsque l'utilisateur sélectionne une ligne (sans saisie). <br><br>**Champ Table en mode "Navigateur"**<br>Exécuté lorsque l'utilisateur sélectionne une ligne (sans saisie) |
| Affichage d'une ligne (Code serveur) | Exécuté lorsque une nouvelle ligne doit être affichée dans le champ Table : ré-affichage du champ Table, scrolling ou déplacement dans le champ Table, etc.<br><br>Permet de gérer les colonnes calculées. |
| Affichage d'une ligne (Code navigateur) | **Champ Table en mode "Navigateur" uniquement**<br>Exécuté lorsque une nouvelle ligne doit être affichée dans le champ Table : ré-affichage du champ Table, scrolling ou déplacement dans le champ Table, etc. |
| Clic de table (Code navigateur) | Exécuté lorsque l'internaute sélectionne une ligne. |
| Sortie d'une ligne (Code serveur) | **Champ Table en mode "Serveur + AJAX" uniquement**<br><br>Exécuté lors du changement de ligne ou lors du changement de champ (touche TAB, changement de champ par la souris, etc.) à partir d'une ligne |
...
<a name="evenements_optionnels_ELTTEXTE000432"></a>
...
<a name="filtrage_des_enregistrements_table_fichier_uniquement_ELTTEXTE000472"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/us/WBS.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) L'événement "Filtrage des enregistrements" est exécuté juste après l'affichage d'une ligne du champ Table. Il est donc exécuté :
...
<a name="avant_affichage_bulle_aide_automatique_ascenseur_ELTTEXTE000496"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) Sous Windows, la bulle d'aide de l'ascenseur des champs Table est affichée lorsque l'utilisateur clique sur la "cage" de l'ascenseur vertical. 
