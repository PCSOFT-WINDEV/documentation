...
**Différences par rapport à la programmation HFSQL**: L'utilisation des fonctions SQL pour manipuler des données MySQL n'implique pas une importation de la structure des fichiers dans le schéma des données. Cependant, les données doivent être manipulées uniquement par requêtes. Il n'est pas possible de lire directement le contenu des fichiers MySQL.
...
La fonction [SQLConnecte](../WDLang4/3072005.md) permet d'ouvrir une connexion à une base de données MySQL par le Connecteur Natif. Il est nécessaire de passer en paramètre à la fonction **SQLConnecte**le type de la base utilisée, ici MySQL.
