...
<a name="diffusez_etats_requetes_avec_vos_applications_ELTTEXTE000385"></a>
...
<a name="presentation_logiciel_etats_requetes_ELTTEXTE000421"></a>
...
Pour lancer le logiciel "Etats et Requêtes", il suffit d'ouvrir un état dans le visualisateur de rapports. Si l'application a été configurée pour utiliser le logiciel "Etats et Requêtes", il suffira de cliquer sur l'icône ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0006%201%202.jpg).
...
<a name="comment_diffuser_logiciel_etats_requetes_avec_votre_application_ELTTEXTE000454"></a>
...
- Etape 2 : définir si nécessaire au niveau du schéma des données les fichiers et rubriques qui seront utilisables avec le logiciel "Etats et Requêtes".
...
<a name="etape_1_parametrage_projet_ELTPARAGRAPHE000062"></a>
...
2. Dans l'onglet "Avancé", cochez l'option "Autoriser le lancement de "Etats et Requêtes"". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0001%201.jpg&type=thumb)
...
### Etape 2 : Définir les tables et les rubriques non visibles dans le logiciel Etats et Requêtes
<a name="etape_2_definir_les_tables_les_rubriques_non_visibles_dans_logiciel_etats_requetes_ELTPARAGRAPHE000092"></a>

Par défaut, toutes les tables et toutes leurs rubriques sont visibles et manipulables avec le logiciel "Etats et Requêtes". Si votre application contient des informations sensibles (mots de passe, etc.) vous pouvez rendre ces tables ou ces rubriques invisibles sous le logiciel "Etats et Requêtes".A partir du logiciel "Etats et Requêtes", l'utilisateur ne peut pas ajouter, modifier ou supprimer des enregistrements. Il n'est également pas possible de créer ou de re-créer des tables.
Nous allons maintenant ouvrir le schéma des données pour paramétrer les fichiers et rubriques utilisables sous le logiciel "Etats et Requêtes".

Ouvrez le schéma des données de votre projet : cliquez sur ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=ICO_Analyse_GAF.jpg) parmi les boutons d'accès rapide du menu de WINDEV.

Nous allons paramétrer le fichier "ModeRèglement" pour qu'il devienne invisible sous le logiciel "Etats et Requêtes" :

1. Sous l'éditeur de schémas des données, cliquez sur la table "ModeRèglement".
...
1. Sous l'éditeur de schémas des données, cliquez sur la table "Produit".
...
Pour prendre en compte les modifications effectuées dans le schéma des données, nous allons regénérer le schéma des données :

1. Sous le volet "Schéma des données" du ruban, dans le groupe "Schéma des données", cliquez sur "Génération". Une modification automatique des tables est proposée pour prendre en compte les modifications effectuées.

2. Validez les différentes étapes de la modification automatique des données en conservant les options par défaut. 

3. Fermez l'éditeur de schémas des données.
...
<a name="etape_3_definir_les_etats_les_requetes_modifiables_non_sous_logiciel_etats_requetes_ELTPARAGRAPHE000149"></a>

Par défaut les requêtes et les états sont modifiables sous le logiciel "Etats et Requêtes". Pour les états, seule l'interface des états est modifiable. La création d'un état est détaillée dans la [leçon 4 du tuto "Application complète avec données"](../TutoWD/1410087535.md).
...
4. Cochez ensuite une des deux options suivantes :
	...
	- "Tout modifiable (interface et code)" pour que l'état soit entièrement modifiable sous Etats et Requêtes. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0002.jpg&type=thumb)
...
<a name="etape_4_creation_executable_diffusion_application_ELTPARAGRAPHE000188"></a>
...
7. Dans l'écran Fichiers installés", sélectionnez si nécessaire les tables (.fic, .mmo et .ndx) présentes dans le répertoire de l'exécutable. Pour cela, cliquez sur "Ajouter" et sélectionnez ces fichiers. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_D%E9ployer%20l'application%20-%20HC%20N%B0006.jpg&type=thumb)
...
11. Dans l'étape intitulée "Etats et Requêtes" :
 ...
 Attention : Incorporer le logiciel "Etats et Requêtes" augmente la taille de votre installation de façon non négligeable (environ 2 Go). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0003.jpg&type=thumb)
...
<a name="installer_utiliser_logiciel_etats_requetes_ELTTEXTE000529"></a>


### Installation de l'application
<a name="installation_application_ELTPARAGRAPHE000253"></a>
...
5. L'étape "Etats et Requêtes" permet d'installer le logiciel "Etats et Requêtes". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0004.jpg&type=thumb)
...
<a name="test_application_ELTPARAGRAPHE000293"></a>
...
2. Sélectionnez l'option "Etats .. Liste des clients". L'état est affiché dans le visualisateur de rapports. Cliquez sur le volet "Aperçu". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0006.jpg&type=thumb)
...
|   |   |
| --- | --- |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0006%201%202.jpg) | Permet de modifier l'état en cours dans le visualisateur de rapports. |
| ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0006%201%201.jpg) | Permet de créer un nouvel état. |
...
1. Cliquez sur l'icône ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0006%201%202.jpg).

2. L'état apparaît en édition sous le logiciel "Etats et Requêtes". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=P4_Diffuser%20EtatsRequetes%20-%20HC%20N%B0007.jpg&type=thumb)
