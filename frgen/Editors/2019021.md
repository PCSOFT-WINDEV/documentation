...
**Remarque** : Pour déboguer une application WINDEV à distance, l'application WINDEV doit inclure la librairie wd310cpl.dll dans son framework.
...
- WDDebug (en version 32 et 64 bits) est une application pouvant être livrée indépendamment. Un pack d'installation est disponible dans le sous-répertoire "Install\\WDDebug" du répertoire d'installation de WINDEV (WX2026PACKWDDEBUG.exe). Attention : WDDebug ne peut être utilisé que sur les postes utilisant Windows 2000 ou supérieur.
...
- Dans le volet "Code", option "Débogueur", cliquez sur "Pause" (![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=main_debog.gif)).
...
**Sur le poste de développement** : Dans la base de registre "HKEY_CURRENT_USER\\SOFTWARE\\PC SOFT\\WINDEV\\31.0\\Common", créez la valeur "DWORD 32 Bits" suivante : "DBG_Port_COM". Cette valeur contient le numéro du port de communication du poste serveur.

**Sur le poste où le débogage doit être effectué** :

1. Créez si nécessaire la clé "HKEY_CURRENT_USER\\SOFTWARE\\PC SOFT\\WDDEBUG\\31.0". 

2. Dans la base de registre "HKEY_CURRENT_USER\\SOFTWARE\\PC SOFT\\WDDEBUG\\31.0", créez la valeur "DWORD 32 Bits" suivante : "DBG_Port_COM". Cette valeur contient le numéro du port de communication du poste serveur. Elle doit être identique à la valeur indiquée sur le poste de développement.

3. Dans la base de registre "HKEY_CURRENT_USER\\SOFTWARE\\PC SOFT\\WDDEBUG\\31.0", créez la valeur "DWORD 32 Bits" suivante : "DBG_Port_DBG". Cette valeur contient le numéro du port de débogage. 
