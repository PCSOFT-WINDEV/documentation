...
- [Comment créer une rubrique Enumération et Combinaison sous l'éditeur de schémas des données](#NOTE2). 
...
## Création de rubriques de type Énumération et Combinaison sous l'éditeur de schémas des données
<a name="creation_rubriques_type_enumeration_combinaison_sous_editeur_schemas_des_donnees_ELTTEXTE000246"></a>


### Création d'une rubrique de type Enumération
<a name="creation_une_rubrique_type_enumeration_ELTPARAGRAPHE000031"></a>

Pour créer une rubrique de type Enumération:

1. Créez une nouvelle rubrique dans la fenêtre de description des rubriques de la table (option "Description des rubriques" du menu contextuel de la table sous l'Éditeur de schémas des données). 

2. Choisissez le type "Enumération, Combinaison". 

3. Dans l'onglet "Général" de la description de la rubrique, cliquez sur le bouton à droite de la combo "Type", puis sélectionnez "Créer une nouvelle énumération". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=TypeRubEnumCombin%20-%20HC%20N%B0003.gif&type=thumb)


4. La fenêtre d'édition d'une énumération s'affiche:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=RubriqueEnumCombin2%20-%20HC%20N%B0001.gif)


	- Précisez le nom de l'énumération.

- Ajoutez des valeurs (bouton ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=RubriqueEnumCombinBoutonPlus.gif)) ou saisissez directement les valeurs dans la table.
...
Remarque: Si vous avez défini une variable de type Enumération dans l'éditeur de code, pour utiliser cette variable dans une rubrique du schéma des données, vous devez recréer cette énumération dans l'éditeur de schémas des données.
<a name="NOTE2_2"></a>


### Création d'une rubrique de type Combinaison
<a name="creation_une_rubrique_type_combinaison_ELTPARAGRAPHE000058"></a>
...
4. La fenêtre d'édition d'une combinaison s'affiche:
	<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=RubriqueEnumCombin2%20-%20HC%20N%B0002.gif)
...
Remarque: Si vous avez défini une variable de type Combinaison dans l'éditeur de code, pour utiliser cette variable dans une rubrique du schéma des données, vous devez recréer cette combinaison dans l'éditeur de schémas des données.
...
<a name="conversion_une_rubrique_existante_vers_type_enumeration_combinaison_ELTTEXTE000276"></a>
Si vos tables existantes utilisent des rubriques que vous souhaitez transformer en Enumération ou Combinaison (pour faciliter la maintenance du code, éviter les affectations de valeurs indésirables, etc.), plusieurs possibilités s'offrent à vous:

- "convertir" la rubrique existante en variable de type Enumération ou Combinaison. Cette solution est la plus rigoureuse mais est plus complexe à mettre en œuvre (surtout si vous avez des tables en production).
...
<a name="convertir_une_rubrique_une_variable_type_enumeration_combinaison_ELTPARAGRAPHE000094"></a>

La "conversion" doit être réalisée en plusieurs étapes:

1. Ajoutez une nouvelle rubrique "Enumération, Combinaison" dans la description de la table voulue (voir paragraphes précédents).

2. Modifiez les tables physiques. Cette modification peut être faite:

	- soit directement depuis l'éditeur de schémas des données via la "Synchronisation de la Structure et des Données déployées". Utilisez cette solution si vous avez accès directement aux tables depuis le poste de développement.

	- soit par programmation via la fonction WLangage [HModifieStructure](../WDLang4/1000019439.md). Utilisez cette solution si vous n'avez pas accès aux tables (applications déployées par exemple).
...
<a name="utilisation_une_rubrique_type_entier_chaine_ELTPARAGRAPHE000127"></a>

Il est également possible d'utiliser une énumération ou combinaison sans modifier votre schéma des données existant. Vous pouvez mémoriser la valeur ou le nom de l'option de l'énumération ou combinaison dans une variable de type entier ou chaîne de caractères.
...
[Enumération (Type de variable)](../Editeurs/1514067.md)

[Combinaison (Type de variable)](../Editeurs/1514074.md)

[HModifieStructure (Fonction)](../Editeurs/1000019439.md)

[Fonctions de gestion des Combinaisons et des Enumérations](../Editeurs/1000020867.md)
