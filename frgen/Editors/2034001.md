...
## Présentation
<a name="presentation_ELTTEXTE001221"></a>
...
**[Le logiciel Etats et Requêtes](../Presentation/3088004.md)** ne permet pas de créer des requêtes en code SQL utilisant les ordres INSERT, UPDATE ou DELETE.
...
FROM NomDesFichiers AS NomDesFichiersUtilisé [FORCE INDEX (NomCléAUtiliser)]
...
<a name="cas_particulier_select_multifichiers_ELTPARAGRAPHE000160"></a>

L'instruction SELECT permet également de sélectionner dans une requête multitable toutes les colonnes d'une table grâce à la syntaxe suivante:
...
<a name="cas_particuliers_select_sans_from_select_utilisant_une_table_virtuelle_ELTPARAGRAPHE000171"></a>
...
<a name="executer_une_procedure_stockee_ELTPARAGRAPHE000187"></a>
...
- Pour connaître la valeur de l'identifiant automatique lors d'une requête de type INSERT sur des tables HFSQL (requête exécutée par la fonction [HExécuteRequêteSQL](../WDLang4/3044084.md), [HExécuteRequête](../WDLang4/3044080.md) ou [SQLExec](../WDLang4/3072007.md)), utilisez la variable SQL.IDAuto.

- Il n'est pas possible d'utiliser l'instruction INSERT pour copier les données d'une table HFSQL dans une autre table HFSQL.
...
<a name="insert_avec_selection_ELTPARAGRAPHE000272"></a>L'instruction **INSERT** peut également être utilisée pour insérer une sélection d'enregistrements réalisée par une requête SELECT. Son format d'utilisation devient:
...
### Types disponibles
<a name="types_disponibles_ELTPARAGRAPHE000410"></a>
...
- Gestion de l'anonymisation: ADD MASKED / DROP MASKED: 
 ...
 avec &lt;ratio&gt; représente un nombre entre 0 et 1 (non compris). Il s'agit d'un pourcentage qui sera appliqué en plus ou en moins.
			...
			- Dans une requête SQL: Ajoute + ou - 20% à la valeur d'origine stockée dans la rubrique prix de la table Produit
...
- **Remplacement de valeur (ou destruction)**
 ...
 avec &lt;valeur chaîne&gt; représente une chaîne de caractères avec des guillemets.
	...
	- Dans une requête SQL: Remplace la valeur d'origine stockée dans la rubrique Statut de la table Commande par la valeur fixe "SOLDEE".
...
- Pour utiliser la table modifiée lors du ALTER (sans schéma des données lié au projet), il faut utiliser la fonction [HDéclareExterne](../WDLang4/3044204.md) après l'exécution de la requête de type ALTER TABLE.
...
La fonction **OPTIMIZE TABLE** permet de réorganiser et réindexer une table et de reconstruire les index de cette table. Cette opération permet notamment d'améliorer les performances.

Cette fonction est équivalente à utiliser la fonction WLangage [HRéindexe](../WDLang4/3044133.md) pour la réorganisation et réindexation d'une table.
...
- Cette fonction est bloquante: la table n'est pas accessible durant l'exécution de la fonction. Il faut s'assurer que le fichier n'est pas en cours d'utilisation lors de l'appel de la fonction.
...
[H.NbEnregRequêteModification](../Editeurs/3087002.md)
