...
L'éditeur de requêtes permet de tester directement le résultat de la requête (icône ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Ico_Go_Fenetre_WD_bl.gif) ou option "Tester la requête" du menu contextuel de la requête).

Lors du test de la requête, le code d'initialisation du projet est exécuté.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Requete_ResultatTestRequete.gif)
**Remarques**:
...
- Lors du test d'une [requête paramétrée](../Editeurs/2032045.md), une fenêtre permettant de spécifier les différents paramètres s'affiche. 
	Dans cette fenêtre, vous pouvez sélectionner une valeur existante grâce à l'icône ![Valeur existante](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=OrdreAffichage.gif). Le menu suivant apparaît:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Requete_MenuVisuValeur.gif)
...
- Le bouton ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=IconeCalculette.gif) permet d'afficher automatiquement la moyenne, le total, les valeurs minimale et maximale des différentes rubriques résultat de type numérique.


[Le logiciel Etats et Requêtes](../Presentation/3088006.md) ne permet pas de tester:
...
Lors du test d'une requête sous [le logiciel Etats et Requêtes](../Presentation/3088006.md), l'événement "Initialisation du projet" est exécuté.
...
<a name="resultat_test_requete_ELTTEXTE000294"></a>


### Résultat du test
<a name="resultat_test_ELTPARAGRAPHE000073"></a>
...
- Le résultat d'une *requête d'insertion* correspond à l'enregistrement qui va être inséré. Lors du test, l'enregistrement est directement inséré dans la table.
...
- Le résultat d'une *requête de modification* correspond aux enregistrements qui vont être modifiés. Lors du test, il est possible:
	...
	- soit de modifier directement les enregistrements dans la table.
...
- Le résultat d'une *requête de suppression* correspond aux enregistrements qui vont être supprimés. Lors du test, il est possible:
	...
	- soit de supprimer directement les enregistrements dans la table.
...
<a name="manipulation_resultat_ELTPARAGRAPHE000091"></a>
...
### Gestion des mots de passe des tables
<a name="gestion_des_mots_passe_des_tables_ELTPARAGRAPHE000113"></a>

Si une des tables participant à la requête est protégée par mot de passe, lors du test de la requête:
...
<a name="prendre_compte_constante_hrequetesanscorrection_lors_test_requete_utilisant_une_connexion_ELTPARAGRAPHE000131"></a>
...
- *Requête de sélection*:
	...
	5. Sélectionnez dans la liste la connexion à utiliser. La liste présente les différentes connexions définies dans l'éditeur de schémas des données. Il est également possible de saisir directement le nom d'une connexion dynamique (définie par programmation grâce à la fonction [HDécritConnexion](../WDLang4/3044205.md)).
...
- *Requête d'insertion, de modification ou de suppression*: 
	...
	3. Sélectionnez dans la liste la connexion à utiliser. La liste présente les différentes connexions définies dans l'éditeur de schémas des données. Il est également possible de saisir directement le nom d'une connexion dynamique (définie par programmation grâce à la fonction [HDécritConnexion](../WDLang4/3044205.md)).
			**Attention**: Si vous utilisez une connexion dynamique, cette connexion doit être définie:
...
[Résultat d'une requête en mode test](../Editeurs/2032017.md)
