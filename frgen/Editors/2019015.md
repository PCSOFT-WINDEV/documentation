...
**Remarque**: Le test de l'état est disponible uniquement pour les états basés sur une requête, une table, un fichier texte et les états associés à aucune source de données. Pour les autres types d'états, il est nécessaire de tester l'élément (fenêtre, page ou projet) à partir duquel l'état est imprimé.**Logiciel Etats & Requêtes**
Lors du test d'un état sous [le logiciel Etats & Requêtes](../Presentation/3088004.md), l'événement d'initialisation du projet est exécuté.
...
## Comment le faire?
<a name="comment_faire_ELTTEXTE000237"></a>


### Tester l'état en cours
<a name="tester_etat_cours_ELTPARAGRAPHE000051"></a>

Pour tester l'état en cours: 

1. Cliquez sur ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=ICO_GO_Fenetre_WD_bl.gif) parmi les boutons d'accès rapide (touche F9).

2. Un écran s'affiche permettant de choisir le mode d'impression de l'état: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Go_Etat_Choix.gif)
...
Remarque: Il est possible de relancer le dernier test effectué (fenêtre, requête, état, ...) depuis n'importe quel élément du projet grâce à l'option "GO .. Relancer le dernier test" présente dans les boutons d'accès rapide. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=GO_Relancer_le_dernier_test%20-%20HC%20N%B0001.gif)

<a name="NOTE2_2"></a>


### Remarque : Iconiser ou non l'éditeur lors du test
<a name="remarque_iconiser_non_editeur_lors_test_ELTPARAGRAPHE000268"></a>Par défaut, lors d'un test, l'éditeur s'icônise. Selon la configuration matérielle (écran large, 2 écrans, ...), cette icônisation n'est pas utile. 
...
<a name="arreter_test_etat_ELTPARAGRAPHE000103"></a>
...
- **3ème méthode**: Revenez dans l'éditeur avec la barre des tâches ou avec la combinaison de touche Alt + Tab et cliquez sur l'icône ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Partie2_PileDesAppels%202.gif) (option "Terminer le test") présente dans le volet "Débogueur".
...
<a name="repertoire_des_tables_utiliser_test_ELTPARAGRAPHE000118"></a>

Pour indiquer les tables utilisées lors du test de l'application sous l'éditeur:
...
2. Dans l'onglet "Tables", sélectionnez l'emplacement des tables de test:
	...
	- ***Répertoire de la configuration courante ou répertoire courant*** (option sélectionnée par défaut): Les tables se trouveront dans le sous-répertoire correspondant à la configuration en cours (sous-répertoire du répertoire EXE du projet).
...
<a name="gestion_des_mots_passe_schema_des_donnees_table_ELTPARAGRAPHE000147"></a>

Lors du test d'un état lié à une table protégée par mot de passe, aucun mot de passe n'est demandé.
