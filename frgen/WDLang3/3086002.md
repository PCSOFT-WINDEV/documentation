...
- Les Webservices REST. Pour plus de détails, consultez [Créer un Webservice REST](../WDLang3/1000022792.md). 
...
<a name="comment_mettre_disposition_webservice_soap_ELTTEXTE000347"></a>
Pour mettre à disposition un Webservice SOAP, il suffit de:

1. [Créer un Webservice SOAP](#NOTE3_1). 

2. [Générer un Webservice SOAP](#NOTE4_1).

3. [Déployer le Webservice SOAP sur un Serveur d'Application WEBDEV](#NOTE5_1).




Remarque: Pour tester le Webservice SOAP, il est possible de le [déployer en local pour test](#NOTE5_2). 
...
<a name="creation_webservice_soap_ELTTEXTE000371"></a>
...
1. Créez un projet WINDEV ou WEBDEV de type Webservice SOAP ou une configuration de type Webservice SOAP dans un projet déjà existant.
	...
	- Lors de la création d'un projet, sélectionnez "Webservice SOAP ou REST". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_Creation_prj.gif&type=thumb)
...
2. Créez une ou plusieurs collections de procédures: 
	...
	- Dans le menu contextuel, sélectionnez "Nouveau".
			<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_Creation_Coll.gif)
...
<a name="gestion_des_contextes_ELTPARAGRAPHE000099"></a>
...
<a name="utilisation_une_base_donnees_hfsql_ELTPARAGRAPHE000144"></a>

Une webservice SOAP peut être lié à un schéma des données et manipuler des tables, par exemple via des requêtes. 

Les structures et les classes du webservice peuvent servir à décrire les tables. Quelques astuces sont à connaître: 

- Pour copier simplement un enregistrement d'une table HFSQL dans une structure ou un objet, utilisez la fonction [TableVersMémoire](../WDLang1/3013071.md). 

- Pour copier simplement le contenu d'une table HFSQL dans une structure ou un objet, utilisez la fonction [TableauRemplit](../WDLang1/1000018833.md). 

- Pour créer facilement les classes correspondant aux tables, utilisez le volet "Explorateur de projet":

	1. Sélectionnez la table voulue. 
...
<a name="generation_webservice_soap_ELTTEXTE000407"></a>
...
2. Générez le Webservice SOAP:  
	...
	- Cliquez sur l'icône de génération parmi les bouton d'accès rapide. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_Ico_generation.gif)
...
6. Sélectionnez les éléments à intégrer dans le Webservice. Ces éléments correspondent à tout sauf ce qui correspond à l'interface (fenêtre, page, état, etc.). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_Elements_integrer.gif)
...
8. Cochez les éléments accessibles. Les éléments accessibles correspondent aux procédures qui contiennent des fonctions visibles par l'application cliente qui appelle le Webservice. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_element_accessibles.gif)
...
<a name="deployer_tester_webservice_ELTTEXTE000431"></a>


### Déployer un Webservice
<a name="deployer_webservice_ELTPARAGRAPHE000230"></a>
...
<a name="cas_particulier_deploiement_test_webservice_sur_poste_local_ELTPARAGRAPHE000257"></a>
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_dep_local.gif)
...
Dans notre exemple, on trouve donc l'accès à la fonction "Addition": <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_test_webservice.gif)
...
Dans notre exemple, il faut passer deux nombres à la fonction "Addition": <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_saisie_parametres.gif)
La page de résultat générée sera: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WS_test_reponse.gif)
...
[AnnuleContexteWebservice (Fonction)](../WDLang3/1000019591.md)

[DéclareContexteWebservice (Fonction)](../WDLang3/1000019460.md)
