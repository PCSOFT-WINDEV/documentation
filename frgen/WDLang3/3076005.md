


## Fonctions de téléphonie
			



<a name="NOTE1"></a>
<a name="NOTE1_1"></a>
Ces fonctions permettent de manipuler facilement toutes les fonctions de téléphonie d'un modem depuis une application WINDEV ou WINDEV Mobile :



|   |   |
| --- | --- |
| [telAppelDébut](../WDLang3/3076006.md) | Renvoie la date et l'heure du début de l'appel (entrant ou sortant). |
| [telAppelEnAttente](../WDLang3/3076033.md) | Permet de savoir si l'appel est en attente. |
| [telAppelEstOccupé](../WDLang3/3076017.md) | Permet de savoir si le numéro appelé est occupé. |
| [telAppelEstTerminé](../WDLang3/3076003.md) | Permet de savoir si l'appel (sortant ou entrant) est terminé. |
| [telAppelFin](../WDLang3/3076002.md) | Renvoie la date et l'heure de la fin de l'appel. |
| [telAppelPasDeRéponse](../WDLang3/3076008.md) | Permet de savoir si une réponse a été donnée à l'appel. |
| [telArrête](../WDLang3/3076007.md) | Force l'arrêt de la lecture d'un message pré-enregistré (fonction [telJoue](../WDLang3/3076023.md)). |
| [telCapacité](../WDLang3/3076014.md) | Renvoie les caractéristiques d'un périphérique de téléphonie. |
| [telCompose](../WDLang3/3076013.md) | Compose un numéro de téléphone pour une prise de ligne vocale. |
| [telComposeLigne](../WDLang3/3076012.md) | Compose un numéro de téléphone pour une prise de ligne vocale, en choisissant le périphérique. |
| [telDémarreDétectionAppel](../WDLang3/3076029.md) | Démarre la détection d'appel (appelé aussi service de détection d'appel). |
| [telDuréeAppel](../WDLang3/3076004.md) | Renvoie la durée de l'appel (différence entre la date et l'heure du début de l'appel et la date et l'heure de fin de l'appel). |
| [telEnregistre](../WDLang3/3076019.md) | Enregistre la communication en cours sous forme d'un fichier ".WAV". |
| [telErreur](../WDLang3/3076020.md) | Indique si la dernière fonction telxxx ([fonctions de gestion des appels téléphoniques](../WDLang3/3076005.md)) a renvoyé une erreur dans le module TAPI. |
| [telExécuteTransfert](../WDLang3/3076021.md) | Effectue un transfert d'appel (avec possibilité de reprise de l'appel). |
| [telFinDétectionAppel](../WDLang3/3076022.md) | Termine la détection d'appel spécifié lancée par la fonction [telDémarreDétectionAppel](../WDLang3/3076029.md). |
| [telJoue](../WDLang3/3076023.md) | Joue un fichier sonore (.WAV) pour la ligne spécifiée. |
| [telListePériphérique](../WDLang3/3076024.md) | Liste les périphériques de téléphonie compatibles TAPI 2.0 et TAPI 3.1 installés sur le poste en cours. |
| [telMetAppelEnAttente](../WDLang3/3076034.md) | Met un appel en attente. |
| [telNuméroAppelant](../WDLang3/3076010.md) | Permet de connaître le numéro de téléphone appelant (qui appelle). |
| [telNuméroAppelé](../WDLang3/3076025.md) | Permet de connaître le numéro de téléphone appelé. |
| [telNumérote](../WDLang3/3076018.md) | Permet de simuler l'utilisation des touches du clavier téléphonique. |
| [telOrigine](../WDLang3/3076028.md) | Permet de connaître l'origine d'un appel (appel entrant, appel sortant, etc.). |
| [telPériphérique](../WDLang3/3076032.md) | Sélectionne le périphérique TAPI qui sera utilisé lors des opérations de téléphonie suivantes : <br><br>- [telCompose](../WDLang3/3076013.md),<br><br>- [telDémarreDétectionAppel](../WDLang3/3076029.md).<br><br><br> |
| [telRaccroche](../WDLang3/3076030.md) | Raccroche une ligne téléphonique ouverte avec la fonction [telCompose](../WDLang3/3076013.md) ou par la fonction [telRépondAppel](../WDLang3/3076026.md). |
| [telRépondAppel](../WDLang3/3076026.md) | Répond à un appel entrant détecté. |
| [telReprendAppelEnAttente](../WDLang3/3076035.md) | Reprend un appel en attente. |
| [telToucheEnfoncée](../WDLang3/3076027.md) | Permet de connaître l'historique des touches enfoncées sur le clavier téléphonique depuis la dernière utilisation de la fonction telToucheEnfoncée. |
| [telTransfertDirect](../WDLang3/3076011.md) | Effectue un transfert d'appel "en aveugle". |




![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png) Ces fonctions sont compatibles avec le protocole TAPI 2.0.




## Voir aussi

[WINDEV et la téléphonie](../WDLang3/3076009.md)

[Fonctions de gestion du téléphone](../WDLang3/1000019476.md)

[telDialerAffiche (Fonction)](../WDLang3/1000019236.md)

[telDialerCompose (Fonction)](../WDLang3/1000019235.md)

