
## Webservice REST: Création des points d'entrée
			

<a name="NOTE1"></a>
<a name="NOTE1_1"></a>


## Création des points d'entrée
<a name="creation_des_points_entree_ELTTEXTE000371"></a>
L'assistant de création des points d'entrée d'un Webservice REST est lancé: 

- lors de la création d'une nouvelle description de Webservices REST. 

- sous le volet "Webservice REST", dans le groupe "Points d'entrée", en cliquant sur "Nouveau". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0009%202.gif)

	Remarque: Le volet "Webservice REST" est disponible uniquement lorsqu'une description de Webservice REST est affichée sous l'éditeur. 




L'assistant de création d'un point d'entrée propose de sélectionner la méthode de création des points d'entrée:

- [Méthode 1: Création manuelle des points d'entrée](#REST_Methode1). 

- [Méthode 2: Création automatique des points d'entrée à partir de la description d'une table (présente dans le schéma des données)](#REST_Methode2).

- [Méthode 3: Création automatique des points d'entrée depuis les procédures d'une collection](#REST_Methode3). 




<a name="NOTE2"></a>
<a name="REST_Methode1"></a>


## Méthode 1: Création manuelle d'un point d'entrée
<a name="methode_1_creation_manuelle_point_entree_ELTTEXTE000395"></a>
Pour créer manuellement un point d'entrée: 

1. Dans l'assistant, sélectionnez l'option "Créer un point d'entrée". 

2. Passez à l'étape suivante.

3. Spécifiez la procédure globale WLangage associée au point d'entrée. Cette procédure peut être créée ou être sélectionnée parmi les procédures globales existantes. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0003.gif&type=thumb)


4. Passez à l'étape suivante. 

5. Choisissez la méthode HTTP représentant l'action à réaliser (GET, PUT, ...). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0004.gif&type=thumb)


6. Passez à l'étape suivante. 

7. Définissez la ressource (ou partie d'URL) correspondant au point d'entrée. Cette ressource peut comporter des éléments statiques ou des éléments variables (paramètres).<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0005.gif&type=thumb)
Pour plus de détails sur la ressource, consultez le paragraphe [Composition de la ressource](#NOTE6_1). 

8. Passez à l'étape suivante. 

9. Définissez le format de la requête et le format de la réponse. Le standard en REST est JSON mais d'autres formats sont disponibles. 

	- Le **format de la requête** indique comment sont formatés les données passées lors de l'appel REST. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0006.gif&type=thumb)
Ce format peut être sous la forme: 

		- Formulaire HTML,

		- JSON,

		- XML,

		- Texte (type MIME "Text/Plain"),

		- Binaire (pour des images par exemple),

		- Variable (le format est libre et doit être indiqué par programmation par la fonction [WebserviceLitTypeMIME](../WDLang3/1000022680.md)).




- **Désérialisation automatique**: Dans le cas d'une action de type POST, PUT ou PATCH, il est possible de désérialiser automatiquement les données passées en paramètre de la requête dans un paramètre de la procédure associée au point d'entrée: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0007.gif&type=thumb)

10. Passez à l'étape suivante. 

11. Saisir la description du point d'entrée (texte libre). 

12. Validez. 




La description des différents points d'entrée est affichée sous l'éditeur. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0009.gif&type=thumb)


<a name="NOTE3"></a>
<a name="REST_Methode2"></a>


## Méthode 2: Création automatique des points d'entrée d'accès à une table
<a name="methode_2_creation_automatique_des_points_entree_acces_une_table_ELTTEXTE000428"></a>
Pour créer automatiquement les points d'entrée et les procédures pour une table du schéma des données:  

1. Dans l'assistant, sélectionnez l'option "Créer les points d'entrée d'accès à une table". 

2. Sélectionnez la table présente dans le schéma des données sur laquelle vous souhaitez décrire les points d'entrée.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebService_Rest_fichier%20-%20HC%20N%B0003.gif&type=thumb)


3. Passez à l'étape suivante.

4. Sélectionnez les rubriques que vous souhaitez exposer dans le Webservice. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebService_Rest_fichier%20-%20HC%20N%B0004.gif&type=thumb)
Ces rubriques seront accessibles en lecture et/ou écriture au travers des points d'entrée du Webservice REST.

5. Passez à l'étape suivante.

6. Sélectionnez la clé qui permettra d'accéder à un enregistrement de la table pour le lire, le modifier ou le supprimer. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebService_Rest_fichier%20-%20HC%20N%B0005.gif&type=thumb)


7. Passez à l'étape suivante.

8. Sélectionnez le format utilisé pour l'échange des données lors de l'appel des points d'entrée du Webservice REST. 3 formats sont disponibles: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebService_Rest_fichier%20-%20HC%20N%B0006.gif&type=thumb)


	- JSON (UTF-8): Les données envoyées ou retournées sont au format JSON.

- JSON (UTF-8) (utilisation de la fonction [HEnregistrementVersJSON](../WDLang4/1000021909.md)): Les données sont formatées dans un objet intermédiaire créé à l'aide de la fonction [HEnregistrementVersJSON](../WDLang4/1000021909.md).

- XML: Les données envoyées ou retournées sont au format XML.

9. Passez à l'étape suivante.

10. Sélectionnez la liste des points d'entrée qui seront générés lors de la création du Webservice REST. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebService_Rest_fichier%20-%20HC%20N%B0007.gif&type=thumb)
Par défaut, tous les types de points d'entrée sont générés: 

	- Création: Ajout d'un enregistrement dans la table.

	- Lecture: Recherche et lecture d'un enregistrement de la table.

	- Modification: Mise à jour d'un enregistrement de la table.

	- Suppression: Suppression d'un enregistrement de la table.

	- Lecture de tous les enregistrements: Lecture de tous les enregistrements de la table.




11. Passez à l'étape suivante.

12. C'est terminé. L'assistant génère le code source en objet (POO) et le Webservice REST avec tous les points d'entrée.  La classe créée contient:

	- la déclaration des membres représentant les rubriques de l'enregistrement qui est manipulé (utilisation du MAPPING).

	- la déclaration des différentes méthodes représentant les différents points d'entrée du Webservice REST. Une méthode pour chaque point d'entrée du Webservice REST.

	- une méthode **sCréation** pour le point d'entrée de type Création.

	- une méthode **sLecture** pour le point d'entrée de type Lecture.

	- une méthode **sModification** pour le point d'entrée de type Modification.

	- une méthode **Suppression** pour le point d'entrée de type Suppression.

	- une méthode **sLectureTous** pour le point d'entrée de type Lecture de tous les enregistrements.







<a name="NOTE4"></a>
<a name="REST_Methode3"></a>


## Méthode 3: Création des points d'entrée depuis une classe ou une collection de procédures
<a name="methode_3_creation_des_points_entree_depuis_une_classe_une_collection_procedures_ELTTEXTE000452"></a>
Pour créer automatiquement les points d'entrée depuis une classe ou une collection de procédures:  

1. Dans l'assistant, sélectionnez l'option "Créer les points d'entrée depuis les procédures d'une collection". 

2. Sélectionnez dans la liste la collection de procédures ou la classe associée au point d'entrée. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebServiceRest_cr%E9er_proc%20-%20HC%20N%B0002.gif&type=thumb)


3. Passez à l'étape suivante. 

4. Sélectionnez les procédures et ou les méthodes qui seront exposées par le Webservice REST. La ressource associée à chaque procédure ou méthode sera automatiquement initialisée en fonction du nom de la procédure et de ses paramètres. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebServiceRest_cr%E9er_proc%20-%20HC%20N%B0003.gif&type=thumb)

	Pour chaque procédure sélectionnée, précisez la méthode HTTP correspondante. 

5. Passez à l'étape suivante. 

6. Définissez le format de la requête et le format de la réponse. Le standard en REST est JSON mais d'autres formats sont disponibles. 

	- Le **format de la requête** indique comment sont formatés les données passées lors de l'appel REST. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebServiceRest_cr%E9er_proc%20-%20HC%20N%B0005.gif&type=thumb)
Ce format peut être sous la forme: 

		- Formulaire HTML,

		- JSON,

		- XML,

		- Texte (type MIME "Text/Plain"),

		- Binaire (pour des images par exemple),

		- Variable (le format est libre et doit être indiqué par programmation par la fonction [WebserviceLitTypeMIME](../WDLang3/1000022680.md)).




- Le **format de la réponse** indique comment sont formatés les données récupérées en retour d'un appel REST. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=WebServiceRest_cr%E9er_proc%20-%20HC%20N%B0006.gif&type=thumb)
Ce format peut être sous la forme: 

	- Aucun,

	- JSON,

	- XML,

	- Texte (type MIME "Text/Plain"),

	- Binaire (pour des images par exemple),

	- Variable (le format est libre et doit être indiqué par programmation par la fonction [WebserviceEcritTypeMIME](../WDLang3/1000022681.md)).

	- Par morceau (chunked): Les données doivent être renvoyées avec la fonction [WebserviceEnvoieMorceau](../WDLang3/1410091271.md). Le format de la réponse doit être précisé par programmation par la fonction [WebserviceEcritTypeMIME](../WDLang3/1000022681.md).
						

7. Terminez l'assistant. 




La description des différents points d'entrée est affichée sous l'éditeur. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0009.gif&type=thumb)






<a name="NOTE6"></a>
<a name="NOTE6_1"></a>


## Remarques
<a name="remarques_ELTTEXTE000485"></a>


### Composition de la ressource (ou partie d'URL)
<a name="composition_ressource_partie_url_ELTPARAGRAPHE000254"></a>

La ressource est composée: 

- d'éléments statiques.

- d'éléments variables.

- de paramètres communs (optionnels).




Les **éléments statiques** représentent les objets manipulés. Par exemple, "/Clients" pour manipuler des clients. 
Dans l'assistant de création de la ressource, un élément statique correspond à une "Composante fixe". 

Les **éléments variables** identifient les ressources manipulées (un numéro de client, un numéro de commande, ...). Par exemple "/Clients/{NumCli}" permet d'indiquer le client manipulé via son numéro de client. 

- Dans l'assistant de création de l'URL, un élément variable correspond: 

	- soit à une composante variable de type paramètre de procédure. Cette composante est récupérée automatiquement dans les paramètres de la procédure WLangage associée. 

	- soit à une composante variable de type programmation, récupéré par programmation par la fonction [WebserviceParamètre](../WDLang3/1000022786.md).  




- Un élément variable est représenté par la syntaxe "{Nom composante variable}".
	Par exemple, "/Clients/10" permet de manipuler les clients ayant l'identifiant 10. 

- Il est habituel de passer les éléments variables dans l'URL lors d'un appel à une fonction REST. 

- Chaque composante variable de type programmation, dans la ressource (URL) du point d'entrée correspond à un paramètre passé à la procédure WLangage associée au point d'entrée.

- Un élément variable est représenté par un nom lors de la définition de l'adresse du point d'entrée dans l'éditeur. **Dans le cas d'une composante variable de type programmation, ce nom doit être identique au nom du paramètre dans la procédure WLangage associé au point d'entrée**. 
	Au moment de l'exécution, la valeur passée dans la ressource (URL) sera récupérée et mise dans la variable. Ce nom est sensible à la casse. Il est donc très important d'utiliser le même nom dans la procédure et dans la ressource (ou inversement) sinon une erreur d'exécution est générée.

<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice-REST.gif)
Il est possible de combiner plusieurs éléments statiques et plusieurs éléments variables. Par exemple:

- "/Clients/{idClient}/Factures" pour récupérer les factures d'un client identifié par son idClient. 

- "/Fournisseurs/{idFour}/Produits/{RefProd}" pour récupérer le produit identifié par "RefProd" du fournisseur identifié par "idFour". 





Pour modifier les paramètres communs, sous le volet "Webservice REST", dans le groupe "Options", cliquez sur "Description". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=213&name=Webservice_REST%20-%20HC%20N%B0010.gif)


La fenêtre de description du Webservice REST (onglet "Composantes communes") permet d'avoir une partie de la ressource identique à tous les points d'entrée. Il n'est donc pas nécessaire de copier cette partie de la ressource systématiquement dans chaque point d'entrée. La modification est réalisée à un seul endroit.

**Astuce**: Il est possible d'utiliser la définition des paramètres communs pour gérer différentes versions du Webservice REST. Par exemple, un paramètre commun nommé "V1" peut être utilisé. Lors de la création d'une évolution ou d'une version 2 du Webservice, il suffit de changer "V1" en "V2" et ainsi tous les points d'entrée ont leur ressource (URL) modifiée. 
<a name="NOTE6_2"></a>


### Gestion des codes retour dans le code des procédures
<a name="gestion_des_codes_retour_dans_code_des_procedures_ELTPARAGRAPHE000303"></a>

Le code des procédures WLangage utilisées pour les Webservices REST doivent gérer des comptes-rendus indiquant si la fonction s'est exécutée correctement ou non.

Par convention, dans un Webservice REST, les comptes-rendus d'exécution sont retournés en utilisant les codes retour HTTP. Les codes retour HTTP sont classés par famille: 

- Les codes 2xx correspondent à une action réalisée avec succès.

- Les codes 3xx correspondent à des erreurs de redirection.

- Les codes 4xx correspondent à des erreurs du client (méthode HTTP incorrecte, format de donnée envoyé incorrect, etc.).

- Les codes 5xx correspondent à des erreurs du serveur (erreur interne à la procédure WLangage, etc).


Le code de retour est renvoyé à l'aide de la fonction [WebserviceEcritCodeHTTP](../WDLang3/1000022785.md).

Exemples de codes retour utilisés:

- Demande de création d'un client via la méthode HTTP POST. Cette demande a réussi: il faut renvoyer le code 201.

- Demande de suppression d'un client via la méthode DELETE. Cette demande a échoué à cause d'une erreur d'intégrité retournée par la base de données: il faut renvoyer le code 500 avec un message complémentaire d'explication. 

- Demande de recherche d'un produit en passant en paramètre une référence via la méthode GET. Cette demande a réussi: il faut renvoyer le code 200.

- Demande de recherche d'un produit en passant en paramètre une référence via la méthode GET. Cette demande a échoué car la référence est inexistante: il faut renvoyer le code 404.





## Voir aussi

[Webservice REST : Appeler une fonction du Webservice](../WDLang3/1410091009.md)

[Webservice REST: Créer un Webservice REST](../WDLang3/1000022792.md)

[Webservice REST : Description et caractéristiques](../WDLang3/1410090999.md)

[Webservices REST : Désérialisation automatique de la requête](../WDLang3/1000022796.md)

[Webservice REST sécurisé](../WDLang3/1410090997.md)

[Webservice REST : Tester et déployer](../WDLang3/1410090998.md)

[WDTestRest : Testez vos Webservices REST](../WDLang3/1000022795.md)

[Webservice REST : Importation d'une API OpenAPI](../WDLang3/1410087809.md)

[WebserviceEnvoieMorceau (Fonction)](../WDLang3/1410091271.md)

[httpRequête (Type de variable)](../WDLang3/1000021158.md)

