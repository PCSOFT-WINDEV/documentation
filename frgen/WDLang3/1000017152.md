
## OBEXRécupèreFichier (Fonction)

***En anglais : OBEXGetFile***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Récupère un fichier sur un périphérique gérant le protocole OBEX.

**Remarque** : Cette fonction ne peut pas être utilisée en cas de transfert par infrarouge.
<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe

`<Résultat> = OBEXRécupèreFichier(<Identifiant de la connexion OBEX> , <Chemin fichier distant> [, <Chemin fichier local>])`
---

**`<Résultat> : (Booléen)`**



- <u><u><u><u>Vrai</u></u></u></u> si le transfert a réussi, 

- <u><u><u><u>Faux</u></u></u></u> dans le cas contraire. La fonction [ErreurInfo](../WDLang1/3013008.md) permet d'obtenir plus de renseignements sur le problème rencontré.




**`<Identifiant de la connexion OBEX> : (Entier)`**

Identifiant de la connexion OBEX à utiliser. Cet identifiant est retourné par la fonction [OBEXConnecte](../WDLang3/1000017153.md).

**`<Chemin fichier distant> : (Chaîne de caractères)`**

Nom et chemin du fichier à récupérer sur le périphérique distant.

**Remarque** : Sur un Mobile, il est possible d'indiquer un répertoire dans les paramètres Bluetooth. Ce répertoire correspond à la racine de l'arborescence OBEX (c'est-à-dire le répertoire "\\" donné par la fonction [OBEXListeFichier](../WDLang3/1000017148.md)).

**`<Chemin fichier local> : (Chaîne de caractères optionnelle)`**

Chemin complet où le fichier sera copié. Si ce paramètre n'est pas précisé, le fichier est créé sous le même nom dans le répertoire courant.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques
Seules les piles bluetooth Microsoft sont supportées par cette fonction. Pour connaître la pile utilisée, utilisez la fonction [BTPile](../WDLang3/1000017229.md).

Pour plus de détails sur les clés supportées, consultez [Quelles piles utiliser ?](../WDLang3/1000017231.md)



<a name="XComposante"></a>

## Composante:
wd310com.dll
## Voir aussi

[Gestion des clés bluetooth](../WDLang3/1000017231.md)

[Fonctions OBEX](../WDLang3/1000017175.md)

[Fonctions Bluetooth](../WDLang3/1000017173.md)

