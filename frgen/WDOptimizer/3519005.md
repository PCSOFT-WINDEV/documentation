...
| --- | --- |
| /Fic=&lt;Répertoire&gt; | Pour traiter une seule table: chemin complet de la table à traiter.<br>Pour traiter un ensemble de tables:<br><br>- chemin d'un répertoire, <br><br>- Fichier INI au format d'export généré par WDOptimizer. <br><br><br>Pour traiter des transactions: chemin complet du fichier de transaction.<br>**Exemple**: c:\\temp\\fichier.fic |
| /Mdp=&lt;Mot de passe&gt; | Mot de passe associé à la table à traiter (traitement d'une seule table) |
| /Option=&lt;Type d'action à réaliser&gt; | Numéro correspondant à l'option de WDOptimizer à lancer:<br>1: Vérification de l'index.<br>2: Optimiser la vitesse des index (Recalculer les statistiques sur les tables).<br>3: Reconstruire les index.<br>4: Reconstruire les index et les mémos.<br>5: Réviser et compresser les index et les mémos. |
...
| /Sauve=&lt;Non&gt; | Par défaut, les tables à traiter sont sauvegardées (uniquement prises en compte si /option = 5). Non permet de ne pas faire cette sauvegarde. |
...
La ligne de commande suivante permet de lancer la ré-indexation des tables présentes dans le répertoire "C:\\MonAppli\\Données".
...
<a name="exemple_procedure_reindexation_ELTPARAGRAPHE000181"></a>
...
// "La réindexation de la table 'C:\.....\MonFic.Fic' s'est déroulée correctement".
...
// - Requis: 
//   - Les tables peuvent être délocalisées par rapport à l'exe 
// (Variables globales gsNomAppli et gsRepData)
//  - WDOptimizer est toujours à côté de l'exécutable
//  - Les tables peuvent avoir un nom physique différent du nom logique 
...
SI _bSauvegarde = Vrai ALORS sSauvegarde = "Vrai"
	...
	TANTQUE sNomFichier <> EOT 
		...
		SI fRep(gsRepData + sNomPhysFichier, frFichier) <> "" ALORS 
			// Prise en compte des mots de passe de certaines tables protégées  
	...
	sNomFichier  = ExtraitChaîne(sListFichier, i, RC)
FIN
Info("Optimisation des tables de '" + gsNomAppli + "' terminée")
```
