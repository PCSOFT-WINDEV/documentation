
## Input suggestions in an Edit control
			

<a name="NOTE1"></a>
<a name="NOTE1_1"></a>


## Overview
<a name="overview_ELTTEXTE000265"></a>
The principle of assisted input in an Edit control field is very simple: when the user starts typing in an Edit control field, a list automatically scrolls below the field to propose the values starting with the current input text. 

Input suggestions can be implemented in the editor or be customized programmatically. 



<a name="NOTE2_Fenêtre"></a>
<a name="NOTE2_Fenêtre_1"></a>


## Principle
<a name="principle_ELTTEXTE000289"></a>
WINDEV, WINDEV Mobile and WEBDEV include several types of input suggestions: 

- Assisted input "in memory": The input field is not linked to a field in a table. Input suggestions are based on a list of data created programmatically (using [AssistedInputAdd](../WDLang1/1000020043.md)). The data that appears in the list of suggestions is based on what the user types.  

- Assisted "file" input: The Edit control is linked to a field in a table or query. The data that appears in the list of suggestions is based on what the user types.

- Programmed assisted input: Assisted input is based on a customized data list. The list shows custom suggestions based on user input.




![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) Please note: In this version, assisted entry is not available in simulator mode.. 

<a name="NOTE3_Fenêtre"></a>
<a name="NOTE3_Fenêtre_1"></a>


## In-memory input suggestions
<a name="inmemory_input_suggestions_ELTTEXTE000313"></a>


### How to?
<a name="how_ELTPARAGRAPHE000058"></a>

To implement in-memory input suggestions: 

1. Use [AssistedInputAdd](../WDLang1/1000020043.md) to add the elements that will appear in the input suggestions. 
	You can add: 

	- a simple value, which will appear in the list of input suggestions,

	- one value that will be displayed and one value that will be stored.

	- a variable of type [AssistedInput](../WDLang1/1410088983.md).




2. If required, use the [AssistedInputConfigure](../WDLang1/1000021074.md) function to set assisted input parameters: filter, opening of proposal list, number of characters to be entered before opening proposal list, etc.




**Note** The memorized value is used to associate a specific value with a line of the assisted entry. You can use this value to perform a specific process according to the type of value selected.


### Examples
<a name="examples_ELTPARAGRAPHE000091"></a>

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) **Example for a window**: 
	
	```wl
	// End of window initialization
	AssistedInputConfigure(EDT_Find, aiMinSize, 3)
	AssistedInputConfigure(EDT_Find, aiFilter, filterContains)
	
	FOR EACH STRING sTitle OF gsPhotoTitles SEPARATED BY CR
		AssistedInputAdd(EDT_Find, sTitle)
	END
	```


- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![WEBDEV - Browser code](https://doc.pcsoft.fr/ext/images/us/WBN.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png) **Example for a page**: 
	
	```wl
	// Load code of the page
	AssistedInputConfigure(EDT_Find, aiMinSize, 3)
	AssistedInputConfigure(EDT_Find, aiFilter, filterContains)
	
	FOR EACH STRING sTitle OF gsPhotoTitles SEPARATED BY CR
		AssistedInputAdd(EDT_Find, sTitle)
	END
	```






### Remarks
<a name="remarks_ELTPARAGRAPHE000119"></a>

- The optional "Select a value in the list of input suggestions" event is executed when an element is selected. This event takes a variable of type [AssistedInput](../WDLang1/1410088983.md) as parameter. This variable allows you to get the selected value and the associated stored value, if any.

- By default, the value displayed in the Edit control is the value is selected in the input suggestions. To change the value in the Edit control, use the "Select a value in the list of input suggestions" event. You can choose to display the stored value instead, for example. 


For more details, see [Edit control events](../WDChamp/1013229.md).





<a name="NOTE4_Fenêtre"></a>
<a name="NOTE4_Fenêtre_1"></a>


## Input suggestions based on a table
<a name="input_suggestions_based_table_ELTTEXTE000349"></a>


### How to?
<a name="how_ELTPARAGRAPHE000140"></a>

To implement input suggestions based on a table:

1. In the edit control description window: 

	- Check whether the Edit control is bound to an item in a table or query ("Binding" tab). 
			Warning: The item must be a key. 

	- On the "Details" tab, check "Assisted input based on data binding". 




2. Validate the Edit control description window. 




At runtime, as soon as the first letter is typed in the control: 

- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) a list appears with different results that match the letter typed. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Saisie_Ass_Fic_WB.gif)


- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) the first result that matches the entered character is displayed. The matching data changes as you type. 




Remarks: 

- If the Edit control is bound to a query item, the query is automatically executed. However, if the query expects parameters, you must specify them (e.g., in the "Global declarations" or "End of initialization" events of the window) and then execute the query with [HExecuteQuery](../WDLang4/3044080.md).  

- Input suggestions are shown using a "Starts with" filter. To use another filter, you must define input suggestions programmatically. 




<a name="NOTE5_Fenêtre"></a>
<a name="NOTE5_Fenêtre_1"></a>


## Input suggestions defined programmatically
<a name="input_suggestions_defined_programmatically_ELTTEXTE000373"></a>


### How to?
<a name="how_ELTPARAGRAPHE000181"></a>

By defining input suggestions programmatically, you can use a custom data source. 

To use input suggestions defined programmatically: 

1. Disable the default filter with [AssistedInputConfigure](../WDLang1/1000021074.md) (using the *filterNone* constant). Thus, the list of input suggestions can be opened and display all the specified elements.  

2. In the "Whenever modifying" event of the Edit control: 

	- Clear the list of suggestions ([AssistedInputDeleteAll](../WDLang1/1000020044.md)). 

	- Rebuild the list of suggested elements based on the letters typed ([AssistedInputAdd](../WDLang1/1000020043.md)).

	- Use [AssistedInputOpen](../WDLang1/1000020048.md) to open the list of input suggestions.








### Example
<a name="example_ELTPARAGRAPHE000209"></a>


```wl
// "Contains" search performed in the name 
// and code of an array of stock shares

FOR EACH sName, sCode of gArrShare
	IF Contains(sCode, EDT_Programmed_mode) _OR_ ...
			Contains(sName, EDT_Programmed_mode) THEN
		AssistedInputAdd(WIN_InMemory.EDT_Programmed_mode, sName)
	END
END
```





## Voir aussi

[AssistedInputAdd (Function)](../WDLang1/1000020043.md)

[AssistedInputClose (Function)](../WDLang1/1000020049.md)

[AssistedInputOpen (Function)](../WDLang1/1000020048.md)

[AssistedInputConfigure (Function)](../WDLang1/1000021074.md)

[AssistedInputDeleteAll (Function)](../WDLang1/1000020044.md)

[Edit control](../WDLang1/1013145.md)

[Control functions](../WDLang1/3025007.md)

[Input suggestions in an Edit control (prefix syntax)](../WDLang1/1000024066.md)

[Input suggestion functions](../WDLang1/1000024067.md)

[AssistedInput (Variable type)](../WDLang1/1410088983.md)

[Events associated with Edit controls](../WDLang1/1013229.md)

