...
No matter how dexterous a development team may be, the end-user of an application will always want to perform an action that was not intended.

To enable advanced users to realize their desires immediately, **WINDEV offers MCU mode** mode: end-users can enter WLanguage code and "graft" it onto an existing application.

This feature is not available by default, but can be switched on or off at any time.

**Warning** Modifying an application by the end user may cause the application to malfunction. This feature must be used carefully.
...
**Note: Where are User Macro-Codes stored?**
...
2. On the "Advanced" tab, click the "User Macro-Code" button. 

<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MCU_Projet.gif)
In this screen:
...
- If the input of UMC is allowed on the project windows, you can:
	...
	- ***Allow the use of HFSQL read commands***. If this option is checked, the end user will be able to read and browse the records found in the HFSQL tables of the application. In any case, the end user will not be able to modify or create new records or new tables.
...
To do so, check "Forbid the use of UMC" in the "Details" tab of the window description.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MCU_Fenetre.GIF&type=thumb)
...
The input of new User Macro-Codes in an application is child's play. If the window allows you to enter a User Macro-Code, the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MCU_Icone.gif) icon is displayed beside the minimize icon of the window. Clicking this icon opens a wizard, allowing the end user to create and use their own User Macro-Codes.
...
3. Select the User Macro-Codes to import.
	The ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MCU_Import_diff.gif) button allows you to see the modifications performed by the user in relation to the existing code.
