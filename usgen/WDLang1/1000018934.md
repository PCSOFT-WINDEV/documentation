...
For example, an application used to configure a service can check whether the service is properly started by doing the following: simply check that the service created a specific shared memory area. If the zone does not exist, an error message can be displayed by the application. 
...
### Handling the content of a shared memory area programmatically
<a name="handling_the_content_shared_memory_area_programmatically_ELTPARAGRAPHE000056"></a>
...
Note: If the two applications want to exchange data, two semaphores must be used to ensure regulation.

**Tip** Manual synchronization is preferred when: 
...
Note: Shared memory zone names are case sensitive under both Windows and Linux.
<a name="NOTE4_2"></a>
...
- Linux, Windows 2000 and earlier: there is only one memory area creation space. The &lt;Share&gt; parameter of [fMemOpen](../WDLang1/1000018917.md) is ignored.

- Windows XP: the &lt;Partage&gt; of the [fMemOpen](../WDLang1/1000018917.md) function is effective if the quick user change service is active, otherwise it is ignored.

- Windows Vista and later: the &lt;Partage&gt; of the [fMemOpen](../WDLang1/1000018917.md) function is managed. The services and the users are located in a different space. To share a memory area between a service and an application in a user session, the *shareGlobal* constant must be used.

- Terminal Server: the &lt;Partage&gt; of function [fMemOpen](../WDLang1/1000018917.md) is managed. Each session opened on the Terminal Server has a different memory space. Applications running in the same TSE session can share a memory area created with the *shareUser* constant. Several applications located in different sessions can share a memory area created by the *shareGlobal* constant.
...
[fMemWait (Function)](../WDLang1/1000018915.md)

[fMemExist (Function)](../WDLang1/1000018916.md)

[fMemOpen (Function)](../WDLang1/1000018917.md)
...
[Shared memory area constants](../WDLang1/1000018933.md)
