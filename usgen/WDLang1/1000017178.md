...
<a name="limits_maximum_number_columns_displayed_ELTPARAGRAPHE000116"></a>
...
<a name="databound_table_control_changes_the_linked_table_ELTPARAGRAPHE000127"></a>
...
If the cascading input is not enabled, the table linked to the data-bound Table control is not modified. To write the record into the table, [TableSave](../WDLang1/3074008.md) must be run after **TableInsertColumn**.
...
<a name="databound_table_control_insert_multiple_columns_ELTPARAGRAPHE000148"></a>

A virtual column is created each time **TableInsertColumn** is called in a data-bound Table control based on a table. The information will be saved in the table during the call to [TableSave](../WDLang1/3074008.md).

If **TableInsertColumn** is successively called in the same process without being followed by [TableSave](../WDLang1/3074008.md), only the FIRST call is effective (the following calls are ignored). Only one column is added to the data-bound Table control based on a table.
<a name="NOTE0_6"></a>
...
[&lt;Table&gt;.InsertColumn (Function)](../WDLang1/1000024261.md)
