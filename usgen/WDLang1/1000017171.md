


## ChangeStyle (Function)

***In french: ChangeStyle***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Modifies the style of a control dynamically.
<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Modify the style of EDT_Edit1 control
ChangeStyle(EDT_Edit1, "Edit_No_caption__Display")
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = ChangeStyle(<Control used> , <Style to apply> [, <Theme to apply>])`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the style was changed,

- <u><u><u><u>False</u></u></u></u> if an error occurs.




**`<Control used>: (Control name)`**

Name of control whose style must be modified.

**`<Style to apply>: (Character string)`**

Name of style defined in edit. This style must be applicable to the specified type of control.

**`<Theme to apply>: (Optional character string)`**

Name of the theme to be applied (name and path of the "\*.WDY" file, if necessary). The extension must not be specified. 

If this parameter corresponds to an empty string (""):

- the theme defined in the window editor will be applied by default. **Please note** If a theme has been dynamically applied using function [ChangeSkinTemplate](../WDLang1/3013026.md), this dynamic theme will be retained.

- the windows already opened will keep the current theme.




If this parameter is not specified, the theme file (\*.WDY) associated with the current project will be used.  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### ".WDY" file
<a name="wdy_file_ELTPARAGRAPHE000053"></a>

The description of a theme is stored in a ".WDY" file. The ".WDY" files corresponding to the themes proposed by WINDEV are provided with WINDEV.

To apply a custom style or a project style, the following syntax must be used: 

```txt
ChangeStyle(<Control>, <CustomStyle>, <ProjectName.wdy>)
```
 Except if it was renamed, the .WDY containing the project styles is named like the project itself. 

Remarks:

- When creating a theme, the ".WDY" file is automatically created.

- To generate the ".WDY" file of an existing theme:

	- Open the "&lt;NomDuGabarit&gt;.STY" file in WINDEV: in the "Home" pane, in the "General" group, click on "Open". 

	- Save this file: under "Home", in the "General" group, click on "Save".






<a name="NOTE0_2"></a>


### Deploying an application
<a name="deploying_application_ELTPARAGRAPHE000097"></a>

When creating the setup program of your application, the following elements must be added into the list of elements to install:

- the ".WDY" file of the project.

- the ".WDY" file corresponding to the new theme applied by **ChangeStyle**.

- the image files used by the new theme applied by **ChangeStyle**. Please note: images used by different themes must have different names, otherwise they will not be modified.




**Note**: To add one or more items to the list of project items:

1. On the "Project" tab, in the "Project" group, expand "List of elements".

2. Select "List of project elements".

3. Click the "Add" button.

4. Select the files to add and validate.



<a name="NOTE0_3"></a>


### Miscellaneous
<a name="miscellaneous_ELTPARAGRAPHE000131"></a>

- You also have the ability to copy the style of a control to another one via [Style](../Proprietes/1000021472.md). 

- When changing style, the positioning of captions is not modified. Indeed, changing the positioning of captions could trigger modifications regarding the size of the control. The control could come into conflict with other controls. 

- Button controls: 

	- If the style includes an image, this image overwrites the initial image of the button. 

	- If the style includes no image, the initial image of the button is kept.







<a name="XComposante"></a>

## Component:
wd310obj.dll
## Voir aussi

[ChangeSkinTemplate (Function)](../WDLang1/3013026.md)

[Control functions](../WDLang1/3025007.md)

[Style (Property)](../WDLang1/1000021472.md)

