...
|   |   |
| --- | --- |
| *DDEActive*<br>(Default value) | The launched program is active: it runs and "takes over" the current program |
| *DDEIconize* | The launched program is inactive: it runs reduced to an Icon |
| *DDEInactive* | The launched program is inactive: it runs while the current program maintains focus |
| *DDEMaximize* | The launched program is active: it runs by "taking over" the current program (the execution window has the maximum authorized size) |
...
- <u><u><u><u>True</u></u></u></u>: the current program resumes execution when the launched program is completed.

- <u><u><u><u>False</u></u></u></u> (default): the current program and the launched program run in parallel.
...
[DDEError (Function)](../WDLang1/3028006.md)
