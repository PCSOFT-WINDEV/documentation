...
<a name="updating_the_table_linked_the_table_treeview_table_control_ELTPARAGRAPHE000281"></a>

**&lt;Table&gt;.Save** saves the current control row in the table linked to the control. The items are automatically assigned with the values of associated columns in the current row.
...
- **&lt;Table&gt;.Save** only manages the table associated with the control. If some columns are linked to items found in other tables, these tables are not automatically managed.
...
- In the Check Box columns, the call to **&lt;Table&gt;.Save** is implicit and it cannot be disabled. In any case, the modification of the value of the check box will be applied to the table.
...
<a name="cascading_input_and_update_linked_table_ELTPARAGRAPHE000314"></a>
...
- **If the cascading input is not enabled**, the table linked to the Table or TreeView data-bound Table control is not modified. To write the added or modified row into the linked table, [&lt;Table&gt;.Save](../WDLang1/1000024249.md) must be used after the addition or modification function.
...
<a name="locking_and_updating_the_table_ELTPARAGRAPHE000416"></a>

If the table is locked in the current process, **&lt;Table&gt;.Save** saves the current row and unlocks the table.
<a name="NOTE0_6"></a>
...
[&lt;Source&gt;.Add (Function)](../WDLang1/1000024187.md)
