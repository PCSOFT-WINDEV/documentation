...
The general parameters of the check must be defined when creating or modifying a check. These parameters are used to define:  <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Robot_Cr%E9er%20un%20controle%20-%20HC%20N%B0003.gif&type=thumb)
...
Note: The control's context menu can be used to simply deactivate a control ("Deactivate control" option).. This option disables the check: 
...
|   |
| --- |
| 2 Network alert(s) \ Highest priority: Normal<br>[----------------------------------------------------------------------]<br>Priority: Normal<br>Category: FTP<br>Control: FTP Etats Et Requetes (xxxx) (n° 38)<br>Error:<br>Error connecting to the FTP server<br>Failure connecting to the FTP server "ftp.pcsoft.com" on port 21 with the username.<br>The last server response is:<br>Solution:<br><br>Internal details:<br>- Syntax: <br>FTP server address: ftp.pcsoft.com<br>Connection port: 21<br>Passive mode connection: True<br>File to Seek: /wx11/latest/WD2026PACKER.exe<br>Max users for check: 0<br>Download file: False<br> |
