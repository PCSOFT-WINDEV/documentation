...
- The WLanguage code typed must necessarily return <u><u><u><u>True</u></u></u></u> at runtime to be considered as being valid.
...
- The monitoring robot supports the Unicode strings at runtime. Therefore, all the strings declared in the WLanguage code are [Unicode strings](../Motscles/1514044.md) by default. When a check must exchange data with another process (via sockets, exchange files, ...), it may be necessary to declare the strings in ANSI if the other process is compiled to support strings in ANSI format. It is then necessary to specify that the string is an ANSI string (declaration of type: MyString is an Ansi string).. 
