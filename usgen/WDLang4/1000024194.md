


## &lt;Source&gt;.LinkMemo (Function)

***In french: &lt;Source&gt;.AttacheMémo***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Used to associate a file with a binary memo item or to cancel the existing link between a file and a binary item.

The file will be read and added to the memo only when the record is added or modified ([&lt;Source&gt;.Add](../WDLang4/1000024187.md), [&lt;Source&gt;.Modify](../WDLang4/1000025038.md) or [&lt;Source&gt;.Write](../WDLang4/1000024331.md)). The function will return an error if the file is opened in write mode and if the *hAcceptOpenedFile* constant was not used.

[&lt;Source&gt;.ExtractMemo](../WDLang4/1000024957.md) will be used to extract the file contained in the memo onto the hard disk.

**Note**: Direct assignment is also possible: 

- To associate an image (found in an Image control) with an image memo: 
	
	```txt
	<FileName>.<NameMemoItem> = <NameImageControl>
	```

	For example: 
	
	```wl
	AFile.AMemoItem = IMG_AnImageControl
	```

	This syntax can be used with both the HFSQL engine and Native Connectors.
	
	```txt
	<FileName>.<NameMemoItem> = <BufferName>
	```

	For example: 
	
	```wl
	bufFile is Buffer = fLoadBuffer("MyFile")
	MyFile.MyMemo = bufFile
	```






<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Attach an image to the record
Product.LinkMemo(Product_Img, "C:\Images\Product1.bmp", hMemoImg)
// Transfers field contents to the table buffer
MyWindow.ToFile()
// Add the record
Product.Add()
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Associating a file with a binary memo item

`<Result> = <Source>.LinkMemo(<Item> [, <File to attach> [, <Type> [, <Information>]]])`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the operation was performed, 

- <u><u><u><u>False</u></u></u></u> if a problem occurs. [HError](../WDLang4/3044088.md) is used to identify the error. The function will return an error if the file to associate is opened in write mode and if the *hAcceptOpenedFile* constant was not used.




**`<Source>: (Type corresponding to the specified source)`**

Name of the table (or query) containing a memo item. 
If only &lt;Source&gt; and &lt;Item&gt; are specified, **&lt;Source&gt;.LinkMemo** cancels the existing link between the binary file and the item.

**`<Item>: (Character string)`**

Name of the memo item of the table into which the binary file must be added. 
If only &lt;Source&gt; and &lt;Item&gt; are specified, **&lt;Source&gt;.LinkMemo** cancels the existing link between the binary file and the item.

**`<File to attach>: (Optional character string)`**

Name of the file that will be associated with the binary memo item. This name can correspond to: 

- the full path of the file to attach, 

- a relative path (respective to the current directory) of the file to attach. 




If this parameter is not specified or is an empty string (""), the current attachment is deleted..
![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) If a path is specified, this path must correspond to an existing file. Otherwise, a fatal error is displayed.

**`<Type>: (Optional combination of Integer constants)`**

Indicates the type of file to attach and its read mode.


|   |   |
| --- | --- |
| *hAcceptOpenedFile* | Allows you to read the source file that will be added to the memo if this source file is opened in write mode somewhere else.<br>This constant can be used with the constants for file type. |
| *hMemoBin*<br>(default value) | The file is an ordinary binary file. |
| *hMemoImg* | The file is an image file. |
| *hMemoOle* | The file is an OLE file. |
| *hMemoWav* | The file is a sound file. |


![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) This parameter is ignored.![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) This parameter is available for the tables in HFSQL format only. 

**`<Information>: (Optional character string)`**

Information message (up to 65535 characters) stored with the file. This message can be read again by [&lt;Source&gt;.InfoMemo](../WDLang4/1000024997.md).

![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) This parameter is ignored.![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) This parameter is available for the tables in HFSQL format only. 



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Canceling the existing link
<a name="canceling_the_existing_link_ELTPARAGRAPHE000349"></a>

If only &lt;Data file&gt; and &lt;Item&gt; are specified, **&lt;Source&gt;.LinkMemo** cancels the existing link between the binary file and the item.
<a name="NOTE0_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Stored procedures](https://doc.pcsoft.fr/ext/images/us/PS.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png)![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) 

### Difference between  BinaryLoad and &lt;Source&gt;.LinkMemo
<a name="difference_between_binaryload_and_docparampagetitleshort_ELTPARAGRAPHE000363"></a>

Unlike **BinaryLoad** (kept for compatibility with WINDEV 5.5), **&lt;Source&gt;.LinkMemo** does not load the binary file in memory. Therefore, there is no risk of memory overload when a large file is associated with a binary memo item.
<a name="NOTE0_4"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png)![IOS Widget](https://doc.pcsoft.fr/ext/images/us/WIDGETIOS.png)![Mac Catalyst](https://doc.pcsoft.fr/ext/images/us/CATALYST.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![External language](https://doc.pcsoft.fr/ext/images/us/LE.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Stored procedures](https://doc.pcsoft.fr/ext/images/us/PS.png) 

### HFSQL thumbnails
<a name="hfsql_thumbnails_ELTPARAGRAPHE000378"></a>

If the item uses HFSQL thumbnails, they will be automatically generated with the new file when the record is added or modified ([&lt;Source&gt;.Add](../WDLang4/1000024187.md), [&lt;Source&gt;.Modify](../WDLang4/1000025038.md) or [&lt;Source&gt;.Write](../WDLang4/1000024331.md)).
<a name="NOTE0_5"></a>
![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) 

### Managing memos in SQLite
<a name="managing_memos_sqlite_ELTPARAGRAPHE000405"></a>

Storing information in memo format is recommended for the data whose size is less than 1 MB. We advise you to directly save the data in external file format on the internal or external memory of the phone and to store the access path to this data in the database.  

<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[&lt;Source&gt;.ExtractMemo (Function)](../WDLang4/1000024957.md)

[&lt;Source&gt;.InfoMemo (Function)](../WDLang4/1000024997.md)

[HFSQL functions (prefix syntax)](../WDLang4/1000025085.md)

[How to save and retrieve an image in a binary memo](../WDLang4/20048.md)

[HLinkMemo (Function)](../WDLang4/3044173.md)

