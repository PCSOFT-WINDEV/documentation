...
**Reminder** A trigger is a WLanguage procedure automatically called by the HFSQL engine each time an HFSQL function is executed. A trigger is defined by [HDescribeTrigger](../WDLang4/3044074.md).
...
`<Result> = HActivateTrigger([<List of HFSQL tables> [, <List of HFSQL functions>]] [, <Type>])`
---
...
To specify several names, the names of the tables must be separated by a comma (","). To use all the tables found in the database schema, this parameter must be equal to "\*". 
