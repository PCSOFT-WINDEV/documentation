


## HDescribeItem (Function)

***In french: HDécritRubrique***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Describes a file item programmatically.

After the call to **HDescribeItem**:

- The item can be handled like any other item described in the database schema.

- The item description cannot be modified anymore.

- The description variable of item is reinitialized. A new item description can be performed.





<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example
<a class="notetitle" target="_blank" href="$DOC$=1000003044233&name=hdescribeitem_function">See additional examples</a>

```wl
HDescribeItem(CustomerBis, CustomerName)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Describing a table item created programmatically

`<Result> = HDescribeItem(<Table> , <Item>)`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the operation was performed, 

- <u><u><u><u>False</u></u></u></u> if a problem occurs. [HErrorInfo](../WDLang4/3044071.md) is used to identify the error.




**`<Table>: (File Description variable)`**

Name of the [File description](../WDLang4/1514065.md) variable that corresponds to the table in which the item must be created. 

**`<Item>: (Variable type Field description)`**

Name of the [Item Description](../WDLang4/1514071.md) variable corresponding to the item to be created.  


<a name="SYNTAXE2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png) 
### Describing a temporary item in Hyper 5.5 format (backward compatibility only)

`<Result> = HDescribeItem(<Hyper File 5.5 table name> , <Item description> [, <Array dimension>])`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the operation was performed, 

- <u><u><u><u>False</u></u></u></u> if a problem occurs. [HErrorInfo](../WDLang4/3044071.md) is used to identify the error.




**`<Hyper File 5.5 table name>: (Character string)`**

Name of temporary table in Hyper File 5.5 format, described with [HDescribeFile](../WDLang4/3044206.md) (Syntax 2).

**`<Item description>: (Character string)`**

Description of item in the following format: &lt;Item name&gt;,&lt;Item type&gt; (no space between &lt;Item name&gt; and &lt;Item type>).

&lt;Item name&gt; contains the name of item

&lt;Item type&gt; contains the item type and it can take one of the following values:

- E: for integer fields

- L: for long integer fields

- C: for short integer fields

- S: for simple real-type items

- D: for double actual type items

- A: for date fields on 8 (YYYYMMDD)

- H: for time type headings HHMM

- T,&lt;Taille&gt; for text fields (&lt;Taille&gt; is the string size)

- M: for text memo headings

- O: for Binary Memo type fields

- F: for monetary items




**`<Array dimension>: (Optional integer)`**

Size of the array of items.  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### How to describe an item through programming? 
<a name="how_describe_item_through_programming_ELTPARAGRAPHE000109"></a>

To describe an item through programming, you must:

1. Use the [File Description](../WDLang4/1514065.md), [Item Description](../WDLang4/1514071.md) and [Link description](../WDLang4/1514039.md) variables (if necessary).

2. Describe the characteristics of the table, its items and links using HFSQL properties.

3. Define the description of the items with **HDescribeItem**.

4. Define the description of the table ([HDescribeFile](../WDLang4/3044206.md)) and the links ([HDescribeLink](../WDLang4/3044191.md)).



<a name="NOTE0_2"></a>


### List of HFSQL properties that can be used to describe an item programmatically
<a name="list_hfsql_properties_that_can_used_describe_item_programmatically_ELTPARAGRAPHE000138"></a>The following properties are available in read/write mode.



|   |   |
| --- | --- |
| [AccentSensitive](../Proprietes/2512077.md) | The **AccentSensitive** property is used to determine if accented characters are taken into account in the different searches performed on the item (with [HReadSeek](../WDLang4/3044050.md), [HReadSeekFirst](../WDLang4/3044036.md) and [HReadSeekLast](../WDLang4/3044034.md), for example). |
| [CaseSensitive](../Proprietes/2512085.md) | The **CaseSensitive** property is used to determine if the different searches performed on the item distinguish between uppercase and lowercase characters (with [HReadSeek](../WDLang4/3044050.md), [HReadSeekFirst](../WDLang4/3044036.md) and [HReadSeekLast](../WDLang4/3044034.md), for example). |
| [Component](../Proprietes/2512020.md) | The **Component** property configures the different components of a composite key. |
| [DefaultValue](../Proprietes/2512092.md) | The **DefaultValue** property is used to:<br><br>- Find out the default value of an item. This item was defined in the data model editor or programmatically.<br><br>- Set the default value of an item (only for items defined programmatically).<br><br><br> |
| [KeyExpression](../Proprietes/2512035.md) | The **KeyExpression** property is used to set the different components of a composite key. |
| [KeyType](../Proprietes/2512060.md) | The **KeyType** property is used to set the characteristics of a key item. |
| [Name](../Proprietes/2510082.md) | The **Name** property is used to get:<br><br>- the name of the font associated with a Font variable.<br><br>- the name of a control, report or report block.<br><br>- the name of a control, group of controls or window.<br><br>- the name of a control, group of controls or page.<br><br><br> |
| [NbArrayElement](../Proprietes/2512053.md) | The **NbArrayElement** property is used to configure the number of elements in an array item. |
| [PunctuationSensitive](../Proprietes/2512087.md) | The **PunctuationSensitive** property is used to determine if punctuation characters (comma, period, etc.) are taken into account in the different searches performed on the item (with [HReadSeek](../WDLang4/3044050.md), [HReadSeekFirst](../WDLang4/3044036.md) and [HReadSeekLast](../WDLang4/3044034.md), for example). |
| [Size](../Proprietes/2510125.md) | The **Size** property is used to determine and change the size of an element. |
| [SortType](../Proprietes/2512091.md) | The **SortType** property is used to:<br><br>- Find out the sort direction defined for an item or for a component of a composite key. This item was defined in the data model editor or programmatically.<br><br>- Define the sort direction for an item or for a component of a composite key (when describing the item through programming).<br><br><br> |
| [Type](../Proprietes/2510131.md) | The **Type** property is used to get:<br><br>- the type of element (table or item) defined programmatically or in the data model editor, view or query.<br><br>- the type of object.<br><br>- the type of data held in a Variant.<br><br><br> |




<a name="NOTE0_3"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) 

### Syntax 2: Temporary field description in Hyper File 5.5 format (compatibility)
<a name="syntax_2_temporary_field_description_hyper_file_55_format_compatibility_ELTPARAGRAPHE000148"></a>

This syntax is used to create temporary items in the temporary tables in Hyper File 5.5 format. This syntax can only be used in an application developed with WINDEV 5.5 or WEBDEV 1.5 then migrated to version 18. To use this function:

- the database schema in WINDEV 5.5 format must be accessible on the development computer.

- the path of the WINDEV 5.5 database schema must be specified in the characteristics of the database schema ("Details" tab).



<a name="NOTE0_4"></a>


### Compatibility
<a name="compatibility_ELTPARAGRAPHE000158"></a>

This function replaces **HDescribeKey**, which is kept for compatibility with WINDEV 5.5. 

<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[](../WDLang4/3044206.md)

[HDescribeLink (Function)](../WDLang4/3044191.md)

[HFSQL functions](../WDLang4/3044156.md)

[File Description (Variable type)](../WDLang4/1514065.md)

[FullTextIndex description (Variable type)](../WDLang4/1000017461.md)

[Link Description (Variable type)](../WDLang4/1514039.md)

[Item Description (Variable type)](../WDLang4/1514071.md)

