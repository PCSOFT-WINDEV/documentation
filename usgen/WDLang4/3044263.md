...
## Example
<a class="notetitle" target="_blank" href="$DOC$=1000003044263&name=hconnect_function">See additional examples</a>
...
Name of the table (table) to open, and for which the connection must be redefined. If no other parameter is specified, the table is reinitialized with the default parameters defined in the database schema.
...
- If this parameter is equal to "\*" and no other parameters are specified, all tables of all Native Connectors defined in the database schema are reset to the default parameters.
...
Username that will be used to open the table.
...
|   |   |
| --- | --- |
| *hHCreationIfNonexistent*<br>(Default value) | Checks the connection by opening or creating a table (if the table does not exist). This operation is only possible with *hOLectureWrite*. |
| *hHOuvre* | Checks the connection by opening a table. This value is used by default when accessing to an *hORead* database. |
...
<a name="configuring_the_connection_ELTPARAGRAPHE000266"></a>

- The table description must be known during the call to **HConnect** (the table must have been described in the database schema, or by [HDeclare](../WDLang4/3044203.md) or [HDescribeFile](../WDLang4/3044206.md)).
...
- When the &lt;Table name&gt; parameter is equal to "\*", the change applies to all the Native Connector tables defined in the database schema. In this case, only the &lt;User&gt;, &lt;Password&gt;, &lt;Database&gt; and &lt;Access&gt; parameters are taken into account.
