...
For each user and each group, the HFSQL Control Center lets you assign read, write, create, delete, etc., rights. ("Rights management" option in the "Rights" group of the server, database and table tabs). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0006.gif&type=thumb)
...
- **Encrypting the tables**: 
	In addition to the HFSQL account, which requires a user and password to log in, each table can also be protected by encryption and a password. The encryption algorithm (128 bits, 12-round RC5, 16-round RC5, etc.) is defined in the WINDEV and WEBDEV database schema by the developer. The password can be defined by the developer or by an option of a client application in order to be defined by a database administrator. 
	An encryption with strong password is recommended for the tables containing sensitive data. When defining the encryption in the database schema, it must be defined for the table (.fic), for the index (.ndx, .ftx) and for the memos. 
	**For maximum encryption security, check "Enable enhanced security "**. This option encrypts data without storing the encryption key in the table. With this option and without the table password, the content of table cannot be retrieved.
	These options are defined in the data model editor, in the "Detail" tab of the table description window. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0001.gif)
...
### Tables over 2 GB
<a name="tables_over_2_ELTPARAGRAPHE000185"></a>

If some tables are likely to exceed 2 GB in size, the HFSQL server must be installed on an operating system and file system that can handle this type of table (such as NTFS). 

In addition, the option to manage tables larger than 2 GB must be enabled in the corresponding WINDEV/WEBDEV database schema ("Compatibility" tab of the table description window): <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0002.gif&type=thumb)


In the case of a logged table, the log takes the same option as the associated table. You should therefore activate this option on a table, even if only its log file is likely to exceed 2 GB.
...
3. Click "System": <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0011%201.gif&type=thumb)
...
To be able to check the operations that have been performed on the database, a tracing mechanism must be implemented on the sensitive tables. 

This operation can be performed by enabling the **log process on these tables**. Logging can be defined at field level: it is therefore possible to exclude certain fields from the log for reasons of performance (size of log) or security (avoid duplication of highly sensitive data).

To enable the log process on the tables:

1. In the data model editor, display the description window of the tables ("Description of tables" in the context menu of a table). 

2. On the "Logging" tab, choose the type of log you want to use: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Base_HFSQLCS_Securite%20-%20HC%20N%B0003.gif)
...
To enable this option in the HFSQL Control Center, select the "Configuration" tab of server description: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0009.gif&type=thumb)
