...
## What is a view?
<a name="what_view_ELTTEXTE000145"></a>
An HFSQL view corresponds to a "memory image" of an HFSQL table.

A view is stored in memory, which insulates it from the modifications made to the associated table.

Once created, a view can be handled like any HFSQL table.
...
<a name="functions_for_managing_views_ELTTEXTE000169"></a>


### The WLanguage functions
<a name="the_wlanguage_functions_ELTPARAGRAPHE000038"></a>The following functions are used to handle the HFSQL views:
...
## Use
<a name="use_ELTTEXTE000251"></a>


### Handling a view
<a name="handling_view_ELTPARAGRAPHE000048"></a>

Follow these operations to handle views on an HFSQL table:

1. Creating the view with [HCreateView](../WDLang4/3044159.md) (while specifying the selection conditions if necessary).

2. The view is automatically executed. View records can be manipulated, browsed, etc. using HFSQL management functions. The table name specified in the functions will correspond to the view name.
...
3. Available operations specific to views:
	...
	- **Modify the sort order of the records found in the view**. By default, records in a view are sorted in the same order as in the corresponding HFSQL table. However, this order can be modified by [HSortView](../WDLang4/3044114.md). If the sort is applied to several items of the view, the sort is applied according to the first sort item, then according to the second one, etc.

	- **Apply the operations performed on the view records** (add, modify, delete, etc.) to the base file of the view ([HViewToFile](../WDLang4/3044021.md)). Please note: this operation can only be performed if the records have been locked in the table when the view was created.
	...
	- **Merge several views** ([HMergeView](../WDLang4/3044124.md)). Merging two views allows you to create a new view containing the data found in the "source" views. Then, this view can be handled like a standard view. [HMergeView](../WDLang4/3044124.md) proposes several merge modes:
		...
		- Union of all the rows found in the first view and in the second view.
						**Caution**: View structures must be identical.
		...
		- Join between the first view and the second view. The items found in the second view that do not exist in the first view and vice versa are added into the destination view
