...
<a name="how_partially_index_the_text_memos_ELTTEXTE000194"></a>
The indexing of text memos can be performed:

- in the data model editor (for the tables described in the database schema)

- in WLanguage for the tables created programmatically.
...
1. In the data model editor, display the description of the items found in a table containing a text memo.

2. Select the item associated with a text memo.

3. On the right-hand side of the screen, you can specify whether the memo should be a key or not: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Indexation%20des%20m%E9mos%20-%20HC%20N%B0001.gif)


4. Specify:

	- type of partial key: non-key, unique key, key with duplicates or primary key.

	- the direction of traversal: ascending or descending.
...
6. Generate the database schema. If tables have been already used, an automatic modification of the tables is performed.
...
### Implement the indexing of text memos programmatically
<a name="implement_the_indexing_text_memos_programmatically_ELTPARAGRAPHE000066"></a>

To implement the indexing of text memos when describing a table through programming, all you have to do is use the available WLanguage properties on the items to specify the characteristics of text memo key.
