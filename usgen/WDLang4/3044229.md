


## HListREP (Function)

***In french: HListeREP***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Returns the list of assignments for the tables handled by the current application, which means the list of physical files corresponding to the tables described in the database schema.

For more details on this list, see [List of tables handled by an application](../WDLang4/3044186.md).
<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
sAssignments is string
// Retrieve all the assignments of the logical file
// CUSTOMER whose physical name is CUSTOMER2000 and that
// exist on disk
sAssignments = HListREP(".", "CUSTOMER", "CUSTOMER2000", hExisting)
```


<a name="Exemple2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![External language](https://doc.pcsoft.fr/ext/images/us/LE.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) 
```wl
// List all the entries in the .REP file of the project
// Equivalent syntax: HListREP()
sList is string
sList = HListREP(".","*")
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Listing the assignments from the .REP file

`<Result> = HListREP([<Name of REP file> [, <Logical file name> [, <Physical file name>]]] [, <Options>])`
---

**`<Result>: (Character string)`**



- List of assignments, 

- Empty string ("") if an error occurred. 


The list of assignments has the following format:

```txt
<Logical name> + <TAB> + <Physical name> + TAB + <Directory> + CR +...
```

Where:

- &lt;Nom logique&gt; Name of the table in the database schema. 

- &lt;Nom physique&gt; Physical file name, without path or extension.

- &lt;Répertoire&gt; Physical file directory. 


![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) &lt;Result&gt; is filled with 2 new details:

- &lt;Nom du serveur&gt; HFSQL Client/Server server name.

- &lt;Nom de la base de données&gt; HFSQL Client/Server database name.




**`<Name of REP file>: (Optional character string)`**

Full name of the analyzed .REP file. 
If this parameter corresponds to an empty string or to **"."**, the current ".REP" file is examined. 
The current ".REP" file is also examined if the first three optional parameters are not specified.
![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) This parameter cannot be specified on its own. The following syntax is not available: HListREP(&lt;Name of .REP file&gt;).

**`<Logical file name>: (Optional character string)`**

Logical name of the table whose assignments must be listed. If this parameter is not specified or if it is equal to **"\*"**, all the assignments of all the tables will be sought.

**`<Physical file name>: (Optional character string)`**

Physical name of the table whose assignments must be listed. If this parameter is not specified, all the assignments of &lt;Logical File Name&gt; will be sought.

**`<Options>: (Optional integer constant)`**

All the assignments are sought by default. The *hExisting* constant is used to find the assignments for which a table actually exists.


<a name="SYNTAXE2"></a>

### Assignment list from .WDD file

`<Result> = HListREP([<Name of WDD file> [, <Logical file name> [, <Physical file name>]]] [, <Options>])`
---

**`<Result>: (Character string)`**



- List of assignments, 

- Empty string ("") if an error occurred. 


The list of assignments has the following format:

```txt
<Logical name> + <TAB> + <Physical name> + TAB + <Directory> + CR +...
```

 where:

- &lt;Nom logique&gt; Name of the table in the database schema. 

- &lt;Nom physique&gt; Physical file name, without path or extension.

- &lt;Répertoire&gt; Physical file directory. 




**`<Name of WDD file>: (Optional character string)`**

Full name of the examined .WDD file. 
The current "WDD" file is examined if this parameter is not specified or if it corresponds to ".".
Multi-database schema projects: If this parameter is not specified or is set to ".", the ".WDD" file for the main database schema is examined.

**`<Logical file name>: (Optional character string)`**

Logical name of the table whose assignments must be listed. If this parameter is not specified or if it is equal to "\*", all the assignments of all the tables will be sought.

**`<Physical file name>: (Optional character string)`**

Physical name of the table whose assignments must be listed. If this parameter is not specified, all the assignments of &lt;Logical File Name&gt; will be sought.

**`<Options>: (Optional integer constant)`**

All the assignments are sought by default. The *hExisting* constant is used to find the assignments for which a table actually exists.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![External language](https://doc.pcsoft.fr/ext/images/us/LE.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) 

### Compatibility with the earlier versions
<a name="compatibility_with_the_earlier_versions_ELTPARAGRAPHE000106"></a>

The following syntax is not available anymore: **HListREP**(&lt;WDD name&gt;, "?").

<a name="XComposante"></a>

## Business / UI classification:
Business Logic
## Component:
wd310hf.dll
## Voir aussi

[HSetREP (Function)](../WDLang4/3044061.md)

[HListAnalysis (Function)](../WDLang4/3044049.md)

[HListFile (Function)](../WDLang4/3044045.md)

[HFSQL functions](../WDLang4/3044156.md)

[REP files](../WDLang4/3044186.md)

[Project with multiple database schemas](../WDLang4/1410087296.md)

