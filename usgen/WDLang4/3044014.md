...
**This function can be used in a log-based replication (between HFSQL tables) or in a universal replication**.

**Remarks**:

- For a log-based replication, only the tables in the database schema with the option "Manage the replication for this table" checked will be taken into account.

- ![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) Only the universal replication operates with the HFSQL Client/Server tables.
...
- an empty string (""). The current tables are considered as the master database. The table corresponding to the master replica (".rpm" file) is searched in the replication directory specified in the database schema (by default, RPL subdirectory of the tables), or in the directory specified with [HChangeRplDir](../WDLang4/3044236.md).
...
- an empty string (""). The current tables are considered as being the subscriber database schema. The file corresponding to the subscriber replica (".rpl" file) is searched in the replication directory specified in the database schema (by default, RPL subdirectory of the tables), or in the directory specified with [HChangeRplDir](../WDLang4/3044236.md).
...
- an empty string (""). The current tables are considered as the master database. The file corresponding to the master replica (".rpm" file) is searched in the replication directory specified in the database schema (by default, RPL subdirectory of the tables), or in the directory specified with [HChangeRplDir](../WDLang4/3044236.md).
...
### Which tables can be replicated
<a name="which_tables_can_replicated_ELTPARAGRAPHE000266"></a>
...
**Attention**: **If you are performing an automation update of your tables**it is necessary to synchronize all replicas prior to this operation. Otherwise, all the modifications performed since the last synchronization and the automatic modification will be lost.
<a name="NOTE0_2"></a>
...
Note: The [HClose](../WDLang4/3044073.md) function deletes all filters defined by the [HFilter](../WDLang4/3044100.md) function on a table.
