...
- This function does not allow access to password-protected tables.
...
<a name="connection_hfsql_database_schema_ELTPARAGRAPHE000376"></a>
...
To connect to an HFSQL database schema via a web service proxy, use the following syntax:


```txt
SQLConnectWS(<Server of web service>, ...
<"<Drive>:<Full Database Schema WDD Path>", "", "<Database Schema Password>")
```
...
// WDD of database schema on the server. 
...
// Connection to the HFSQL database according to what is specified in the database schema
...
- After the connection to an HFSQL database schema, the SQL.Connection variable returns -1.

- The HFSQL tables will be searched for on the server in the current directory of the web service programs or in the directory described in the database schema. No path can be specified programmatically.

- If a password has been set for the database schema, it must be specified in the third parameter passed to **SQLConnectWS**.

- To manipulate HFSQL Client/Server tables, the connection defined in the database schema must contain all the necessary information (server name or IP address, port, database, etc.). This information cannot be specified programmatically. 
