...
**HFSQL-specific functionality** SQL queries executed on HFSQL Classic, Mobile or Client/Server databases can contain WLanguage functions. For more details, see [Using a WLanguage function in an SQL query](../Motscles/1513004.md).

**Please note** To position yourself on the first record in the query result, use the [HReadFirst](../WDLang4/3044051.md) function, for example.
...
## Example
<a class="notetitle" target="_blank" href="$DOC$=1000003044084&name=hexecutesqlquery_function">See additional examples</a>
...
|   |   |
| --- | --- |
| *hCheckDuplicates* | Enables the management of duplicates during the query execution. The query is not run if a duplicate error occurs.<br>This constant forces the *hWithTransaction* constant. A transaction is started, the records are locked in read/write during the execution of the query and they are unlocked at the end of its execution, regardless of the result (failure or success). If the application is stopped during the query execution (power outage for example), the transaction will be canceled when the tables of query are used for the next time.<br>Note: the execution of the query can be significantly slowed down.<br>![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The management of duplicates is disabled if the *hCheckDuplicates* constant is not specified. An error occurs if the user has no rights to disable the management of duplicates ([HModifyDatabaseRights](../WDLang4/3044326.md)) and if the management of duplicates must be performed.![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
| *hCheckIntegrity* | Enables the management of integrity during the query execution. The query is not run if an integrity error occurs.<br>This constant forces the **hWithTransaction** constant. A transaction is started, the records are locked in read/write during the execution of the query and they are unlocked at the end of its execution, regardless of the result (failure or success). If the application is stopped during the query execution (power outage for example), the transaction will be canceled when the tables of query are used for the next time.<br>Note: the execution of the query can be significantly slowed down.<br>![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The integrity management is disabled if the *hCheckIntegrity* constant is not specified. However, an error occurs if an integrity management operation must be performed, but the user does not have the rights to disable integrity management ([HModifyDatabaseRights](../WDLang4/3044326.md)).![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
| *hModifyFile*<br>(constant that can be combined with the other constants) | <br><br>- **On HFSQL tables** When modifying the query result (functions [HAdd](../WDLang4/3044147.md), [HWrite](../WDLang4/3044092.md), [HModify](../WDLang4/3044042.md), [HCross](../WDLang4/3044121.md), [HDelete](../WDLang4/3044018.md)), these modifications will be transferred to the tables contributing to the query. <br>	If this option is not specified, only the query result is modified. <br>	For more details on modifying the result of a query, see [Modifying the query content](../WDLang4/3044234.md).<br><br>- **On non-HFSQL tables**, the constant *hModifiesFile* is unnecessary: tables contributing to the query are automatically modified when the query result is modified.<br><br><br>![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
...
![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) You cannot execute queries with SQL code containing WLanguage functions. ![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This parameter can only be set to *hQueryDefault*. An error is generated if another constant is used.This parameter is not available if &lt;Query name&gt; corresponds to an SQL Query variable.
...
<a name="reading_data_during_the_initialization_ELTPARAGRAPHE000488"></a>

Depending on the query (sorts, groups, ...), the data can be read (or not) when the initializing the query.

Note: The [ExecutionCompleted](../Proprietes/2512117.md) property indicates whether the query has completed filling (and can therefore be read without blocking).. 
...
### SQL query text
<a name="sql_query_text_ELTPARAGRAPHE000518"></a>

If a table or item name contains spaces, this name must be enclosed in square brackets in the query text. For example:
...
<a name="executing_sql_query_with_parameters_ELTPARAGRAPHE000529"></a>
...
2. Define variable type **Data source**. The name of this variable must correspond to the &lt;Query name&gt; parameter of **HExecuteSQLQuery**. 
	Example: 
...
<a name="sql_injection_how_protect_yourself_ELTPARAGRAPHE000576"></a>
...
<a name="inserting_binary_data_ELTPARAGRAPHE000611"></a>
...
<a name="why_should_the_hquerywithoutcorrection_constant_used_ELTPARAGRAPHE000647"></a>
...
<a name="condition_composite_key_sql_query_ELTPARAGRAPHE000700"></a>
...
<a name="NOTE0_6"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![External language](https://doc.pcsoft.fr/ext/images/us/LE.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) 

### Queries on HFSQL tables
<a name="queries_hfsql_tables_ELTPARAGRAPHE000717"></a>
...
- To optimize query processing time, use [HStatCalculate](../WDLang4/3044134.md) or [HIndex](../WDLang4/3044133.md) on all tables included in the query.
...
- If the tables used by the query are not found in the database schema, these tables must be dynamically declared by [HDeclareExternal](../WDLang4/3044204.md). Otherwise, an error such as **Unable to initialize the query. Unknown FILENAME** will occur. 
	Example:
...
<a name="optimization_for_running_the_same_query_several_times_native_oracle_and_sql_server_accesses_only_ELTPARAGRAPHE000762"></a>
...
<a name="NOTE0_8"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) 

### Specifying and retrieving values when running a query (stored procedures)
<a name="specifying_and_retrieving_values_when_running_query_stored_procedures_ELTPARAGRAPHE000803"></a>
...
<a name="sql_queries_docparampagetitleshort_execution_sql_queries_created_the_query_editor_ELTPARAGRAPHE000856"></a>
...
![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The integrity management is disabled if the *hCheckIntegrity* constant is not specified. However, an error occurs if an integrity management operation must be performed, but the user does not have the rights to disable integrity management ([HModifyDatabaseRights](../WDLang4/3044326.md)).**To perform an automatic check for duplicates**, simply specify the *hCheckDuplicates* constant. This constant is used to enable the management of duplicates during the query execution. The query is not run if a duplicate error occurs. Indeed, a transaction is started, the records are locked in write mode during the execution of the query and they are unlocked at the end of its execution, regardless of the result (failure or success). If the application is stopped during the query execution (power outage for example), the transaction will be canceled when the tables of query are used for the next time.

Note: If a duplicate error is detected when executing an UPDATE query, the **HExecuteQuery** function returns False and processing continues. The corresponding error is returned by [HErrorInfo](../WDLang4/3044071.md).

**Remark**: Default automation support is not called for duplicate errors. A simple duplicate error is generated. If an assistance was redefined by [HOnError](../WDLang4/3044017.md), then this custom assistance is called.
![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The management of duplicates is disabled if the *hCheckDuplicates* constant is not specified. An error occurs if a duplicate management operation must be performed, but the user has no rights to disable the management of duplicates ([HModifyDatabaseRights](../WDLang4/3044326.md)).**The UPDATE, DELETE and INSERT queries only exist during their execution**. No WLanguage function can be used on this type of query after its execution.
<a name="NOTE0_10"></a>
...
### Looping through queries ([HReadFirst](../WDLang4/3044051.md))
<a name="looping_through_queries_hreadfirstwdlang43044051md_ELTPARAGRAPHE000895"></a>

- By default, [HReadFirst](../WDLang4/3044051.md) re-runs the query to refresh the result. It is recommended to use the *hNoRefresh* constant to avoid re-running the query. 
...
[Cross-database queries](../WDLang4/2034010.md)

[Overloading a WLanguage function or using a WLanguage function in an SQL query](../WDLang4/1513004.md)

[How to create and run an SQL query through programming?](../WDLang4/20009.md)
