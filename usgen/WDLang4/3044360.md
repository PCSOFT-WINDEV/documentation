...
In fact, when the way in which data is accessed changes (following a change in the structure of tables, for example), it's simply a matter of modifying the procedure stored on the server without making any changes to the deployed applications that use this procedure.
...
### The stored procedures are created in the database schema.
<a name="the_stored_procedures_are_created_the_database_schema_ELTPARAGRAPHE000039"></a>

To create a stored procedure:

1. Create (if necessary) the set of stored procedures in the database schema:
...
- or via the "database schema" pane: select the "Stored Procedures" folder. Select "New set of procedures" in the context menu. The set of stored procedures is automatically created. This set of procedures is named "Set_&lt;Database Schema Name&gt;" by default.

- or in the data model editor ribbon: under the "Database schema" pane, in the "Creation" group, pull down "New" and select "Stored procedure collection". The set of stored procedures is automatically created. This set of procedures is named "Set_&lt;Database Schema Name&gt;" by default.

2. This set is physically saved in the "Code" subdirectory of database schema directory.
...
2. In the "Database schema" pane, select the "Stored procedures" folder.
...
4. The following operations are performed:
	...
	- ***If the procedure collection belongs to the current project*** If the procedure collection belongs to the current project: the project's procedure collection is automatically copied into the "Code" directory of the database schema. The logical name of this procedure is automatically renamed. 
			Caution: The two procedure collections are dissociated: a modification made in the project procedure collection will not be carried over to the database schema procedure collection, and vice versa.

	- ***If the procedure collection does not belong to the current project*** the procedure collection is automatically copied into the "Code" directory of the database schema, and defined as a stored procedure.
...
- Method 1: 
	...
	1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_nouveau.gif) in the quick access buttons.
		...
		- The wizard for query creation is automatically started. The query is automatically created. The query is physically saved in the "Code" subdirectory of database schema directory.




2. The query can be used in one of the stored procedures of the database schema.

- Method 2: 

	1. Open the data model editor.

	2. Display the "Database schema" pane and select "New query" in the context menu. The wizard for query creation is automatically started. The query is automatically created. The query is physically saved in the "Code" subdirectory of database schema directory.

	3. The query can be used in one of the stored procedures of the database schema.
...
2. In the "Database schema" pane, select the "Queries"; folder.

3. Select "Import a query" in the context menu. In the window that opens, select the query to import and validate.

4. The following operations are performed:

	- ***If the query belongs to the current project*** the project query is automatically copied into the "Code" directory of the database schema, and relocated: a single query is used by the project and the database schema. Therefore, a modification performed in the project query will be automatically applied to the database schema query and vice versa.

	- ***If the query does not belong to the current project*** the query is automatically copied into the "Code" directory of the database schema, and defined as a query linked to the database schema.
...
- To find out whether or not a function can be used in a stored procedure, simply consult the function's help: if the image ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=proc_stock.gif) is displayed in the function's title bar, the function can be used in stored procedure code.
...
- To handle the tables, you must use [HDeclareExternal](../WDLang4/3044204.md). 
	For example: 
	
	```wl
	PROCEDURE NameOfStoredProcedure()
	
	// Check existence of logical table
	IF NOT HFileExist(ZIPCODES) THEN
		...
		// The table will be searched in the current database. 
		// You have the ability to specify a subdirectory of the database. 
		// - give full name of table, including extension (.FIC)
		IF NOT HDeclareExternal("ZIPCODES.FIC", "ZIPCODES") THEN
			// Table declaration error
			RETURN HErrorInfo()
		END
	END
...
1. Open the database schema description window. 

	- Open the data model editor if necessary. 

	- Open the context menu of the database schema (right-click the database schema schema) and select "Database schema description". 
...
- **for test** Stored procedures and/or queries are deployed on the chosen server, but the database schema is not updated. This operation is recommended for large-sized database schemas whose generation can take quite a long time. 
...
- **actually** the database schema must be re-generated and the tables automatically modified when the client version is installed.
...
1. In the data model editor, on the "Database schema" tab, in the "Database schema" group, expand "Synchronization" and select "Update stored elements (HFSQL Client/Server)".
...
2. In the window that is displayed, select:
	...
	- the connection affected by the update. You can also directly enter the characteristics of the connection. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=proc_stock_Test.gif&type=thumb)
...
**Note** To deploy for tests, you can also generate the database schema and accept automatic table modification.
<a name="NOTE3_3"></a>
...
1. Generate the database schema containing the stored procedures ("Generate the database schema" in the context menu of database schema). The tables can be modified during this step.

2. Create the Customer version by requesting automatic modification of tables: in the "Project" pane, in the "Generation" group, scroll down to "Setup procedure" and select "Create installation procedure". 
	Note: simply by modifying the stored procedure code or the database schema queries, automatic modification of the tables is proposed.
...
In order for this set of procedures to operate, you must select "Enable compilation errors for 64-bit compatibility" ("Database schema description" in the database schema context menu, "Compilation" tab).
