...
- The database schema must be shared between the WINDEV and WINDEV Mobile application (Android or iOS). 
...
- IMPORTANT: Date and time must be identical on server and Android/iOS terminals. Internal dates (those used by replication in HFSQL tables) are in UTC. 
...
**Note** Don't forget to set the connection to the local database (HFSQL Classic table on the mobile device) in the Android (or iOS) project code. 
