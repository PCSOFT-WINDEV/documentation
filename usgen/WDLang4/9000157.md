...
4. The SQL view is created. By default, it is not associated with a connection. To associate the view with a connection defined in the data model editor: 
	...
	- On the "Database schema" pane, select the SQL view that was just created. 
...
5. Generate the database schema to use the SQL view in the application code: under the "database schema" pane, in the "database schema" group, pull down "Generation" and select "Generation". 




Note: In a query to create a SQL view, some options are not available: 
...
Note: The **HFSQL connection** parameter must be specified if the view has not been created in the data model editor, or if the view defined in the data model editor has not been associated with a connection. 
...
- The SQL view must be based on a single table. 
...
[HInfoViewRights (Function)](../WDLang4/1000021475.md)
