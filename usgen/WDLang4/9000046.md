...
Note: Automatic assisted replication is available for mobile data replication (Android or iOS).. For more details, see [Replication of mobile data (Android or iOS)](../WDLang4/1000021071.md). 
...
<a name="principle_automatic_assisted_universal_replication_ELTTEXTE000295"></a>
...
<a name="the_steps_for_implementing_automatic_assisted_universal_replication_are_follows_ELTPARAGRAPHE000037"></a>
...
Note: Some features are not available in this version.. For more details, see the [Limitations regarding the assisted universal replication](../WDLang4/9000048.md).
...
<a name="adapting_the_application_necessary_ELTTEXTE000325"></a>
Assisted universal replication can be implemented on applications that use HFSQL Classic, HFSQL Client/Server and OLE DB tables, as well as tables handled by Native Connectors (also called Native Accesses).

**Tables in the database schema are in HFSQL Classic format.** format: no modifications are required. The master files can be configured:
...
**Tables in the database schema use a connection** (HFSQL Client/Server, Native Connectors, OLE DB files, ...): it is necessary to ***define the connection to the master server in the database schema, and associate the tables with this connection***.
...
**Caution: Regardless of the table format, to use universal automatic assisted replication**DO NOT modify file access information programmatically. YOU MUST NOT USE:
...
<a name="using_nonhfsql_databases_ELTPARAGRAPHE000102"></a>
...
<a name="preparing_the_files_required_the_assisted_universal_replication_ELTTEXTE000355"></a>
The files required by the automatic assisted universal replication are prepared on the development computer:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Preparation_replic.gif)
The replication file (.WER file) is created by [ReplicEdit](../ReplicEdit/3526500.md).
...
<a name="using_the_replication_ELTTEXTE000379"></a>


### Direct assisted universal replication
<a name="direct_assisted_universal_replication_ELTPARAGRAPHE000164"></a>

The principle for the direct assisted replication is as follows:
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=replication_directe.gif)
...
Note: the principle described here is identical for an application using a system with automatic updates.
<a name="NOTE5_2"></a>


### Assisted universal replication by replication server
<a name="assisted_universal_replication_replication_server_ELTPARAGRAPHE000185"></a>
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=replication_serveur.gif)
...
<a name="offline_modeonline_mode_ELTPARAGRAPHE000204"></a>
...
**When the application is in offline mode configured by ReplicAdmin**, the application is always using the local tables. No automatic mechanism is implemented. To use the master database, a synchronization must be performed and ReplicAdmin must be used to switch to the online mode.
...
Note: to keep local data as up-to-date as possible when switching to nomad mode, local data can be immediately synchronized each time the application is closed (option in the application installer).
...
**Note** This mechanism can be used for 32-bit and 64-bit Windows executables.
