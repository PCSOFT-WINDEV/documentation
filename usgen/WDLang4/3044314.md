


## HDeleteLink (Function)

***In french: HSupprimeLiaison***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Deletes an integrity rule between two tables on the server. The integrity rule to delete is defined by a [Link description](../WDLang4/1514039.md) variable.

The deletion will be effective for the application that performed it once the connection has been closed and re-opened.




<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
MyConnection is Connection
MyLink is Link Description
// Describe the "LIVES" link
MyLink.SourceFile = ".\city.fic"
MyLink.LinkedFile = ".\customer.fic"
MyLink.SourceKey = "CITYNAME"
MyLink.LinkedKey = "CUSTCITY"

// Default link (0-1, 0-1)
HDeleteLink(MyConnection, MyLink)
```

```wl
// Delete all links from a database 
// found on an HFSQL Client/Server server 
// independently of the current database schema
CntHFSQL is Connection
CntHFSQL.Provider = hAccessHFClientServer
CntHFSQL.User = "admin"
CntHFSQL.Password = ""
CntHFSQL.Server = "HFSQLServer:4900"
CntHFSQL.Database = "CRM"
HOpenConnection(CntHFSQL)

sListLinks is string = HListLink("*", CntHFSQL)
sALinkInfo is string
ALink is Link Description

FOR EACH STRING sALinkInfo OF sListLinks SEPARATED BY CR
	IF sALinkInfo <> "" THEN
		ALink.Name = ExtractString(sALinkInfo, 1, TAB)
		UneLiaison.FichierSource = Replace(ExtractString(sInfoUneLiaison, 2, TAB), CnxHFSQL.BaseDeDonnées, ".", SansCasse)
		ALink.SourceKey = ExtractString(sALinkInfo, 3, TAB)
		OneLink.LinkedFile = Replace(ExtractString(sInfoOneLink, 5, TAB), CnxHFSQL.Database, ".", NotCase)
		ALink.LinkedKey = ExtractString(sALinkInfo, 6, TAB)
		// Delete the link from the HFSQL Client/Server server
		Trace("Deletion >> " + UneLiaison.Nom + " >> " + HSupprimeLiaison(CnxHFSQL, UneLiaison))
	END
END
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = HDeleteLink(<Source connection> [, <Linked connection>] , <Link description>)`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the operation was performed, 

- <u><u><u><u>False</u></u></u></u> if a problem occurs. [HErrorInfo](../WDLang4/3044071.md) is used to identify the error.




**`<Source connection>: (Character string or Connection variable)`**

Connection to which belongs the source file of the link. This connection corresponds to:

- the name of the connection defined in the data model editor or with [HDescribeConnection](../WDLang4/3044205.md) or [HOpenConnection](../WDLang4/3044107.md).

- the name of a variable of type [Connection](../WDLang4/1514073.md).




**`<Linked connection>: (Optional character string or Connection variable)`**

Connection to which belongs the linked file of the link. This connection corresponds to: 

- the name of the connection defined in the data model editor or with [HDescribeConnection](../WDLang4/3044205.md) or [HOpenConnection](../WDLang4/3044107.md).

- the name of a variable of type [Connection](../WDLang4/1514073.md).


If this parameter is not specified, both tables will belong to the same connection (&lt;Source connection&gt;).

**`<Link description>: (Link Description variable)`**

Name of the [Link description](../WDLang4/1514039.md) variable containing the characteristics of the link to delete. The link to delete is defined by the source table, the source item and the linked item.  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Link description
<a name="link_description_ELTPARAGRAPHE000129"></a>

- The names of the tables in the link description ([SourceFile](../Proprietes/2512034.md) and [LinkedFile](../Proprietes/2512031.md) properties) must correspond to the physical name of the tables, relative to the relevant database schema. The database of each table is returned by the &lt;Source connection&gt; and &lt;Linked connection&gt; parameters

- To delete a link, there is no need to describe all the characteristics of the link. Only the following properties are required:
	


|   |   |
| --- | --- |
| [LinkedKey](../Proprietes/2512007.md) | Item of the linked table used in the link. |
| [SourceKey](../Proprietes/2512003.md) | Item of the source table used in the link. |
| [LinkedFile](../Proprietes/2512031.md) | Name of the linked table used in the link. |
| [SourceFile](../Proprietes/2512034.md) | Name of the source table used in the link. |
| [Name](../Proprietes/2510082.md) | Link name. |






<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[HAddLink (Function)](../WDLang4/3044312.md)

[HFSQL Client/Server functions](../WDLang4/3044316.md)

[&lt;Connection variable&gt;.DeleteLink (Function)](../WDLang4/1000022759.md)

