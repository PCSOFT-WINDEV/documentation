


## HRetrieveRecord (Function)

***In french: HRécupèreEnregistrement***

<DIV class="specObsolete">
	<IMG style="float:left;margin-right:10px;" src="$IMGLOCAL$=warning_big.png"><B>Warning</B><BR>
	From version <B>26</B>, This function is kept for backward compatibility. It is recommended to use [HRecordToString](../WDLang4/1000025983.md).  
</DIV><a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Returns the content of the current record (in an HFSQL table, view, query, ...).


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
Rec is string = HRetrieveRecord(Customer)
```
<a name="Exemple2"></a>

```wl
// Retrieves the string containing the entire record
MyRecord is string = HRetrieveRecord(Customer)
// Extracts the last name of the customer
CustomerLastName is string = ExtractString(MyRecord, 1)
// Extracts the first name of the customer
CustomerFirstName is string = ExtractString(MyRecord, 2)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Retrieving the content of a record for the specified file

`<Result> = HRetrieveRecord([<Table> [, <Item separator> [, <Array separator> [, <Format>]]]])`
---

**`<Result>: (Character string)`**

Value of the record, in the following format: 

```txt
<Content of item 1> + <Item separator> + <Content of item 2> + ...
```


The contents of items are extracted according to the physical order of items in the table. If the item corresponds to NULL, &lt;Content of item&gt; corresponds to an empty string ("").

Depending on the &lt;Format&gt; specified, this character string is in ANSI (default) or Unicode format.

**`<Table>: (Optional character string)`**

Name of the table (view or query) used. If this name is not specified, **HRetrieveRecord** will use the last table used by the last HFSQL function (function starting with "H").

**`<Item separator>: (Optional character string)`**

Character string used to separate the different items in &lt;Result&gt;. The default separator is TAB.

**`<Array separator>: (Optional character string)`**

Character string used to separate the values of array items (or sub-items) in &lt;Result&gt;. The default separator is TAB.
Please note: for compatibility with WINDEV 5.5, separators are not shown when array fields are empty.

**`<Format>: (Optional Integer constant)`**

Format used to return the value of the record. By default, an ANSI format character string is returned. To obtain a string in Unicode format, use the following constant:


|   |   |
| --- | --- |
| *hUnicode* | &lt;Résultat&gt; is a Unicode character string. |




<a name="SYNTAXE2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) 
### Retrieving the content of a record for the specified Record variable

`<Result> = HRetrieveRecord(<Record> [, <Item separator> [, <Array separator> [, <Format>]]])`
---

**`<Result>: (Character string)`**

Value of the record, in the following format: 

```txt
<Content of item 1> + <Item separator> + <Content of item 2> + ...
```


The contents of items are extracted according to the physical order of items in the table. If the item corresponds to NULL, &lt;Content of item&gt; corresponds to an empty string ("").

Depending on the &lt;Format&gt; specified, this character string is in ANSI (default) or Unicode format.

**`<Record>: (Optional Record variable)`**

Name of the [Record](../WDLang4/1000020962.md) variable used. 

**`<Item separator>: (Optional character string)`**

Character string used to separate the different items in &lt;Result&gt;. The default separator is TAB.

**`<Array separator>: (Optional character string)`**

Character string used to separate the values of array items (or sub-items) in &lt;Result&gt;. The default separator is TAB.
Please note: for compatibility with WINDEV 5.5, separators are not shown when array fields are empty.

**`<Format>: (Optional Integer constant)`**

Format used to return the value of the record. By default, an ANSI format character string is returned. To obtain a string in Unicode format, use the following constant:


|   |   |
| --- | --- |
| *hUnicode* | &lt;Résultat&gt; is a Unicode character string. |





<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


- The content of Binary Memo items is not retrieved but the item separator is present (to avoid a shift).

- The content of composite keys is not retrieved but the item separator is present (to avoid a shift).




<a name="XComposante"></a>

## Business / UI classification:
Business Logic
## Component:
wd310hf.dll
## Voir aussi

[HListItem (Function)](../WDLang4/3044230.md)

[HRetrieveItem (Function)](../WDLang4/3044128.md)

[HToItem (Function)](../WDLang4/3044025.md)

[HFSQL functions](../WDLang4/3044156.md)

[&lt;Source&gt;.RetrieveRecord (Function)](../WDLang4/1000025057.md)

