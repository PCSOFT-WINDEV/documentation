...
- the filter is created: the **&lt;Source&gt;.Filter** function returns the optimal route key to be used to route the table, view or query.
...
Name of key item used to loop through the table, view or query. This item must be a search key of table, view or query. The lower and upper bounds will be applied to this item.

**`<Lower bound>: (Type of search item)`**

Minimum value (inclusive) of search item (if the search key is defined with an iteration in ascending order in the database schema). The records corresponding to this minimum value will be included in the filter. For a descending iteration (i.e., if the search key is defined with an iteration in descending order in the database schema), it is the maximum value of the browse item. 
...
After the call to **&lt;Source&gt;.Filter**, the iteration must be performed on the item returned by **&lt;Source&gt;.Filter**. The filter will be ignored if another item is used to loop through the table.
...
- To optimize the iteration, use [&lt;Source&gt;.StatCalculate](../WDLang4/1000025064.md) on the table before creating the filter. The HFSQL engine analyzes the selection condition and uses these statistics to determine which items will be used to optimize the iteration performed on the table.
...
A single filter can exist at a given time on a table (query or view). If the **&lt;Source&gt;.Filter** function is used several times, only the last filter will be taken into account: the previous filter(s) will be deleted.
<a name="NOTE0_5"></a>
...
### Non-HFSQL tables
<a name="nonhfsql_tables_ELTPARAGRAPHE000590"></a>
