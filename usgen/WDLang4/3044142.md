


## WithSpace (Function)

***In french: EspaceSignificatif***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Adds or deletes the spaces found on the right of a text item when reading it.

**Please note** This function is only available for tables for which the "5.5 compatible mode: text fields with spaces" option has been checked in the data model editor ("Detail" tab of the table description).




<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example
<a class="notetitle" target="_blank" href="$DOC$=1000003044142&name=withspace_function">See additional examples</a>

```wl
// Manage significant spaces (add spaces) for all the files
WithSpace(True)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = WithSpace([<File name>] [, <Item name>] , <Options>)`
---

**`<Result>: (Boolean)`**

Former management mode of significant spaces.

**`<File name>: (Optional character string)`**

Name of the table used. If this parameter is not specified, **WithSpace** will handle **all tables**.
![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) This parameter is ignored.

**`<Item name>: (Optional character string)`**

Name of the item used. If this parameter is not specified, **WithSpace** will handle **all items** in the specified table.
![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png) This parameter is ignored.

**`<Options>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u>: the content of text headings is completed with spaces to the right. 

- <u><u><u><u>False</u></u></u></u> spaces to the right of text headings are removed.


![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/us/HF55.png)  The management of significant spaces will be performed for all the tables and for all the text items found in these tables.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Generalizing the management of spaces
<a name="generalizing_the_management_spaces_ELTPARAGRAPHE000068"></a>

- To apply the management mode of spaces to **all the items found in a data file**, the &lt;Item name&gt; parameter must be equal to "\*". 
	Example: 
	
	```txt
	WithSpace(<File name>, "*", <Options>)
	```


- To apply the management mode of spaces to **all the items of all the data files**, the &lt;File name&gt; and &lt;Item name&gt; parameters must be equal to "\*". 
	Example: 
	
	```txt
	WithSpace("*", "*", <Options>)
	```
In this case, **WithSpace** returns <u><u><u><u>False</u></u></u></u> if the management of spaces was different on at least one of the relevant items.

- If only the &lt;Options&gt; parameter is specified, the specified management mode is applied to all the tables and to all the items.





### Replication and WithSpace function
<a name="replication_and_withspace_function_ELTPARAGRAPHE000090"></a>

The replication is not compatible with **WithSpace** in the code of the client application. This function changes the nehavior of HFSQL. Using this function in the client application can affect the efficiency of replication.
Starting with version 21 update 3 (version 210065), the synchronization functions (programmed or assisted universal replication) return an error to the client application if **WithSpace** is used on at least one of the replicated files.

<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[HReadSeekFirst (Function)](../WDLang4/3044036.md)

[HReadSeekLast (Function)](../WDLang4/3044034.md)

[HFSQL functions](../WDLang4/3044156.md)

