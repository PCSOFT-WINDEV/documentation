...
Note: These manipulations are also available via programming.
...
<a name="history_incidentsnotifications_ELTTEXTE000221"></a>
...
- Incident category: Information, Warning, Critical, Error. 

	- Information: Information about events on the server. 
			Example: 
...
- Warning: Events that do not lead to malfunctions, but which need to be addressed.
...
- Critical: Malfunctions whose errors cannot be passed on to the user.
			Examples: WLanguage error in a stored procedure used by a scheduled task, connection failure to a replicated server, etc.

- Error: Windows cache management problem. 
...
<a name="history_notifications_displayed_the_hfsql_control_center_ELTPARAGRAPHE000074"></a>
...
4. The list of notifications sent by the server is displayed. It is possible to: 
...
<a name="accessing_the_history_notifications_programmatically_ELTPARAGRAPHE000091"></a>

Notification history is recorded in an HFSQL table in the server's system database: "Alert.fic". 

This table can be handled through programming as follows: 


```wl
PROCEDURE HFCS_RecoverLastNotifications(sServer, sUser="Admin", sPassword=""): string
...
ELSE
	...
	Error("Notification retrieval error: " + HErrorInfo(hErrComplet))
END
```
...
<a name="receiving_the_notifications_email_wdbal_ELTTEXTE000257"></a>
...
<a name="hfsql_control_center_ELTPARAGRAPHE000116"></a>
...
4. In the list of possible configurations, click "Notifications". The window for configuring the notifications is displayed: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0014.gif&type=thumb)
...
6. Create the recipients of notifications: 
	...
	- The ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Notif_HF_Email.gif) button is used to add a recipient for the sending by email. Specify the email address of the recipient and validate. 

	- The ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Notif_HF_Bal.gif) button is used to add a recipient for the sending via the messaging (WDBal) of Control Centers. Select the relevant recipients and validate. 
...
### Programming
<a name="programming_ELTPARAGRAPHE000147"></a>Several WLanguage functions can be used to configure the sending of HFSQL notifications through programming: 
