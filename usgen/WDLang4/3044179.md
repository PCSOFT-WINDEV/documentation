...
The driver is available in read/write. An application written in an external language can read and write in HFSQL tables.
...
8. Use the browse buttons to select the WDD file corresponding to the database schema.

9. In the list of database schemas, select the requested database schema and validate.

10. If the database is in "HFSQL Classic" format, check "HFSQL Classic" and select the directory of the tables via the browse button. All the HFSQL Classic tables corresponding to the selected database schema are grouped in this directory. 
	Caution: a file directory must be selected for each database schema. 
	Confirm ("OK" button).
...
You can use OPEN FILE queries on HFSQL tables by specifying:
...
### Access HFSQL Client/Server tables
<a name="access_hfsql_clientserver_tables_ELTPARAGRAPHE000127"></a>
