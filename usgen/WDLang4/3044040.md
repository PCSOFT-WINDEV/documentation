...
- **HModePerso(False)**: Blocking tests should be performed after each call to a Hyper File function, except for non-blocking read Hyper File functions.
...
**Note**: Block management is not automatic on Memo or Binary Memo type items.
...
- **hMode(HModeMulti)**: multi-user mode management (network)
...
**Note**: A project migrated to version 2026 will retain its own blocking management. The code is migrated in order for this management of locks to operate properly.
