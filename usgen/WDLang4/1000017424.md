...
The advantage of a spare server is the simple implementation of the infrastructure. Compared to a replication, it is not necessary for the table to have an 8-byte auto ID item.
...
- Integrity management and secure password: For integrity management on tables with passwords to be taken into account by replication, all tables contributing to the integrity constraint must have the same password. Otherwise, the replication will not work. 
