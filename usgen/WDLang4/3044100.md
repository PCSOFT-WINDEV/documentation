


## HFilter (Function)

***In french: HFiltre***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Defines and enables a filter on a table, view or query.

After its execution, the most common cases are as follows:

- the filter cannot be created: the function **HFilter FUNCTION** function returns an empty string. [HError](../WDLang4/3044088.md) returns the error identifier.

- the filter is created: the **HFilter** function returns the optimal route key to be used to route the table, view or query.





<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Simple filter with one condition: Find customers in a city 
SearchKey is string
mycity is string
mycity = "MONTPELLIER"
SearchKey = HFilter(Customer, "CITY='" + mycity + "'")
HReadFirst(Customer, SearchKey)
WHILE HOut() = False
	// Process the record 
	...
	HReadNext(Customer, SearchKey)
END
// Cancels the filter
HDeactivateFilter(Customer)
```

```wl
// Filter made with a bounded path key and a condition: the path key is fixed
SearchKey is string
// Filters the invoices found between 1/1/2005 and
// 12/31/2005, whose total is greater than 1500 Euros IOT
// with a discount whose type is passed as parameter (text item)
SearchKey = HFilter(Invoice, InvoiceDate, "20050101", "20051231", ...
	StringBuild("TotalIOT>1500 AND DiscountType='%1'" + , ...
	Replace(sDiscountType, "", "\")))
// The Replace function is used to prefix the quotes 
// contained in sDiscountType with a backslash
IF SearchKey <> "" THEN
	HReadFirst(Invoice, SearchKey)
	WHILE HOut() = False
		// Record Line: send a thank-you letter
		Send_Letter()
		HReadNext(Invoice, SearchKey)
	END
END
...
// Cancels the filter
HDeactivateFilter(Invoice)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Filter built with a bounded search key and a condition

`<Result> = HFilter(<Table> , <Search key> , <Lower bound> [, <Upper bound> [, <Selection condition>]])`
---

**`<Result>: (Character string)`**

Browse item. Corresponds to: 

- the search key of table if the filter is enabled. 

- an empty string ("") if the filter cannot be implemented. 




**`<Table>: (Character string)`**

Name of table, HFSQL view or query used.

**`<Search key>: (Character string)`**

Name of key item used to loop through the table, view or query. This item must be a search key of table, view or query. The lower and upper bounds will be applied to this item.

**`<Lower bound>: (Type of search item)`**

Minimum value (inclusive) of search item (if the search key is defined with an iteration in ascending order in the database schema). The records corresponding to this minimum value will be included in the filter. For a descending iteration (i.e., if the search key is defined with an iteration in descending order in the database schema), it is the maximum value of the browse item. 
The type of this parameter must correspond to the type of search item. For example, if the search item is a string, the lower bound must also be a string.

**`<Upper bound>: (Type of search item, optional parameter)`**

Maximum value (inclusive) for the browse item (ascending iteration). The records corresponding to this maximum value will be included in the filter. The type of this parameter must correspond to the type of search item. For example, if the search item is a string, the upper bound must also be a string. 
For a descending iteration, it is the minimum value of the search item.
If this parameter is not specified, the upper bound corresponds to the value of &lt;Lower bound&gt;.

**`<Selection condition>: (Optional character string)`**

Selection condition to create the filter (see Notes). This selection condition can be an Ansi or Unicode character string.


<a name="SYNTAXE2"></a>

### Filter built with a condition

`<Result> = HFilter(<Table> , <Selection condition>)`
---

**`<Result>: (Character string)`**

Browse item. Corresponds to: 

- the search key of table if the filter is enabled

- an empty string if the filter cannot be implemented




**`<Table>: (Character string)`**

Name of table, HFSQL view or query used.

**`<Selection condition>: (Character string)`**

Selection condition used to create the filter (see the Notes). This selection condition can be an Ansi or Unicode character string.  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Lower bound and upper bound on a Text or Composite Key key item
<a name="lower_bound_and_upper_bound_text_composite_key_key_item_ELTPARAGRAPHE000133"></a>

If the lower bound and the upper bound are the same:

- to implement **an exact-match filter on a value**, all you have to do is specify this value in the "Lower Bound" parameter.
	For example, to select the customers whose name corresponds to "Smith":
	
	```wl
	HFilter(Customer, Name, "Smith")
	```

	The customer named "Smither" will not be selected.

- to implement **a generic filter on a value**, you must:

	- fill the lower bound with the *hMinVal* constant to give it the minimum value.

	- fill the upper bound with the *hMaxVal* constant to give it the maximum value.


 For example, to select the customers whose last name starts with "Smith":
	
	```wl
	HFilter(Customer, Name, "Smith" + hMinVal, "Smith" + hMaxVal)
	```

	The customers named "Smith" and "Smither" are selected.


Remarks:

- The *hMinVal* constant is equivalent to Charact(0).

- The *hMaxVal* constant is equivalent to Charact(255).



<a name="NOTE0_2"></a>


### Filter and filtered iteration
<a name="filter_and_filtered_iteration_ELTPARAGRAPHE000174"></a>

After the call to **HFilter**, the iteration must be performed on the item returned by **HFilter**. The filter will be ignored if another item is used to loop through the table.

When a filter is defined and enabled on a table (view or query), all records read correspond to the filter. If no other record corresponds to the filter during the iteration:

- [HOut](../WDLang4/3044094.md) returns <u><u><u><u>True</u></u></u></u>.

- the current record corresponds to the last record read with the filter.




For example:

| On a filtered table, after the function: | the current record is: |
| --- | --- |
| HReadFirst | the first file record corresponding to the filter |
| HReadLast | the last file record corresponding to the filter |
| **HReadNext** (or **HForward**) | the next record (or the next nth record) corresponding to the filter |
| **HReadPrevious** (or **HBackward**) | the previous record (or the previous nth record) corresponding to the filter |


<a name="NOTE0_3"></a>


### Selection condition
<a name="selection_condition_ELTPARAGRAPHE000221"></a>

The general syntax of a condition has the following format:

"CustomerName&gt;'Smith' and (ZipCode=34 or ZipCode=32)"

The supported operators depend on the type of items used in the condition:


|   |   |   |
| --- | --- | --- |
| &gt; | Greater than | Valid for all types |
| &gt;= | Greater than or equal to | Valid for all types |
| &lt; | Less than | Valid for all types |
| &lt;&gt; | Different | Valid for all types |
| &lt;= | Less than or equal to | Valid for all types |
|  = | Strictly equal to | Valid for all types |
| ] | Contains: case-sensitive characters | Valid for string types only |
| ]= | Begins with: takes into account character case | Valid for string types only |
| ~] | Contains: does not take character case into account | Valid for string types only |
| ~~ | Very flexible equality: does not differentiate between uppercase and lowercase characters, ignores spaces before and after the string to be tested, ignores lowercase accented characters, ignores spaces and punctuation within strings. | Valid for string types only |
| ~= | Approximately equal: ignores right-hand spaces, accents and case. | Valid for string types only |


![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) The ~~ and ~= operators must not be used. These operators are not allowed.

**Notes about the selection condition**:

- Constant strings must be enclosed in single quotes.

- If the item name contains single quotes, they must be doubled. 
	For example: Rubricwithquotes

- The comparisons between strings are performed according to the parameters specified for the indexes.

- Binary memos and composite keys cannot be part of a selection condition. 
	For the composite keys, we recommend that you use the first syntax. The following syntax cannot be used:
	
	```wl
	HFilter(FILENAME, "COMPKEY~]'AAA' AND COMPKEY]='" + HBuildKeyValue(FILENAME, COMPKEY, 0) + "'")
	```


- If a character string (constant or variable) contains a single quote, the quote must be preceded by a backslash ( \\ ).

- To optimize the iteration, use [HStatCalculate](../WDLang4/3044134.md) on the table before creating the filter. The HFSQL engine analyzes the selection condition and uses these statistics to determine which items will be used to optimize the iteration performed on the table.

- This selection condition can be an Ansi or Unicode character string.

- It is not possible to use: 

	- more than 6000 operands in the condition.




- more than 420 logical AND/OR/XOR operands in the condition.



<a name="NOTE0_4"></a>


### Enabling/Disabling a filter
<a name="enablingdisabling_filter_ELTPARAGRAPHE000344"></a>

[HDeactivateFilter](../WDLang4/3044085.md) is used to disable a filter.

[HActivateFilter](../WDLang4/3044158.md) is used to re-enable a filter.

The filter is deleted when the table (query or view) is closed ([HClose](../WDLang4/3044073.md) for example).

A single filter can exist at a given time on a table (query or view). If the **HFilter** function is used several times, only the last filter will be taken into account: the previous filter(s) will be deleted.
<a name="NOTE0_5"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/us/HF.png)![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png)![Stored procedures](https://doc.pcsoft.fr/ext/images/us/PS.png) 

### Filter on a composite key
<a name="filter_composite_key_ELTPARAGRAPHE000370"></a>

Several methods can be used to implement a filter on a composite key:

- Method 1: Using a list of values in the function **HFilter FUNCTION**.

- Method 2: Use [FOR EACH](../Motscles/1510013.md) instruction.

- Method 3: Use the [HBuildKeyValue](../WDLang4/3044258.md) function.




For more details on how to use composite keys on filters, see [Composite keys and filters](../WDLang4/3044178.md).

**Example of method 1: Using a list of values**

The value of the composite key on which the filter will be set can be directly built in **HFilter** using the following syntax:


```txt
HFilter(<File name>, <Name of composite key>, 
	[<List of values for the lower bound of composite key>], 
	[<List of values for the upper bound of composite Key>])
```


**Example** Search for all records in the CLIENT file whose first and last names are between "AA", "Barnabé" and "Philomène" and "Tartuffe".

```wl
HFilter(Customer, LastNameFirstName, ["AA", "Philomene"], ["Barnaby", "Tartuffe"])
```
 In this example, "AA","Zorro" is returned by the filter while "Philomene","Zorro" is not.

**Example** Search for all records in the Tasks file whose tasks fall between 03/15/2021 00:00 and 03/25/2021 00:00.

```wl
HFilter(Tasks, TaskStarDateTaskStartTime, [20210315,0000], [20210325,0000])
```

<a name="NOTE0_6"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/us/AJAX.png)![Stored procedures](https://doc.pcsoft.fr/ext/images/us/PS.png)![OLE DB](https://doc.pcsoft.fr/ext/images/us/OLEDB.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) 

### Non-HFSQL tables
<a name="nonhfsql_tables_ELTPARAGRAPHE000423"></a>

- The filter can be created from any item (key or non-key item).

- The filter is automatically enabled when **HFilter** is executed.

- The changes made with **HFilter** and [HDeactivateFilter](../WDLang4/3044085.md) are only taken into account when the following functions are used [HReadFirst](../WDLang4/3044051.md)/[HFirst](../WDLang4/3044113.md), [HReadLast](../WDLang4/3044140.md)/[HLast](../WDLang4/3044101.md) or [HReadSeekFirst](../WDLang4/3044036.md)/[HReadSeekLast](../WDLang4/3044034.md).




<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[HFSQL functions](../WDLang4/3044156.md)

[Filtering a table, a view or a query](../WDLang4/3044196.md)

[HActivateFilter (Function)](../WDLang4/3044158.md)

[HDeactivateFilter (Function)](../WDLang4/3044085.md)

[HFilterStartsWith (Function)](../WDLang4/3044098.md)

[HFilterIncludedBetween (Function)](../WDLang4/3044097.md)

[HFilterContains (Function)](../WDLang4/1000020267.md)

[HFilterIdentical (Function)](../WDLang4/3044105.md)

[How do I run a table with a filter?](../WDLang4/20011.md)

[&lt;Source&gt;.Filter (Function)](../WDLang4/1000024960.md)

