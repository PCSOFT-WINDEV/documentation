


## HStatNbRec (Function)

***In french: HStatNbEnr***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Returns the number of entries for a given key item. This number was calculated during the last statistical calculation.


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Calculate statistics for the CustomerName key in the Customer table
ResStat = HStatNbRec(Customer, CustName)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = HStatNbRec([<Table> [, <Key item>]])`
---

**`<Result>: (Integer)`**

Number of entries found for the specified item. 
Corresponds to the *hInvalidRecNum* constant if:

- the statistics have never been calculated for the specified item.

- the statistics are not supported by the specified type of item. HFSQL allows you to perform statistical calculations on key items only.




**`<Table>: (Optional character string)`**

Name of the table used. If this name is not specified, **HStatNbRec** will use the last table used by the last HFSQL function (function starting with "H").

**`<Key item>: (Optional character string)`**

Name of the key item used to calculate statistics. If this name is not specified, **HStatNbRec** will use the last item used by the last HFSQL management function (function starting with "H").  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


- The statistics can be updated by [HIndex](../WDLang4/3044133.md) and [HStatCalculate](../WDLang4/3044134.md).

- In the HFSQL Classic tables, the statistical calculations can only be performed on key items.




<a name="XComposante"></a>

## Component:
wd310hf.dll
## Voir aussi

[HStatDate (Function)](../WDLang4/3044117.md)

[HStatTime (Function)](../WDLang4/3044116.md)

[HFSQL functions](../WDLang4/3044156.md)

[HStatCalculate (Function)](../WDLang4/3044134.md)

[HStatNbRecRange (Function)](../WDLang4/3044029.md)

[HStatNbDuplicates (Function)](../WDLang4/3044031.md)

[&lt;Source&gt;.StatNbRec (Function)](../WDLang4/1000025068.md)

