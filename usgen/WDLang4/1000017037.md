...
| &lt;Table&gt; | Physical name of the table to which the trigger applies. |
...
Physical name of one or more HFSQL Client/Server tables associated with the connection ("CUSTOMER.FIC" for example). To specify several names, the names of the tables must be separated by a comma (","). 
If this parameter is not specified or is an empty string (""), the trigger will be enabled on all the tables of the database.  
