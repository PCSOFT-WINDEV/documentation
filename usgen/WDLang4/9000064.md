...
1. In Microsoft Excel 2007, go to the "Data" tab, click on the "From other sources" button and choose the "Provenance: Data Connection Wizard" option.

2. In the "Wizard for data connection" window, choose "Other/Advanced".

3. On the "Properties of data links" window, in the "Provider" tab, select "PC SOFT OLE DB Provider for HyperFileSQL" and click the "Next" button.

4. On the "Connection" tab, specify the parameters for connecting to the HFSQL database (Classic or Client/Server).
...
- In the "Data source" edit control, type the full path of.WDD file of database schema description (optional, used to retrieve the integrity constraints).

- Leave the "Username" and "Password" controls empty.

- In the "Initial catalog to use" combo box, enter the full path for accessing the tables.




The "Advanced properties" tab allows you to enter the "Extended Properties" element of the connection string if necessary.
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=oledb_excel.gif)
...
- **Connection to an HFSQL Client/Server database with a user who has a password**: 
...
5. In the window of "Advanced Information", add a property named "Extended Properties" if necessary:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=oledb_crystal2.gif)
...
- In the "Username" and "Password" controls, enter the information for connecting to the HFSQL server.
...
- In the "File or server name" edit control, enter the full name of the database schema (optional).

- Leave the "Username" and "Password" controls empty.

- In the "Initial catalog" list, enter the full path for accessing the tables.
...
### Example of code that uses the OLE DB provider for HFSQL in C#
<a name="example_code_that_uses_the_ole_provider_for_hfsql_ELTPARAGRAPHE000094"></a>
...
### Example of code that uses the OLE DB provider for HFSQL in PHP
<a name="example_code_that_uses_the_ole_provider_for_hfsql_php_ELTPARAGRAPHE000105"></a>
...
[OLE DB provider for HFSQL](../WDLang4/9000059.md)
