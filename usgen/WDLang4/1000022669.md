...
sRights = MyConnection.InfoViewRights("Moore", hRightsRead + hRightsInsert, "MyDatabase", "MyCustomerView")
...
Name of the database containing the SQL or materialized view for which to get the rights. 
...
Name of the SQL view or materialized view on the server for which to get the rights.
...
| *hEffective* | The rights returned correspond to the rights that will be actually granted to the user or to the group on the view. Inherited rights are taken into account (for example, rights granted to the groups to which the user belongs). |
