...
1. Open the database schema associated with the project: click on ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Ico_Analyse.gif) among the quick access buttons.
...
**If you choose to keep the data in Access format**, the Access tables can be directly handled by the WLanguage functions for managing tables (starting with the letter H). A connection to the Access database is automatically established. 
...
**If you choose to convert the data to HFSQL format**, tables in HFSQL format will be created from the structure of Access tables. These HFSQL tables can be manipulated by table management functions (functions beginning with the letter H).
