...
## What is a port?
<a name="what_port_ELTTEXTE000159"></a>
...
- An HFSQL server is accessible by default on port 4900 and MantaManager on port 4999.




In other words: on a server, there are applications that run. To communicate with these applications, you need a "pipe" identified by a number: the port number.

Examples: a web server uses port 80. Telnet port 23. HFSQL server: 4900 (default).
...
<a name="hfsql_clientserver_ports_used_ELTTEXTE000183"></a>


### HFSQL server
<a name="hfsql_server_ELTPARAGRAPHE000030"></a>

**The HFSQL server** uses port 4900 by default (TCP port).

This port can be configured via the [HFConf.ini](../WDLang4/3044345.md) file found in the installation directory of the HFSQL server.
...
### MantaManager
<a name="mantamanager_ELTPARAGRAPHE000044"></a>

**MantaManager** uses a fixed port 4999 (5002 by compatibility). This service is not required to access an HFSQL server: it allows you to list the servers on a machine and start a server on the machine.

Therefore, there is no need to open this port on Internet.

Note: It is possible to choose another port provided that the port is valid:
...
<a name="registering_the_ports_used_hfsql_clientserver_iana_ELTPARAGRAPHE000059"></a>
...
- 4999: administration of HFSQL Client/Server engines. The port 4999 is a TCP port for listing servers and starting servers, and a UDP port for detecting servers on a local area network (LAN).
...
## Use via Internet
<a name="use_via_internet_ELTTEXTE000219"></a>
...
**If your server can be identified by a name** (e.g. zeus.org), you can use this in the connection. This name must be saved in a DNS service (that can be dynamic as dyndns, no-ip for example)
...
<a name="how_specify_the_port_the_client_application_ELTTEXTE000243"></a>
To specify the port in the client application:

- **Solution 1** Select the port in the database schema ("Advanced" tab in the connection description).

- **Solution 2** Specify the programmed port by following the server name or IP with: (colon) then the port number (e.g. "zeus.org:4920" or "192.172.1.99:4920" ).
...
<a name="why_are_some_ports_closed_default_ELTTEXTE000267"></a>
Limiting the number of open ports on the machine improves security: OS software vulnerability, misconfigured service, unprotected service.
