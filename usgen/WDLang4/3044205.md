...
To associate this connection with a table or a table, use [HChangeConnection](../WDLang4/3044150.md).
...
With **HDescribeConnection**, an Oracle table description imported into the database schema can be directly used by HFSQL Classic tables. Simply describe a temporary connection to an HFSQL Classic database and associate it with the table description from in the database schema. For more details, see the Examples.
...
To assign this connection to a table, use [HChangeConnection](../WDLang4/3044150.md).

**`<Connection>: (Character string)`**

Name of new connection to define. 

If this name is already used by an existing connection, the former connection is replaced with the new one. The former connection (as well as the tables that used it) are automatically closed.
...
Name of new connection to define. If this name is already used by an existing connection, the former connection is replaced with the new one. The former connection (as well as the tables that used it) are automatically closed.
...
1. In the data model editor, display the connection description window: under the "Database schema" pane, in the "Connections" group, click on "Connections".
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=HOuvreConnexion.gif)
...
OLE DB tables or native tables are described in the data model editor. To handle these descriptions through programming with an HFSQL database, simply specify the *hAccessHF7* constant as provider name.

Then, this connection can be assigned to the tables affected by [HChangeConnection](../WDLang4/3044150.md).

The tables in HFSQL format corresponding to the description of the OLE DB or native tables will be created (if necessary) in the directory defined by &lt;Data source&gt;. By default, these tables are empty when they are created.
<a name="NOTE0_3"></a>
...
**Method 2** To describe a connection programmatically and open it automatically when the table is opened, simply:
...
This solution can be useful when an application handles both HFSQL tables and Oracle tables. For example, if the application uses a single Oracle file for a specific process, the connection to the Oracle database will only be made when necessary.
