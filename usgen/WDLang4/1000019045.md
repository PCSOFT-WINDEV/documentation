...
## Questions/Answers
<a name="questionsanswers_ELTTEXTE000314"></a>
...
| --- | --- |
| Answer | A minimum of three physical servers is required to create an HFSQL Cluster configuration: two of them will be the cluster nodes, the third will run the ClusterManager service. |
...
| --- | --- |
| Answer | Because of the throughput required to perform a synchronous replication of the databases, we highly recommend not to dissociate the nodes of a cluster geographically. This configuration is only possible if latency is very low (less than 10 ms).. <br><br>It is also possible to use an asynchronous replication mechanism between two geographically distant clusters. |
...
| Question | After changing the file structure in the database schema (adding, changing or deleting items), how to apply the changes to the tables on the different cluster nodes? |
| --- | --- |
...
| --- | --- |
| Answer | Starting with version 27, automatic data modification support is more comprehensive: <br><br>- Links are managed ([HAddLink](../WDLang4/3044312.md), [HDeleteLink](../WDLang4/3044314.md), etc.)<br><br>- Secure files are supported. <br><br>- etc.<br><br><br> |





| Question | Is it possible to perform Update and Delete SQL queries?  |
| --- | --- |
| Answer | Update and Delete SQL queries can be performed on clustered data. However, these operations can be relatively time-consuming.  |
...
| --- | --- |
| Answer | The current version of HFSQL Cluster has the following limitations compared to HFSQL Client/Server.<br>These limitations will disappear in future versions.<br><br>- Replication is prohibited. <br><br>- A backup cannot be restored without stopping the cluster.<br><br>- The following functions are not allowed:<br><br>	- HAddScheduledOptimization/HModifyScheduledOptimization/HDeleteScheduledOptimization<br><br>	- HAddScheduledBackup/HModifyScheduledBackup/HDeleteScheduledBackup/HExecuteScheduledBackup<br><br>	- HCreateServerTrigger/HDescribeServerTrigger/HActivateServerTrigger/HDeactivateServerTrigger/HDeleteServerTrigger <br><br>	- HSetServer (the following options are not available: hLevelLog, hMaxSizeLog, hPeriodActivityStatistics, hMaxSizeActivityStatistics, hServerPort, hMode2Go and hDebugPort)<br><br>	- HManageTask/HAddTask/HModifyTask/HDeleteTask<br><br>	- HNoDatabaseAccess/HEndNoDatabaseAccess<br><br>	- HNoModif/HEndNoModif<br><br>	- HLogRecreate<br><br>	- HLogRestore<br><br>	- HFree<br><br>	- HModifyDatabaseProperty/HModifyFileProperty/HModifyServerProperty<br><br>	- HBackup/HDeleteBackup<br><br>	- HDeleteDirectory<br><br>	- HDeleteAll<br><br><br><br><br><br> <br> |





| Question | My HFSQL Cluster is "Shutdown" but I can't connect to the nodes. There seems to be a blockage. How to proceed? |
| --- | --- |
| Answer | In this case, you must: <br><br>- cut Manta services. <br><br>- restart HFSQL Cluster administrator. <br><br>- restart Manta services one by one (ignoring synchronization). <br><br><br> |
