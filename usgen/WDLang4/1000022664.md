...
**Remark**: Server management rights are required to use this function.. These rights correspond to the *hRightsManageServer* constant used in [&lt;Connection variable&gt;.InfoServerRights](../WDLang4/1000022668.md) and [&lt;Connection variable&gt;.ModifyServerRights](../WDLang4/1000022710.md).
...
| *hAutoStatisticalCalc* | &lt;Enable&gt; parameter of AUTOANALYSIS section of HFConf.Ini file.<br>Enable the automatic calculation of statistics for the tables (optimization of indexes). <br>Value:<br><br>- 0, the automatic statistical calculation is not enabled, <br><br>- 1, the automatic statistical calculation is enabled.<br><br><br> |
| *hBackupPath* | Parameter &lt;BackupDirectory&gt; in the HFConf.Ini file.<br>Path of the directory in which backups will be made. |
...
| *hConserveHistoryReindexing* | Parameter &lt;ManagerHistoryOptimEnable&gt; in the HFConf.Ini file.<br>Indicates whether the optimization history should be retained: <br><br>- 1 (Default), history is enabled, <br><br>- 0, history is not activated. <br><br><br> |
| *hDaemonUser* | &lt;DaemonUser&gt; parameter of HFConf.Ini file.<br>Username under which the server will operate. |
| *hDatabasePath* | &lt;DBRootPath&gt; parameter of HFConf.Ini file.<br>Path of directory containing all databases (which means the tables of different HFSQL Client/Server applications). |
...
| *hJNLPath* | &lt;JNLRootPath&gt; parameter of HFConf.Ini file.<br>Path of directory containing all the logs of different HFSQL Client/Server applications (history of write operations performed in the HFSQL tables). |
...
| *hMaxNumberConnection* | &lt;MaximumConnectionsNumber&gt; parameter of the HFConf.ini file<br>Maximum number of connections allowed per application. The number of connections is determined by the number of applications (processes) with at least one non-superuser connection on the server. For example: <br><br>- an application that has opened a single connection in super-user mode counts as zero.<br><br>- an application that has opened a single connection with a normal user counts as 1. <br><br>- an application that has opened two connections of a normal user counts as 1.<br><br>- an application that has opened superuser and normal user connections counts as 1. <br><br><br> |
| *hMaxSizeHistoryPlanning* | Parameter &lt;ScheduledHistoryMaxValues&gt; in the HFConf.Ini file.<br>Maximum number of executions to be kept in the scheduled execution history. |
| *hMode2GB* | &lt;MoreThan2GB&gt; parameter of HFConf.Ini file.<br>Specifies whether the size of system tables can exceed 2 GB. <br>Note: the system must handle files larger than 2 GB. |
| *hNdxCacheSize* | &lt;NdxCacheSize&gt; parameter of HFConf.Ini file.<br>Size of the index cache (in MB). The default value is 10 MB. |
| *hServerLanguage* | &lt;ServerLanguage&gt; parameter in the HFConf.Ini file.<br>Language of the HFSQL server. <br>Value: <br><br>- FR: the server language is French. <br><br>- US: server language is English. <br><br>- ES: server language is Spanish. <br><br><br> |
| *hServerPort* | &lt;AccessPort&gt; parameter of HFConf.Ini file.<br>Network port used. |
| *hTelemetryEnable* | &lt;Active&gt; parameter of TELEMETRY section of HFConf.ini file<br>Configure the transmission of data in order to improve the HFSQL server. <br>Value:<br><br>- 1 if the transmission of data (telemetry) was enabled when installing the HFSQL server,<br><br>- 0 if the transmission of data (telemetry) was refused when installing the HFSQL server,<br><br>- another value if no choice was made during the setup regarding the use of telemetry. The question will be asked again during the next setup of HFSQL server.<br><br><br>Note: If the "TELEMETRY" section does not exist in the HFConf.ini file, transmission of product usage data is not enabled for the HFSQL server. |
| *hTempDirectory* | &lt;TmpPath&gt; parameter of HFConf.Ini file.<br>Path of temporary directory. This directory is used to create the temporary files if necessary (temporary files created when re-indexing large tables for example). |
| *hWindowsDiskCacheSize* | &lt;WindowsFileCacheSize&gt; parameter of HFConf.ini file<br>Configure the management of the size for the disk cache of Windows. <br>Value:<br><br>- -1 for a size that is automatically calculated,<br><br>- 0 for an unlimited cache size,<br><br>- a positive value corresponds to a custom cache size.<br><br><br>Note: Windows' standard disk cache management in 64-bit mode (independent of HFSQL) can lead to memory saturation.. This constant is used to customize the management of memory caches. |
...
**Note**: A more detailed description of each parameter is available on page [HFConf.ini](../WDLang4/3044345.md).

- List of options for a standard use: 
	**Remark**: Modifying these options with the **&lt;Connection variable&gt;.SetServer** function takes effect without having to restart the server..
...
| *hAutoStatisticalCalc* | &lt;Enable&gt; parameter of AUTOANALYSIS section of HFConf.Ini file.<br>Enable the automatic calculation of statistics for the tables (optimization of indexes). <br>Value:<br><br>	- 0, the automatic statistical calculation is not enabled, <br><br>	- 1, the automatic statistical calculation is enabled.<br><br><br> |
| *hBackupPath* | Parameter &lt;BackupDirectory&gt; in the HFConf.Ini file.<br>Path of the directory in which backups will be made. The path is created on the server if it does not exist. |
| *hFindKey* | &lt;KeySearch&gt; parameter of HFConf.Ini file.<br>Enable the search for keys. <br>Value: <br><br>	- 0, the automatic search for keys is not enabled,<br><br>	- 1, the automatic search for keys is enabled.<br><br><br> |
| *hkaInterval* | &lt;KeepAliveInterval&gt; parameter of HFConf.Ini file.<br>Maximum time (in seconds) after which, if no communication is established between the server and the client, the server will check the presence of client.<br>This parameter is set to 30 minutes by default. |
| *hkaTimeout* | &lt;KeepAliveTimeout&gt; parameter of HFConf.Ini file.<br>Maximum timeout (in seconds) for the server when checking the presence of client. If the client is not found at the end of this timeout, the client is automatically disconnected.<br>This parameter is set to 2 minutes by default. |
| *hLogLevel* | &lt;ServerLogLevels&gt; parameter of HFConf.Ini file.<br>Defines the elements that must be in the log file. |
...
| *hMaxNumberConnection* | &lt;MaximumConnectionsNumber&gt; parameter of the HFConf.ini file<br>Maximum number of connections allowed per application. The number of connections is determined by the number of applications (processes) with at least one non-superuser connection on the server. For example: <br><br>	- an application that has opened a single connection in super-user mode counts as zero.<br><br>	- an application that has opened a single connection with a normal user counts as 1. <br><br>	- an application that has opened two connections of a normal user counts as 1.<br><br>	- an application that has opened superuser and normal user connections counts as 1. <br><br><br><br><br>By default, this parameter is 0 (no limit). |
...
| *hTelemetryEnable* | &lt;Active&gt; parameter of TELEMETRY section of HFConf.ini file<br>Configure the transmission of data regarding the use of HFSQL server in order to improve the product. <br>Value:<br><br>	- 1 if the transmission of data (telemetry) was enabled when installing the HFSQL server,<br><br>	- 0 if the transmission of data (telemetry) was refused when installing the HFSQL server,<br><br>	- another value if no choice was made during the setup regarding the use of telemetry. The question will be asked again during the next setup of HFSQL server.<br><br><br>Note: If the "TELEMETRY" section does not exist in the HFConf.ini file, product usage data transmission is not enabled for the HFSQL server. |
| *hTempDirectory* | &lt;TmpPath&gt; parameter of HFConf.Ini file.<br>Path of temporary directory. This directory is used to create the temporary files if necessary (temporary files created when re-indexing large tables for example).<br>The path is created on the server if it does not exist. |



- List of options for an advanced use:
	**Remark**: Modifying these options using the **&lt;Connection variable&gt;.SetServer** function will take effect the next time the server is restarted..
	


|   |   |
| --- | --- |
| *hDaemonUser* | &lt;DaemonUser&gt; parameter of HFConf.Ini file.<br>Username that will be used by the server. |
| *hDatabasePath* | &lt;DBRootPath&gt; parameter of HFConf.Ini file.<br>Path of the directory containing all the databases (which means the tables of different HFSQL Client/Server applications). <br>The path is created on the server if it does not exist when restarting the server. |
...
<a name="dynamic_load_balancing_ELTPARAGRAPHE000904"></a>
...
### Log management
<a name="log_management_ELTPARAGRAPHE000918"></a>
...
[HFConf.Ini file](../WDLang4/3044345.md)
