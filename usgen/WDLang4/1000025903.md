...
Note: It is possible to analyze queries created with the query editor, WLanguage queries or SQL queries. The different types of queries can be mixed in the list. 
...
|   |   |
| --- | --- |
| *hSKSimplified* not specified<br>(Default value) | &lt;Result&gt; is a readable message for the developer, containing the list of keys to be added to the tables in order to optimize the queries. This text uses RTF. |
| *hSKSimplified* specified | &lt;Result&gt; is the list of missing keys. If no key is needed, &lt;Result&gt; is an empty string. The list of missing keys has the following format: <br><br><pre><code><File 1> + TAB + <Key 1> + <EOT> + <Key 2> + ...<br> + <Key N> + CR + ... + CR + <File N> + TAB + <Key 1> + <br><EOT> + <Key 2> + ... + <Key N></code></pre><br> where: <br><br>- &lt;File N&gt; is the table to be optimized. <br><br>- &lt;Key N&gt; is the name of the key to be added. <br><br><br> |
...
- Keys are suggested according to the data contained in the table. If a table contains no data, no key will be suggested.
...
[HSuggestKeyStart (Function)](../WDLang4/1000025905.md)

[HSuggestKeyEnd (Function)](../WDLang4/1000025906.md)
