...
**Please note** To position yourself on the first record in the query result, use the [&lt;Source&gt;.ReadFirst](../WDLang4/1000025022.md) function, for example.
...
|   |   |
| --- | --- |
| *hCheckDuplicates* | Enables the management of duplicates during the query execution. The query is not run if a duplicate error occurs.<br>This constant forces the *hWithTransaction* constant. A transaction is started, the records are locked in read/write during the execution of the query and they are unlocked at the end of its execution, regardless of the result (failure or success). If the application is stopped during the query execution (power outage for example), the transaction will be canceled when the tables of query are used for the next time.<br>Note: the execution of the query can be significantly slowed down.<br>![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The management of duplicates is disabled if the *hCheckDuplicates* constant is not specified. An error occurs if the user has no rights to disable the management of duplicates ([&lt;Connection variable&gt;.ModifyDatabaseRights](../WDLang4/1000022707.md)) and if the management of duplicates must be performed.![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
| *hCheckIntegrity* | Enables the management of integrity during the query execution. The query is not run if an integrity error occurs.<br>This constant forces the **hWithTransaction** constant. A transaction is started, the records are locked in read/write during the execution of the query and they are unlocked at the end of its execution, regardless of the result (failure or success). If the application is stopped during the query execution (power outage for example), the transaction will be canceled when the tables of query are used for the next time.<br>Note: the execution of the query can be significantly slowed down.<br>![HFSQL Client/Server](https://doc.pcsoft.fr/ext/images/us/HFCS.png) The integrity management is disabled if the *hCheckIntegrity* constant is not specified. However, an error occurs if an integrity management operation must be performed, but the user does not have the rights to disable integrity management ([&lt;Connection variable&gt;.ModifyDatabaseRights](../WDLang4/1000022707.md)).![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
| *hModifyFile*<br>(constant that can be combined with the other constants) | <br><br>- **On HFSQL tables** When modifying the query result (functions [&lt;Source&gt;.Add](../WDLang4/1000024187.md), [&lt;Source&gt;.Write](../WDLang4/1000024331.md), [&lt;Source&gt;.Modify](../WDLang4/1000025038.md), [&lt;Source&gt;.Cross](../WDLang4/1000025051.md), [&lt;Source&gt;.Delete](../WDLang4/1000025071.md)), these modifications will be transferred to the tables contributing to the query. <br>	If this option is not specified, only the query result is modified. <br>	For more details on modifying the result of a query, see [Modifying the query content](../WDLang4/3044234.md).<br><br>- **On non-HFSQL tables**, the constant *hModifiesFile* is unnecessary: tables contributing to the query are automatically modified when the query result is modified.<br><br><br>![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![Android Widget ](https://doc.pcsoft.fr/ext/images/us/ANDROIDWIDGET.png) This constant is available for HFSQL databases only. This constant is not available for SQLite databases.![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This constant is not available.![Java](https://doc.pcsoft.fr/ext/images/us/JAVA.png) JDBC access: This constant is not available. |
...
<a name="reading_data_during_the_initialization_ELTPARAGRAPHE001023"></a>
...
### SQL query text
<a name="sql_query_text_ELTPARAGRAPHE001058"></a>
...
<a name="executing_sql_query_with_parameters_ELTPARAGRAPHE001070"></a>
...
2. Define variable type **Data source**. The name of this variable must correspond to the &lt;Query name&gt; parameter of **&lt;Source&gt;.ExecuteSQLQuery**. 
	Example: 
...
<a name="sql_injection_how_protect_yourself_ELTPARAGRAPHE001118"></a>
...
### Joins
<a name="joins_ELTPARAGRAPHE001156"></a>
...
<a name="why_should_the_hquerywithoutcorrection_constant_used_ELTPARAGRAPHE001170"></a>
...
| --- | --- |
| The connection associated with the tables in the query is defined automatically. | The connection to use must be specified in **&lt;Source&gt;.ExecuteSQLQuery**. |
...
<a name="condition_composite_key_sql_query_ELTPARAGRAPHE001224"></a>
...
<a name="queries_hfsql_tables_ELTPARAGRAPHE001242"></a>
...
- To optimize query processing time, use [&lt;Source&gt;.StatCalculate](../WDLang4/1000025064.md) or [&lt;Source&gt;.Index](../WDLang4/1000025060.md) on all tables included in the query.
...
- If the tables used by the query are not found in the database schema, these tables must be dynamically declared by [&lt;Source&gt;.DeclareExternal](../WDLang4/1000024214.md). Otherwise, an error such as **Unable to initialize the query. Unknown FILENAME** will occur. 
	Example:
...
<a name="optimization_for_running_the_same_query_several_times_native_oracle_and_sql_server_accesses_only_ELTPARAGRAPHE001296"></a>
...
<a name="specifying_and_retrieving_values_when_running_query_stored_procedures_ELTPARAGRAPHE001340"></a>
...
<a name="sql_queries_docparampagetitleshort_execution_sql_queries_created_the_query_editor_ELTPARAGRAPHE001394"></a>
...
**Remark**: Default automation support is not called for duplicate errors. A simple duplicate error is generated. If an assistance was redefined by [&lt;Source&gt;.OnError](../WDLang4/1000025073.md), then this custom assistance is called.
...
<a name="looping_through_queries_readfirstwdlang41000025022md_ELTPARAGRAPHE001440"></a>
