...
`<Result> = HCreateView(<View to create> , <Table> [, <Items> [, <Sort items> [, <Selection condition>]]] [, <Creation mode> [, <Page size>]])`
---
...
Name of the table used. This table can correspond to a table described in the database schema or through programming ([HDescribeFile](../WDLang4/3044206.md)). You also have the ability to use a [data source](../WDLang4/1514053.md).
...
- To optimize the browse, use [HStatCalculate](../WDLang4/3044134.md) on the table before creating the view. Indeed, the HFSQL engine analyzes the selection condition and it uses these statistics to define the most discriminating items that will be used to optimize the browse operations performed on the tables.
...
<a name="creating_view_filtered_table_ELTPARAGRAPHE000277"></a>

**HCreateView** takes into account the filter set on the source table.
