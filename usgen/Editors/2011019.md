...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_nouveau.gif) in the quick access buttons: 
	...
	- In the window that appears, click "Table", then "Database schema (CDM)"

	- The CDM creation wizard opens.
...
2. Specify:
	...
	- the ***name and directory of CDM***. The CDM corresponds to a ".MCD" file. By default, this table will be created in the directory of the project database schema (&lt;Project name&gt;.ANA directory). This directory must be accessible in read/write mode.

	- the ***CDM caption*** that briefly describes the subject of CDM.

	- whether the CDM must be directly ***associated with a database schema***. This database schema can be an existing database schema or a new database schema that will be created.
