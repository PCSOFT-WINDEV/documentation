...
**Caution**: depending on how project elements are declared (class, collection of procedures, etc.), the elements that are accessible or not may vary.. The table below presents the different methods for accessing an element according to its declaration mode.
...
<a name="summary_table_about_the_elements_and_the_visibility_options_ELTTEXTE000192"></a>
...
|   |   | If the procedure has no access attribute in the component, this procedure is invisible in the project that uses the component.<br><br>If the procedure is public in the component, the procedure will be public in the project that uses the component.<br><br>If the procedure is private in the component, the procedure will not be accessible in the project that uses the component. |
| Project (WDP, WWP or WPP file) | The project can be included (or not) in the component. No visibility option can be defined. The project will always be invisible.<br><br>If the project is included in the component, the project initialization code and the project closing code will be automatically run when loading/unloading the component.<br><br>Please note: if the analysis integrated into the component is protected by a password, it is necessary to integrate the project into the component to specify this password. |
| Analysis | The analysis can be included (or not) in the component. No visibility option can be defined. The analysis will always be invisible.<br><br>Caution: if an analysis is included in the component and if a password was defined for this analysis, you must include the project with the component. |
...
**Note**: By default, component elements use a [independent HFSQL context](../WDChamp/1010029.md).
<a name="NOTE2_2"></a>
...
### Special case: managing enumerations
<a name="special_case_managing_enumerations_ELTPARAGRAPHE000119"></a>

You cannot use an Enumeration variable from an external Android component. 
