...
- The HFSQL engine adds security layers. The password (i.e. the string) is salted and hashed, and the result of these processes will be stored in the table.
...
HFSQL stores only the following elements in the table:
...
- in the database schema associated with your project, change the item type used to store passwords. The "Text" item becomes a "Password" item. 
...
1. Create a new item in the table item description window ("Item description" option in the table context menu in the data model editor).  
...
- Queries are supported on tables containing Password items. The item is read-only and doesn't support any specific operations. The following operations are supported: 
