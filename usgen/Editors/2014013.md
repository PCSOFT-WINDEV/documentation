...
**Remark**: If your application uses the automation menu (menu "?"), the Feedback component is automatically integrated into your application if you select the "Send a suggestion, malfunction or business rule" option.

**Multilingual application**: Feedback component available in English and French only. For the other languages, the wizards of the component will be displayed in English by default. However, the different messages found in the Feedback component can be translated. For more details, see [Translating the components](../Editeurs/2014014.md).
...
<a name="sending_suggestion_problem_business_rule_with_the_feedback_component_ELTTEXTE000237"></a>
...
2. In the wizard, choose the type of request to perform: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MesIncidents2.gif&type=thumb)
...
<a name="configuring_the_feedback_component_ELTTEXTE000267"></a>
...
**Conseil**: enter an email address, which will be used automatically if users are not connected (network access problems, etc.).
<a name="NOTE3_2"></a>
...
- **Control Center database in HFSQL Classic** mode: No special configuration required.

- **Control Center database in HFSQL Client/Server mode**: If you have integrated the Feedback component via the automatic menu (menu "?"), the user taken into account for connection to the database is the current user.. We advise you to:
	...
	- use the parameters of the "Guest" contributor in the code of "? .. Send suggestions, ...".
		...
		3. Modify the line of code used to specify the path of the data files. Use the following line of code:
			...
			- &lt;utilisateur&gt;: Control Center user name.

			- &lt;mot de passe&gt; Password associated with the user.

			- &lt;serveur&gt;: IP address or name of the server where the Control Center database is installed.

			- &lt;port&gt;: Server port.

			- &lt;base&gt;: Name of the HFSQL Client/Server database corresponding to the control center files..
										Note: if you update your application's automation menu, this line of code will be automatically updated with the name of the current developer..
...
<a name="retrieving_and_processing_the_user_feedback_ELTTEXTE000297"></a>
...
## Feedback component: its various functions
<a name="feedback_component_its_various_functions_ELTTEXTE000321"></a>
...
|   |   |
| --- | --- |
| **AddRequest** | Used to add a request to the spool of Project Management Hub. |
...
[How to retrieve the bugs or suggestions of my users? ](../Editeurs/3540719.md)

[Using the Control Centers in Client/Server mode](../Editeurs/3540712.md)
