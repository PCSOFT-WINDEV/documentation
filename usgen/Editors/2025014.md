...
This help page briefly presents the UAC mechanism. For more details, see the online help of Windows (Vista or later).
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=UAC_install.gif)


Note: The aspect of this window will vary depending on whether the executable is signed or not.
...
This redirection is per user: if an application using the redirection mechanism is launched by two different users, these applications will not be able to exchange information.
...
- Warning: When launching an application with "Administrator" privileges, network connections are lost.. This is a Windows security option linked to UAC: when user privileges are increased, network drives are disconnected.. To keep on using them, they must be specifically reconnected. To do so, use the [NetworkConnect](../WDLang1/3066002.md) function.
	
	```wl
	NetworkConnect("S:","\\Server\share", "user", "password")
	```

	Note: the user and password parameters are mandatory in this case, as Windows considers that the user has changed.. 
...
- If an application requires administrator privileges, a manifest must be included in the executable (requesting administrator privileges).
...
Reminder: WINDEV allows you to include a manifest when creating an executable.
<a name="NOTE5_2"></a>
...
Here are the different families of functions or features whose operation can be modified by the activation of the UAC mechanism (Reminder: the UAC mechanism is activated by default on Windows Vista (and higher):
...
**Programming tip**: To create/modify a file without requiring administrator privileges, we recommend following the Windows Vista (and higher) programming standard. For more details, see [Programming standard for Windows Vista](../Editeurs/2025017.md).
...
[Creating an executable: Manifest management](../Editeurs/2025013.md)
