...
- ***on non-shared projects***: For example, the external component is available on the network and can be used by any project. For more details, see [Creating and distributing an external component](../Editeurs/2014002.md).

- ***on projects present in an SCM repository***: The external component can be created from a project present in the SCM and shared with other projects also present in the SCM.




This help page explains how to create and share an external component via the SCM.
...
Note: An SCM project can also use an external component that has not been published in the SCM.
<a name="NOTE2_2"></a>
...
2. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_generation_Composant.gif) in the quick access buttons and select "Generate a component".
...
2. Select the publication directory. This directory is relative to the SCM repository. You can for example group all the external components found in the SCM in a single directory. You can create a specific directory via the "New directory" button in the directory selection window. 
	Note: If the external component is published in the SCM, it cannot be integrated into the reusability center. 

3. In the following step, the wizard prompts you to label the sources to find the version of the elements used during the creation of the component. 
...
## SCM: Using an external component in the SCM
<a name="scm_using_external_component_the_scm_ELTTEXTE000259"></a>
...
3. The screen allows you to:
	...
	- find all the external components found in the SCM repository ("Find" tab). Note: This operation may take some time.
...
### Special case: Replacing an external component with an external component from the SCM
<a name="special_case_replacing_external_component_with_external_component_from_the_scm_ELTPARAGRAPHE000139"></a>

Your project found in the SCM uses an external component that was included via the standard method (file available on network for example) and you want to replace it by an external component found in the SCM: 
...
**Note**: You cannot use the "Relocalize" option to replace the external component integrated in the project with the same component from the SCM.
