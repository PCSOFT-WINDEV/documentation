...
- Edit the text of the control (default option). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=LibelleModifDirect.gif)
...
### Dotted lines for size of mobile devices
<a name="dotted_lines_for_size_mobile_devices_ELTPARAGRAPHE000149"></a>

By default, several types of information are displayed along with the window in the WINDEV Mobile window editor: 

- size of the current target device (landscape and portrait mode).

- size of the recommended device. 
...
- **Automatically set a hotkey when creating a control**: This option is used to automatically create a shortcut when creating the control. This shortcut can be modified thereafter. For more details, see [Keyboard shortcut and hotkey](../WDChamp/1014018.md). 

- **Show a placeholder image when editing Image controls**: This option is used to display (or not) a crosshair in the Image controls. 
	<br>![Image with crosshair](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Image_Mire%20-%20HC%20N%B0001.gif)<br>![Image without crosshair](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Image_Mire%20-%20HC%20N%B0001%201.gif)

	This option can also be disabled control by control. For more details, see [The Image control](../WDChamp/1013218.md). 

- **Calculate anchors when copying and pasting an element**: This option is used to automatically calculate the best anchoring options for a control when a copy/paste is performed in a window. 

- **Set internal window preview zoom to 100%**: Used to force the preview of internal windows at 100% in the Internal Window controls. Therefore, the internal window is properly viewed and the size of the Internal Window control can be adapted in the editor. For more details, see [The Internal Window control](../WDChamp/1013255.md). 

- **Highlight controls when hovered over**: Used to view the limits of the control during the rollover. 

- **Show a UI compilation error when the window theme is not up to date**: Used to display (or not) a specific UI error if the skin template is not udpated. 

- **Adapt zoom to the screen DPI when a window is opened**: Used to optimize the display size of the window in the editor. 

- **Show window shadows**: Used to display (or not) a shadow under the outline of the windows being edited in the window editor. 
...
- **Default input size for edit controls**: Used to define the default size (in number of characters) of the edit control when they are created.
