...
- [Test and debugging of the current project in the editor](#NOTE2).
...
1. On the "Project" tab, in the "Test mode" group, expand "Test mode" and select "Debug project" (Ctrl + F9). You also have the ability to click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Projet_WD.gif) in the quick access buttons.
...
<a name="note_minimizing_not_the_editor_during_the_test_ELTPARAGRAPHE000317"></a>
...
Note: In this mode, function [InTestMode](../WDLang1/3013011.md) returns <u><u><u><u>False</u></u></u></u>.
...
Some problems can only be reproduced when testing the executable in real-use conditions. This test mode allows you to find a problem that only occurs at runtime (and not in test mode). In this case, the code executed comes from the libraries (and not from the code in the elements on the computer). Therefore, the code of the current project and the code of the executable can become out-of-sync: the out-of-sync code is highlighted in pink during the debug operation.
...
- **Starting the debugger from the editor**. In this case, perform the following operations:
	...
	- Go to the "Debugger" pane, "Debug" group, and click the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=main_debog.gif) icon.  
...
- Method 2: Go back to the editor with the taskbar (or press Alt + Tab) and confirm the end of the test. WINDEV goes back to editor that was open when the test was started.




**Remark**: If window UI modifications have been carried out during project testing (modification of field size, position, etc.), at the end of the test WINDEV suggests modifying the interface of the various corresponding windows. For more details, see [Using the UI modification during a project test](../WDLang1/1000021927.md).
