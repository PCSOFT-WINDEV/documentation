...
An environment configuration is a backup of the work environment in the WINDEV, WEBDEV and WINDEV Mobile editors: the different panes and toolbars (and their position) are saved. This configuration can be restored at any time. Up to 4 configurations can be stored per user/developer.
...
**Remark**: The environment configurations are stored in a .INI file found in the "\\Personal" directory of the installation directory of the product used. This file has the following format: "ConfigVoletEnv_&lt;User login&gt;.INI" where &lt;User login&gt; is replaced with the login of the user.
<a name="NOTE2_2"></a>
