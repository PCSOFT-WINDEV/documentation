...
- URL rewriting only applies to Active WEBDEV Pages and pages in SEO-ready Session mode.
...
A configuration window gives an overall view of all rules for rewriting the project. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=URLRewriting_fenppale.gif)
...
- WEBDEV generates a rewriting URL by default. To configure the rules for a specific page, click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Btn_Parcourir.gif) (2) on the right of the table.

- The character used to separate the parameters during the rewriting can also be chosen (3). You can use the "-" or "/" character. 

- The border at the bottom of the window (4) is used to enhance the rules by directly testing the URL rewriting engine. All you have to do is enter an address and the engine indicates whether it corresponds to a page and how the parameters will be extracted.
...
### Configuring page rewriting rules
<a name="configuring_page_rewriting_rules_ELTPARAGRAPHE000072"></a>

The option window proposes the following information for each page whose URL must be rewritten: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=URLRewriting_page.gif)
...
- The "Type" column (1) is used to choose whether the element of the URL is a fixed component or a parameter:
	...
	- A fixed component is an unchanging section of the URL. Fixed components can be multilingual (click the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=URLRewriting_multilangue.gif) column to enter the values in the other languages of the project).
...
2. **Check the configuration of the Web server used.** The URL rewriting will operate only if the Web server calls the WEBDEV Application Server properly.
	**Note**: These points are checked by the diagnostic of the WEBDEV Application Server by entering the name of the virtual server in the "Name/IP" edit control beside the "Diagnostic" button of the "Advanced" tab.
	The following information must be checked: 

	- ".awp" must be associated with the WEBDEV 2026 engine in the virtual site.
...
[Managing short URLs automatically](../Editeurs/2030058.md)
