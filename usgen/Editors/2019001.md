...
**Principle of debugging**: Debugging an application consists of:
...
The debugger allows you to perform these operations. The debugger also allows you to debug the components used in an application. For more details, see [Debugging a component](../Editeurs/2014012.md).

Note: WINDEV, WEBDEV and WINDEV Mobile also provide you with various trace tools (trace window, information box, etc.). For more details, see [Debug without using a debugger](../Editeurs/2019002.md).
...
## Debugger
<a name="debugger_ELTTEXTE000549"></a>
...
- **As soon as you start testing the** project: in the "Project" pane, in the "Test mode" group, pull down "Test mode" and select "Debug project".. The debugger is started when the project is run.
...
- **During the test, by starting the debugger from the code editor**. This method is recommended to start the debugger when a process seems to take an unusual long time (endless loop for example). In this case, perform the following operations:
	...
	- On the "Debugger" tab, click "Pause" ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=main_debog.gif). 
...
- **Through programming**, by adding the **[Stop](../Motscles/1511014.md)** keyword where the debugger must be started.
	...
	**Note** The [Stop](../Motscles/1511014.md) keyword is only taken into account when testing the application in the WINDEV, WINDEV Mobile or WEBDEV editor. The [Stop](../Motscles/1511014.md) keyword is ignored when using the executable.

- **In the code editor** by adding a breakpoint in the code at the point from which the debugger is to be launched. For more details on breakpoints, see [Breakpoints](../Editeurs/2019005.md).
...
<a name="note_minimizing_not_the_editor_during_the_test_ELTPARAGRAPHE000125"></a>
...
![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) **Tests in WEBDEV**
In WEBDEV, if your computer is equipped with a 32-bit system, the debugger will be run in 32-bit mode. On the contrary, if your computer is equipped with a 64-bit system, the debugger can be run either in 32-bit (default), or 64-bit mode. 

To run 64-bit tests: under the "Project" pane, in the "Test mode" group, pull down "Test mode" and select "Debug 64-bit".
...
- by indicating the line of code to run (Backspace).




![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![WEBDEV - Browser code](https://doc.pcsoft.fr/ext/images/us/WBN.png) The server code and browser code can be run.

**1. Running code line by line**
To run the current line of code, use:

- The F7 key to run the code of the line and the code of the procedures called by this line. The execution of the line and the procedures it calls are displayed in the debugger. The corresponding icon in the "Debugger" pane is: icon ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Partie2_PileDesAppels%205.gif).  
...
Note: If the procedure has the extension attribute &lt;sans pas à pas&gt;, its code will be ignored during debugging. For more details, see [Ignoring functions during step-by-step debugging](#NOTE4_6).
...
To run the lines of code found between the debugger cursor and the caret, press the F6 key. The icon that corresponds to this option in the "Debugger" pane is the following: ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Partie2_PileDesAppels%208.gif)

**3. Running code ignoring the breakpoints**
To run code ignoring breakpoints, click the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeIgnorerpoint.gif) icon in the "Debugger" tab of the ribbon.
You can also 
...
**Note** To stop code execution in the debugger, click on the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeContinuerTest.gif) icon. The project test continues without debugger.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=config_debug.gif)
...
## Notes
<a name="notes_ELTTEXTE000621"></a>


### Hierarchy of events/processes
<a name="hierarchy_eventsprocesses_ELTPARAGRAPHE000314"></a>

The events/processes currently run in the debugger are organized hierarchically. This hierarchy is listed in the call stack in the "Debugger" pane. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Partie2_PileDesAppels.gif&type=thumb)
...
**Note**: This feature does not stop debugging under the debugger.
<a name="NOTE4_2"></a>
...
- In the debugger:
	...
	- The ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeVariableLocale.gif) icon is used to display local variables.

	- The ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeVariableGlobale.gif) icon is used to display global variables.

	- You can also view the content of the variables in hexadecimal format via the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Partie2_PileDesAppels%2016.gif) button. 
...
**Watch window**
The contents of variables can be viewed in a special window: the watch window. To display this window, all you have to do is select "Edit value" in the context menu of variable in the "Debugger" pane. This window can be used to display the contents of both simple and structured variables: array, multiline RTF string, structured language type, class, etc.

This window is non-modal: it is possible to open several watch windows on different variables simultaneously. These windows can remain opened when debugging the application. These windows are updated for each statement run. 

This window also allows you to: 

- View the variable displayed in hexadecimal mode.

- Save variable contents to disk (useful for RTF, HTML, image variables, etc.). 
...
The debugger allows you to insert an expression to evaluate. This expression can be of any type: variable, function, operation on variables, etc. The result of the expression is calculated and displayed.
...
***An expression can be auto-stop*** for example, the debugger can be launched as soon as a condition is verified or the value of a variable is modified.
...
1. Select "Add an expression" in the context menu of the "Debugger" pane.
...
The HFSQL status report (![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeCpteRenduHF.gif)) shows the result of the following HFSQL functions (if they exist) in the "Debugger" pane:
...
The WLanguage status report (![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debug_IconeCpteRenduWL.gif)) shows the following information in the "Debugger" pane:
...
### Managing threads
<a name="managing_threads_ELTPARAGRAPHE000436"></a>
...
- Toggle information (call stack, variables, etc.) from different threads using the combo in the "Debugger" pane.  

<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Debog_thread.gif)


When the code to be debugged contains different threads, the "[Real-Time Performance Profiler](../Editeurs/9500237.md)" (or ATR) pane is automatically displayed. 

ATR freezes and releases threads during debugging. For more details, see [Real-Time Performance Profiler](../Editeurs/9500237.md). 
...
<a name="ignore_certain_functions_when_debugging_step_step_mode_ELTPARAGRAPHE000464"></a>
...
[Debugging an external component](../Editeurs/2014012.md)
