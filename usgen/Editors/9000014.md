...
More specifically, you have the ability to generate the Eclipse project. Indeed, the call to the WINDEV environment can be included in the Eclipse menus. A plug-in is provided with WINDEV.
...
To generate the Java source codes of the project: 

1. Depending on the current project configuration: 

	- If the current project configuration corresponds to the Java archive, on the "Project" tab, in the "Generation" group, click "Generate".

	- If the current project configuration does not correspond to the Java archive, use the quick access buttons: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_generation_java.gif)
...
**Note**: To start the Java application after generating the Java sources, you must either import and run the project in Eclipse, or compile and run the content of the scr directory located at the root of the EXE directory.
...
1. Run the WDEclipsePlugin.zip file located in the "\\Programs\\Data" folder of WINDEV.

2. Unzip the WDEclipsePlugin.zip files in the "Eclipse" software installation directory.
...
From the "Package Explorer", you have the ability to directly start the WINDEV project via the context menu of the project.
<a name="NOTE3_3"></a>
...
Only the following controls can be handled through programming in native Java code in the Eclipse project: 
