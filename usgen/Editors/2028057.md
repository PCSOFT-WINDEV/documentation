...
| The FTP server is not configured properly: wrong username, ... | Check the configuration of the FTP server for the user. <br><br>Warning: If you are using the IIS FTP server, check that:<br><br>- the user is defined at Windows level<br><br>- the alias of FTP transfer directory corresponds to the Windows username.<br><br><br> |
...
**User password verification failed: the user is unknown to the server.**
...
| Possible problem | Solution |
| --- | --- |
| The update failed to be encrypted. | The host must free some memory space and/or disk space on the computer that performs the update (computer of site manager for instance). |
