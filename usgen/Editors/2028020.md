...
- the options for remote control are as follows:
	...
	- delay between each remote control: 5 minutes.

	- logout message: "This application has been locked by its administrator for an update and is currently unavailable."

	- imminent stop message: "This application will be interrupted for an update. Please save all your changes and exit the application."

	- forced shutdown message: "ATTENTION: Application shutdown for update".
...
Advanced network installation options are set in the WDInst installation editor: in the "Tools" pane, in the "Utilities" group, click on "WDInst".
...
**Note** Control option settings can be modified by the application manager from the [program for remote check of WINDEV executables](../WDAdminExe/2028052.md) (WDADMINEXE) installed on the server workstation.
<a name="NOTE3_2"></a>
...
Note: If no message is entered, the following message will be displayed: "The previous update of the application did not complete successfully and the application cannot run. Do you want to restart the update?".
<a name="NOTE3_3"></a>


### Important: changing control options in WDADMINEXE
<a name="important_changing_control_options_wdadminexe_ELTPARAGRAPHE000109"></a>
...
To inform the users that an application update is available, the following window is displayed when the application is started. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=FenMiseAJour.gif)
...
- ***Apply the update automatically if it is proposed*** (default option). 
	...
	During this timeout, the user has the ability to cancel the update or to start the application if the update is not mandatory.
...
- ***Apply the update only if it is mandatory***. 
	...
	If the update is optional, a click on the "Ignore" button will be performed at the end of the timeout for "Automatic validation". During this timeout, the user has the ability to install the update.
...
- ***Cancel the start of the application if the update is mandatory***. 
	...
	During this timeout, the user has the ability to install the update or to start the application.




**Note** By default, regardless of the language in which the installation program is run, this window is displayed in French only. To display this window in a language other than French, you must translate the WDSetup project.
<a name="NOTE4_2"></a>
...
2. Specify the timeout (in seconds) after which the information window will be validated or closed (60 seconds by default). 
...
This timeout is ignored if the "Don't validate the updates automatically" option is checked.
...
- **the interactions with the user when checking the availability of an update**. 
	...
	- If there is no network connection when opening the application, a warning message is displayed by default. 
			This message can be useless for an application used on a mobile device, connected intermittently. In this case, simply select "Silent check: Don't inform user". 
...
The help about the new features is configured in the "Help on the new features" tab of the advanced options of the wizard. 
<a name="NOTE5_2"></a>
...
2. Select the file containing the history ("Path of history file"). This file can be in text format (".TXT") or in RTF (".RTF" or ".WRI" for example).




**Remark**: To view the selected file directly, click on the "Preview" button.. The file will be displayed:
