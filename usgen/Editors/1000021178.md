...
**Note** User Groupware works with Active Directory. It does not work with openLDAP.
...
3. In the "Tables" tab:
	...
	- Check "Login window only (no management of rights)". In this case, the User Groupware will create no table. A supervisor will not be able to define the rights for the windows and for the pages.
...
3. In the "Tables" tab:
	...
	- If necessary, uncheck "Login window only (no management of rights)". In this case, the User Groupware creates the tables for managing the users and the rights. A supervisor can define the rights for the windows and for the pages.
...
4. A window allows you to import all the users.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=gpu_ldap.gif)
If you are not authenticated on the LDAP server (1st use for example), a login and password of an LDAP user will be requested. The selected LDAP user must have the rights to list the users of LDAP directory.
	**Please note** Depending on the LDAP strategies defined in Active Directory, import may be limited to 1,000 users. In this case, to remove this limitation, it is necessary to modify the LDAP MaxPageSize parameter. For more details, see [https://support.microsoft.com/kb/315071](https://support.microsoft.com/kb/315071). 
...
**Note: Using an LDAP directory with management of rights enables you to easily switch between the LDAP mode and the non-LDAP mode. Simply check "Use an LDAP directory"**.
<a name="NOTE2_3"></a>
...
LDAP server characteristics must be entered in the "tables" tab of the User Groupware settings window.
...
**In manual run mode**: 
To use the LDAP directory, an LDAP session must be started with [gpwOpen](../WDLang6/3041002.md) before calling [gpwOpenLDAPSession](../WDLang6/1000017055.md):
