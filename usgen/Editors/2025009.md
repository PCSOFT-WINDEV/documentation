...
**Attention**: When the executable program is launched, the libraries (".DLL" files) are automatically extracted and replace the existing files.. These replacements may trigger problems if several WINDEV applications are installed on the same computer.
...
**Solution**: To avoid overwriting existing libraries (".DLL" files), we recommend renaming the libraries used by the application when creating the executable program.
<a name="NOTE2_2"></a>
