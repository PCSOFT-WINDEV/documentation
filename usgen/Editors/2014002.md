...
1. **Elements of the external component**: 
	...
	2. Right-click to open the context menu and select "Configurations". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Configuration_de_projet2%20-%20HC%20N%B0001.gif&type=thumb)
In the menu appears: 
...
4. **Declaration and visibility of the elements**
	...
	Note: By default, if an external component uses a database schema and tables, the HFSQL functions will handle these elements in an independent HFSQL context. This operating mode can be modified in the advanced options of the component.
...
- Mode for managing the themes on the component elements.
...
- Access advanced options by clicking on "Component options". You can then define: 
	...
	- [Theme management for component elements](#NOTE4_4).
...
<a name="access_rights_the_component_the_client_project_ELTPARAGRAPHE000225"></a>There are three options:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Creer_un_composant_et_le_distribuer%20-%20HC%20N%B0002.gif)


- **The component will not access the tables, windows, pages and reports of the client project.**
	The component and its host application use independent contexts (default option). The component cannot handle the tables or the elements of the host project.
...
- **The component will access the tables of the client project.**
	The component is allowed to handle only the tables of the host application. In this case, the HFSQL contexts will be identical.
	If this option is selected, the "Initialization" event of the component project will be called when the component is loaded by the host project.

- **The component will access the tables, windows, pages and reports of the client project.**
	The component is allowed to handle the tables of the host application as well as its elements (windows, reports, etc.). In this case, the execution contexts will be identical.
...
**When should the component be allowed to handle the tables of the host application?**
When the final application consists of a main application and options provided as components. In this case, the components handle the tables of the main application.

**Note**: If the component can handle tables and has its own database schema, the database schema of the component will be ignored. The component will use the database schema of the host project.

In this example, the component can be associated with the database schema of the main application. Therefore, there is no need to use the external declarations to handle the tables. Furthermore, auto-completion suggestions will appear in the code editor.

For more details on HFSQL contexts, see [HFSQL context](../WDChamp/1010029.md).
<a name="NOTE4_4"></a>


### Style: Theme of the component elements.
<a name="style_theme_the_component_elements_ELTPARAGRAPHE000280"></a>

By default, the theme of the host project can be applied to the different elements of the external component. For more details on themes, see [Themes](../Editeurs/2016011.md).
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Creer_un_composant_et_le_distribuer%20-%20HC%20N%B0003.gif)


Remarks: 

- You can allow or prevent the client project from applying the theme. Go to the "Style" tab of the advanced options of the external component. 

- To avoid applying the theme of the client project to a window of the external component:

	1. Open the component description window. 

	2. On the "Style" tab, in "Themes", check "Always keep the initial theme/theme".
...
- in the quick access buttons, click on the component generation icon: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_generation_Composant.gif)
...
7. The list of files integrated into the component is displayed. This list is built from the files in the "Other" folder of the "Project explorer" pane. For example, you can integrate: 
	...
	- HFSQL tables used by the application, etc. 
...
9. Indicate if there are libraries to be integrated into the Android component. You can include: 
	...
	- the HFSQL library. 
			 This library is required if your component uses HFSQL tables.
