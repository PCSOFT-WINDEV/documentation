...
The CustomMenu allows you to create a custom menu in the menu bar of WINDEV, WEBDEV and WINDEV Mobile. This lets you add shortcuts to your favorite applications (Calculator, etc.), open different files (Charter, etc.) and more..<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=PersoMenu18.gif)
...
1. Click the logo corresponding to the product version. Select "CustomMenu .. Customize the &lt;CustomMenu&gt;" from the menu that is displayed. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=persomenu_creation.gif)


2. The CustomMenu configuration window appears.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=PersoMenu_description.gif&type=thumb)
...
5. Enter the command lines that will be run when this option is selected. You can type:
	...
	- the path of an executable: enter the path directly in the field, or use the "Browse..." button.

	- Macros: click on the "Macro" button and double-click on the macro you wish to add to your command line.. For example, "Name of Current Project" {$ProjectName} will be replaced with the name of your project currently edited.
			The main macros are as follows: 

		- {$ProjectName}: Name of the current project. 

		- {$RepProjet}: Current project directory. 

		- {$CompleteProjectPath}: Complete path of the current project. 

		- {$ExeName}: Name of the executable generated from the current project. 

		- {$ConfigName}: Name of the current configuration for the current project. 
...
6. Select the runtime options of the command line:
	...
	- Leave the console opened If a console is displayed, it will remain open until the program is finished.
...
**Note**: It's also possible to add a folder or separator to a PersoMenu. A folder is used to create a sub-menu.
<a name="NOTE2_2"></a>
