...
The update of a WEBDEV Session or AWP website does not affect connected users. Active users stay on the old version. They will only have access to the new version of the site the next time they log in. 

This deployment mode allows session sites to be deployed with zero downtime. 
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=D%E9ploiement_sans_deconnexion%20-%20HC%20N%B0002.gif)


WARNING: this mode is **not recommended** for: 
...
Note: This feature is only available for sites deployed with WEBDEV version 24 and higher.. 
...
1. **Case of a first site deployment**: 
	...
	- Deploy your site for the first time. 
			...
			Note: Installation is similar to classic mode (as the site has never been deployed, there can be no logged-in user!).
...
2. **Case of a site already deployed**: If the site was already installed using the "classic" method: 
...
**Caution**: It is not possible to redo a "classic" installation after a "no-disconnect" installation.. If you want to reinstall your site using the "classic" method, you must: 
...
[Multiple setup of same site on the same server](../Editeurs/2028099.md)
