...
1. In the editor, create a new project of "Library" type.
	...
	- Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_nouveau.gif) in the quick access buttons. Click "Project" in the wheel that is displayed. The project creation wizard starts. 
...
4. Create the library: ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=genererbibliotheque.gif)The wizard for library creation starts. 
...
8. Specify the information about the library version: 
	...
	- the version of the library. All the characters (digits and letters) are allowed. For example: "1.01A".
		...
		- automatically increment the last characters (digit or letter) of the version number whenever the library is created (check "Increment version automatically"). 
						For example:
...
If a library is closed with [UnloadWDL](../WDLang1/3013013.md), the HFSQL data files of that library are not automatically closed.

Any access to the HFSQL data files will trigger the following error: "The XXX WDL file was unloaded". It is recommended to use [HClose](../WDLang4/3044073.md) on the data files opened in a library that will be unloaded.
