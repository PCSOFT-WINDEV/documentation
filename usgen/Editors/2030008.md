...
3. Specify the elements to include in the new patch (windows, reports, ...). You can also:
	...
	- **Add elements** ("Add" button): 
			Any type of file can be added to the list of items inserted in the patch: images, text files, etc.
...
6. Validate. The application patch is created in the PATCH subdirectory of the current project directory as well as in the EXE subdirectory of the project. The file name is in the form: ***&lt;Project Name&gt;.WDL.001*** where 001 corresponds to the patch version number. Rename &lt;Project name&gt; with the name of the library only if the project name differs from the library name.
...
***&lt;Project name&gt;.001.EXE*** where 001 is the patch version number.

**Note: Patch containing** classes: If your patch contains a class, the class declaration code must not be modified from the version used with the executable.. Declaration code must be identical: same members, same structures, same declaration order. Otherwise, the executable must be recreated.
...
**If you haven't created an** installation file: copy the file corresponding to the patch into the application directory (the directory containing the application library).

**If you have created an** installation file: run the file corresponding to the self-extracting executable on the user's workstation and extract the patch into the application directory (the directory containing the application library).
...
**Note**: This mechanism is not supported for network installation via HTTP.
...
<a name="translating_the_interface_for_selfextraction_ELTPARAGRAPHE000145"></a>The interface of the self-extracting program can use a language other than the default language (English).<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Zipcreeexe.gif)
To do so, create a .TXT file containing the following information:
...
When a new patch is created on this new executable, you must respect the numbering of patches proposed by WINDEV. The patch number cannot be reinitialized (see the next paragraph).					
<a name="NOTE5_3"></a>
...
[Application patch](../Editeurs/2030031.md)
