...
**Remarque**: Please note that this feature is subject to Apple's goodwill, which may decide to remove the downloadable application from its store..
...
2. If necessary, select the IP address of the development computer that corresponds to the network to which the iPhone or iPad is connected: 
	...
	- On the "Home" tab, in the "Environment" group, expand "Options" and select "Code editor options".
...
3. Select "GO .. Debug on new iPhone/iPad". The window for connecting to an iOS device appears. This window contains a specific QR code. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Test_Appli_iOs%20-%20HC%20N%B0001.gif)
...
- Custom fonts added to the project are used during testing. 
...
"Unable to connect to this device: WMDev version is too old to use with the debugger"


Solution: Check the App Store to see if an update for the WMDev app is available (https://itunes.apple.com/fr/app/wmdev/id1437792304) and update the app on the iOS device.

Please note: The WMDev application update may be available several days after the message appears (time required for Apple to validate the application).. 
