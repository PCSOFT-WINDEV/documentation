...
**Note**: Analyses, queries, sets of procedures, reports and classes have the same format in WINDEV and WINDEV Mobile. These elements can be shared between a standard WINDEV project and a WINDEV Mobile project. For more details, see [Sharing WINDEV elements](../WDLang3/3023045.md).
...
**Note**: The size of the imported window corresponds to its original size. It is necessary to check the UI and window size in order to adapt them to the platform (Android, iOS, etc.).
...
- Controls that do not exist in WINDEV Mobile are automatically removed (Toolbars, etc.).
...
### File dependency
<a name="file_dependency_ELTPARAGRAPHE000056"></a>

Windows are imported with their dependencies, this means that all files linked to the imported window are also imported. Images are automatically converted into a format supported by WINDEV Mobile. For example, the background image of a window is automatically imported and converted into GIF format.
<a name="NOTE3_3"></a>
