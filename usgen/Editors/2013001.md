...
- ***complete the current entry*** with an intelligent list of possible values (function name, property name, etc.). All the values suggested in the list contain the word being typed. 
	...
	For example, when entering the characters "OKA", all elements (functions, fields, constants, code snippets, etc.) containing these characters are proposed.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=completion.gif)
 **Remarks**: 
...
- ***suggest the value of the possible parameters*** according to the WLanguage function used.
	For example, when using [HRead](../WDLang4/3044047.md), the first expected parameter corresponds to the name of a data file. The automation will only propose data files from the analysis associated with the current project. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=completion2.gif)
...
- ***select a combination of constants for a given parameter***. 
	...
	Note: constants are separated into different colored zones to identify constants that can or cannot be combined (two constants present in a colored zone with a picker cannot be selected simultaneously). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Completion_cst.gif)
