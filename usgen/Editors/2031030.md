...
- other windows found in the pattern (a window for configuring the emails for instance, ...).
...
**Important: don't forget to create the first project window in your RAD pattern. In most cases, this first window corresponds to the menu window.**
<a name="NOTE3_2"></a>
...
2. Create the menu: under the "Window" pane, in the "Bars and menus" group, pull down "Main menu" and select "Add main menu".

3. The menu options used for the specific RAD options must have a specific name: "MENUFIC_". The menu with this name will be duplicated as many times as necessary for the different tables of the database schema that require a menu entry. The code associated with the options will also be duplicated and adapted (if necessary) to the file used.
...
**Example**: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Pattern_Menu_deroulant.gif)
...
2. Create a Carousel control: under the "Creation" pane, in the "Other controls" group, click on "Carousel".
...
END
```
In this example, the "//RADBEGINNING" and "//RADEND" comments are mandatory: they are used to identify the code that will be duplicated for each file processed by RAD.
<a name="NOTE3_4"></a>
...
For example: MENUCTRFIC_RADFileA button, with "Manage RADFileA" for caption and the following code: 
