...
It is a "Design Pattern" that proposes to divide and structure the architecture of user interfaces into layers. It can be combined with a data access layer (DAL), which separates in-memory information from physical access to the database. It allows you to separate the UI management code from the code that uses business data.

This layered architecture of application and UI simplifies the maintenance and evolution of projects.

**Note**: This development mode uses OOP. It requires a good knowledge of these concepts. This development mode is an advanced development mode. 
...
## How it works
<a name="how_works_ELTTEXTE000243"></a>
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=MVP.gif)
...
Some operations may require interaction with the user (error display, confirmation, etc.), and these interactions must be handled by the VUE.
...
The PRESENTER layer does not have access to the VIEW, which means that the PRESENTER layer must not directly access the controls of the VIEW.

On the contrary, the PRESENTER layer can request from the VIEW to be updated (via [RequestRefreshUI](../WDLang1/1000023899.md)).

The same PRESENTATION layer can be used by several VIEWS (e.g., a window or report displaying data from the same MODEL, such as a customer list).
...
The PRESENTER layer contains data and decides which "data" will be displayed in the VIEW. For example, change the status of a field, change the color of a line in a Table control, etc.
...
The advantage of separating this layer is that the logical structure of the data and storage format (relational DB, SQL DB, XML files, Webservice, etc.) can be modified and upgraded centrally.
...
- information common to the application (e.g. centralize connection to a DB, etc.),
