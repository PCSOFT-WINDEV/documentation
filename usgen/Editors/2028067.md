...
- **Clear directory before generating setup**: If this option is checked, the contents of the directory in which the installation program is generated will be emptied before a new installation is generated.. This option allows you to delete any useless file.

- **Protect the setup via a password**: This option is used to specify the password that will be requested when the application is installed.

- **Sign the setup with the certificate**: To guarantee the integrity of the installation package, it is possible to sign the installation.. Simply select the certificate to use. The certificates available on the current computer, in the personal stored or Windows certificates, are automatically proposed. 




Note: For installation via a Private Store, only self-extracting executable support is available. 
...
**Note**: For more details on how to use WDSetup (customization, testing, usage), see [Using a custom WDSetup](../Editeurs/2028037.md).
<a name="NOTE4_2"></a>
