...
When several tables are used in a query, the query editor automatically searches for the links between these tables defined in the database schema. These links are called joins in the query editor.

For example, the following query selects the numbers of the orders placed by the customers who live in Paris. The ORDERS and CUSTOMER tables are joined by the CustomerNum item. The query editor automatically defines the joins between these two tables.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Requete_exemplePlusieursFic.gif)
**Note**: Table bindings appear in the graphical representation of the query only if both linked tables are used by the query.
...
This gives you the ability to create multiple joins between two tables.
<a name="NOTE2_2"></a>
...
3. Click the "Delete join" button. The join is automatically deleted. The joins between tables are automatically optimized in order for the query to operate properly.
...
When calculating a join between two tables ("Parent" table and "Son" table), by default only records with a match in both tables are retained. However, the notion of "External join" allows you to define a rule to include: 

- records in the "Parent" table which have no match in the "Son" table.

- records in the "Son" table which have no match in the "Parent" table.
...
1. Right click the join and select "Edit join" in the context menu. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0001.gif)
...
5. Select the option(s) corresponding to the desired result.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0002.gif&type=thumb)
...
2. The description window of the query joins appears. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0002.gif&type=thumb)


3. Select the join to edit.

4. Click the "Edit join conditions" button. The edit window of conditions is displayed.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0003.gif)


5. You can: 

	- Add a new condition. ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0003%201.gif)

	- Modify a condition. ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0003%202.gif)

	- Delete a condition. ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0003%203.gif)
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0003%201.gif). The condition creation window appears.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Jointure_Requ%EAte%20-%20HC%20N%B0004.gif)
...
4. Select the type of element to compare for the operation to perform (3): 
	...
	- the field: this option is selected in the case of a comparison with a field in another table in the query. Only the "key" items are proposed.
...
<a name="NOTE3_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png)![Native Connectors (Native Accesses)](https://doc.pcsoft.fr/ext/images/us/AN.png) 
...
[How to create a query with external join?](../Editeurs/20063.md)
