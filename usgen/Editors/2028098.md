...
This setup mode is an option of the setup in local network with automatic update (Live Update). For more details on installation modes, see: [The WDInst setup editor](../Editeurs/2028042.md).
...
1. Launch the installation creation wizard: under the "Project" pane, in the "Generation" group, pull down "Setup procedure" and select "Create installation procedure".
...
**Remarque**: The "WDAdminExe" tool is present in the "INSTALL" subdirectory of the application's reference installation.. WDAdminExe can be redistributed.  
...
2. Go to the "Push" tab of the main window: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=WDAdminExe_2%20-%20HC%20N%B0002.gif&type=thumb)
...
4. Click "List the computers" to validate the connection information and to start enumerating the client computers.
	**Caution**: depending on the LDAP strategies defined in Active Directory, import may be limited to 1,000 users. In this case, to remove this limitation, it is necessary to modify the LDAP MaxPageSize parameter. For more details, see [https://support.microsoft.com/kb/315071](https://support.microsoft.com/kb/315071). 
...
8. Click "Start the PUSH setup..." to start the setup.
	The window for starting the setup is displayed: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=WDAdminExe_2%20-%20HC%20N%B0003.gif&type=thumb)
...
10. The summary (2) displays the list of target computers for the setup. Some computers can be deleted from this list via the context menu.
...
**Remark**: An "Advanced" button lets you set the waiting time for push installations.. If several setups fail with timeout errors, you may have to adjust this parameter.


### Services required on the target computers
<a name="services_required_the_target_computers_ELTPARAGRAPHE000096"></a>

**Note**: Depending on the version of Windows used, the names of these services in the Control Panel may vary.
