...
- **Directly from the WINDEV, WINDEV Mobile or WEBDEV editor** In the "Project" pane, in the "Audit and performance" group, scroll down to "Analyze performance" and select "Analyze performance". In this case, the project is automatically executed in test mode. The process to optimize can be run in your application. 
	...
	The performance profiler displays the result of the database schema. This result is saved as a WPF file.
...
- **During debugging** with a performance database schema between two breakpoints. All you need to do is set the start and end breakpoints for the performance profiler. Only the code between these two breakpoints will be analyzed. The report will be automatically displayed in the editor at the end of the database schema. For more details, see [Breakpoints](../Editeurs/2019005.md).  
...
- **From any application** Press the key combination **WIN + Shift + F10** has the same effect as calling function [ProfilerStart](../WDLang1/3013012.md). 
	In this case, you can terminate the database schema by closing the application, or by pressing the Windows logo key + Shift + F11. 
	When the recording stops, the location of the database schema report is copied to the clipboard.
...
The performance profiler presents the result of the database schema in two tabs:
...
The following information is displayed for each process:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Analyseur%20performances%20-%20HC%20N%B0004%201.gif)



|   |   |   |
| --- | --- | --- |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle1.gif)<br> | Parent | Element or process that called the function. |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle2.gif)<br> | Function | Function, process, event or procedure run. |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle3.gif)<br> | Total time | Function execution time. |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle4.gif)<br> | Calls | Number of calls made to the function (procedure, event or process). |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle5.gif)<br> | Time per call | Execution time of a call to the function (procedure, event or process) |
| <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Cercle6.gif)<br> | % code | Percentage of time it took to process the function or procedure (developer code that can be optimized). |
...
3. The following tab is displayed:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Analyseur%20performances%20-%20HC%20N%B0004.gif&type=thumb)
...
- To quickly view the current process, all you have to do is click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Detail_Description_FEN_btnCode%20-%20HC%20N%B0001%201.GIF). The profiler remains opened and the current code can be modified.
