...
2. Specify the batch name, its location and its description.
	**Remark**: The file corresponding to a batch has the extension ".WDBatch".
...
4. Specify the processes that must be run by the batch. A list of processes is proposed by default. You have the ability to add or delete processes.
	...
	- the management of automated tests. If the option "Continue the batch execution regardless of errors" is selected, a "warning" will be written into the status report whenever an error occurs in the automated tests.
...
5. Specify the post-batch actions to run. You have the ability to run an executable after the batch, to close the editor, to switch the computer off, ... A status report can also be sent into WDBal.
...
[Batch processing: creation of executables, WDLs, components, etc.](../Editeurs/2020004.md)
