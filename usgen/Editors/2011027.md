...
The content of a database schema can be exported in the format of SQL scripts for a specified database (SQL Server, Oracle, ...). This script generates the integrity constraints, the indexes, ...
...
To generate the SQL script corresponding to a database schema (or to a table):

1. On the "Database schema" tab, in the "Database schema" group, expand "Generation" and select "Generate SQL script". The wizard starts.
...
[Generating a description of a table from a window or from a page](../Editeurs/9000079.md)
