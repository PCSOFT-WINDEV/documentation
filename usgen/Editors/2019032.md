...
WEBDEV offers several methods to test and debug a site on the development computer (see [The different types of WEBDEV tests](../Editeurs/2019003.md)). However, in some cases, you may have to debug the site directly in a production environment.
...
To debug a dynamic WEBDEV website on a remote application server, you must:
...
- **enable remote debugging** on WEBDEV Application Server. To do so, check "Allow the remote debugging" in the "Configuration" tab of the administrator.  
...
- If the debug operation is performed on a WEBDEV Application Server for deployment, the code run is the code found in the library (WDL file). The code can therefore be out of phase between the current project code in the editor and the site code: out-of-phase code is overlined in pink when debugging.. [InTestMode](../WDLang1/3013011.md) returns <u><u><u><u>True</u></u></u></u>.
...
## Debugging a site in use on a remote application server
<a name="debugging_site_use_remote_application_server_ELTTEXTE000282"></a>
...
- Method 1: Close the browser. WEBDEV displays the editor open when the test was started.
