...
## Overview
<a name="overview_ELTTEXTE000691"></a>
...
- [Android 10: New behaviors](../Editeurs/9000195.md).

- [Android 11: New behaviors](../Editeurs/9000201.md).
...
<a name="available_project_elements_ELTTEXTE000715"></a>
<a name="NOTE1_1"></a>
...
- Queries.
	**Note**: in a parameterized query, all parameters are mandatory.
...
- External components. 
	 **Note** Internal components are available for Android development. However, if the internal component includes a database schema, this database schema will be ignored during the execution.
...
- [Combo Box control](../WDChamp/1013023.md)
	**Note** It is not possible to assign a negative or zero value to a Combo Box control. If the Combo Box control is not empty, an element has to be selected. An error will occur if a negative or null value is assigned to a Combo Box control.

- [Image control](../WDChamp/1013218.md)

- [Check Box control](../WDChamp/1013151.md)

- [Static Text control](../WDChamp/1013179.md)
...
- [Static Text control with display mask](../WDChamp/1013179.md)
...
## Databases
<a name="databases_ELTTEXTE000745"></a>
...
- Memo management with HFSQL: The Unicode or ANSI runtime string usage mode defined in [project configuration](../Editeurs/9000030.md) determines the character set used for storing. By default, ANSI strings assigned to HFSQL memo items are encoded in UTF-8. [ChangeCharset](../WDLang1/3054001.md) can be used to change the character encoding

- SQLite database: If your Android project shares its database schema with another (non-Android) project, you can use the [HChangeConnection](../WDLang4/3044150.md) function to associate the application's tables with an SQLite connection. For more details, see [Android application: Using and sharing data](../Editeurs/9000123.md). 

- Memo management with SQLite: Storing information in the form of memos is recommended only for small amounts of data (less than 1 MB).. We advise you to directly save the data in external file format on the internal or external memory of the phone and to store the access path to this data in the database.  

- You also have the ability to access an external database (Oracle, SQL Server, ...) via a web service. For more details, see [Accessing a database via a web service](../WDLang4/1000021063.md).
...
## Application menus
<a name="application_menus_ELTTEXTE000769"></a>
...
- On the devices without "right click" (phones for example), the context menus are displayed after a "long press" on the control to which they are associated.
...
## Modal windows
<a name="modal_windows_ELTTEXTE000799"></a>
The modal widows are not supported by the Android operating system. The [Open](../WDLang1/3038035.md) function is not allowed.
...
- When closed, a child or sibling window can call a local procedure of the window that opened it in order to pass it a return value. A return value can also be returned via a global variable.
...
<a name="managing_the_back_and_home_buttons_ELTTEXTE000823"></a>
...
Note: It is possible to change this behavior and execute custom processing (e.g. to ask the user for confirmation and possibly not close the window, especially if this is the application's first window). To do so, the window must have a button with a "'Back' button" shortcut ("UI" tab in the control description window).


### Home button
<a name="home_button_ELTPARAGRAPHE000299"></a>

Pressing the "Home" button does not close the current application: the application is put into the background.. The "Close" event is not executed. 
**Note** Even if the window has a Button control with the keyboard shortcut "Home Button" (defined in the "UI" tab), the code for this button will not be executed. Unlike the Back button, the default behavior cannot be changed.
...
**Note** The Android system does not allow this option to be programmed.

<a name="NOTE6_1"></a>


## Notification about the application inactivity
<a name="notification_about_the_application_inactivity_ELTTEXTE000853"></a>
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=android_anr.gif)
...
**Note** We strongly advise against manipulating application interface elements from a secondary thread.
...
<a name="operating_mode_focus_and_selection_ELTTEXTE000877"></a>
...
- The clickable but non-editable controls (buttons, list boxes, etc.) behave like the controls in interruption. They do not gain focus when they receive a click.
...
**Warning** In a List Box control or RepeatString field, the banner displayed does not represent the selection, only the focus.

<a name="NOTE9_1"></a>


## Automatic scrollbars in the windows
<a name="automatic_scrollbars_the_windows_ELTTEXTE000901"></a>
...
<a name="management_directories_ELTTEXTE000925"></a>
...
<a name="changing_the_screen_orientation_ELTTEXTE000949"></a>
...
Note: The [GPS functions](../WDLang3/1000019192.md) family is used to manage the phone's geolocation.. 

<a name="NOTE14_1"></a>


## Miscellaneous
<a name="miscellaneous_ELTTEXTE000973"></a>
...
- DPI-dependent image management: Depending on the device's DPI, images may be pixelated or blurred. You can use the automatic DPI management. For more details, see [Android/iOS: Provide one image per DPI](../WDChamp/9500137.md).
...
## Thread management
<a name="thread_management_ELTTEXTE000997"></a>
...
<a name="indirection_operators_ELTTEXTE001021"></a>
...
<a name="using_native_android_fonts_ELTTEXTE001045"></a>
...
## Managing ADB logs
<a name="managing_adb_logs_ELTTEXTE001069"></a>
...
- in the [Android Control Center](../CCAndroid/1410086630.md) ("Logcat" tab): under the "Tools" pane, in the "Mobile tools" group, click on "CCAndroid").
...
## Printing to PDF
<a name="printing_pdf_ELTTEXTE001093"></a>
Under Android, PDF printouts support only Adobe standard fonts (not Unicode): Courier, Helvetica, Times, Symbol and ZapFDingbats. 
<a name="NOTE20_1"></a>


## API level
<a name="api_level_ELTTEXTE001117"></a>
...
| From WINDEV Mobile 28 -> WINDEV Mobile 28 "Update 1" | 31 / Android 13 |
| Starting with WINDEV Mobile 28 "Update 2" | 33 / Android 13 |
| Starting with WINDEV Mobile 2024 "Update 2" | 34 / Android 14 |
| From WINDEV Mobile 2025 | 34 / Android 15 |
| As of WINDEV Mobile 2025 Update 1 | 35 / Android 15 |
...
[Summary table: Managing application directories by platform](../Editeurs/1000020167.md)

[Accessing a database via a web service](../Editeurs/1000021063.md)

[Testing and debugging an application on an Android device](../Editeurs/9000193.md)

[Android 10: Changing application behavior](../Editeurs/9000195.md)

[Android 11: Changes to application behavior](../Editeurs/9000201.md)
