...
- to a user directly. The user installs the application on his computer. This application can be a single-user application, a multi-user application... The setup is stand-alone.
...
## Individual setup
<a name="individual_setup_ELTTEXTE000159"></a>
...
- **Standard setup**. The setup program used is the one provided with WINDEV or a customized version (the WDSetup example corresponds to the source code of the installer and it can be modified).
...
<a name="setup_with_automatic_update_ELTTEXTE000183"></a>
...
- [Local network (LAN) setup](../Editeurs/2028112.md), with "Push" support. A first application setup will be created on the network, in a shared directory. This directory will contain the setup program used by the end users and the reference version used to manage the updates.

- [Setup by HTTP](../Editeurs/9000026.md). The application is first installed on an HTTP server. The users will connect to a Web page in order to download the setup program of application.
...
### Checking versions
<a name="checking_versions_ELTPARAGRAPHE000060"></a>
...
### Connected users
<a name="connected_users_ELTPARAGRAPHE000075"></a>
