...
## Overview
<a name="overview_ELTTEXTE001437"></a>
...
- **SQL query execution with SQL code verification** This option is available by default for all SQL queries executed from WINDEV, WEBDEV and WINDEV Mobile. The SQL code of the query must use the commands listed below. In this case, the HFSQL engine checks the SQL code of the query.

- **Execute SQL query without checking SQL code (use constant hQueryWithoutCorrection)** This option is recommended for queries executed via native or OLE DB access. In this case, the HFSQL engine does not check the query. This option must be used if the query contains commands specific to a connection type (Oracle, SQL Server, etc.). All the SQL commands supported by the accessed database can be used.




**Note** The *hQueryWithoutCorrection* option is also available when testing a query from the query editor:

- for a Select query, click the "Advanced" button of the description window. On the "hQueryWithoutCorrection" tab, check "Run with hQueryWithoutCorrection in test mode". You have the ability to choose the connection that will be used to run the query in test mode.
...
## Available commands
<a name="available_commands_ELTTEXTE001461"></a>
...
|   |   |
| --- | --- |
| [ADDDATE](../Editeurs/2034005.md) | Adds a value (year, month, etc.) to the specified date. |
...
| [CURDATE](../Editeurs/2034005.md) | Returns the date and the local time of server.<br><br> |
| [CURRENT_DATE](../Editeurs/2034005.md) | Returns the date and the local time of server.<br><br> |
| [CURRENT_TIME](../Editeurs/2034005.md) | Returns the date and the local time of server.<br><br> |
...
| [CURTIME](../Editeurs/2034005.md) | Returns the date and the local time of server.<br><br> |
...
| [EOMONTH](../Editeurs/2034005.md) | Returns the last day of the month containing the specified date. |
...
| [GROUP_CONCAT](../Editeurs/2034005.md) | Concatenates the non-null values of an item from a series of records into a string. |
...
| [HOUR](../Editeurs/2034005.md) | Extracts the number of hours from an hour or date-time. <br><br> |
...
| [INSERT](../Editeurs/2034001.md) | Adds a record to a table. |
...
| [LOCALTIME](../Editeurs/2034005.md) | Returns the server's local time.<br><br> |
| [LOCALTIMESTAMP](../Editeurs/2034005.md) | Returns the server's local time.<br><br> |
...
| [MINUTE](../Editeurs/2034005.md) | Extracts the number of minutes from an hour or date-time. <br><br> |
...
| [MAINTENANT](../Editeurs/2034005.md) | Returns the server's local time (in date-time format). |
...
| [ROLLBACK](../Editeurs/2034011.md) | Rolls back a transaction. |
...
| [ROLLBACK TRANSACTION](../Editeurs/2034011.md) | Rolls back a transaction. |
| [ROLLBACK WORK](../Editeurs/2034011.md) | Rolls back a transaction. |
...
| [SECOND](../Editeurs/2034005.md) | Extracts the number of seconds from an hour or date-time. <br><br> |
| [SEC_TO_TIME](../Editeurs/2034005.md) | Convert seconds to duration. <br><br> |
...
| [SHIFT_LEFT_OP](../Editeurs/2034005.md) | Bit operator: << |
| [SHIFT_RIGHT_OP](../Editeurs/2034005.md) | Bit operator: >> |
...
| [SPLIT_PART](../Editeurs/2034005.md) | Cuts the string according to a separator and returns the nth part. |
| [ST_AREA](../Editeurs/2034012.md) | Calculates the area occupied by a geometry. |
| [ST_CONTAINS](../Editeurs/2034012.md) | Find out whether a geometry is contained within another geometry. |
| [ST_COVEREDBY](../Editeurs/2034012.md) | Finds out whether ALL the points in the first geometry are contained in the second geometry, or whether at least one of the points in the first geometry is not part of the second geometry. |
| [ST_COVERS](../Editeurs/2034012.md) | Finds out whether ALL points in the second geometry are contained in the first geometry, or whether at least one point in the second geometry is not part of the first geometry. |
| [ST_CROSSES](../Editeurs/2034012.md) | Finds out whether one of the two geometries has some of its interior points in common with the second geometry (and not necessarily all of its points). |
| [ST_DIFFERENCE](../Editeurs/2034012.md) | Returns a geometry corresponding to the difference between the two geometries passed as parameters. |
| [ST_DISTANCE](../Editeurs/2034012.md) | Calculates the distance between 2 geometries. |
| [ST_EQUALS](../Editeurs/2034012.md) | Determines whether 2 geometries are identical. |
| [ST_GEOMFROMTEXT](../Editeurs/2034012.md) | Convert geometry described as a string into 2-dimensional geometry. |
| [ST_INTERSECTS](../Editeurs/2034012.md) | Find out if 2 geometries have points in common. This command calculates the common area (intersection) between the two geometries. |
| [ST_LENGTH](../Editeurs/2034012.md) | Returns the length in meters of a linear geometry. |
| [ST_MAKEPOLYGON](../Editeurs/2034012.md) | Returns a Polygon geometry corresponding to the description of the lines described in the geometry to be analyzed. |
| [ST_OVERLAPS](../Editeurs/2034012.md) | Determines whether the dimension of the geometry resulting from the intersection of the 2 geometries is equal to the dimension of the compared geometries, but different from each of the compared geometries. |
| [ST_PERIMETER](../Editeurs/2034012.md) | Returns the perimeter in meters of the specified geometry. |
| [ST_SIMPLIFY](../Editeurs/2034012.md) | Returns a 'simplified' geometry of the specified geometry. |
| [ST_SYMDIFFERENCE](../Editeurs/2034012.md) | Returns a geometry corresponding to the union of 2 geometries passed in parameter, from which are subtracted the shapes corresponding to the points common to the 2 geometries. |
| [ST_TOUCHES](../Editeurs/2034012.md) | Determines whether two geometries have at least one of the external points describing their respective contours in common. |
| [ST_UNION](../Editeurs/2034012.md) | Returns a geometry corresponding to the union between two geometries. |
| [ST_WITHIN](../Editeurs/2034012.md) | Finds out whether the first geometry is contained in the second geometry, i.e. whether ALL the points making up the first geometry are contained in the second geometry. |
...
| [SUBDATE](../Editeurs/2034005.md) | Subtracts a value (year, month, etc.) from the specified date. |
...
| [DATESYS](../Editeurs/2034005.md) | Returns the current date and time. |
