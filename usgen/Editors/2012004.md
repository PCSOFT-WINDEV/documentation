...
The .NET technology is Microsoft's new development platform. This .NET platform simplifies the development of distributed object applications whose modules communicate via the Internet.
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_generer_exenet.gif) in the quick access buttons. The wizard for generating a .NET application starts. 
	Note: If this icon is not present, the selected configuration does not correspond to a .NET application.. You can: 

	- Select the ".NET application" configuration of your project in the project explorer. 

	- Create a new configuration for your project. This configuration is automatically proposed by expanding the generation icon and by selecting "Other generations .. Generate a .NET application". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Generation_Exenet.gif)
...
2. Specify [general options of the executable](#NOTE3_1): executable name, executable program icon, splash screen name.

3. Specify [properties of the executable](#NOTE3_3): company, application description and copyright. 
...
6. Select the version of the .NET Framework to be used and the WINDEV Framework (32-bit or 64-bit) to be delivered with the .NET assemby.
	Note: If a .NET executable is to run in both 32-bit and 64-bit versions, it is necessary to generate 2 .NET executables in WINDEV. The same .NET executable cannot operate in 32 bits and in 64 bits.

7. Select the [libraries of WINDEV framework](#NOTE3_5) (".DLL" files) used by the executable program.
	**Warning** It is not possible to rename the wdxxxnet.dll DLL.
...
9. Select the options specific to the .NET executable:
	...
	- The **"Advanced properties"** button is used to define whether the .NET executable must be accessible by a program that uses the COM technology. For more details, see [Create a .NET assembly that can be accessed using COM](../Editeurs/2012003.md).
...
- the icons must have been created by a tool that organizes the icons by increasing size and by increasing color resolution. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=icoxp.gif)
...
- **Format for compatibility.** Until version 12, this format was the default for WINDEV applications..
	**Note**: All characters (numbers and letters) are allowed. For example: "1.01A".
	If the "Automatic version increment with each generation" option is checked, the version number will be changed with each generation: version number "1.01A" will become "1.01B" and version number "A14Z" will become "A15A".
...
**Note** To find out which WINDEV library a function uses, go directly to the function's online help (see "Component" section).
...
- **rename this library** ("New name" column). If several WINDEV applications are installed on the same computer, each executable uses its own libraries.
	Note: WINDEV libraries must not be renamed for programs created using an external language.
...
When creating the executable, all the loaded WINDEV libraries will be automatically copied into the "EXE" subdirectory of the project.

**Reminder** By default, the installation program automatically selects all files in a project's "EXE" sub-directory, with the exception of tables (".FIC", ".NDX", ".MMO"). For more details, see [Setup program](../Editeurs/2028001.md).
