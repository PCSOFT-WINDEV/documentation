...
- **Geolocation**: Impossibility of combining ACCESS_FINE_LOCATION and ACCESS_BACKGROUND_LOCATION permission requests into a single operation. This implies a change in the code of applications that use background location. 

- **Access to files**: an Android application running on Android 11 can no longer access files created by another application by manipulating these files via their path.. This implies a verification of the file handling processes and changes in the code, if necessary. 

- **Limitations on access to Mac** addresses: [NetMACAddress](../WDLang3/1000016006.md) function no longer available. It always returns an empty string. 
...
<a name="reminder_the_targetsdkversion_target_api_level_ELTPARAGRAPHE000021"></a>
...
- almost anywhere in the external storage: 
	...
	- in application directories on external storage: SysRepExternalStorage function with sseAppXXX constants.

	- in public directories: Documents, Download, Images, etc.. on external storage: SysDirExternalStorage function with ssePublicXXX constants.Android applications can access files created by other applications on the external storage without restrictions. 
...
However, there is a special permission that lets you override these restrictions: MANAGE_EXTERNAL_STORAGE. To activate it, you must: 
...
Please note: if you publish your application on the store, you must be able to prove that you have this permission.. Only file manager, antivirus or backup manager applications are allowed. In other cases, if this permission is used, the publication is very likely to be refused.

**Alternative solution (for existing applications only)**: Add *preserveLegacyExternalStorage* attribute to application manifest. The process for adding the *preserveLegacyExternalStorage* attribute to an application is detailed in FAQ 21 548:
[How to allow an existing application to access files on the external storage? (preserveLegacyExternalStorage attribute)](https://faq.windev.com/faqread.awp?idfaq=21548).
