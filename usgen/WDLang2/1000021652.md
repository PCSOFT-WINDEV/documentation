


## &lt;Cell&gt;.DisplayDialog (Function)

***In french: &lt;Cellule&gt;.AfficheDialogue***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Displays a Cell control  in a page with a DDW effect (Dim Disabled Windows). This function is used to easily simulate a dialog box in browser code, by using a Cell control in a page.

For more details, see [Communicating with the Web user](../WDChamp/1012026.md).
<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Display a Cell control containing a question
CELL_Question.DisplayDialog(cellCenter, True, 50)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Cell control>.DisplayDialog([<Display position> [, <Take DDW into account> [, <Dimming rate>]]])`
---

**`<Cell control>: (Control name)`**

Name of the Cell control that will be displayed.

**`<Display position>: (Optional Integer constant)`**

Display position of cell:


|   |   |
| --- | --- |
| *cellBottomLeft* | The Cell control is at the bottom left of the visible part of the current page. |
| *cellBottomRight* | The Cell control is at the bottom right of the visible part of the current page. |
| *cellCenter*<br>(Default value) | The Cell control is centered in the visible part of the current page. |
| *cellPositionXY* | The Cell control remains at its current position. |
| *cellTopLeft* | The Cell control is at the top left of the visible part of the current page. |
| *cellTopRight* | The Cell control is at the top right of the visible part of the current page. |



**`<Take DDW into account>: (Optional boolean)`**



- <u><u><u><u>True</u></u></u></u> (default) to display the Cell control with a DDW effect (the page displayed behind the Cell control is grayed)

- <u><u><u><u>False</u></u></u></u> to disable the DDW effect.




**`<Dimming rate>: (Integer)`**

Rate of gray (included between 0 and 100) used by the DDW. 
The maximum value (100) will be used if the specified value is greater than 100. 

The DDW is ignored if the specified value is less than 0. The default dimming rate is set to 18%.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Characteristics of Cell controls
<a name="characteristics_cell_controls_ELTPARAGRAPHE000222"></a>

- The Cell control must be overlayable ("UI" tab of the Cell control description window).

- To avoid displaying unnecessary scrollbars in the browser, check the "Force position at the top left of the page when loading" option ("Advanced" tab of the Cell control description window). This option is used to automatically position the Cell control at position (0,0) when loading the page. Then, the display position defined by **&lt;Cell&gt;.DisplayDialog** is taken into account.




**Note** To execute server code from a Cell control, use [AJAXExecute](../WDLang2/3055114.md) or [AJAXExecuteAsynchronous](../WDLang2/3055115.md).
<a name="NOTE0_2"></a>


### Special cases
<a name="special_cases_ELTPARAGRAPHE000258"></a>

- **&lt;Cell&gt;.DisplayDialog** must be used to display the Cell control:

	- in a "None" button. In this case, only the browser code of the button will be executed.

	- in a "Submit" button and activate AJAX in the code of this button. In this case, the browser and server codes of the button will be executed.




- The Cell control displayed can be hidden with [&lt;Cell&gt;.CloseDialog](../WDLang2/1000021653.md).

- Browsers don't refresh the display in the middle of Javascript code: the functions **&lt;Cell&gt;.DisplayDialog** and [&lt;Cell&gt;.CloseDialog](../WDLang2/1000021653.md) functions must be used in different events/processes. If these functions are used in the same event/process, the Cell control is not displayed.

- It is not possible to display two different Cell controls at the same time using **&lt;Cell&gt;.DisplayDialog**.




<a name="XComposante"></a>

## Component:
WDJS.DLL
## Voir aussi

[Communicating with the Web user](../WDLang2/1012026.md)

[&lt;Cell&gt;.CloseDialog (Function)](../WDLang2/1000021653.md)

[Functions for managing pages (prefix syntax)](../WDLang2/1000021657.md)

[CellDisplayDialog (Function)](../WDLang2/1000017139.md)

