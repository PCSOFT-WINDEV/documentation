


## &lt;Page&gt;.DisplayDialog (Function)

***In french: &lt;Page&gt;.AfficheDialogue***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Displays a page in modal mode. This function is used to establish a dialog with the user. The page is displayed in the foreground while the opening page is displayed in the background, grayed by the DDW mechanism. For more details, see "Resulting display". 

The code of the page is locked until the page is closed by [PageCloseDialog](../WDLang2/1000017166.md). The page can return a value.


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Displays a page allowing the user to exit from the site
IF PAGE_Exit.DisplayDialog() = True THEN 
	// Displays an end page
	PAGE_End.Display()
	// Close the session
	EndProgram()
END
```
<a name="Exemple2"></a>

```wl
// Displays a page to validate the information entered in the form
PAGE_Info.DisplayDialog(EDT_LastName, EDT_FirstName, EDT_Email_address)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = <Page>.DisplayDialog([<Parameter 1> [... [, <Parameter N>]]])`
---

**`<Result>: (Type corresponding to the retrieved value (optional))`**

Value retrieved when closing the page to open. This value is returned:

- by the [Return](../Motscles/1510007.md) keyword in the page closing code,

- by the [PageCloseDialog](../WDLang2/1000017166.md) function,

- by the [ReturnedValue](../Proprietes/2510134.md) property used before closing the page,

- by the [ContextClose](../WDLang2/3058003.md) function.




&lt;Result&gt; corresponds to <u><u><u><u>False</u></u></u></u> and an error is displayed if the user presses the browser "Back" button.

**`<Page>: (Page name)`**

Name of the page to open.

**`<Parameter 1>: (Type of value sent to the page (optional))`**

First parameter that will be passed to the "Global declarations" event of the page to open. This parameter is passed by reference and is considered as a variable global to the page.



**`<Parameter N>: (Type of value sent to the page (optional))`**

Nth parameter that must be passed to the "Global declarations" event of the page to open. This parameter is passed by reference and is considered as a variable global to the page.



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Resulting display
<a name="resulting_display_ELTPARAGRAPHE000228"></a>

The page displayed by **&lt;Page&gt;.DisplayDialog** is displayed in the foreground.

Only the border and the content of the page are displayed. This page has no title bar, no taskbar, etc. The page background is ignored, regardless of its color.

The page that used **&lt;Page&gt;.DisplayDialog** is in the background. This page is grayed by DDW (Dim Disabled Pages). The DDW feature can be configured by [PageEnableDDW](../WDLang2/1000017087.md) and [PageRateDDW](../WDLang2/1000017086.md).

**Note**: The open page is not automatically centered. The page display mode must be defined in the page characteristics.. We advise you to include a large part of the page in a cell with a background color in order to avoid the display effects caused by the DDW feature.


### Limitations
<a name="limitations_ELTPARAGRAPHE000254"></a>

- The page displayed must be a non-AWP dynamic page. 

- **&lt;Page&gt;.DisplayDialog** must be used in a non-AWP dynamic page. 

- If your project uses pre-launched sessions, this function must not be used in the project initialization event. This function must be used in the "Initialization in pre-launched session mode" event. 






<a name="XComposante"></a>

## Component:
wd310page.dll
## Voir aussi

[Functions for managing pages (prefix syntax)](../WDLang2/1000021657.md)

[PageCloseDialog (Function)](../WDLang2/1000017166.md)

[PageEnableDDW (Function)](../WDLang2/1000017087.md)

[PageRateDDW (Function)](../WDLang2/1000017086.md)

[Communicating with the Web user](../WDLang2/1012026.md)

[PageDisplayDialog (Function)](../WDLang2/1000017124.md)

