...
Starts a dynamic WEBDEV website from a static page or from a dynamic WEBDEV page. The dynamic site to start must be on the same Web server as the current site.
<a name="Exemple1"></a>
...
Name of page to display. If this parameter is not specified or is an empty string (""), the first page of the site will be displayed.

**`<HTTP parameters>: (Optional character string)`**

Parameter that must be passed to the dynamic WEBDEV website. These parameters can be retrieved by [PageParameter](../WDLang2/3058026.md). The list of parameters must have the following format:
...
Other JavaScript parameters to use when opening a new browser if &lt;Target&gt; is set to the *NewBrowser* constant. For example, "dependent = 1" is used to force the closing of the new browser if the current browser is closed.  
...
**DynamicSiteDisplay** automatically sends the language of the current page to the WEBDEV website to be displayed. Therefore, if the current page is in French (*Nation(5)*), the WEBDEV website will be started in French.
<a name="NOTE0_2"></a>
...
The &lt;Page&gt; parameter can correspond to any page of the site. For this page, the option "Accessible via the DynamicSiteDisplay function" must be checked in the "General" tab of the page description.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=SiteDynamiqueAffiche%20prob.gif)
...
SWITCH PageNumber
	CASE 1: PageDisplay(PageAAfficher)
END
...
Another Active WEBDEV Page has a Button control for which the option "During the action" is set to "Don't send anything to the server".
