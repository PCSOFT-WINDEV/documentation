...
**Remark**: The **LooperDisplay** function can be used on either a file or memory Looper control.
...
- An integer. The refresh is performed from this position. The selected row is not modified. The "Select a row" event of &lt;Position&gt; is run. The current element in the Looper control is the same as the current record in the table. If this integer is invalid (outside the authorized bounds), the call to **LooperDisplay** triggers an error.
...
| *taCurrentSelection* | <br><br>	- Refreshes the Looper control from the position of selection bar in the Looper control.<br><br>	- The position of selection bar is not modified. The current record in the table will correspond to the record selected in the Looper control (after **LooperDisplay** is called).<br><br><br> |
...
[&lt;Looper&gt;.Display (Function)](../WDLang2/1000023451.md)
