


## LooperAddBeginning (Function)

***In french: ZoneRépétéeAjouteDébut***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Adds a row at the beginning of an Infinite Looper control. The row is inserted before the first row in the Looper control. The scrollbar of the Looper control remains at the same position. 

Note: It is recommended to use this function in the "Add additional elements at the top" event of the Looper control.
<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
LooperAddBeginning(LOOP_Email, Email.Date, Email.Text, Email.Sender)
LooperAddInProgress(LOOP_Email, taRefreshIndicatorUp, False)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`LooperAddBeginning(<Looper control> [, <Value 1> [... [, <Value N>]]])`
---

**`<Looper control>: (Control name)`**

Name of the Looper control to be used.
If this parameter is an empty string (""), the row will be added in the current Looper control.

**`<Value 1>: (Type of associated attribute (optional))`**

Value of the 1st attribute of the row to be inserted in the specified Looper control.
The different values must be specified in the order of the attributes (Looper control description window).
If you don't want to specify a value for one of the attributes, use an empty string ("") or 0 (depending on the attribute type).
If none of these parameters is specified, an empty row will be inserted into the Looper control.



**`<Value N>: (Type of associated attribute (optional))`**

Value of the Nth attribute of the row to be inserted into the specified Looper control.
The different values must be specified in the order of the attributes (Looper control description window).
If you don't want to specify a value for one of the attributes, use an empty string ("") or 0 (depending on the attribute type).
If none of these parameters is specified, an empty row will be inserted into the Looper control.  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### Use conditions
<a name="use_conditions_ELTPARAGRAPHE000074"></a>

**LooperAddBeginning** can be used on: 

- a Looper control based on a data file, 

- a Looper control populated programmatically. 


![WEBDEV - Browser code](https://doc.pcsoft.fr/ext/images/us/WBN.png) This function is not available for Linear Looper controls. This function is available for Looper controls in browser mode only. ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This function is available for Looper controls in AJAX mode, Looper controls in classic mode and Linear Looper controls. 
<a name="NOTE0_3"></a>


### Row characteristics
<a name="row_characteristics_ELTPARAGRAPHE000083"></a>

If a value is the result of a calculation, enclose the numerical expression in brackets. 

For example:

```wl
LooperAddBeginning(LOOP_Calculation, (53+29), (83-21))
```

<a name="NOTE0_4"></a>


### Sort
<a name="sort_ELTPARAGRAPHE000102"></a>

**LooperAddBeginning** ignores the sort.

<a name="XComposante"></a>

## Component:
wd310obj.dll
## Voir aussi

[Infinite Table and Looper controls](../WDLang2/1013307.md)

[LooperAddInProgress (Function)](../WDLang2/1000021638.md)

[Looper control functions](../WDLang2/3083007.md)

[&lt;Looper&gt;.AddBeginning (Function)](../WDLang2/1410090639.md)

