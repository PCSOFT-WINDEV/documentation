


## xlsUnmergeCell (Function)

***In french: xlsDéfusionneCellule***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Unmerges a range of cells. The range of cells is identified by one of its cells. 


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
MyXLSDocument is xlsDocument
// Unmerges the range of cells identified by cell B2
xlsUnmergeCell(MyXLSDocument, "B2")
```

<a name="XSYNTAXE"></a>

## Syntax
<a name="SYNTAXE1"></a>

### Unmerging a range of cells identified by the row and column of a given cell

`<Result> = xlsUnmergeCell(<Excel document> , <Cell row> , <Cell column>)`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the range of cells was unmerged.

- <u><u><u><u>False</u></u></u></u> otherwise.




**`<Excel document>: (xlsDocument variable)`**

Name of the variable of type [xlsDocument](../WDLang5/1000017464.md) to be used.

**`<Cell row>: (Integer)`**

Row number of one of the cells in the range of cells to unmerge.

**`<Cell column>: (Integer)`**

Column number of one of the cells in the range of cells to unmerge.


<a name="SYNTAXE2"></a>

### Unmerging a range of cells identified by the reference of a given cell

`<Result> = xlsUnmergeCell(<Excel document> , <Cell>)`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the range of cells was unmerged.

- <u><u><u><u>False</u></u></u></u> otherwise.




**`<Excel document>: (xlsDocument variable)`**

Name of the variable of type [xlsDocument](../WDLang5/1000017464.md) to be used.

**`<Cell>: (Character string)`**

Cell in the range of cells to unmerge. This parameter has the following syntax: &lt;Column letter&gt;&lt;Row number&gt;. For example: "A32".



<a name="XComposante"></a>

## Component:
wd310xls.dll
## Voir aussi

[xlsMergeCell (Function)](../WDLang5/1410089537.md)

[xlsDocument (Variable type)](../WDLang5/1000017464.md)

[XLS file functions](../WDLang5/3080009.md)

[&lt;xlsDocument variable&gt;.UnmergeCell (Function)](../WDLang5/1410089571.md)

