...
<a name="NOTE1"></a>
<a name="NOTE1_1"></a>
The purpose of a page is to display, consult and enter information. This information can come from the tables found in a database schema, external files, queries, etc.

WEBDEV offers several options for creating the project pages:

- Create a blank page with the wizard.

- Build all the site pages from the database schema description.

- Create a page from the database schema description (with or without its code).

- Create pages based on a template, etc.


The page can be modified afterwards regardless of the method used to create it. You can add, modify or delete controls and change the page characteristics.
...
- [Opening a page in a WEBDEV website](#NOTE10_1).
...
## Creating a page
<a name="creating_page_ELTTEXTE000668"></a>
...
<a name="creating_blank_pages_ELTPARAGRAPHE000085"></a>

To create a blank page:

1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P1_ICO_Cr%E9er_cpt.gif) in the quick access buttons of WEBDEV. In the window that appears, click "Page" then "Page". The page creation wizard opens. 
...
<a name="creating_preset_pages_ELTPARAGRAPHE000118"></a>
...
- the "RAD" tab is used to create RAD pages that allow data display and input. This type of page can be bound to a table, query, etc.
...
<a name="creating_the_project_pages_automatic_site_building_rad_ELTPARAGRAPHE000143"></a>

For dynamic sites (Session, AWP, or PHP), the project pages can be automatically created in a single operation from the database schema description, by automatic site building (RAD).
...
1. On the "Project" tab, in the "Generation" group, click "Full Application RAD".
	**Caution**: To use this option, the project must be associated with a database schema generated at least once.

2. Select the RAD pattern to use.

3. Select the tables used for the generation.

4. Select the main tables that will be the entry points in the application (these tables will be mainly used to implement the menu options).
...
<a name="simple_operations_page_ELTTEXTE000710"></a>
The page editor allows you to perform the following operations on the pages:

- Open a page in the editor. To do so, simply double-click the page name in the "Project explorer" pane.
...
<a name="main_characteristics_page_ELTTEXTE000734"></a>
A page can accept all types of controls. A page can:

- use a ***background image***: The image is used as the background of the page and controls are laid out over it.

- have a ***menu***: This menu will give users quick access to the site's main features. To create a drop-down menu in a page, use a "Menu" control. The first two options are displayed in the page. The options can be manipulated via the context menu. They can also be manipulated programmatically.

- have a ***status bar***: This bar will display help messages associated with the different page controls. .

- be ***multilingual***: All languages defined for the page will be available for all page controls. A page can be associated with more languages than the project (e.g., pages shared between multiple projects). For more details, see [Multilingual websites](../Concepts_WB/1410087463.md).


**Remark**: Web pages designed to be viewed on iPhone can benefit from specific features, such as splash screen at launch, full-screen mode, etc. These features make it possible to develop pages that behave like real applications. WEBDEV allows you to easily configure these specific features while keeping the compatibility with other browsers (mobile or not) on the market.
...
## Internal page
<a name="internal_page_ELTTEXTE000758"></a>
...
**Note**: The internal page to be shared can come from a component.
...
## Page templates
<a name="page_templates_ELTTEXTE000782"></a>
...
## Popup page
<a name="popup_page_ELTTEXTE000806"></a>
...
Note: It is also possible to display a page as a popup. In this case, the popup will be displayed using [PopupDisplayPage](../WDLang2/1410087258.md). 
...
<a name="refreshing_page_session_mode_ELTTEXTE000830"></a>


### Overview
<a name="overview_ELTPARAGRAPHE000299"></a>
...
### Implementation
<a name="implementation_ELTPARAGRAPHE000310"></a>
...
## Page contexts
<a name="page_contexts_ELTTEXTE000860"></a>


### Overview
<a name="overview_ELTPARAGRAPHE000332"></a>

In a Windows application, the information relative to a window is kept with the window.

In a WEBDEV website in Session mode, a page context exists on the server for each page displayed in the browser of the Internet user.
...
<a name="automatic_operating_mode_ELTPARAGRAPHE000348"></a>
...
## Opening a page in a WEBDEV website
<a name="opening_page_webdev_website_ELTTEXTE000890"></a>


### Overview
<a name="overview_ELTPARAGRAPHE000377"></a>

Several methods can be used to open a new page in a WEBDEV website:
...
<a name="opening_page_from_control_description_ELTPARAGRAPHE000387"></a>
...
2. Go to the "Button action" section, expand "Type" and select "Display a site page".

3. Select the page to display. 

4. Select the action destination: Current page, current browser, etc. The previously selected action will be performed in this destination.
...
<a name="opening_page_programmatically_ELTPARAGRAPHE000403"></a>
...
<a name="events_associated_with_pages_ELTTEXTE000926"></a>


### Default events
<a name="default_events_ELTPARAGRAPHE000438"></a>
...
- **Whenever changing layout (sever code)**: 
	Executed when the page layout is changed.  
...
### Optional events
<a name="optional_events_ELTPARAGRAPHE000494"></a>
...
<a name="menus_main_context_ELTTEXTE000956"></a>
...
## Menu options
<a name="menu_options_ELTTEXTE000980"></a>


### Overview
<a name="overview_ELTPARAGRAPHE000538"></a>
...
2. Double-click on the menu. A yellow border will appear.
...
<a name="manipulating_menu_options_the_editor_ELTPARAGRAPHE000552"></a>
