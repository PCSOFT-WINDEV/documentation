...
To manage these access levels in your WEBDEV sites, simply integrate the User Groupware into your site. In just a few clicks, a standard application can be transformed into an application that manages several access levels. This feature can be easily implemented.
...
**Note**: The User Groupware is available for dynamic WEBDEV sites running on Windows or Linux. The User Groupware is not available in static, PHP or AJAX sites.
...
<a name="how_does_user_groupware_work_ELTTEXTE000584"></a>
...
Note: You can also log in using an LDAP directory.
<a name="NOTE2_3"></a>
...
The supervisor logs in to the application via a login page and accesses an advanced menu to configure users and their rights, or to launch the application.
...
**Note**: All user information and rights are stored in HFSQL tables. 
...
<a name="implementing_the_user_groupware_ELTTEXTE000620"></a>


### Adding the User Groupware to a site
<a name="adding_the_user_groupware_site_ELTPARAGRAPHE000086"></a>

To implement the User Groupware in a WEBDEV website: 
...
3. The User Groupware settings window allows you to enable the User Groupware and define: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_WB%20Groupware%20U%20-%20HC%20N%B0001.gif&type=thumb)
...
- The type of database and the location of the User Groupware tables (Standard User Groupware). 
...
- The authentication server settings. 
...
There are various types of groupware: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_WB%20Groupware%20U%20-%20HC%20N%B0001%201.gif&type=thumb)


- **Standard User Groupware**: The User Groupware will be associated only to your site, and the rights will be defined from the site. 
...
- **User Groupware with authentication server**: This option allows you to integrate a BaaS (Backend as a Service) User Groupware and manage rights via a server. 
	If this option is selected, two specific tabs are displayed: 

	- the "Server" tab, which allows you specify the characteristics of the BaaS server, the user accounts and the URLs of the administration sites. 
 ...
 **If the "SSO: Single sign-on on multiple sites" option is checked**, two specific tabs are displayed: 

- the "SSO" tab, which allows you to specify the Client ID and Client Secret required for SSO authentication. 
...
- **If you are not using a PCSCloud platform**, you will need to install the following elements:
 ...
 For more details, see [Configuring an authentication server (User Groupware and Private Store)](../WDLang3/1000021311.md).
...
Several start modes are available: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_WB%20Groupware%20U%20-%20HC%20N%B0001%202.gif&type=thumb)
...
- The "Manual start (programmatically)" option allows you to define when the User Groupware should be started.
	The login page will only be opened if [gpwOpen](../WDLang6/3041002.md) (or [gpwLogin](../WDLang6/3041001.md)) is used. This option can be used to execute the project initialization code before opening the login page. For example, in a multilingual project, this option can be used to display a language selection page.
...
### User Groupware options (standard and with authentication server)
<a name="user_groupware_options_standard_and_with_authentication_server_ELTPARAGRAPHE000185"></a>

There are multiple User Groupware customization modes: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_WB%20Groupware%20U%20-%20HC%20N%B0002.gif&type=thumb)


- **No customization**: All User Groupware programming is automatically integrated into your site. No customization can be done. The project theme is automatically applied to all pages used by the User Groupware.
...
- tables are in English. To use data in French, simply check "Tables in French". 

- User Groupware tables have a specific password. This password is: "PCSGPW2001".
	...
	Note: This password is used when reindexing or opening tables with WDMAP for example.
...
<a name="options_specific_the_standard_user_groupware_ELTPARAGRAPHE000223"></a>

If you use the standard User Groupware, the "Tables" tab allows you to configure:
...
In "Standard User Groupware" mode, the User Groupware uses several specific tables. These tables are mainly used to manage users, groups, rights and elements in the site. The standard User Groupware can be used:

- with HFSQL Classic tables. In this case, you can specify the location of the tables via the "Default directory" link.

- with HFSQL Client/Server tables. In this case, you can define the parameters for accessing the HFSQL Client/Server database as well as the file directory by selecting "Click here to enter the server parameters".
...
- **Authentication entirely managed by the User Groupware**: 
	The username and password are saved in the User Groupware tables.
...
- **Authentication from an LDAP directory or Active Directory**: 
	...
	- If this option is checked, no Groupware table will be created. If the application is started in automatic mode, the user will need to log in. If the user is saved in the LDAP directory, the application will start; otherwise, it will be closed. A supervisor cannot configure the rights on the windows.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_WB%20Groupware%20U%20-%20HC%20N%B0004.gif&type=thumb)



### Options specific to the User Groupware with authentication server
<a name="options_specific_the_user_groupware_with_authentication_server_ELTPARAGRAPHE000264"></a>

When using a BaaS User Groupware, the "Server" and "Email" tabs allow defining specific options. 

The "Server" tab enables you to define:  
...
<a name="testing_the_site_standard_user_groupware_ELTPARAGRAPHE000282"></a>
...
By default, a single user exists: the supervisor. To log in as supervisor, simply enter the following information in the login page:
...
- **To avoid starting the User Groupware when testing the site**, select "Disabled" in the "General" tab of the User Groupware options.
	If you re-enable the User Groupware again, the tables previously used by the User Groupware will not be deleted.
...
<a name="installing_site_that_uses_the_standard_user_groupware_ELTPARAGRAPHE000310"></a>

Sites that use the standard User Groupware are installed in the same way as any other site. Simply go to the "Project" tab, "Generation" group, and select "Deploy site".

A window from the standard mode lets you configure the location of User Groupware tables.
...
- If the groupware tables have not been configured, the Supervisor will be the only user in the site the first time it is started.
...
<a name="configuring_site_that_uses_the_standard_user_groupware_ELTTEXTE000674"></a>
...
### Managing users
<a name="managing_users_ELTPARAGRAPHE000367"></a>
...
### Managing rights
<a name="managing_rights_ELTPARAGRAPHE000400"></a>
...
- ***Default***: The behavior of the element is the default behavior defined in the application.

- ***Disabled***: The element is displayed but users cannot interact with it.

- ***Grayed out***: The element is displayed but is grayed out. No input supported.
...
<a name="displaying_statistics_ELTPARAGRAPHE000423"></a>
...
### Migrating data
<a name="migrating_data_ELTPARAGRAPHE000432"></a>

In version 19, some important changes were made to the User Groupware. The tables used by the User Groupware have changed.
...
<a name="tips_for_site_that_uses_the_user_groupware_ELTTEXTE000722"></a>


### Using groups of controls
<a name="using_groups_controls_ELTPARAGRAPHE000445"></a>
...
<a name="visibility_controls_ELTPARAGRAPHE000460"></a>
...
- programmatically ([State](../Proprietes/2510039.md) or [Visible](../Proprietes/2510138.md) properties).
...
### Defining rights
<a name="defining_rights_ELTPARAGRAPHE000482"></a>

To define the rights corresponding to the features of the site, it is recommended to:

- set the rights according to the site manager's requirements before creating the setup procedure . Simply add the groupware tables when you create the site setup.
