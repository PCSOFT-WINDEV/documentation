...
Sites that use one or more components are deployed in the same way as standard sites. Simply go to the "Project" tab, "Generation" group and expand "Deploy site".
...
Compile the project and redistribute the site with the component. In this case, no version problem or compatibility problem will occur. Compilation is required in the following cases:
...
- ***backward compatibility***: The version of the component (.WDK file) used to compile the sites must always be greater than or equal to the version currently used.
