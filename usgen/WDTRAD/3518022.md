...
- **Language settings**: For each language, it is possible to set the language characteristics, such as the keyboard used and the symbols.. The setting of symbols is used by WDTRAD when a translation is rebuilt from an existing translation. 
	To configure the characteristics of a language: 

	1. Select the desired language. 

	2. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=icone_Proprietes.gif). The "Keyword" tab and the "Symbols" tab are used to configure the specific features of the selected language. 




- **Dictionary reindexing**: For each language, it is possible to reindex the dictionary.. This operation optimizes the access to dictionary data. This reindex operation can take a few minutes. 
...
Note: Use the context menu in the found resources table to add an entry to the dictionary ("Add an entry to the dictionary" option).. 

<a name="NOTE4"></a>
<a name="NOTE4_1"></a>


## Search/Replace" tab: Available options
<a name="searchreplace_tab_available_options_ELTTEXTE000261"></a>
...
2. Enter the search text in the specified language. 
	Note: The characters \* and? can be used. The ~ character allows you to exclude a term from the search. 
