...
| --- | --- |
| Test of the entire project ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_GO_Projet_WB_bl.gif) | Runs the application like the executable file would. Tests the entire application, even if it is not fully developed.<br><br>When a problem occurs during the execution of the project, you can start the debugger to identify and fix the problem.<br><br>Note: If the number "64" appears on the "Go" icon, it means that the test will be run in 64-bit mode in a 32-bit editor. |
| Test of the current element ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fenetre_WD_bl.gif) | Tests an element once it has been developed.<br>You can start the debugger when a problem occurs in reports and windows.<br><br>Note: If the number "64" appears on the "Go" icon, it means that the test will be run in 64-bit mode in a 32-bit editor. |
| Test of a project step by step<br>(via the debugger) | Allows you to start the debugger when the application is run. This solution is used to monitor the operating mode of the application via the debugger.<br><br>You can also debug a specific executable, or a running executable on the current computer or on a remote computer. |
