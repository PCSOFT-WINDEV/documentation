...
- test of a single query (for more details, see the online help),

- test of a single report (for more details, see the online help),
...
**The window test** is used to run the current window only. This allows you to test your project from a given window or to test a window once it has been finished. Like for the project test, you can start the debugger if a problem occurs.
...
In this case, the code executed comes from the libraries (and not from the code in the elements on the computer). Therefore, the code of the current project and the code of the executable can become out-of-sync: the out-of-sync code is highlighted in pink during the debug operation.
...
**Note**: To debug a WINDEV application remotely, the application must include the WD310CPL.DLL library in its framework.
...
2. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fenetre_WD_bl.gif) in the quick access buttons (or press F9). The editor is automatically minimized and the window is run.
...
**Note**: WINDEV also includes multiple trace tools (trace window, dialog box, etc.). For more details, see [Debugging without using a debugger](#NOTE5_4).
<a name="NOTE5_2"></a>
...
<a name="debugging_without_using_debugger_ELTPARAGRAPHE000229"></a>
...
When you test your application, the performance profiler keeps track of all the actions performed and the corresponding processes executed.
...
The performance profiler displays the result of the database schema.

**Note**: Use the performance profiler to optimize your application (before publishing, for example).
<a name="NOTE6_3"></a>
...
The performance profiler presents the result of the database schema in several tabs:
...
| Calls | Number of calls made to the function (procedure, event or process). |
| Time per call | Execution time of a call to the function (procedure, event or process). |
