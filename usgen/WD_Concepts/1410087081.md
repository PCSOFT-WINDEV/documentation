...
| Start mode | On the "Project" tab, in the "Generation" group, click "Setup procedure". | In the "Tools" pane, in the "Utilities" group, click on "WDInst". | WDSETUP project available in the "Examples" subdirectory of the WINDEV installation directory. |
...
- **Individual installation**: recommended for single-user applications.
	The setup program installs the application directly on the client computer.

- **Installation with automatic update**: recommended for multi-station applications.
...
- ***provide other files***, you must use the setup program creation wizard, started from WDInst (select "File .. New" from WDInst).
	Reminder: To launch the WDInst installation editor, under the "Tools" pane, in the "Utilities" group, click on "WDInst".
...
1. Install the reference version: launch your reference version installer from any user workstation.. The files required by the reference application are installed in the specified installation directory. The reference application corresponds to a client setup program.

2. Install the application: from each user workstation, launch the client installation program. The files required by the client application are installed in the specified installation directory.
	Note: You can provide the path to the customer installation directory directly to users if required.. Thus, they will be able to install the application. 
...
1. Install the reference version update: launch the reference version installer from any user workstation.. The update replaces the necessary files of the reference application. You can save these replaced files.
