...
The supervisor logs in to the application via a login window and accesses an advanced menu to configure users and their rights, or to launch the application.
...
**Note**: All user information and rights are stored in HFSQL tables (Classic or Client/Server).
...
3. The User Groupware settings window allows you to enable the User Groupware and define: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0001.gif&type=thumb)
...
There are various types of groupware: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0001%202.gif&type=thumb)


- **Standard User Groupware**: The User Groupware will be associated only to your application, and the rights will be defined from the application. 
...
- **User Groupware with authentication server**: This option allows you to integrate a User Groupware and manage rights via a server. 
	...
	- the "Server" tab, which allows you specify the characteristics of the authentication server, the user accounts and the URLs of the administration sites. 
...
Several start modes are available:  <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0001%201.gif&type=thumb)
...
- The "Manual start (programmatically)" option allows you to define when the User Groupware should be started.
	The login window will only be opened if [gpwOpen](../WDLang6/3041002.md) (or [gpwLogin](../WDLang6/3041001.md)) is used. This option can be used to execute the project initialization code before opening the login window. For example, in a multilingual application, this option can be used to display a language selection window.
...
<a name="user_groupware_options_standard_and_with_authentication_server_ELTPARAGRAPHE000172"></a>

There are multiple User Groupware customization modes: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0002.gif&type=thumb)


- **No customization**: 
	WINDEV takes care of everything. You just need to start the application to configure user access. 
	The windows of the User Groupware can automatically use the theme of the application. Simply check "Apply project skin template to non-customized windows at runtime".
...
- **Full customization (expert mode)** - Standard User Groupware only: 
	The programming of the User Groupware is automatically included in your application. The internal component named **WDGPU** is included in your project. This component contains a database schema as well as all the windows, queries, classes and procedures required to manage the User Groupware. This allows you to customize all the windows used by the User Groupware.




The option "Apply project theme to non-customized windows at runtime" applies the theme of your project to the windows specific to the User Groupware if they have not been customized. 
...
- User Groupware tables have a specific password. This password is: "PCSGPW2001".
	...
	Note: This password is used when re-indexing or opening tables with WDMAP for example.




For more details on these tables and their configuration (installation directory, etc.), see the online help.
...
In "Standard User Groupware" mode, the User Groupware uses several specific tables. These tables are mainly used to manage users, groups, rights and elements in the application. The standard User Groupware can be used:
...
- **Windows authentication**:
	...
	Note: You can also use a unique authentication. In this case, the username and password will be requested only once per session.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0004.gif&type=thumb)



### Options specific to the User Groupware with authentication server
<a name="options_specific_the_user_groupware_with_authentication_server_ELTPARAGRAPHE000266"></a>

When using a User Groupware with authentication server, the "Server" and "Email" tabs allow you to define specific options. 
...
By default, a single user exists: the supervisor. To log in as supervisor, simply enter the following information in the login window:
...
This allows you to define the location of the User Groupware tables.
<a name="NOTE3_4"></a>
...
Applications that use the User Groupware are installed just like any other application. Simply go to the "Project" tab, "Generation" group, and select "Setup procedure".

The wizard displays a screen specific to the standard User Groupware to set the location of the User Groupware tables.
...
1. Start the application (executable or test from WINDEV) and log in as supervisor:
	...
	- Password: supervisor <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0007.gif&type=thumb)





2. Select "Configure groupware". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0008.gif&type=thumb)


3. The User Groupware management window appears. This window allows you to:
...
- associating users with groups. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Int%E9grer%20le%20GPU2%20-%20HC%20N%B0009.gif&type=thumb)
...
In version 18, some important changes were made to the User Groupware. The tables used by the User Groupware have changed.
...
- configure the rights required by the user of your application before creating the setup procedure. Simply add the groupware tables when you create the application setup.
