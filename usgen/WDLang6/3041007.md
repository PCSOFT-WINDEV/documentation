...
**gpwOpenConnection** sets and opens a connection to the User Groupware tables. To use the same connection for all the tables of the application, use [HChangeConnection](../WDLang4/3044150.md). Then, [gpwOpen](../WDLang6/3041002.md) is used to specify the location of tables for managing rights as well as the location of tables for identifying the users.
<a name="Exemple1"></a>
...
ELSE 
	...
	IF nRes <> gpwOK THEN 
		...
		END 
	END 
	// Use the same connection for all tables 
...
```wl
// Change the password of the user groupware tables
...
Use [gpwOpen](../WDLang6/3041002.md) to specify the location of tables for managing rights as well as the location of tables for identifying the users. The directories must be specified in relation to the server directory.

**`<Connection name>: (Character string)`**

Name of new connection to define and to open. This name must not be used by another connection (defined in the database schema, or with [HOpenConnection](../WDLang4/3044107.md) or [HDescribeConnection](../WDLang4/3044205.md)).
...
Name of the [Connection](../WDLang4/1514073.md) variable to open. This name must not be used by another connection (defined in the database schema, or with [HOpenConnection](../WDLang4/3044107.md) or [HDescribeConnection](../WDLang4/3044205.md)).
...
1. In the data model editor, open the connection description window. Go to the "Database schema" pane, "Connection" group and click "Connections".

2. Select the provider to use ("Connect via" combo box).

3. The character string corresponding to the provider is displayed. This character string can be used in the syntax 2 instead of constants. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=HOuvreConnexion.gif)
...
OLE DB tables or native tables are described in the data model editor. To handle these descriptions with an HFSQL Classic database, &lt;OLE DB provider or Native Connector&gt; must correspond to the *hAccessHF7* constant.
...
[User Groupware and Client/Server tables](../WDLang6/1000021177.md)
