


## LeftIndent (Property)

***In french: RetraitGauche***
	



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
The **LeftIndent** property gets and sets the space to the left of the text in a control. 

This property is applied to the following controls: 

- the columns of Table and TreeView Table controls (between the beginning of column and the beginning of text).

- Edit controls.

- the Combo Box and List Box controls.

- Button controls.

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) the Ribbon controls.

- Tab controls.

- Static Text controls.




This space is expressed in pixels.
<br>![Table control without left indent](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=RetraitGauche.gif)

<br>![Table control with left indent set to 20 pixels](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=RetraitGauche2.gif)



<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Indents all the columns in a Table control by 20 pixels
TABLE_Table1.LeftIndent = 20
// Sets an additional indentation to the Day column of the current row
COL_Day.LeftIndent = 50
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

### Finding out the left indent of a control

`<Result> = <Control used>.LeftIndent`
---

**`<Result>: (Integer)`**

Number of pixels used to indent the text. The indent is set to 0 pixels by default (no indent). 

**`<Control used>: (Control name)`**

Name of control to use: 

- column of a Table control.

- column of a TreeView Table control.. 

- edit control.

- List Box or Combo Box control.

- Button control.

- Ribbon control.

- Tab control.

- Static Text control.





<a name="SYNTAXE2"></a>

### Changing the left indent of a control

`<Control used>.LeftIndent = <New indent>`
---

**`<Control used>: (Control name)`**

Name of control to use: 

- column of a Table control.

- column of a TreeView Table control.. 

- edit control.

- List Box or Combo Box control.

- Button control.

- Ribbon control.

- Tab control.

- Static Text control.




**`<New indent>: (Integer)`**

Number of pixels that will be used to indent the text in the specified control. 
Note: Indentation on the entire Table field: If one of the columns has a specific indentation, the new indentation will not be applied to this column..  




## Voir aussi

[The Table control](../Proprietes/1013224.md)

[Properties associated with windows, pages and controls](../Proprietes/2510107.md)

[RightIndent (Property)](../Proprietes/1000021110.md)

[BottomIndent (Property)](../Proprietes/1000022891.md)

[TopIndent (Property)](../Proprietes/1000022890.md)

