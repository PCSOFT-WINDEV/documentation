...
<a name="filtering_the_row_column_headers_ELTPARAGRAPHE000040"></a>
...
<a name="using_filter_procedure_ELTPARAGRAPHE000055"></a>
...
1. Open the description window of the row or column header to filter. 

2. In the details of the header, click "None" next to "Filter". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=TCDFiltre.gif)
...
3. A window is displayed, allowing you to select the filter procedure: 
	...
	- If this procedure does not exist, click the "Create a procedure" button. In this case: 
		...
		- A window is displayed, allowing you to select the items whose value will be automatically assigned to the parameters of the procedure: 
			...
			4. Click (if necessary) the "Fill the procedure" button to wirte the code of the procedure in the code editor. The code editor is displayed
...
**To take into account the current record** in the pivot table, the procedure must use the following line of code:
...
**To ignore the current record** in the pivot table, the procedure must use the following line of code: 
...
**Note**: You can also associate a filter procedure with a row or column header using the [FilterProcedure](../Proprietes/1000020655.md) property. 
