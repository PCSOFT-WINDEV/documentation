...
In this case, the report contains a Scheduler control only. This type of control is only created when a Scheduler control report is created: it cannot be created independently of this report. 

Example: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Etat_Planning.gif&type=thumb)
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Etat_Planning_Editeur.gif&type=thumb)
...
<a name="creating_scheduler_report_ELTTEXTE000234"></a>
...
4. In this wizard, specify:
	...
	- the type of report to create ("Scheduler report") with the [information specific to the creation of a scheduler control](#NOTE2_2): 
		...
		- the display mode of resources (resources displayed by rows or in columns).  
...
- the table source of the report (table, query, Scheduler control, ...). For more details, see [Report data source](../WDChamp/1011055.md).
...
- Number of days to be displayed in the Scheduler control: The schedule can be printed: 
	...
	- with a custom number of days. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Ass_Creation_pln.gif)
...
**Note**: If the Planning report is linked to a data source (query, table), it is necessary to indicate in the rest of the wizard the fields to be taken into account for displaying the schedule and appointments. The following information must be specified: 
...
<a name="characteristics_scheduler_report_and_scheduler_control_ELTTEXTE000264"></a>
...
Note: Check that the daily appointment display area is correctly sized in the report editor. It can be resized if necessary.
	...
	- the custom report to display the appointments. 
		...
		- If "Preset report" is selected, a preset report is automatically added to the project. This report is named "RPT_WinDevViewAPT" by default and it is automatically used to display the appointments.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Etat_WinDevvueRDV.gif)
...
- resource display mode: rows or columns. If the resources are displayed by columns, you have the ability to specify the number of resources displayed by columns. 

- whether or not to display the week number. 
			If the week number is displayed, the ISO 8601 format can be used.
			

- the display mask for the dates and times. 

- If the Scheduler control is linked to a data source (table, query, ...), the "Content" tab found in the description window of the control is used to link the appointment data with the data found in the data source. 
...
## Handling a schedule in a report programmatically
<a name="handling_schedule_report_programmatically_ELTTEXTE000297"></a>


### WLanguage functions
<a name="wlanguage_functions_ELTPARAGRAPHE000149"></a>Several WLanguage functions are available in the report editor for manipulating the Scheduler controls. These functions are as follows: 
...
<a name="wlanguage_properties_ELTPARAGRAPHE000157"></a>Several WLanguage properties are available in the report editor for manipulating the Scheduler controls. These properties are as follows: 
...
[Report based on a Scheduler control](../WDChamp/1011080.md)
