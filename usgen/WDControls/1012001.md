...
- A page contains a Table control linked to an "ITEM" table and a "Next" link.

- The ITEM table contains a single item, each record includes a letter of the alphabet.
...
When the page is opened, the data-bound Table control displays the first 6 records in the table (from 'A' to 'F'). The user performs the following series of actions:
...
This can have undesired effects when modifying a record in a table (e.g., the user sees a record but actually modifies another).
