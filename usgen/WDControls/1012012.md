...
1. On the "Home" tab, in the "General" group, expand "Open" and select "Open" (Ctrl + O).
...
3. Save the frameset: in the "Page" pane, in the "Edit" group, pull down "Frame" and select "Save frameset".
...
1. On the "Page" tab, in the "Edit" group, expand "Frame" and select: 
...
It is necessary to define the initial pages of a frameset: these pages will be opened by default when the frameset is opened in the surfer's browser.

To define the initial pages of the frameset:

1. Associate the requested pages with the different frames.

2. On the "Page" tab, in the "Edit" group, expand "Frame" and select "Define the initial status".




**Reminder** A frameset displays several pages in a single browser window. The HTML file corresponding to the frameset contains:
...
By default, a frameset is used to display several pages in the same window of the browser. It's possible that one of the pages displayed in the frameset is itself another frameset: this is known as a "nested frameset". The nested framesets can be used to implement outstanding layout in your pages for example.
