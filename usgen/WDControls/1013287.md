...
A break in a Looper control is used to group the data corresponding to one or more arguments.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=RuptureZR.GIF)
...
- RepeatString fields linked to data loaded into memory.

- Memory Looper controls.

- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![WEBDEV - Browser code](https://doc.pcsoft.fr/ext/images/us/WBN.png) Looper controls in Standard, AJAX and Browser mode.
...
- You have the ability to manage the [breaks found in a Looper control programmatically](#NOTE6_1). 




![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png) Breaks in Looper controls are not available in Multicolumn Looper controls. 
...
1. Display the RepeatString Looper control description window ("Description" option in the context menu).

2. On the "Content" tab, click <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=BtnRupture.GIF)
. The window for managing breaks is displayed: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=FenRupture.GIF)
...
3. Select the item or the attribute on which a break must be performed. Several items and/or attributes can be selected in order to implement several breaks. 
	...
	- ***In a Looper control linked to a table***:

		- *automation path* The browse item, the attributes linked to this browse item and the attributes linked to no browse item are proposed. If the search item corresponds to a composite key, the components of this composite key, the attributes linked to these components and the attributes linked to no item are proposed.

		- *programmed route*: table items, attributes linked to these items and attributes linked to no item are proposed.
...
- ***In a Looper control linked to a query***:
	...
	- *automation path* The browse item, the attributes linked to this browse item and the attributes linked to no browse item are proposed. If the search item is not specified ("&lt;Automatic&gt;" option), the sorted query items, the attributes linked to these items and the attributes linked to no item are proposed.
...
<a name="top_and_break_footer_ELTPARAGRAPHE000119"></a>
...
Each break header and break footer is associated with the "Display a row" event. This event is executed when a new break header or break footer is displayed in the Looper control.

These break headers and footers can contain controls. These controls can be handled programmatically.
...
3. On the "Style" tab, select the "Break, border in creation" element.

4. Click the "Plus/Minus image of breaks" button. 

5. In the area that is displayed, enter the path of the image for the break button. This image is an image with 2 states (plus and minus) in 24 x 12 format. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Rupture_Perso_Btn.gif&type=thumb)
...
<a name="manipulating_the_controls_found_the_break_headers_and_footers_ELTTEXTE000404"></a>
...
**Remark**: These lines of code must be present in the "Display top / break footer line" event.
<a name="NOTE3_2"></a>
...
1. Insert a Static Text control or an edit control into the break header and footer.
...
5. Select the element on which the calculation must be performed. The calculation can be performed on:
	...
	- A field: the calculation will be performed on the field's value property.

	- An attribute of the RepeatString zone: the calculation will be performed using the attribute's property.

	- A field in the table (only for table-related RepeatString fields).
...
<a name="handling_the_break_headers_and_footers_programmatically_ELTTEXTE000458"></a>
...
<a name="properties_associated_with_the_break_headers_and_footers_ELTPARAGRAPHE000269"></a>Several properties are used to handle the breaks through programming, especially: 
...
<a name="managing_breaks_the_looper_controls_programmatically_ELTTEXTE000559"></a>
The following functions are used to create and manipulate breaks in the Looper controls: 
...
[Looper control break properties](../WDChamp/1010045.md)
