...
3. In the "Report viewer" option, select "Standard (no theme)".
...
3. In the "Report viewer" option, select "With project theme".
...
4. Select the type of customization to perform:
	...
	- &lt;Visualisateur de rapports standard (sans gabarit)&gt; This option imports standard print windows directly into your project.

	- &lt;Visualisateur de rapports au gabarit du projet&gt; This option directly imports the print windows corresponding to your project's skin template (supplied with WINDEV 10 and higher only).
...
**Remark**: If you only want to customize a few windows (e.g. the report viewer only), simply delete the unnecessary windows from your project.. The standard print windows will be used.
<a name="NOTE4_2"></a>
...
- *** delete these controls*** (Del key). The corresponding properties will no longer be proposed to the users.
	Caution: To configure the options available in the ribbon, it is advisable to: 
...
- ***add controls to the viewer window***. 
	Warning: The code of these fields must not use [WLanguage print functions](../WDLang5/3046065.md).
...
4. Select the window from your project that will be used to view the printouts. This window can be found in a component (the window must be an accessible element of the component).
	Note: If you have chosen to integrate the standard report viewer windows, the "WD_Viewer" window is automatically selected.. 
...
**Reminder**: WDMSG simplifies project translation by extracting all project messages (field labels, window titles, etc.) and reintegrating messages after translation.. WDMSG is an additional module of WINDEV. Contact PC SOFT Sales Department for more details about WDMSG.


## Voir aussi

[Customizing the report viewer: Available windows](../WDChamp/9000162.md)
