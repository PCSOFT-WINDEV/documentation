...
**Attention**: The procedure declaration (keyword PROCEDURE) must correspond to the first line of the "Global declarations" event.
...
// sDateSel: date specifying the date to be selected.
```
...
**2.** Pass the parameter expected by the page when it is opened ([PageDisplay](../WDLang2/3058008.md), [PageUse](../WDLang2/3058016.md), etc.). For example, BTN_CALENDAR is used to open PAGE_CALENDAR. When this page is opened, today's date is passed as parameter.
...
<a name="testing_parameterized_page_ELTTEXTE000156"></a>
To run the test of a page with parameters:

1. Open the page with parameters in the page editor.

2. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_Go_Page_bl.gif) (or press F9). The following window is displayed:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Page_Parametree%20-%20HC%20N%B0001.gif)
...
2. This variable is passed as a parameter to the PAGE_CALENDAR page.. The *parameter sDateSel* contains the value of the *variable MaDate*.

3. The value of the *parameter sDateSel* is modified in the PAGE_CALENDAR page (for example: sDateSel = "20020701").
