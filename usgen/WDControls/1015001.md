


## The TreeView Table control
			



<a name="NOTE1"></a>
<a name="NOTE1_1"></a>


## Overview of the TreeView Table control
<a name="overview_the_treeview_table_control_ELTTEXTE000329"></a>
The TreeView Table control is a cross between the [Table control](../WDChamp/1013224.md) and the [TreeView control](../WDChamp/1013037.md).

This control is based on the structure of a Table control (column, row, cell) but it also contains a TreeView column that introduces the notion of tree structure. This notion does not exist in a Table control.

The columns of a TreeView Table control and the [columns of a Table control](../WDChamp/1013245.md) have the same characteristics.

![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) In WINDEV, the TreeView column of a TreeView Table control can also be a [container column](../WDChamp/1013263.md). A TreeView Table control allows you to perform a single selection or a multiple selection as well as a sort on the columns (sort by level, among sibling elements).

The information displayed in the TreeView Table control can:

- can be programmatically determined: this is referred to as a field. **Programmatically populated hierarchical table**.

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) come from a variable: this is referred to as a field **[TreeView Table control based on a variable](../WDChamp/1015006.md)**. 

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) come from a table or a query: this is referred to as an **field [Hierarchical table linked to a table](../WDChamp/1015005.md)**. Only TreeView Table controls linked to a table loaded in memory are available. In this case, the control handles the records of the table loaded in memory.








<a name="NOTE2"></a>
<a name="NOTE2_1"></a>


## How to?
<a name="how_ELTTEXTE000353"></a>


### Creating a TreeView Table control
<a name="creating_treeview_table_control_ELTPARAGRAPHE000079"></a>

To create a TreeView Table control:

1. On the "Creation" tab, in the "Data" group, 

	- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) expand "Table and List Box" and select "TreeView Table".

	- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) expand "Table" and select "TreeView Table". The TreeView Table control creation wizard opens. This wizard allows you to: 

		- select the data source for the TreeView Table control.

		- select the specific parameters of the TreeView Table control.




2. Click where you want to create the control in the window or page.


Note: The dimensions of the control are optimized to take up the available space at the specified position. If you are not happy with the size of the control, press Ctrl + Z key to restore the control to its default size.
<a name="NOTE2_2"></a>


### Control characteristics
<a name="control_characteristics_ELTPARAGRAPHE000115"></a>

To view the characteristics of the control, select "Description" in the context menu. There are two types of information:

- information about the entire TreeView Table control (simply select the field name).

- information about each column in the TreeView Table control (simply select the name of one of the columns).




Table and TreeView Table controls, as well as their columns, share most of their characteristics. 

![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) TreeView Table control fields offer the following option: "Expand by clicking the row". If this option is checked, the hierarchy can be expanded by clicking on the row (and not only on the expand icon). 
![Android](https://doc.pcsoft.fr/ext/images/us/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) **Special case: Option "Android/iOS selection mode" selected** ("UI" tab in the control description window): 

- If the "Expand by clicking the row" option is enabled, the "Select a row" event is not called when the user clicks the row (if it is a node) and the value of the control does not change. The "Collapsing/Expanding" event of a node is called. In this event, the value of the control ([Value](../Proprietes/2510130.md) property) corresponds to the row clicked. 

- If the "Expand by clicking the row" option is not enabled, the "Select a row" event is not called when the "+" and "-" buttons are clicked (if it is a node). The "Collapsing/Expanding" event of a node is called. In this event, the value of the control ([Value](../Proprietes/2510130.md) property) corresponds to the row clicked.






For more details on the characteristics of Table controls, see: 

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WINDEV Mobile](https://doc.pcsoft.fr/ext/images/us/WM.png) [Characteristics of Table controls in a window](../WDChamp/1013200.md).

- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) [Characteristics of Table controls in a page](../WDChamp/9500133.md).




For more details on TreeView Table controls, see:

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/us/IPHONE.png) [Hierarchical table linked to a table](../WDChamp/1015005.md).

- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) [TreeView Table control based on a variable](../WDChamp/1015006.md).



<a name="NOTE2_3"></a>


### TreeView column
<a name="treeview_column_ELTPARAGRAPHE000191"></a>

To choose the column containing the hierarchy: 

1. Open the TreeView Table control description window.

2. On the "General" tab, enter the number of the desired column in "Number of TreeView column".



<a name="NOTE2_4"></a>


### Handling the nodes of the tree structure programmatically
<a name="handling_the_nodes_the_tree_structure_programmatically_ELTPARAGRAPHE000201"></a>

To modify the images of the treeview nodes in a TreeView Table control, use the following properties:

- [ExpandedImage](../Proprietes/2513015.md) to modify the images of expanded nodes.

- [CollapsedImage](../Proprietes/2513016.md) to modify the images of collapsed nodes.




For more details on how to manipulate TreeView Table controls programmatically, see [Manipulating TreeView Table controls programmatically](../WDChamp/1015004.md).

<a name="NOTE3"></a>
<a name="NOTE3_1"></a>


## Vocabulary
<a name="vocabulary_ELTTEXTE000399"></a>
To easily handle a TreeView Table control, you must be familiar with the following vocabulary:

| Name | Definition |
| --- | --- |
| Selection bar | Used to view the selected element. A specific color is used in most cases. |
| Branch | Part of the tree structure that can define a path:<br><br>- from the root to a leaf,<br><br>- from a node to another node,<br><br>- from a node to a leaf,<br><br>- from the root to a node. <br><br><br> |
| Element | Generic name corresponding to the contents of a tree: Root, Node or Leaf are elements of a tree structure in a hierarchical table |
| Leaf | Last element of the tree structure. There are no lower levels. |
| Node | Intermediate level, can have higher levels (also called "Parent" levels) and lower levels (also called "Child" levels).<br>Note: the root and leaves correspond to specific nodes. |
| Root | First element in the tree structure of the TreeView Table. The entire tree structure can be expanded from this element. |



<a name="NOTE4"></a>
<a name="NOTE4_1"></a>


## Swapping the Table/TreeView Table controls
<a name="swapping_the_tabletreeview_table_controls_ELTTEXTE000423"></a>
A Treeview Table control can be automatically transformed into a Table control and conversely: 

1. Select the TreeView Table control to transform. 

2. On the "Modification" tab, in the "Transformations" group, expand "Refactoring and swapping" and select "Table / TreeView Table".







## Voir aussi

[TreeView Table control linked to a table](../WDChamp/1015005.md)

[TreeView Table control based on a variable](../WDChamp/1015006.md)

[Characteristics of the columns in a Table control in a window](../WDChamp/1013245.md)

[Defining colors in Table controls](../WDChamp/1013231.md)

[Calculations in the columns of a Table control](../WDChamp/1013201.md)

[Combo Box columns in Table controls](../WDChamp/1013222.md)

[TreeView Table control events](../WDChamp/1015002.md)

[TreeView Table control properties](../WDChamp/1015003.md)

[Properties associated with columns of Table controls](../WDChamp/1013247.md)

[Referring to an element in a WLanguage event / process](../WDChamp/1014007.md)

[TreeView Table control functions](../WDChamp/1000010002.md)

[Container column in a Table control](../WDChamp/1013263.md)

[How to associate an image with an element found in a TreeView Table? ](../WDChamp/20054.md)

