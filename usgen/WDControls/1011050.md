...
For example:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=EtatEtiquette3.gif&type=thumb)
...
1. To create a "Label" report, click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ico_nouveau.gif) in the quick access buttons: 
...
2. In this wizard, you must specify:
	...
	- the data source of the report (table, query, ...). For more details, see [Report data source](../WDChamp/1011055.md).
...
- a **preset format**:
	...
	3. The information specific to the labels used (caption, size of labels and number of labels printed across the page width) is displayed. 
						Note: Check the "Inch" option to display label sizes in inches.
...
Once the "Label" report has been created, you can modify label characteristics at any time: under the "Modification" pane, in the "Report" group, click on "Format and labels".
...
The following properties are associated with the "Label" reports. These properties are used to dynamically configure (through programming) the format and print mode of labels.

**Note** These properties can only be used on "Label" reports.
