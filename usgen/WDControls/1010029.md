...
- Information about the current record: record number, item content, ...
...
**Note**: Only one transaction is possible per HFSQL context.
...
<a name="what_hfsql_context_ELTTEXTE000265"></a>


### Independent HFSQL context
<a name="independent_hfsql_context_ELTPARAGRAPHE000046"></a>
...
When the window (or report) is opened, the existing HFSQL context is automatically "doubled" (a copy of the existing context is made): the operations performed in each of the contexts will be independent of those performed in the other contexts.. The use of an independent context provides the same result as if two instances of the same application were running in parallel.
...
- All Native Connectors manage independent HFSQL contexts: the context is copied and the operations performed are independent.. However, some Native Connectors do not save the current position when copying the context.. In this case, it is necessary to set the position back to the current record after the context is copied.. 
...
<a name="transactions_and_independent_hfsql_context_ELTPARAGRAPHE000083"></a>
...
<a name="limitation_data_files_55_format_and_views_hyper_file_55_format_ELTPARAGRAPHE000093"></a>
...
<a name="limitation_independent_hfsql_context_and_use_odbc_driver_ELTPARAGRAPHE000104"></a>

Warning: the query will only be duplicated if the context copy (window or report opening) is performed AFTER initializing the result path with function [SQLFirst](../WDLang4/3072017.md).


|   |   |
| --- | --- |
| **Not to do**<br><br><pre><code>SI SQLExec("Requête1") ALORS <br>	Ouvre(FEN_Résultat)<br>FIN</code></pre><br> | **Do**<br><br><pre><code>SI SQLExec("Requête1") ALORS<br>	SQLPremier("Requête1")<br>	Ouvre(FEN_Résultat)<br>FIN</code></pre><br> |
...
<a name="hfsql_contexts_and_windows_ELTTEXTE000307"></a>


### How to manage the HFSQL context of a window?
<a name="how_manage_the_hfsql_context_window_ELTPARAGRAPHE000125"></a>
...
<a name="the_hfsql_contexts_are_not_independent_for_each_window_option_not_checked_ELTPARAGRAPHE000140"></a>
...
<a name="the_hfsql_contexts_are_independent_for_each_window_option_checked_ELTPARAGRAPHE000151"></a>
...
Furthermore, an HFSQL function used in a context will have no effect outside this context. So if memo management is active when independent contexts are opened, it is possible to deactivate this feature in a window: memos will only be deactivated in the context corresponding to the window.
...
<a name="hfsql_contexts_and_reports_ELTTEXTE000343"></a>


### How to manage the HFSQL context of a report?
<a name="how_manage_the_hfsql_context_report_ELTPARAGRAPHE000167"></a>
...
2. On the "Data" tab, check "The report execution does not affect iterations (independent HFSQL context)", if necessary.
...
<a name="the_hfsql_contexts_are_not_independent_for_each_report_ELTPARAGRAPHE000181"></a>
...
<a name="the_hfsql_contexts_are_independent_for_each_report_ELTPARAGRAPHE000190"></a>
