...
The "Print" tab allows the user to configure the print job on the selected printer. Then, the print job can be started via the "Start print job" button.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Volet_Imprimer.gif)
...
**Note**: When a printout is started from the report viewer by selecting a printer other than the default, this printer will be retained:
...
- **Color or black and white printing**. 
 ...
 Similarly, if a new printer is selected in the report viewer, the "Color/Black and white" radio button option changes according to the characteristics of the new printer.
	Physical printing launched from the viewer takes into account the "Color/Black & White" picker: any choice made by the end-user takes precedence over printer settings..
...
- **Report orientation, paper size, ...**: A combo box can be displayed to select document orientation, paper size, ..., ... 
