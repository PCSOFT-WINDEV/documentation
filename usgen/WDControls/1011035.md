...
The sort is used to organize the information displayed in the report. Several sorts can be performed at the same time: these sorts will be performed according to the display order in the array listing the sort criteria.

In the following example, in the report creation wizard, the sorting mode is defined: the data selected in the "Customer" table will be sorted by country name, then for each region by customer name.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_Appli%20compl%E8te%20-%20Imprimer%20liste%20clients%20-%20HC%20N%B0005.jpg)
...
- or to a **procedure name** in the project (for example: "MaProc( )"). This procedure must return the value on which the sort will be performed. The procedure can accept parameters. This procedure can be global to the project or local to the report.
	For more details, see [Sort procedure](#NOTE6_1).

- or to **an expression based on a WLanguage function** (for example: "Gauche(Client.CodePostal,2)"). This function must return the value on which the sort will be performed.
...
**Note** Sorting will be performed in the order displayed in the array listing the sort criteria.
...
**Note** If the report is based on a query, it is also possible to add a sort criterion from the query editor. For more details, see [Sorting the query records](../Editeurs/2032005.md).
<a name="NOTE3_2"></a>
...
2. Click the "New sort criterion" button (![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=etatIconeNewcritereTri.gif)).

3. To add:

	- **a sort item** Select the "Sort on an item" option, then select the item on which you wish to sort.
			The "&lt;Multi-file&gt;" option and the "Other" button are used to select an item via one or two cascading joins.

	- **a sorting procedure** Select the "Sort on WLanguage code" option and enter the name of the sorting procedure (e.g. "MaProc( )"). 
			For more details, see [Sort procedure](#NOTE6_1).

	- **a WLanguage function** select the "Sort on WLanguage code" option and enter the WLanguage function returning the value to be sorted on. 
...
- A sort criterion can be modified from the editor for the reports based on a table or on a memory area.
...
### Modifying a sort criterion in a report based on a table or on a memory area
<a name="modifying_sort_criterion_report_based_table_memory_area_ELTPARAGRAPHE000123"></a>

To modify a sort criterion in a report based on a table or on a memory area:
...
3. To modify the selected sort criterion by adding:
	...
	- **a sort item** Click on the "Sort item" button and select the sort item you wish to sort by.
	...
	- **a sort procedure**:
			...
			- enter the name of the sorting procedure (for example: "MaProc( )").
			For more details, see [Sort procedure](#NOTE6_1).
...
**Note** You can also change the sort direction of a criterion in the query editor. For more details, see [Sorting the query records](../Editeurs/2032005.md).
...
**Note** If a break is defined on this sort criterion, the break will also be deleted.
<a name="NOTE5_2"></a>
...
3. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=IconeSuppcritereTri.gif).
...
A sort procedure can be implemented in a report based on a table or in a report based on a memory area. This procedure can be used to modify the sort criteria dynamically.

To implement a sort procedure:

1. On the "Modification" tab, in the "Report" group, click "Edit sorting". The list of sort criteria is displayed.

2. Click the "New sort criterion" button (![New sort](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=etatIconeNewcritereTri.gif)).

3. Check "Sort on WLanguage code".

4. Enter the name and value of the parameters for the sort procedure (for example: "SortVal("City")").
...
6. Create the sort procedure. This procedure must return the sort argument. For example:
	...
	SWITCH Criterion
		Case "City": SEND Customer.City
		Case "Name": SEND Customer.Name
	END
	```
