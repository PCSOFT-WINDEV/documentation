...
| Query | Query items |   |
| Table | Items of a table |   |

The menu options and the items found in a table are handled differently. For more details, see [Special case: Headings](#NOTE3_2) and [Special case: Menu options](#NOTE4_2).

**Note** To simplify WLanguage processing, WINDEV, WINDEV Mobile and WEBDEV make it easy to retrieve the name of an element in a code window. For more details, see [Retrieving the name of an element in the code](#NOTE5_1).
...
To handle a page, window, report, query or table in a WLanguage event or process, simply use the name of this element. This name can be used:
...
To manipulate a second-level element (control or menu of a window/page, control or block of a report, etc.) in a WLanguage event or process, simply:
...
// equivalent to: ComboOpen(FEN_Client.COMBO_VilleFrance)
...
// equivalent to: LooperDisplay(PAGE_PageClient.ZR_ZoneProduct, taBegin)
```

<a name="NOTE3_2"></a>


### Special cases: Table or query fields
<a name="special_cases_table_query_fields_ELTPARAGRAPHE000105"></a>

To handle a table item or query through programming, simply:

- use the name of the item directly. This syntax is valid only if the item is handled by a function or by a property into which the name of the table or the name of the query to which the item belongs is specified.
...
// of the CLIENT table according to the NAMECLI field
...
<a name="NOTE4_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png)![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png)![User code (UMC)](https://doc.pcsoft.fr/ext/images/us/MCU.png) 
...
// equivalent to: FEN_Client.OPT_FicheClient.Couleur = DarkBlue
```
...
- use the automatic completion. 
	Auto-completion for assisted WLanguage code entry. For more details, see [Automatic completion](../Editeurs/2013001.md).
