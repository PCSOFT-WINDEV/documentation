...
- from WEBDEV websites (Windows or Linux).
...
<a name="creating_report_via_wizard_ELTTEXTE000445"></a>
<a name="Creer_etat"></a>
To create a report:
...
2. In the different steps of the wizard, specifiy:
	...
	- the report data source (table, query, etc.). For more details, see [Report data source](../WDChamp/1011055.md).

	- the elements of the report: 

		- [Sort: Data print order](#NOTE3_3). 
...
<a name="the_different_steps_the_wizard_ELTTEXTE000469"></a>


### The different types of reports
<a name="the_different_types_reports_ELTPARAGRAPHE000090"></a>
...
<a name="the_available_data_sources_ELTPARAGRAPHE000131"></a>
...
- **Existing table** The data to be printed comes from a table described in the project database schema. This table can be in any format (HFSQL Classic, Oracle, etc.).
	**Reports and Queries**: 
The tables and the table items that can be used in the reports created with Reports & Queries are defined by the developer in the data model editor. For more details on the requirements to use a WINDEV application with Reports and Queries, see [Distributing Reports & Queries](../Presentation/3088002.md).





- **Query**: Data to be printed comes from a query. This query corresponds to:
...
- **Table or hierarchical Table control, Agenda or Scheduler, RepeatString field** present in a window or in a page: the data to be printed comes from a specific field present in one of the application's windows or in one of the site's pages. The records are selected by the control and they are printed by the report.
	**Reports and Queries**: 
...
- **Existing variable** The data to be printed comes from a WLanguage variable in the project or report. This variable can be a simple array, a class member, a structure member or an advanced variable member. 
	**Reports and Queries**: 
...
- **No data source**: The report is not linked to any data source.
...
- **Other available data sources**
	...
	- **Text file type** The data to be printed is read from a text file.

	- **Memory area populated programmatically (not available in WINDEV Mobile)** Data to be printed correspond to records stored in a memory zone.
			**Reports and Queries**: 
Reports & Queries cannot be used to create reports on a memory area. However, you can customize reports based on a memory area, if they were created in the standard report editor (color, font, size, etc.).
	...
	- **HFSQL view**: The data to be printed comes from an HFSQL view on an HFSQL Classic table..
			**Reports and Queries**: 
...
**Note** If you select one of the above-mentioned data sources, the report editor takes full charge of programming the path of records to be displayed in the report.
...
### Breaks and sorts
<a name="breaks_and_sorts_ELTPARAGRAPHE000216"></a>
...
- If the report is based on a table, the search items of the table are automatically proposed as sort items.

- If the report is based on a query, the sorts performed in the query are automatically proposed for the breaks.

- If the report is based on a variable, only the breaks can be defined: the sorts must be performed programmatically.
...
- to list the customers and the companies by city, sort the customers by city then by company name, and perform a break on the city.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=EtatRupture.gif)
...
<a name="performing_calculations_report_ELTPARAGRAPHE000291"></a>
...
**Remark**: The various calculations performed in a report are selected when the report is created.. However, new calculations can be added to the created report via the calculated controls.
<a name="NOTE3_5"></a>


### Theme
<a name="theme_ELTPARAGRAPHE000300"></a>

If the project is associated with a theme (or with a template), this theme (or this template) can be selected for your reports. Your printed reports and your application will have the same style. This theme (or template) can be modified once the report is created.

Starting with version 28 Update 1, you can choose not to use a theme by selecting "None". In this case, no theme will be applied to the report.
...
<a name="print_format_the_report_ELTPARAGRAPHE000318"></a>
...
### Saving a report
<a name="saving_report_ELTPARAGRAPHE000340"></a>
...
- ![WEBDEV - Server code](https://doc.pcsoft.fr/ext/images/us/WBS.png) when deploying the site.
