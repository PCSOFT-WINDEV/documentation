...
**Remark**: Only a Scheduler control report can be created on a Scheduler control.. 
...
**Attention**: It is not possible to test a report on a Scheduler control directly from the report editor, as the window or page containing the Scheduler control must be open.. To run the test of a report based on a Scheduler control, you must:
...
[**Reports & Queries**](../Presentation/3088004.md) cannot be used to create reports on a Scheduler control. However, it is possible to customize reports on a Scheduler control created in the integrated report editor (color, font, size, etc.).
