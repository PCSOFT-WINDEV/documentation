...
![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=IconeCroissant.gif) and ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=IconeDecroissant.gif) allow you to know the sort direction of the column at any time.

**Programming**: Sorting can also be programmed using the:
...
<a name="search_column_magnifying_glass_ajax_mode_only_ELTPARAGRAPHE000085"></a>
...
**Attention**: Search is not available:
...
Columns can be resized: users can manipulate the size of columns.

**Programming**: Column sizes can be adjusted using the [TableAdjust](../WDLang1/3074004.md) function.
<a name="NOTE2_5"></a>
...
In a Table based on a data file field, it is possible to create calculated columns: these columns are not linked to a data file.. These columns can display a calculation, for example, ...
...
**Example**: The Day column is of type List of values. This column contains the following values:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Champ_table_ColContenu.gif)
...
2. Select "Clickable column   " in the "Details" tab of the column description window.

3. Specify the action and the target to use ("Details" tab of the column description window). For more details, see the following paragraphs.
...
Different preset actions are available in a drop-down list. These predefined actions depend on the elements present on the page and on the project pages.. You can for example choose to display a project page, to run the click code of a button or to point to one of the page anchors.

**Caution**: Actions are performed after the Table field click code.
...
|   |   |
| --- | --- |
| **New browser (_blank)**: <br>Displays the result in a new browser window. | <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Dest_nouveau.gif)<br> |
| **Current frame (_self)**: <br>Displays the result in the same frame. | <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Dest_FrameEnCours.gif)<br> |
...
| **Other frames in the frameset (if the page is part of a frameset)**: <br>Displays the result in a specific frame. |   |
...
[Characteristics of columns found in a Table control in a page (prefix syntax)](../WDChamp/1013334.md)

[Column of type Attribute](../WDChamp/1013350.md)
