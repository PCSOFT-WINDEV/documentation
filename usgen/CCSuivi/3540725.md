...
This dashboard contains several widgets allowing you to follow the evolution, tasks, incidents, fixed bugs in one or more projects. 

Each contributor can customize their dashboard according to their objectives. 
...
If the dashboard is not displayed in the Project Management Hub, simply create a new Dashboard tab: 

1. Click the "+" button found on the right of existing tabs. 

2. Select the type of tab you wish to create: "Dashboard". 
...
Note: It is possible to create several tabs of the same type but displaying different data. 
...
- **Vision développement**: In this mode: 
...
- **Vision documentation**: In this mode: 
...
**Note** By default, widgets display data corresponding to the filters specified in the Ribbon. However, each widget can display specific information. All you have to do is expand the arrow found on the right of the widget and select "Select...". 
<a name="NOTE2_3"></a>
...
- **Contributor widget**. Displays the characteristics of the current contributor: name, photo, workstation, e-mail address. 
...
- **Progress widget**. Displays the percentage of progress of the development tasks for the current contributor. 
	Clicking on the ProgressBar displays the associated list of requirements: a new Requirements tab is displayed. 
...
- **Incidents widget**. Displays the number of incidents corresponding to the specified list of requirements. 
	...
	- view the incidents from the global set of incidents. 
...
- **Requirements widget** Displays the list of requirements associated with the contributor in the form of a TreeMap. The completed requirements are displayed in green, the scheduled requirements in blue and the locked requirements in red. Clicking a requirement opens the description of the requirement. 
	The drop-down menu of this widget allows you to: 
...
- **Requests widget** Displays the number of requests to be processed. 
...
- **Current task widget** Displays the contributor's current task (and its duration since the last activation). 
