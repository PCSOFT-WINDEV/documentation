...
By default, only the names of the tables are displayed. 
...
Note: The "Hide backup directories" option displays only directories containing database tables.. 
...
- Optimize and repair: This option displays the configuration window for optimizing and repairing HFSQL tables.
...
This tab presents the information regarding the links between the tables found in the HFSQL Client/Server database.
...
- The tables affected by the trigger,
...
This tab is used to manage the locks for all the tables of the current database. 
...
For the selected HFSQL Client/Server database, the ribbon proposes the following actions: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Gestion_Serveur%20-%20HC%20N%B0001%201.gif)
...
- In the "Tables" group, the "Optimize and repair" option is used to optimize one or more tables. For more details, see [Optimize and repair tables](../CCHF/3540007.md). 
