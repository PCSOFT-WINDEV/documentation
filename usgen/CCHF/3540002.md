...
**Reminder** HFSQL server replication consists of automatically replicating data from server to server. For more details, see [HFSQL server replication](../WDLang4/1000020588.md). 


	<B><B><B><B>Warning</B></B></B></B>

	 **The replication of HFSQL servers must not be implemented without the agreement of the developer of the application**. Indeed, implementing the replication between several HFSQL servers requires some adaptations in the application. 
...
<a name="implementing_replication_between_hfsql_servers_ELTTEXTE000276"></a>
...
5. Check "The server is subscriber or spare (will receive replicated data)". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=CC_HF_Replication_Serveur%20-%20HC%20N%B0001.gif&type=thumb)
...
9. Specify the priority of the server. 
	Note: In the case of a modification conflict, this number is used if the conflict resolution method corresponds to priority resolution.. The smaller the priority value defined for the server is, the higher the priority granted to the server will be.
...
1. If no replication has been defined, click the link "Click here to create a new server replication". 
	...
	The replication creation wizard opens. 
...
4. Select the elements to replicate. You can replicate:
	...
	- a selection of databases or tables. 


 Remarks: 

- You can exclude certain tables from replication. If this option has been selected, the next step of the wizard allows you to select the elements to exclude. 

- If necessary, you can make an initial copy of the elements to replicate. 

5. Schedule the replication: 

	- Periodic: replication will be performed regularly. A wizard allows you to define the periodicity. 

	- Continuous mode: replication is carried out continuously, each time a change is made.. 
...
6. ***In the case of a unidirectional or bidirectional replication,*** select the conflict resolution mode: 
	...
	- **Most recent**: In a conflict, the most recent element takes precedence.
			Caution: This method requires synchronization of the clocks on the different servers. The date and time (with second precision) of records is used for the comparison (YYYYMMDDHHmmSS). If two records in conflict strictly have the same date and time, the identifier of servers is used in order for the comparison to be deterministic.

	- **Priority** In the event of a conflict, only the priority value defined for the server is taken into account. The smaller the priority value defined for the server is, the higher the priority granted to the server will be.

	- **Stored procedure with all items**: In the event of a conflict, a stored procedure is launched. In this procedure, all the items found in the record will be accessible. Specify the name of the stored procedure to use. 

	- **Stored procedure with modified items**: In the event of a conflict, a stored procedure is launched. In this procedure, only the modified items will be accessible. Specify the name of the stored procedure to use. 
...
<a name="managing_the_existing_replications_ELTTEXTE000318"></a>
...
- **Modify a replication** ("Modify" button). The replication description window appears. This window is used to modify the main characteristics of the replication. Some characteristics cannot be modified. In this case, you may have to delete the replication and recreate it. 
	The following characteristics can be modified: 

	- the mode for solving the conflicts ("General" tab). This option is only available for monodirectional and bidirectional replications.  
	...
	- the description of the replication ("General" tab). 
			...
			**Caution**: This modification should only be made if the server remains identical, except for its DNS name or data replication port..  

	- the replicated elements ("Replicated elements" tab). 
			Please note: it is only possible to exclude items from replication. 
...
- **Delete a replication** ("Delete" button). After confirmation, the configuration of the replication is deleted.
	Please note: The publisher server must be accessible.. If the subscriber server is not accessible, you have the ability to force the deletion. In this case: 
...
- **Troubleshooting corrupted files** ("Troubleshoot" button): Available only on subscribed replication, this button lets you ignore a corrupt log file (.hyl file). 
	If the replication fails due to the corruption of one of the replication log files (.hyl), you will not be able to ignore the contents of the current log file and continue with the replication. 
