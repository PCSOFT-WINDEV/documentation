...
- In the call to the stored procedure, you must use the syntax specific to the database used, including for the syntax of parameters. For Sybase, the parameters are specified with the following syntax: **@ParamName**. Warning: the "@" character must be followed by at least one letter (the @1 syntax is forbidden).. 
