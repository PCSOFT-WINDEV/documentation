...
In our example, this search will be performed on the "Product" table.
...
The window we are going to create will be as follows: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche_Fin%20-%20HC%20N%B0002.jpg&type=thumb)


To create this window, we will: 

- Create the query for selecting records in the Product table.
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Nv_WM_GAF.jpg) in the quick access buttons. In the window that appears, select "Query". The query creation wizard opens.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0001.jpg)
...
4. In "Query name", replace "QRY_NoName" with "QRY_Products", and in "Caption", enter "Find products by name": <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0003%201.jpg)
...
1. Under "Database schema", select the "Product" table.

2. Click the icon to the left of the "Product" table to view all its items. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0002%201.jpg)


3. Select the "ProductID" item and click the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0002.jpg) button. The item appears in the list of query elements. 

4. Repeat the same steps for the "Caption", "Description" and "Photo" items. 

5. The query description window is as follows: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0003.jpg&type=thumb)
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fen_WM_GAF.jpg).

2. The result is displayed in a window: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0004.jpg)
...
To set a parameter based on the name of the product, we will define a condition on one of the items of the Product table:

1. Select the Product.Caption item in the list of query elements. 

2. Open the context menu and select "Selection condition .. New condition". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0005.jpg)


3. In the window that appears, we will specify that the selection condition corresponds to a parameter: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0006.jpg)
...
4. Validate the condition description window. The number "1" appears on the right of "Product.Caption", indicating that a selection condition has been defined. <br>![Condition in the query description window](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0007.jpg)


5. Validate the query description window. 

6. The query graph is modified to take into account the selection condition that was defined. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0008.jpg)


7. Save the query by clicking ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Sauver_WM_GAF.jpg) in the quick access buttons.
...
3. Select the ParamCaption parameter. In the lower section of the window, enter "Polo". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0009.jpg)


4. Validate the window. The query result corresponding to the specified parameters is displayed.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0010.jpg)
...
We will create the search window using the window creation wizard. We are going to create a "Looper" window to list the products as we did in lesson 2. However, in this lesson, the window will not use data directly from a table, but from the query. This will allow us to perform a search using the parameter we defined in the query.  The window creation wizard includes different preset windows. These windows have modern UIs for your applications.
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Nv_WM_GAF.jpg) in the quick access buttons. In the window that appears, click "Window" then "Window".
...
5. The wizard prompts you to choose the data source associated with the window. In our case, it is a query:
	...
	- Select the query we just created: "QRY_Products". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0011.jpg&type=thumb)
...
12. Finish the wizard. The window is automatically created, displayed in the editor and saved. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche%20-%20HC%20N%B0012.jpg)
...
Let's analyze this code:
	...
	- The aim is to open the previously created window when a row is clicked . This window is based on the Product table. 

	- The record selected by the query must be in the "Product" table in order to load the buffer of the selected data in memory. The operation is performed by [&lt;Table&gt;.ReadSeek](../WDLang4/1000025023.md).
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fen_WM_GAF.jpg) in the quick access buttons. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Fen_RAD_Recherche_Go%20-%20HC%20N%B0001.jpg&type=thumb)
...
3. On the "Details" tab, check "Enable search in the Action Bar". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche_Fin%20-%20HC%20N%B0001.jpg)
...
10. Save changes by clicking ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Sauver_WM_GAF.jpg) in the quick access buttons.
...
13. Test the window you have just created in the simulator (click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fen_WM_GAF.jpg) in the quick access buttons).
	...
	- The list of products containing "Polo" is displayed. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20FEn_RAD_Recherche_Fin%20-%20HC%20N%B0002.jpg&type=thumb)
...
3. In the "Details" tab of the control description window, in the "Moves and gestures" area, check "Pull to refresh". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Pull%20to%20Refresh%20-%20HC%20N%B0001.jpg)
...
Test the window in the simulator (click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_Fen_WM_GAF.jpg) in the quick access buttons).

1. Click and drag the top of the looper down.

2. Release the mouse. The looper is refreshed. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Pull%20to%20refresh%20Go%20-%20HC%20N%B0001.jpg&type=thumb)
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Nv_WM_GAF.jpg) in the quick access buttons. In the window that appears, click "Window", then "Internal window".

2. The internal window is automatically opened in the editor. 

3. The save window appears. Name the internal window "IW_MLZ_Options". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Fen%EAtre%20coulissante%201%20-%20HC%20N%B0003.jpg)
...
6. On the "UI" tab, specify the dimensions of this internal window:
	...
	- Height: This value must be the window height above which the sliding menu is displayed. In our case, this height is set to 248. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Fen%EAtre%20coulissante%201%20-%20HC%20N%B0002.jpg)
...
9. Save the window by clicking ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_Sauver_WM_GAF.jpg) in the quick access buttons.
...
3. On the "Details" tab, in "Left sliding window", select "IW_MLZ_Options". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Fen%EAtre%20coulissante%201%20-%20HC%20N%B0004.jpg&type=thumb)
The "Swipe" option makes the sliding window appear after a swipe gesture. 

4. Confirm. 

5. Since the window is associated with an Action Bar, the editor prompts you to enable the option to open the sliding window. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=P4_MP%20-%20Fen%EAtre%20coulissante%202%20-%20HC%20N%B0001.jpg&type=thumb)
...
2. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=ICO_GO_simu_PRJ_WM_GAF.jpg) in the quick access buttons.
