...
Displays the permission request for user activity tracking (iOS only). <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=AppleATT.gif)
...
SWITCH AppleTrackingAuthorizationStatus()
     CASE atasNotDetermined
             AppleRequestTrackingAuthorizationAsynchronous(AppleTrackingDemandeAuthorizationAsynchronous_Callback)
     CASE atasAuthorized
             TrackUser()
END
