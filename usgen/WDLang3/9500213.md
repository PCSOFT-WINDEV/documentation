...
In the SaaS model, the application is accessible remotely via an Internet browser. For the user, there's nothing to install locally, nothing to maintain, and so on.
...
<a name="operating_mode_and_use_saas_site_from_the_point_view_the_user_ELTPARAGRAPHE000020"></a>
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=SaaS_Principe.gif)
...
## Prerequisite
<a name="prerequisite_ELTTEXTE000273"></a>
...
The SaaS site management webservice will be used via its web alias (e.g. WDSAAS310_WEB), which must correspond to the directory of the same name in the Application Server installation (default: C:\\WEBDEV31\\WDSaaS\\WDSaaS310\\WDSAAS310_WEB).
...
<a name="developing_saas_site_ELTTEXTE000297"></a>
The method for developing a SaaS site is the same as the method for developing any other WEBDEV website. For more details, see [Developing a SaaS site](../WDLang3/9500214.md).
...
<a name="deploying_saas_site_ELTTEXTE000321"></a>
When SaaS is enabled for a site, the deployment requires the following additional parameters:

- **Choice of web service associated with the site** WEBDEV Application Server: it is possible to use the Webservice located on the same Application Server as the one hosting the site (this is the most frequent case) or a Webservice located on another WEBDEV Application Server (when several SaaS sites share a Webservice).

- **The name and password of the administrator of the SaaS web service**. This information is required to perform some deployment steps (automatic modification of HFSQL tables for example).
...
- In some cases, the site developer may not know the username and password of the administrator of the SaaS web service. In this case, you can use the setup by physical media or the installation packages and ask a person with sufficient rights to perform the setup.

- If the address of the SaaS web service was not specified during the deployment, the alias of the SaaS web service (WDSAAS310_WEB) must exist on the same virtual server as the SaaS site.
...
<a name="managing_saas_site_ELTTEXTE000345"></a>
...
[SaaS administrator](../WDLang3/1600000.md)
