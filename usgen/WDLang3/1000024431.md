...
The use cases are numerous: demonstration application, application for a Numerical bound, application for children, application dedicated to a particular task, etc.
...
A "device owner" application can perform operations normally reserved for system applications: create users, modify global parameters (access to wifi, bluetooth), define security policies, hide applications, and in particular in kiosk mode: lock the application so that the user can no longer exit it by himself.. 
 ...
 **Note**: There can only be one application in "device owner" mode per device, and this operation is only possible if no user has been configured on the device.. Otherwise, it is necessary to delete all users before running the command.
...
- "Device management application (can become "device owner" for kiosk mode)" has been checked in the Android configuration description. 
...
- In case of system restart (battery problem, etc.), the kiosk mode will not be kept the next time the application is launched. Don't forget to:  
	...
	- Automatically relaunch the application: check the "Automatically start the application when starting the device" option in the Android application generation wizard.. 
