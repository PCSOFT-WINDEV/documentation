...
**Conditions of use** The response ProgressBar can only be progressed if the Web server processing the HTTP request indicates the length of the data to be returned in its response. 
...
Frequency for refreshing the receive progress bar (in bytes). The default frequency is set to 10.000 (the progress bar is refreshed every 10 KB). Please note: this frequency is indicative only.
...
Frequency for refreshing the progress bar used to send data (in bytes). The default frequency is set to 10.000 (the progress bar is refreshed every 10 KB). Please note: this frequency is indicative only.. 
...
- &lt;Progress&gt; is the number of bytes already received. 
	Note: If the server does not return the expected total size, then &lt;Total&gt; will be equal to &lt;Progression&gt;. 
...
Frequency for refreshing the receive progress bar (in bytes). The default frequency is set to 10,000 (the procedure is called every 10 KB). Please note: this frequency is indicative only.. 
...
Frequency for refreshing the send progress bar (in bytes). The default frequency is set to 10,000 (the procedure is called every 10 KB). Please note: this frequency is indicative only.. 
...
Frequency for refreshing the receive progress bar (in bytes). The default frequency is set to 10,000 (the event is triggered every 10 KB). Please note: this frequency is indicative only.
...
Frequency for refreshing the progress bar (in bytes). The default frequency is set to 10,000 (the event is triggered every 10 KB). Please note: this frequency is indicative only..  
