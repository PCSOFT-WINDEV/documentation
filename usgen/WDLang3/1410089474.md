...
Deep linking allows you to redirect users to specific content in a mobile application (rather than in a browser) when they click on a link in a web page or in an email. 
...
**If the user does not have the application installed on their phone**, the link can for example: 
...
**Note** To find out whether an application has been launched via a deep link, use the [ExeInfo](../WDLang1/3035001.md) function with the constant *exeLancement*.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Deeplink_Android%20-%20HC%20N%B0002.gif&type=thumb)
...
Caution: The "assetlinks.json" file must be accessible from the outside, without redirection, via an HTTPS connection. 
...
**Attention**: Some mobile browsers don't allow applications to be launched directly from page links. Some browser settings may also prevent this.
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=Deeplink_ios%20-%20HC%20N%B0001.gif&type=thumb)
...
Note: The "Developer mode" option in the generation wizard enables the application to retrieve the **apple-app-site-association** file directly, without passing through an Apple-managed CDN.. This option can only be used in simulator mode on a Mac, using a development provisioning profile.
...
[ExeInfo (Function)](../WDLang3/3035001.md)
