...
Address of agent with which the communication via SNMP will be established. This address can be specified as follows:
...
## Remarks
<a name="NOTE0_2"></a>
SNMP uses the UDP protocol that is a connectionless protocol. Therefore, **SNMPStartSession** does not allow you to find out whether the contacted agent can actually be joined. **SNMPStartSession** returns a session once the SNMP request was sent, even if the agent is not available. To check whether an agent is actually connected, you have the ability to use: 
...
[SNMPCloseSession (Function)](../WDLang3/1000017068.md)
