...
Optional parameters expected by the report for its execution. These parameters are defined in the opening code of the report. These parameters can be the name of a view, the name of a memory area, etc.
...
- In standard, the report editor locks no data file.
	...
	To avoid having this error in custom mode, the lock mode must be changed before the running the report: 
