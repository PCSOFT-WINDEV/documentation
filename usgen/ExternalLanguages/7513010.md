...
Note: Other external language programming modes are available. For more details, see [The programming modes of the external languages](../LangageExt/7510002.md)
...
<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=214&name=mode3.gif)
...
<a name="2_including_the_hfsql_declarations_ELTPARAGRAPHE000044"></a>

An inclusion (#include) must be added for each table declared in the database schema of the WINDEV project. The declarations of tables are included in the file whose extension is.h (this file replaces the.wdr created with the earlier WINDEV version).
...
<a name="1_declaring_the_hfsql_context_and_the_working_buffers_each_file_ELTPARAGRAPHE000057"></a>

If the application must manage tables, an HFSQL working context must be declared as well as a buffer for each table.
...
**Declaring the map**
The following declaration is used to associate the created methods with the user events. The events correspond, as for the previous method, to the content of the **WDKey** string filled in the WLanguage code.
...
The CWDDialog class takes the Enter and Esc keys into account (standard behavior for validation and cancellation). To retrieve these events yourself, you can inherit the mechanism described in the base class with the following line (placed at the end of your method declarations): HERITE_TOUCHE_MAP(CWDDialog)
...
## Using HFSQL in C++
<a name="using_hfsql_ELTTEXTE000327"></a>
The functions for accessing tables can be used by calling nWDExecute (see the definition of this function). However, the C++ language interface provides an HFSQL table management class named CVHFContext.
