...
### Data-bound Table control loaded in memory
<a name="databound_table_control_loaded_memory_ELTPARAGRAPHE000018"></a>

The new records added into a data-bound Table control loaded in memory cannot be modified straightaway. To modify these new records directly in the Table control, you must use [TableDisplay](../WDLang1/3074003.md) with the *taInit* constant after the additions.
<a name="NOTE2_2"></a>


### Locking a table in a transaction
<a name="locking_table_transaction_ELTPARAGRAPHE000028"></a>
...
- The precision of read and written values is limited to the precision of the WLanguage: one millisecond. 
	**Special case** The datetime type: precision is limited by SQLServer to 0.00333 seconds.

- The new SQLServer 2008 date and time types are supported only from SQLnCLI for SQL Server 2008.. For earlier client libraries, these items are considered as "unicode text" items.
...
<a name="database_schema_options_ELTPARAGRAPHE000071"></a>
...
- The database schema options regarding the full-text indexes are not supported by the Native Connector.

- The database schema options regarding the partial indexing of text memos are not supported by the Native Connector.
...
**Limit**: The [HPrepareSQLQuery](../WDLang4/3044119.md) function cannot be used to prepare a stored procedure that returns:
...
The specifics for using HFSQL functions with the SQL Server Native Connector are as follows:  
...
| **Multi-table query** <br>**HAdd**, **HModify** and **HDelete** are not supported. | [HAdd](../WDLang4/3044147.md), [HModify](../WDLang4/3044042.md)<br>[HDelete](../WDLang4/3044018.md) |
...
| The ***hForceIDAuto*** constant is ignored when used with **HModify**. An "IDENTITY" column cannot be modified in SQL Server. | [HModify](../WDLang4/3044042.md) |
...
| [HCreation](../WDLang4/3044255.md) creates DATE and TIME items if the version of the server and the client library correspond to SQL Server 2008 (or later). | [HCreation](../WDLang4/3044255.md) |
...
Calculated items can be used in the tables. 
