...
Warning: the tilt angle can only be changed if the following conditions are met:
...
- 10 &lt;= zoom &lt;= 14: the maximum increases linearly from 30 to 45 degrees. 

- 14 &lt;= zoom &lt;= 15.5: the maximum increases linearly from 45 to 67.5 degrees. 
...
- The property **Tilt PROPERTY** property allows you to change the tilt angle only in "satellite" mode on certain areas with a sufficient level of detail and with a sufficiently strong zoom: the tilt button must appear on the map. The bearing can be changed only if the tilt is set to 45. 
