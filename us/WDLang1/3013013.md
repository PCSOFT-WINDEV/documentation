
## UnloadWDL (Function)

***In french: DéchargeWDL***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Frees the library that was loaded in memory by [LoadWDL](../WDLang1/3013020.md). The memory is freed.
<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Closing code of the first project window
UnloadWDL(hWnd) // Close the library
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`UnloadWDL(<Library Identifier>)`
---

**`<Library Identifier>: (Integer)`**

Identifier of the library to unload, returned by [LoadWDL](../WDLang1/3013020.md).  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


- The main project library is always loaded and it cannot be unloaded.

- When a library is closed using the **UnloadWDL FUNCTION**function, HFSQL data files open in the library are not automatically closed. In this case, any access to these HFSQL data files will result in the error "WDL xxx has been unloaded".
	It is recommended to use [HClose](../WDLang4/3044073.md) on open files of a library that is going to be unloaded.




<a name="XComposante"></a>

## Component:
wd310vm.dll
## Voir aussi

[LoadWDL (Function)](../WDLang1/3013020.md)

[Various functions](../WDLang1/3013036.md)

[Miscellaneous WEBDEV functions](../WDLang1/3012026.md)

