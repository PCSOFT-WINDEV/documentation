...
Minimum size of the shared memory area in bytes.

- If the zone already exists: 

	- if &lt;Taille&gt; is 0: this parameter is ignored and the zone size can be obtained using the [fSize](../WDLang1/3036055.md) function.
...
Constants used to define the access mode to the shared memory area.
...
Name of WLanguage procedure ("callback") that will be called automatically if the memory area is modified by another WLanguage application.
...
- Windows Vista and later: the &lt;Sharing&gt; parameter of the **fMemOpen** function is managed. The services and the users are located in a different space. To share a memory area between a service and an application in a user session, the *shareGlobal* constant must be used.

- In Terminal Server: the &lt;Partage&gt; parameter of the **fMemOpen** function is supported. Each session opened on the Terminal Server has a different memory space. The applications started in the same TSE session can share a memory area created with the *shareUser* constant. Several applications located in different sessions can share a memory area created by the *shareGlobal* constant.

- Windows: To access an *Global* memory area for writing or reading, you need administrator rights.
...
**Note** The mechanism for automatically recalling a WLanguage procedure whenever the memory area is modified by another application only works between WLanguage applications.
<a name="NOTE0_4"></a>
...
[Shared memory area functions](../WDLang1/1000018927.md)
