...
Creates a Repositionable Note control programmatically.
<a name="Exemple1"></a>
...
NOTE_MyNote <- MyWindow.EdCreateNote("New note")
...
`[<Result> <- ] <Window>.EdCreateNote(<Initial text> [, <X> [, <Y>]])`
---
...
If **&lt;Window&gt;.EdCreateNote** is directly used, the Note control is created in the window and it is visible by default.

**`<Window>: (Window name)`**

Name or alias of window where the Note control will be created. 
If this parameter is not specified or is an empty string (""), the Note control will be created in the current window.
...
<a name="characteristics_repositionable_note_control_ELTPARAGRAPHE000216"></a>

- The creation can be canceled by Ctrl + Z. 

- The Repositionable Note control created is a persistent note: if the window is closed and reopened, the note will be restored.. 
...
- The Repositionable Note control will be saved during the call to the functions: 
	...
	- [&lt;Window&gt;.EdSave](../WDLang1/1000022330.md),
...
### Equivalences
<a name="equivalences_ELTPARAGRAPHE000245"></a>
