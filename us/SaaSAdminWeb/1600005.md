...
**Remark**: Deleting a customer account permanently deletes all current subscriptions, users and visits.
...
For more details, see [Managing subscriptions](../SaaSAdminWeb/1600006.md).
...
## Manage users
<a name="manage_users_ELTTEXTE000235"></a>
...
| Element | Role |
| --- | --- |
| *LastName* | User's last name (optional parameter). |
| *First name* | User's first name (optional parameter). |
| *Login* | Identifier for connecting to the SaaS sites.<br>**Note** Once a user's account has been created, Subscription licenses must be allocated to enable access to SaaS Sites. |
...
| *Password to enter* | If "Password to enter" is checked, during the next connection of the user, a message will indicate that his password has expired and that a new password must be entered.<br><br>Note: If this box is checked when the user is created, a password must still be specified for the user's first connection. |
...
When declaring a SaaS site in the SaaS administrator, you have the ability to define a set of parameters. These parameters can be used through programming to customize the interface or the operating mode of the SaaS site according to the client.
...
[SaaSAdminFindClient (Function)](../SaaSAdminWeb/1000019114.md)

[SaaSAdminCreateClientDatabase (Function)](../SaaSAdminWeb/1000019141.md)

[SaaSAdminListClient (Function)](../SaaSAdminWeb/1000019116.md)

[SaaSAdminModifyClient (Function)](../SaaSAdminWeb/1000019115.md)

[SaaSAdminDeleteClient (Function)](../SaaSAdminWeb/1000019117.md)
