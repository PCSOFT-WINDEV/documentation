


## ImageLayerVisible (Function)

***In french: ImageCalqueVisible***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Used to make a layer found in a WDPIC image visible or invisible.


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
// Displays the product image in an Image control
PROCEDURE DisplayProduct(sImageControl is Control, sWDPicName is string, ...
		bPromo is boolean)

MyImage is WDPic
MyImage.FileName = sWDPicName
ImageLayerVisible(MyImage, "Promo layer", bPromo)

sImageControl.Value = MyImage
```

<a name="XSYNTAXE"></a>

## Syntax
<a name="SYNTAXE1"></a>

`<Result> = ImageLayerVisible(<WDPic image> , <Layer name> , <Visible>)`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the layer exists,

- <u><u><u><u>False</u></u></u></u> if the layer does not exist. In case of failure, the [ErrorOccurred](../WDLang1/3087001.md) variable is set to <u><u><u><u>True</u></u></u></u> and [ErrorInfo](../WDLang1/3013008.md) returns the details of the error.




**`<WDPic image>: (WDPic variable)`**

Name of the [WDPic](../WDLang1/1000023503.md) variable corresponding to the image used. 

**`<Layer name>: (Character string)`**

Name of layer to use. This name must be identical to the name defined in the image editor.

**`<Visible>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the layer must be visible,

- <u><u><u><u>False</u></u></u></u> if the layer must be invisible. 






<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks
A layer can be made visible or invisible through programming only if the layer includes the "Visible/Invisible programmatically" option. This option is available in the context menu of layer. This option can be enabled: 

- in the image editor,

- in the Image Editor control.


For more details, see [Managing layers](../Editeurs/1410086606.md). 

<a name="XComposante"></a>

## Business / UI classification:
UI Code
## Component:
wd310pic.dll
## Voir aussi

[WDPic (Variable type)](../WDLang2/1000023503.md)

[Functions for managing images](../WDLang2/1000019452.md)

[Image editor: Features](../WDLang2/1410086606.md)

[&lt;Image&gt;.LayerVisible (Function)](../WDLang2/1000023747.md)

