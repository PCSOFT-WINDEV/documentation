...
|   |   |
| --- | --- |
| *jsEventBeforeUnload* | The event is triggered before unloading the page. <br><br>**Remarque**: It is possible to suggest that the user leave the page using the code: <br><br><pre><code>JSInfoEvent("returnValue") = "Message"<br>	RETURN "Message"</code></pre><br><br>The browser will display the message with the "Exit from the page" and "Stay on the page" buttons (or a variation according to the browser used). |
...
| *jsEventFocus* | The event is triggered when the target object gains focus. |
...
| *jsEventTouchCancel* | The event is triggered when the user cancels the action performed with his or her finger: displaying the context menu, for example (event managed during a display on a Phone, for example). |
