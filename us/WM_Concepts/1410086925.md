...
The data that is not linked to the data file is lost if the Table is closed, then reopened (for example, checked or unchecked rows).
...
The WLanguage functions starting with "Table" are used to manipulate Table controls with direct access to the data source. When a row is added to or deleted from the Table control, the record in the bound data file is also added or deleted.
