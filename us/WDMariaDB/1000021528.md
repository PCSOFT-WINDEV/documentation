...
**Differences from SQL programming**: To manipulate MariaDB data, queries are not mandatory. In fact, you can directly use HFSQL functions to read your data, perform searches, filters, etc. However, it is necessary to import the structure of MariaDB files into the WINDEV or WEBDEV analysis.
...
3. The wizard starts.  A connection to the MariaDB database is automatically established and associated with the files imported into the analysis. This connection will have to be used to handle the MariaDB data file programmatically.
