


## xlsMergeCell (Function)

***In french: xlsFusionneCellule***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Merges the specified cells in an Excel document.


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
MyXLSDocument is xlsDocument

// Merges the cells from B2 at the top-left to D4 at the bottom-right
xlsMergeCell(MyXLSDocument, "B2", "D4")
```

<a name="XSYNTAXE"></a>

## Syntax
<a name="SYNTAXE1"></a>

### Merging a range of cells identified by row and column

`<Result> = xlsMergeCell(<Excel document> , <Start row> , <Start column> , <End row> , <End column> [, <Type of merge>])`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the cells were merged, 

- <u><u><u><u>False</u></u></u></u> otherwise.




**`<Excel document>: (xlsDocument variable)`**

Name of the variable of type [xlsDocument](../WDLang5/1000017464.md) to be used.

**`<Start row>: (Integer)`**

Row number of the upper-left cell in the range of cells to merge.

**`<Start column>: (Integer)`**

Column number of the upper-left cell in the range of cells to merge.

**`<End row>: (Integer)`**

Row number of the lower-right cell in the range of cells to merge.

**`<End column>: (Integer)`**

Column number of the lower-right cell in the range of cells to merge.

**`<Type of merge>: (Optional Integer constant)`**

Type of merge to be performed:



|   |   |
| --- | --- |
| *xlsMergeAndCenter*<br>(default value) |  Merges the cells and centers the contents. |
| *xlsMergeSimple* | Merges the cells without centering the contents. |




<a name="SYNTAXE2"></a>

### Merging a range of cells identified by reference

`<Result> = xlsMergeCell(<Excel document> , <Start cell> , <End cell> [, <Type of merge>])`
---

**`<Result>: (Boolean)`**



- <u><u><u><u>True</u></u></u></u> if the cells were merged, 

- <u><u><u><u>False</u></u></u></u> otherwise.




**`<Excel document>: (xlsDocument variable)`**

Name of the variable of type [xlsDocument](../WDLang5/1000017464.md) to be used.

**`<Start cell>: (Character string)`**

Upper-left cell in the range of cells to merge. This parameter has the following format: &lt;Column letter&gt;&lt;Row number&gt;. For example: "A32".

**`<End cell>: (Character string)`**

Lower-right cell in the range of cells to merge. This parameter has the following format: &lt;Column letter&gt;&lt;Row number&gt;. For example: "A32".

**`<Type of merge>: (Optional Integer constant)`**

Type of merge to be performed:



|   |   |
| --- | --- |
| *xlsMergeAndCenter*<br>(default value) |  Merges the cells and centers the contents. |
| *xlsMergeSimple* | Merges the cells without centering the contents. |





<a name="XComposante"></a>

## Component:
wd310xls.dll
## Voir aussi

[xlsDocument (Variable type)](../WDLang5/1000017464.md)

[XLS file functions](../WDLang5/3080009.md)

[xlsUnmergeCell (Function)](../WDLang5/1410089538.md)

[&lt;xlsDocument variable&gt;.MergeCell (Function)](../WDLang5/1410089568.md)

