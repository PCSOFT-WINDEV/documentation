...
From 2020, all companies must use Chorus Pro.
...
- Invoice recipients: public authorities,

- the services that submit invoices to the government.


**Note** All documentation concerning the operation of the "Chorus Pro" portal and the APIs is available at the following address: [https://communaute.chorus-pro.gouv.fr/?lang=en](https://communaute.chorus-pro.gouv.fr/?lang=en)
...
**Note**: test and production environments can be "linked" but remain independent. An account must be created for every environment.
...
**Step 1: Create a Chorus Pro account**: 
...
Note: the address associated with the account must be a valid and consulted Email address. This address will be used by Chorus to communicate on the changes and maintenance periods of the different environments.
...
6. Log in to the qualification portal using the login details provided in the downloaded file (Partner supplier: login and password).
...
7. Log in to the portal using the login details provided in the downloaded file (Partner supplier: login and password).
...
Note: you can create several technical accounts and give them limited access to certain APIs.
...
When using APIs (and not EDI), only one type of certificate is required for Chorus Pro: an RGS-compliant client server authentication certificate\* a minima.
...
Warning: If you're using a proxy, remember to configure proxy access in your code using the [Proxy](../WDLang3/3043002.md) function. 

Note: See LST 115 for an example of how to use APIs to send PDF invoices to the Chorus Pro portal. 
...
Note: From version 24 onwards, it is possible to use a certificate stored in the application's library (thus avoiding its distribution). Simply specify its name in string format.
...
Tip: you can download a zip file containing all the JSON formats used by the "Chorus Pro" APIs at:
...
"filename": "facture_capgemini.pdf"
}
```
