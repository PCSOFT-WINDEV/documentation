


## XMLPath (Function)

***In french: XMLPosition***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Returns the current position in the document in XPATH format.








<a name="Exemple1"></a>
<a name="sample_code"></a>

## Example


```wl
Res is string
Res = XMLPath("MyDocument")
// Res is of the form: /COMMANDE/LIGNECDE[2]/@NUMERO on an attribute
// Res is of the form: /COMMANDE/LIGNECDE[1] on a child tag
// Res is in the form: /COMAND on a tag 
// Caution, the XMLPosition function can also return a string
// of this (syntactically valid) form: /*/*[8]/*[1]
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntax

`<Result> = XMLPath(<XML document>)`
---

**`<Result>: (Character string)`**

Position of current element, in XPATH format.

**`<XML document>: (Character string)`**

Name of the XML document used. This document contains the XML code to study and it was created by [XMLDocument](../WDLang5/3081008.md).  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarks


### XML functions and threads
<a name="xml_functions_and_threads_ELTPARAGRAPHE000188"></a>

If your application uses threads, the XML document is shared between all these threads. For more details on threads, see [Managing threads](../WDLang1/3077026.md).

<a name="XComposante"></a>

## Business / UI classification:
Business Logic
## Component:
wd310xml.dll
## Voir aussi

[Management of XML](../WDLang5/3081018.md)

[XML functions](../WDLang5/3081005.md)

