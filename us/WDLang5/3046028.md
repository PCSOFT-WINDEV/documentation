...
Initializes the query linked to the report. This function must be used if the request uses data files accessed via a specific connection (e.g., a connection using an OLE DB provider). If the query does not use a specific connection, use [iInitReportQuery](../WDLang5/3046021.md).
