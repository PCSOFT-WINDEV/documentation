...
SWITCH nEvent
	CASE SocketOpening: 
		...
		RETURN
	CAS SocketMessage: gsRes += [RC] + "Message received: " + sMessage
		
	CASE SocketClosing: gsRes += [RC] + "Socket closing"
		
	CASE SocketError: gsRes += [RC] + "Socket error: " + sMessage
		RETURN
	OTHER CASE
...
3. Create a local procedure (**pServerSocketListen** for example) to listen to the messages sent by the client from its Web site. The code of this local procedure is as follows: 
	...
	```wl
	// Local procedure code: Listens to messages sent by the socket client.
	...
	// In our case, 2 carriage returns in a row
...
[SocketConnect (Function)](../WDLang3/3070016.md)
