...
**Remark**: This function can only be called if the [SNMPGet](../WDLang3/1000017069.md) function has been used at least once.. In this case, **SNMPGetNext** reads the next value of the SNMP agent. Otherwise, the OID preceding the OID to be read must be specified in **SNMPGetNext**.
<a name="Exemple1"></a>
...
- Value of the OID returned by the agent. The type of value is retrieved in an integer variable (&lt;Returned Type&gt; parameter).
...
| *snmpTypeCounter32* | The OID is a counter (signed integer on 32-bit). |
| *snmpTypeCounter64* | The OID is a counter (signed integer on 64-bit). |
...
<a name="tip_browse_oid_values_ELTPARAGRAPHE000115"></a>
...
[SNMPGet (Function)](../WDLang3/1000017069.md)
