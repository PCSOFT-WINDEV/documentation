...
| Feature | Table control populated programmatically | Table control linked to a data file with direct access to the data source | Table control linked to a data file loaded in memory |
| --- | --- | --- | --- |
...
| **Sort and magnifier available for all the columns**<br>![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) As of version 2025 Update 2, Table control fields linked to a data file with direct access feature sorting and a magnifying glass on all columns (if the number of records is less than 100,000). | X | X <br>![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) Only in WINDEV, if less than 100,000 records | X |
...
| Data file positioned during the "Display a row" event |   | X | X |
...
(\*) For tables on data files with direct access, if cascade input has been disabled, the [TableAddLine](../WDLang1/3074006.md) function can be used to add a line to the field, allowing the user to enter data. In this case, [TableAddLine](../WDLang1/3074006.md) can be called once only.
