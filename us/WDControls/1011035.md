...
In the following example, in the report creation wizard, the sorting mode is defined: the data selected in the "Customer" data file will be sorted by country name, then for each region by customer name.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=P4_Appli%20compl%E8te%20-%20Imprimer%20liste%20clients%20-%20HC%20N%B0005.jpg)
...
- A sort criterion can be modified from the editor for the reports bound to a data file or on a memory area.
...
### Modifying a sort criterion in a data-bound report or on a memory area
<a name="modifying_sort_criterion_databound_report_memory_area_ELTPARAGRAPHE000123"></a>

To modify a sort criterion in a data-bound report or on a memory area:
...
A sort procedure can be implemented in a data-bound report or in a report based on a memory area. This procedure can be used to modify the sort criteria dynamically.
...
4. Enter the name and value of the sorting procedure parameters (for example: "ValTri("Ville")").
