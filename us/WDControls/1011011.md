...
**Characteristics of a report on a text file** Each line in the text file must correspond to a record. Each line contains various information, called items. The items are separated by a separator.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=EtatFichierTexte.gif)
**To create a report on a text file**, you can:

- use ***the wizard for creating a report based on a text file***. The report editor automatically creates the report and reads the records. No programming required.
...
9. Select the text file containing the records.
	...
	- When deploying an application containing a report based on a text file with automatic reading of records, some rules must be complied with. For more details, see "Characteristics of the text file when deploying the application".
...
- If **the text file is supplied with the application**, we recommend that you include this text file in the application library.
	**Warning** The contents of this file cannot be modified on the end user's workstation.

- If **the content of the text file might be modified on the end-user computer**, when printing the report, the text file will be searched for on the end-user computer according to the following order:
	...
	4. in the application's installation directory.
...
8. Specify the format of the sheet on which the report will be printed. By default, the report is printed in A4 format. 
	Note: By default, the report controls are displayed in a single column. To create a multi-column report, simply specify the desired number of columns in the "Format" tab of the report description window. 

9. Select the skin template of the report if necessary.

10. Type the name and title of the report (name of the ".WDE" file corresponding to the report). This name will be used to identify the report in your programs. 
	![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) You can also specify whether the report can be modified by the end user in "Reports & Queries". 
...
13. If necessary, specify the mode for reducing the size of the report: 
...
14. The report is displayed in the report editor.
...
The programming required to play back recordings is illustrated by the following example: 
...
1. Open the text file in the "Open" event of the report ([fOpen](../WDLang1/3036036.md)).
	**Note** Heading labels must not be specified in this text file.
...
2. In the "Reading the report data" event:
	...
	- Check whether the end of file was reached. The following values are returned according to the case:
		...
		- <u><u><u><u>True</u></u></u></u> end of file not reached (step 3 executed).

		- <u><u><u><u>False</u></u></u></u> end of file reached (step 4 executed).
...
4. The "Close" event of the report is used to close the text file when the end of this file has been reached ([fClose](../WDLang1/3036027.md)).
...
7. Delete all the lines of code specific to the reading of records.
...
<a name="NOTE5_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png)![Reports and Queries](https://doc.pcsoft.fr/ext/images/us/ER.png)![Windows](https://doc.pcsoft.fr/ext/images/us/WINDOWS.png) 
...
### WEBDEV example
<a name="webdev_example_ELTPARAGRAPHE000468"></a>
