...
This opening mode allows you to manage the modeless windows.
...
## Opening a window
<a name="opening_window_ELTTEXTE000405"></a>
...
A child window is opened from an MDI parent window. These child windows can have the following types:
...
1. Executing the "Global declarations" event of the window.

2. Initialization events of the controls (in any order).

3. Displaying the window (this window is always a child window of the MDI parent window). 
	Note: The MDI child windows are always opened in the parent window (they cannot be displayed outside the parent window). [MDIWindowMenu](../WDLang1/3052004.md) is used to organize the display of MDI child windows in the MDI parent window (cascading display or tiled display).
...
2. Running the initialization events of the controls (in any order),
...
4. Display the child window (the parent window remains the current window).
	Note: Non-MDI daughter windows can be displayed outside the parent window, or at any screen coordinate.
...
[MDIOpen](../WDLang1/3052005.md) opens a modeless "MDI Child" window from an "MDI parent" or "MDI child" window. The different child windows are sibling windows.
...
4. Displaying the sibling window.
	**Remark**: MDI sister windows are always opened in the parent window (they cannot be displayed outside the parent window).. [MDIWindowMenu](../WDLang1/3052004.md) is used to organize the display of MDI child windows in the MDI parent window (cascading display or tiled display).
...
**Important**: A window can only be opened with function [OpenSister](../WDLang1/3038045.md) if the window requesting opening has been opened with function [OpenChild](../WDLang1/3038021.md) or function [OpenSister](../WDLang1/3038045.md).
<a name="NOTE2_4"></a>
...
**Note** The alias can be changed using the [WinChangeAlias](../WDLang1/3038028.md) function.
...
OpenSibling("CUSTOMER1=CUSTFORM")
...
## Overlaying windows
<a name="overlaying_windows_ELTTEXTE000447"></a>
...
## Minimizing a window
<a name="minimizing_window_ELTTEXTE000471"></a>
...
## Current window
<a name="current_window_ELTTEXTE000513"></a>
...
- use [SetFocusAndReturnToUserInput](../WDLang1/1410088107.md) to force input in the window passed as parameter (the window must include at least one editable control).
 ...
 returns the name of the MDI child window that is displayed in the foreground.
...
<a name="closing_child_window_sibling_window_ELTTEXTE000537"></a>
...
**If the parent window is closed**, its child windows are closed. If a child window refuses to be closed, closing is interrupted: sister windows following the child are not closed, nor is the mother window; child windows that have been closed remain closed.
