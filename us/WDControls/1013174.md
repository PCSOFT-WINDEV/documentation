...
- or automatically: No specific code is required to initialize the List Box control. The options to loop through the data file linked to the control can be defined in the "Content" tab of the control description.

- or by programming: enter the path code of the data file linked to the ListView control in the initialization code of the field. This method is not recommended.
...
**Note** When creating an Image control ListView control, the wizard suggests the various types of image list. For a ListView control linked to a data file or to a query, the wizard asks for the necessary information (data file used, items used, etc.).
...
**Example** Initialization of a ListView control with photographs from the VOYAGE data file.
