...
## Overview
<a name="overview_ELTTEXTE000785"></a>
...
The Spreadsheet control provides essential calculation functions and operators as standard: sum, subtraction, multiplication, division, percentage, average, min-max, condition (SI), calculation and comparison operators, etc.

WLanguage functions such as math, string, date, financial functions, etc. can also be used in formulas.

In the formulas, you have the ability to use the WLanguage procedures that have been created for this purpose in the application ([SpreadsheetAllowProcedure](../WDLang1/1000021473.md)). For example, the application can propose the "Postage" procedure, which calculates the amount based on advanced parameters: weight, volume, delivery time, country of delivery, etc, ... 
...
<a name="formulas_available_standard_ELTTEXTE000809"></a>
...
| DATEDIF | Returns the difference in days between two dates. <br><br> |
...
| HEX2BIN | Converts a hexadecimal number into binary number. |
| HEX2DEC | Converts a hexadecimal number into decimal number. |
| HEX2OCT | Converts a hexadecimal number into octal number. |
...
| CELL | Returns information about the layout, the location and the content of a cell. <br>Note: This function is not available in Excel Web App. |
...
| BDNBVAL | Counts the non-empty cells in a cell range. <br><br> |
| GROWTH | Returns a value relative to an exponential trend. <br><br> |
...
[SpreadsheetAddFormula (Function)](../WDChamp/1000021373.md)
