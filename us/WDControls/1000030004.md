...
- [Use the AAF context menu](#NOTE3_1). 
...
## Manipulating Gantt Chart controls programmatically
<a name="manipulating_gantt_chart_controls_programmatically_ELTTEXTE000416"></a>
...
- "Start to End" link: The target task can only end if the source task is started (uncommon case).

- End-to-start link (default case): The destination task cannot start before the source task has finished. 

- "End to End" link: The target task can only end if the source task has ended.
...
**Example** To save a task added by the user via the context menu in a "Task" data file, simply enter the following in the "Task entry" event: 
...
<a name="management_public_holidays_ELTPARAGRAPHE000280"></a>
