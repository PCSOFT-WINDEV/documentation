...
- Calculated: If the column is not linked to a variable, it is a **calculated column**. For calculated columns, the content of the column is recalculated each time a row is displayed.
...
**Caution: the synchronization between the Table control and its source is not automatic. Depending on the action performed, changes must be propagated to the Table control or to the source.**
...
- Case 2. Changes in the Table control
	...
	**Warning** Depending on the operations carried out previously, it is possible that the transfer is made at the wrong place if the source and Table control are not in phase: it is necessary to maintain synchronization at all times.
...
- Case 1. Deleting an element using its index in the data source of the Table control
	...
	**Limitation** For simple source types (e.g., an array of integers), you need to use the [TableDisplay](../WDLang1/3074003.md) function with the constant *taInit* to update "links" between Table control lines and source elements. The selections and sorts performed by the user will be lost.  
...
<a name="handling_table_control_based_variable_wlanguage_code_stored_value_ELTTEXTE000268"></a>
