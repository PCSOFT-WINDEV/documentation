...
**Note**: If the Planning report is linked to a data source (query, data file), it is necessary to indicate in the rest of the wizard the fields to be taken into account for displaying the schedule and appointments. The following information must be specified: 
...
Note: Check that the daily appointment display area is correctly sized in the report editor. It can be resized if necessary.
	...
	- the custom report to display the appointments. 
		...
		- If "Preset report" is selected, a preset report is automatically added to the project. This report is named "RPT_WinDevViewAPT" by default and it is automatically used to display the appointments.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Etat_WinDevvueRDV.gif)
