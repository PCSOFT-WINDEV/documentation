...
- Table fields linked to a data file are mainly used to view data. To enter data, use the Edit controls.
...
- Edit control is not available in Table fields linked to a data file. The functions [TableAdd](../WDLang1/3074017.md), [TableAddLine](../WDLang1/3074006.md), [TableInsert](../WDLang1/3074025.md), [TableInsertLine](../WDLang1/3074026.md), [TableModify](../WDLang1/3074019.md) and [TableModifyLine](../WDLang1/3074028.md) are not available on this field type. 
...
Table controls are available for viewing only: editing is not available in this version. 
...
Two types of Table fields linked to a data file are available:

- Table control fields linked to **a direct-access data file**. 
	...
	![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) Table controls with direct access to the data source offer sorting and magnifying on all columns (if the number of records is less than 100,000 and the field doesn't use a bounce scrollbar).



- Table controls based on **a data file loaded into memory**. Only a specific number of data file records (defined in the editor) are loaded into memory. These Table controls can be used to sort and search on any column.
...
In Java, cascading is not available for Table control fields based on a direct-access data file. Cascading is only available for Table controls based on a data file loaded in memory.
...
A Table control linked to a data file can be automatically refreshed as soon as the associated HFSQL Client/Server data file is modified. This feature can be defined: 

- in the editor: 

	1. Open the Table control description window. 

	2. On the "Content" tab, select the display synchronization mode: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Table_Fichier_Synchro%20-%20HC%20N%B0001.gif)
...
In a Table control linked to a data file, it is possible to scroll down a line to view the records of other data files linked to the selected line. 
...
2. On the "Content" tab, check "Show row details with [ + ]". <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Visu_enreg_li%E9_auto%20-%20HC%20N%B0001.gif&type=thumb)
...
At runtime, a "+" sign allows the user to see the data related to the data file currently displayed in the Table control. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Visu_enreg_li%E9_manuel%20-%20HC%20N%B0003.gif)
