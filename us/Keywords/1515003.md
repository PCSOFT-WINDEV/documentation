...
## Default assignment: '='
<a name="default_assignment_ELTTEXTE000314"></a>
The default assignment (= operator) is used between all simple types (integers, reals, strings, etc.) to copy the value of one variable into another. 

During this copy, an implicit conversion can take place. 
...
It is possible to force conversion by using the explicit conversion operator (cast). For more details, see [Explicit conversion operator for simple types](../Motscles/1515004.md). 


### Assigning complex types
<a name="assigning_complex_types_ELTPARAGRAPHE000068"></a>How assignment works depends on the type of variable used:   
...
### Multiple assignment
<a name="multiple_assignment_ELTPARAGRAPHE000078"></a>

The operator **'=**operator can be used to assign several variables in a single line of code.
...
// Then assigns "ABC" in t to the index i
```
...
## Copy assignment: &lt;=
<a name="copy_assignment_ELTTEXTE000427"></a>
...
<a name="assignment_copy_the_arrays_ELTPARAGRAPHE000127"></a>

The &lt;= operator can be used on the arrays.
These arrays can have diferent types.
The assignment uses the implicit conversions of WLanguage (like the = operator would do).
...
// (Reminder: all numbers other than zero are "True" in Boolean logic)
...
### Homonymic copy
<a name="homonymic_copy_ELTPARAGRAPHE000142"></a>
...
- transfer the contents of structure members to fields in an HFSQL data file,
...
- For the structures and classes, each member can be followed by a "Mapping" attribute. This attribute is used to specify the name of the member that must be "mapped" to the structure member or to the class member. 
	![PHP](https://doc.pcsoft.fr/ext/images/us/PHP.png) This extension attribute is not available.
	Example:  
...
- Special case: homonymous copying between structures or classes: 
...
- Limitation: Homonymous copying is not available: 
...
<a name="assignment_reference_ELTTEXTE000463"></a>
...
## Remarks
<a name="remarks_ELTTEXTE000487"></a>
The operators used to assign values by reference (=, &lt;= and &lt;-) return "Null" if the assignment or reference is invalid. You have the ability to write the following code:
...
[Clone (Function)](../Motscles/1000024843.md)

[Explicit conversion operator for simple types](../Motscles/1515004.md)
