...
There are two methods to define an analysis:

- **1st method**: Creating the analysis (or Logical Data Model (LDM)) directly. 
...
<a name="creating_analysis_ldm_ELTTEXTE000260"></a>
In WEBDEV, the terms "LDM" and "Analysis" are used interchangeably to define the structure of the database associated with a project.
<a name="NOTE2_2"></a>


### Creating an analysis (or LDM)
<a name="creating_analysis_ldm_ELTPARAGRAPHE000024"></a>

To create an analysis:

1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=P1_ICO_Cr%E9er_cpt.gif) in the quick access buttons. In the window that appears, click "Data", then "Analysis". The analysis creation wizard opens.
...
<a name="adding_data_file_analysis_ELTPARAGRAPHE000031"></a>

The analysis allows you to define the structure of data files used by the project. In the analysis, a data file can be:
...
**Note**: To modify a data file or the items in a data file:
...
This is an Oracle database on the server. SImply drag the name of the database and drop it onto the data model editor to transfer its description. <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=P5_Base%20Oracle%20WB%20-%20HC%20N%B0001.gif&type=thumb)
...
**Note**: You can also create an item using the meta-types provided by WINDEV. To do so, click the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=MLD_Metatype_ico.gif) button. The list of available metatypes appears.
<a name="NOTE2_5"></a>
...
<a name="characteristics_the_links_defined_analysis_ELTTEXTE000492"></a>


### Owner data file and member data file
<a name="owner_data_file_and_member_data_file_ELTPARAGRAPHE000054"></a>
...
To put it simply, these two questions can be broken down into four questions. For example, to describe a link between the Club and Member data files: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=CardinaliteFenDescriptionWB.gif)
...
This example presents two different cardinalities: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=CardinaliteMLDwb.gif)
...
This example presents two different cardinalities: <br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=CardinaliteAvMLDwb.gif)
...
For a parallel link, ***each record*** in a data file (Product) is linked to ***a record*** in another data file (Info), and ***vice versa***.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=LiaisonParallele.gif)
...
**Note**: This type of link is rare, as the two data files can be combined into a single file.
...
- ***Each record*** from the other data file (Details) ***has at least one matching record*** in the first data file (Product).<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Liaisoncomplement.gif)
...
**Note**: This is a common type of link. WINDEV allows you to automatically create windows that manage data files with a 0,N - 1,1 relationship.
...
<a name="possible_operations_analysis_ELTTEXTE000751"></a>
WEBDEV allows you to perform the following operations on an analysis (LDM):

- Duplicate or copy an analysis. This makes it possible to have two identical analyses with different names.
...
WEBDEV allows you to perform the following operations on a data file item found in the data model editor:
...
<a name="generating_the_analysis_ELTTEXTE000775"></a>
...
<a name="managing_the_analysis_versions_ELTTEXTE000827"></a>
...
**Note**: If an analysis is damaged, you can select it and restore one of its previous versions, for example.
...
### Restoring an analysis
<a name="restoring_analysis_ELTPARAGRAPHE000131"></a>
...
**Caution**: Data files from an analysis that is more recent than the restored version can no longer be opened. In this case, you must also restore the data files of the restored analysis version or delete the existing data files and create them again.
<a name="NOTE6_3"></a>
...
You can reset the analysis generation number to "1". To do so, go to the "Generation" tab, "Analysis" group, click "Analysis" and select "Reset analysis version to 1".
