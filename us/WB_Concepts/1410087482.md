...
- ***to test the dynamic part of the site (Session or AWP)***: Follow the steps to test a dynamic site.
...
2. On the "Connections" tab, select the site and click the "Test" button.
...
### Load test/Regression test
<a name="load_testregression_test_ELTPARAGRAPHE000231"></a>

WDTestSite runs load tests. It opens multiple simultaneous connections to a dynamic WEBDEV site (Session or AWP).
...
**Important**: If this information is displayed on screen, it should only be shown during tests.
...
- ***message boxes***: WLanguage [Info](../WDLang1/3021011.md) function.
	Important: Messages boxes are displayed modally.
...
**Note**: In the Details tab, "Execution completed" indicates the total time of the test run with the performance profiler.
<a name="NOTE6_4"></a>
...
WDTestSite runs different tests on a WEBDEV site.
