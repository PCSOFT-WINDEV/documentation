...
WINDEV and WEBDEV offer a Native xBase/FoxPro Connector (or Native xBase/FoxPro Access) module. This Native Connector allows you to handle xBase/FoxPro data files from a WLanguage program without using any external driver.

**System requirements**: The following files are supplied as standard with WINDEV and WEBDEV: wd310db.dll. This file is required by the Native Connector on xBase/FoxPro files in order to operate with an HFSQL Classic analysis.
...
<a name="importing_the_structure_data_files_ELTTEXTE000324"></a>
...
2. Select database type: xBase. A connection to the xBase/FoxPro database is automatically established and associated with the data files imported into the analysis. This connection will have to be used to handle the xBase/FoxPro data file programmatically.

3. Specify whether the data must be kept in the current format (option required to use the Native xBase/FoxPro Connector).

4. Select the data file access mode (xBase Native Connector for WINDEV) and specify the data file directory.
...
5. Select the tables that will be imported into the WINDEV analysis. These tables can be handled through programming with the HFSQL functions of WINDEV. These tables will be displayed in blue in the data model editor.
	Note: the FoxPro databases (.dbc extensions) are not supported. Each data file (.dbf extension) must be individually imported into the analysis.  
...
<a name="taking_into_account_the_changes_the_xbasefoxpro_data_files_ELTPARAGRAPHE000070"></a>
...
<a name="new_features_for_managing_the_xbasefoxpro_data_files_ELTPARAGRAPHE000096"></a>

The management of xBase/FoxPro data files is similar to the management of HFSQL data files except for the cases explained in this help page.

**New features for xBase data file management since WINDEV 5.5**:
...
<a name="dynamic_description_xbase_data_file_ELTPARAGRAPHE000136"></a>
...
HDBCreation()
```
**Note** FoxPro data files cannot be described dynamically. Only the dBase 3 format is supported.
<a name="NOTE4_3"></a>


### Opening data files described dynamically
<a name="opening_data_files_described_dynamically_ELTPARAGRAPHE000175"></a>

An xBase data file is not automatically opened: you must open this data file with function [HDBOpen](../WDLang4/3044163.md) before using it for the first time, unless it has been created with function [HDBCreation](../WDLang4/3044171.md) (which creates and opens the data file).
<a name="NOTE4_4"></a>
...
<a name="links_between_the_xbase_foxpro_data_files_ELTPARAGRAPHE000202"></a>

The links are not automatically managed by WINDEV and WEBDEV. They must be managed programmatically. Therefore, to access a record in the linked data file, you must find the corresponding record according to the link key ([HReadSeek](../WDLang4/3044050.md)).
