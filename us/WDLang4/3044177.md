...
**Remark**: The [HErrorInfo](../WDLang4/3044071.md) function returns detailed information on the last error encountered when managing HFSQL data files.
...
The data files must be synchronized in WINDEV's data model editor: in the "Analysis" pane, in the "Analysis" group, scroll down to "Synchronization" and select Synchronization of Structure and Deployed Data (SDD, Modif Auto)".

**70018: UNKNOWN FILE**

This error occurs when the name of the data file passed as parameter to a file management function is not recognized by the HFSQL engine (analysis file, file described dynamically, query, alias, view, ...).
...
The data file and index file are no longer in phase: re-index with [HIndex](../WDLang4/3044133.md).

Probable cause: the automation modified the data files but was unable to re-index.
...
Solution: Re-index the data file with [WDOptimizer](../WDOptimiseur/3519002.md) (or with the [HIndex](../WDLang4/3044133.md) function).

**70055: INVALID OR CORRUPTED ANALYSIS**

The version of the analysis (.WDD file) is not recognized by the HFSQL engine.

Solution: Generate the analysis again in the data model editor: in the "Analysis" pane, in the "Analysis" group, scroll down to "Generation" and click on "Generate".
...
**71038: INVALID ANALYSIS PASSWORD**
