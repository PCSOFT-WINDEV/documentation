...
<a name="required_configuration_ELTTEXTE000232"></a>
...
**Reminder** A cluster must be made up of at least three machines. In order for the coordinator to be installed on the same computer as a node, the cluster must include at least three nodes. If the cluster contains two nodes only, the coordinator must necessarily be found on a third computer.

**Attention**: All servers and workstations must be Windows workstations. The clusters are not available in Linux. 
...
- Operating system: all Windows server versions supported by the publisher.  32 and 64-bit editions are supported. 

- Memory: 512 MB

- Disk: 20 MB for cluster elements + size of databases

- Network: 100 Mbps between nodes. The IP addresses of the nodes must be fixed.
...
- Operating system: Windows Server in a version currently supported by the publisher.
...
- The port 4997 used by the cluster coordinator must be open in the Windows firewall.
...
## Creating a cluster
<a name="creating_cluster_ELTTEXTE000270"></a>
To create a cluster:

1. Start the HFSQL Cluster setup wizard.

2. Accept the terms in the license agreement to continue the setup.

3. Select "Create a new cluster".

4. A short text summarizes the principle of a cluster and the steps for its implementation. Go to the next step.  
...
6. Enter the setup parameters of the coordinator. These parameters are:
	...
	- The password for managing the cluster. This password will be used in the [HFSQL Control Center](../CCHF/3540001.md) or in programming for cluster maintenance operations (adding nodes, stopping and restarting, etc.).

	- The cluster's DNS name: this is the name from which the cluster will be accessible on the network. For more details, see ["Configuring the DNS for the access to the cluster"](#DNS).
...
8. Enter the port number that will be used for the HFSQL connections. This port number will be shared by all the cluster nodes. Therefore, it must be available on each computer that hosts a cluster node. Go to the next step. 

9. Indicate whether you authorize PC SOFT to collect information on the use of the HFSQL Cluster. This data collection is optional, and is used to improve the functionality of the HFSQL Cluster.

10. The summary of the selected configuration is displayed. You can return to the previous wizard steps to modify the cluster configuration.

11. Click "Next". A custom installation package is automatically created. You can: 
...
**Note**: Only one coordinator can be installed on a given machine. If two clusters are installed on the same network, their respective coordinators must be found on two different computers.

**Attention**: If you are installing a blank cluster, the following rules must be followed: 

- never manually copy/paste HFSQL data files into the nodes (even after stopping the nodes). 

- always use the cluster to import data: 

	- via the HFSQL Control Center. 

	- via an application using the [HCopyFile](../WDLang4/3044282.md) function.
...
<a name="configuring_the_dns_for_accessing_the_cluster_ELTTEXTE000294"></a>
...
**Note** To ensure correct operation of the DNS system, it is imperative that the IP addresses of the nodes do not vary. If the servers get their IP addresses via a DHCP, it must be configured so that a fixed address is reserved for each one of the cluster nodes.
...
When a client application wants to connect to the cluster, it retrieves the DNS records corresponding to the chosen name for the cluster: this name is the one given as the server address in the [Connection variable](../WDLang4/1514073.md) or in the [HOpenConnection](../WDLang4/3044107.md) or [HDescribeConnection](../WDLang4/3044205.md) functions. The client computer will select the node used for connection among the different IP addresses.
...
<a name="configuring_the_dns_from_the_hfsql_control_center_ELTPARAGRAPHE000134"></a>
...
<a name="installing_new_node_ELTTEXTE000324"></a>
...
<a name="installing_new_hfsql_server_the_local_computer_cluster_node_ELTPARAGRAPHE000159"></a>
...
3. Enter the coordinator access parameters: machine name and administration password.
...
## Uninstallation
<a name="uninstallation_ELTTEXTE000354"></a>


### Uninstalling a node or a cluster
<a name="uninstalling_node_cluster_ELTPARAGRAPHE000175"></a>
...
5. Confirm. The uninstall operation is performed. 




**Remark**: This uninstall can also be performed from the HFSQL Control Center.. In this case, you have the ability to temporarily exclude a node from the cluster without having to uninstall it. 
