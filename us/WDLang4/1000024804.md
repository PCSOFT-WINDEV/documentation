...
WLanguage procedure ("callback") called by [HWatch](../WDLang4/1000022407.md) when a change is made on the specified data file.
<a name="Exemple1"></a>
...
IF BitwiseAND(Action, hsAdd) = hsAdd THEN
	Trace("An element was added to the following data file: "+ FileName)
END
```
...
|   |   |
| --- | --- |
| *hwAdd* | Procedure started once a record is added into the data file. |
| *hwCross* | Procedure started once a record is crossed in the data file. |
| *hwDelete* | Procedure started once a record is deleted from the data file. |
| *hwModify* | Procedure started once a record is modified in the data file. |
