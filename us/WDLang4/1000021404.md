...
- The function **SourceFromPage FUNCTION** function does not write to the data file: only variables in data file fields are assigned (i.e. variables in the record loaded into memory). To modify the records, use [HAdd](../WDLang4/3044147.md), [HModify](../WDLang4/3044042.md), [HWrite](../WDLang4/3044092.md), etc.
...
<a name="binding_between_control_and_wlanguage_variable_between_control_and_data_file_item_ELTPARAGRAPHE000100"></a>
...
- If multiple controls in the window or page are bound to the same data file item, **SourceFromPage** may not update the bound item properly. ***It is not recommended to bind multiple controls of a window or page to the same item***.
