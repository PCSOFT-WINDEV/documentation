...
- [Principle: transaction management on HFSQL data files and special cases.](../WDLang4/3044335.md)
...
3. Perform your operations. All the write operations performed on the data files in transaction are automatically saved in the transaction file. Please note: processing is relatively slower (since each operation is recorded in a specific data file).
...
| If a record in the specified data file is considered to be in a transaction, but does not belong to any current transaction, it is automatically freed. | [HTransactionFree](../WDLang4/3044016.md) |
