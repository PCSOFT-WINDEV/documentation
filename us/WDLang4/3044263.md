...
Name of the table (data file) to opened, and for which the connection must be redefined. If no other parameter is specified, the data file is reinitialized with the default parameters defined in the analysis.
...
|   |   |
| --- | --- |
| *hHCreationIfNonexistent*<br>(Default value) | Checks the connection by opening or creating a data file (if the data file does not exist). This operation is only possible with *hOLectureWrite*. |
| *hHOuvre* | Checks the connection by opening a data file. This value is used by default when accessing to an *hORead* database. |
