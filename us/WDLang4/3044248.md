...
2. The view is automatically executed. View records can be manipulated, browsed, etc. using HFSQL management functions. The data file name specified in the functions will correspond to the view name.
...
3. Available operations specific to views:
	...
	- **Modify the sort order of the records found in the view**. By default, records in a view are sorted in the same order as in the corresponding HFSQL data file. However, this order can be modified by [HSortView](../WDLang4/3044114.md). If the sort is applied to several items of the view, the sort is applied according to the first sort item, then according to the second one, etc.

	- **Apply the operations performed on the view records** (add, modify, delete, etc.) to the base file of the view ([HViewToFile](../WDLang4/3044021.md)). Please note: this operation can only be performed if the records have been locked in the data file when the view was created.
