...
The **mongoOperationDeleteOne** type is used to define all the advanced characteristics of a document delete operation that will be added to a bulk operations list. The first document that matches the filter will be deleted. You can define and change the characteristics of this operation using different WLanguage properties. 
...
| --- | --- | --- |
| Filter | Type of filter used | Description of the MongoDB filter(s) that will be applied to select the document to delete. This filter corresponds to:<br><br>- a JSON document, in string format.<br><br>- a BSON document, in buffer format.<br><br>- a variant.<br><br><br> |
