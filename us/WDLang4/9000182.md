...
## Prerequisites
<a name="prerequisites_ELTTEXTE000211"></a>
...
Note: To make high-performance hot backups, you must have the corresponding local right to the volume ("Perform volume maintenance tasks").. 
...
### Manual installation
<a name="manual_installation_ELTPARAGRAPHE000045"></a>
...
2. Modify the account running the service of HFSQL server to use the user account of domain created in the prerequisite. 
	Please note: This account must have access rights to the database directory and the HFConf.INI file.. 
...
2. Check whether the server gets information from the Active Directory controller by typing the command: 
	...
	```
 where &lt;account name&gt; is the name of user account of domain created in the prerequisite. 
...
This connection allows you to connect to the server using the current user's identity. The user does not have to type his login information again (Single Sign-On).

Note: Under Windows and Linux with a compatible Kerberos layer, you can force the use of another domain account using the following syntax: 
