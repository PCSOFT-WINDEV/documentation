...
**Data files in the analysis are in HFSQL Classic format.** format: no modifications are required. The master files can be configured:
...
**Data files in the analysis use a connection** (HFSQL Client/Server, Native Connectors, OLE DB files, ...): it is necessary to ***define the connection to the master server in the analysis, and associate the files with this connection***.
...
**Caution: Regardless of the data file format, to use universal automatic assisted replication**DO NOT modify file access information programmatically. YOU MUST NOT USE:
