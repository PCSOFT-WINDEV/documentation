...
```wl
// Recovers data files described in the current analysis and by programming
FileList is string
...
Case of projects with multiple analyses: If several analyses are in progress, all the files from all the analyses in progress are listed (without duplicates). 

The descriptions of each analysis are separated by a carriage return. The result is more or less detailed depending on the constant used:
...
| *hLstNormal*<br>(default value) | List not sorted, not detailed, without the data files described programmatically. |
...
List of data files belonging to the group. The descriptions of each data file are separated by a carriage return.
...
List of data files accessible by the connection. The descriptions of each data file are separated by a carriage return.
