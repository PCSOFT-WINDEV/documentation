...
| Heterogeneous databases (HFSQL, Oracle, etc.) |   | X |   |




### Usage example of the data replication
<a name="usage_example_the_data_replication_ELTPARAGRAPHE000071"></a>

In a company, the "Customer" data file must be accessible to all sales staff, both from their cell phones and from the company's head office. Each sales rep updates the data file on his or her laptop, and regularly updates the information contained in the company's main database.

***In an application that does not use the replication of data***, the database is accessible by network in most cases. The salesman works via a modem connection to the network, with all the constraints regarding access time. Furthermore, all the users must be connected to the same network to access the data.
...
<a name="vocabulary_specific_the_replication_ELTTEXTE000198"></a>


### Databases
<a name="databases_ELTPARAGRAPHE000084"></a>
...
### Type of replication
<a name="type_replication_ELTPARAGRAPHE000101"></a>
...
<a name="files_created_during_replication_ELTPARAGRAPHE000115"></a>
...
- The **master replica** (file extension "rpm"): this file is associated with the master database. This file contains the names and identifiers of the different subscriber databases, as well as the date of the last synchronization.

- The **subscriber replica** (file extension "rpl"): this file is associated with the subscriber database. This file contains the identifier of the subscriber database, the date of the last synchronization, ...

- The **transportable replica** (or transportable medium, file extension "rpa"): this file is needed to synchronize 2 remote databases that are inaccessible to each other. The movable replica contains the information required to update the remote database. This file can be transmitted to the remote database via FTP, email or physical media.
