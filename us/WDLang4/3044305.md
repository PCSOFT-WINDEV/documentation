...
**If you are modifying an existing WINDEV application into an HFSQL Client/Server**there are two ways of converting HFSQL Classic data files into HFSQL Client/Server data files: from the data model editor or from the HFSQL Control Center.
For details of how to convert HFSQL Classic data files to Client/Server mode, see [Converting an HFSQL Classic database into HFSQL Client/Server](../WDLang4/9000155.md). 
...
- [HSubstDir](../WDLang4/3044028.md) is ignored. In fact, this function only allows you to substitute the directories of Network HFSQL data files.
...
It is also possible to import file structures from an existing HFSQL server into an analysis: in the "Analysis" pane, in the "Creation" group, pull down "Import" and select "Import/table file descriptions".
