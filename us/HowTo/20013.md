...
By default, a single row can be selected in a Table control. However, the Table control can be configured in order to become multi-selection. In this case, the user will have the ability to select several rows and to retrieve these rows programmatically.
...
This method consists in iterating over rows with [TableSelect](../WDLang1/3074020.md). 

A variable representing the rank of the selected list will be incremented starting at 1. 

- If [TableSelect](../WDLang1/3074020.md) returns -1, it means no rows are selected.
...
Note: To get the number of selected rows, use [TableSelectCount](../WDLang1/3074030.md). This allows you to perform a loop with a [FOR](../Motscles/1510001.md) statement instead of a [WHILE](../Motscles/1510010.md) statement.
...
[TableSelect (Function)](../CommentLeFaire/3074020.md)

[FOR EACH/FOR ALL statement (loop through controls)](../CommentLeFaire/1510017.md)
