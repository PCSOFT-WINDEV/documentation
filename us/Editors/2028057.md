...
Error messages may appear at various stages of the remote installation process.
...
<a name="failure_checking_the_user_password_information_was_returned_the_server_ELTPARAGRAPHE000027"></a>

The possible problems are as follows:

| Possible problem | Solution |
| --- | --- |
| The Web server does not work | Restart the Web server on the server. |
| The FTP server does not work. | Restart the FTP server on the server. |
| FTP server misconfigured: incorrect user name, ... | Check the configuration of the FTP server for the user. <br><br>Warning: If you are using the IIS FTP server, check that:<br><br>- the user is defined at Windows level<br><br>- the alias of FTP transfer directory corresponds to the Windows username.<br><br><br> |
...
| Possible problem | Solution |
| --- | --- |
| The FTP password specified by the site manager is incorrect. | The hosting provider must check the FTP password and communicate this password to you. |
| The FTP server does not work. | Restart the FTP server. |
...
| The Web server address is incorrect. | The hosting provider must tell you the exact address of the Web server where the site must be installed by FTP. |


**Failure checking the user password: the user is unknown.**
**Check the address of the server and the username**
The possible problems are as follows:

| Possible problem | Solution |
| --- | --- |
| WEBDEV Account Manager does not recognize the site manager. | The hosting provider must check that a WEBDEV account was created in the account manager and communicate the corresponding "login" to you. |
...
| Possible problem | Solution |
| --- | --- |
| The password specified during the remote setup does not correspond to the password defined in the account manager. | The hosting provider must check the password associated with your account in WEBDEV Account Manager and communicate this password to you. |
...
| Possible problem | Solution |
| --- | --- |
| The Windows account that runs the protocol manager WD31AWP has no sufficient rights to create a temporary directory. | The hosting provider must grant the necessary rights to the Windows account. |
...
| Possible problem | Solution |
| --- | --- |
| WEBDEV Account Manager does not recognize the site manager | The hosting provider must check that your WEBDEV account was created in the account manager and communicate the corresponding "login" to the site manager |
...
| Possible problem | Solution |
| --- | --- |
| An error occurred during the FTP transfer. The file installed on the server cannot be read. | You must restart the setup. <br>Warning: If this is a first-time installation, you must remove the server description from the "Remote update" list. |
...
| Possible problem | Solution |
| --- | --- |
| Insufficient memory | The hosting provider must check your password associated with the site supervisor in the account manager and communicate this password to you |
...
| Possible problem | Solution |
| --- | --- |
| The base directory of sites does not exist | The hosting provider must check the base directory of data specified in the account manager and check whether this directory exists. |
| The access rights to the base directory of data are insufficient | The hosting provider must grant the necessary rights to the Windows account that runs the protocol manager WD31AWP.EXE. |
...
| Possible problem | Solution |
| --- | --- |
| The Windows account that runs the protocol manager WD31AWP has no sufficient rights to write into the registry. | The hosting provider must grant the necessary rights to the Windows account. Use the Regedt32.exe program to grant rights to write into the registry ("Security .. Permissions" in Regedt32). |
...
| The site is no longer referenced in the local WEBDEV administrator | The hosting provider must reference the WEBDEV website in the local WEBDEV management tool ("Sites" tab, "Add" button). |
...
| Possible problem | Solution |
| --- | --- |
| This site was installed by another site manager. | In WEBDEV Account Manager, the hosting provider must to assign the site to the corresponding WEBDEV account and check that the home directories (or the directory of data, site and FTP transfer) are correct. |
...
| Possible problem | Solution |
| --- | --- |
| The update failed to be encrypted. | The hosting provider must free memory space and/or disk space on the computer that performs the update (computer of site manager for example). |
...
| Possible problem | Solution |
| --- | --- |
| Unable to copy temporary file that describes the site status. The user does not have sufficient rights in the FTP directory | The hosting provider must grant the rights to write into the FTP directory. |
| Unable to copy temporary file that describes the site status. Insufficient disk space | The hosting provider must free disk space on the server. |
