...
A new element can be created from a previously saved version at any time.

This means you no longer need to save the same window under different names depending on the version of the window. The window editor automatically manages the different versions and integrates them into the window registration file. A window corresponds to a ".WDW" file. This file contains the description of the window as well as all the versions saved for this window. The size of the ".WDW" file is proportional to the number of versions saved.

The same principle applies to the queries, pages, reports and project (WDP, WWP or WPP file depending on the product used).
...
**Remark**: The different versions of an element are displayed in descending order (from the most recent version) according to the date and time it was saved.
<a name="NOTE2_2"></a>
...
3. Click "Delete". The selected version is automatically deleted from the window for version management. This deletion will be effective during the next backup of the element.
