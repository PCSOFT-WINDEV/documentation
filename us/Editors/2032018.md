...
When several data files are used in a query, the query editor automatically searches for the links between these files defined in the analysis. These links are called joins in the query editor.
...
When calculating a join between two data files ("Parent" data file and "Son" data file), by default only records with a match in both data files are retained. However, the notion of "External join" allows you to define a rule to include: 
...
1. Click ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Jointure_Requ%EAte%20-%20HC%20N%B0003%201.gif). The condition creation window appears.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Jointure_Requ%EAte%20-%20HC%20N%B0004.gif)
...
4. Select the type of element to compare for the operation to perform (3): 
	...
	- parameter: this option is selected when parameters are passed during query execution.
