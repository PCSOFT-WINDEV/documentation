...
- ![WINDEV](https://doc.pcsoft.fr/ext/images/us/WD.png) **When the test is interrupted by pressing Ctrl + Pause**. The action performed once the Ctrl + Pause keys have been pressed (click on a button, ...) will start the debugger.
...
- **During the test, by starting the debugger from the code editor**. This method is recommended to start the debugger when a process seems to take an unusual long time (endless loop for example). In this case, perform the following operations:
	...
	- On the "Debugger" tab, click "Pause" ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=main_debog.gif). 




- **Programmatically**, by adding the **[Stop](../Motscles/1511014.md)** keyword where the debugger must be started.
...
- indicating the line of code to be executed.
...
**Note** To stop code execution in the debugger, click on the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Debug_IconeContinuerTest.gif) icon. The project test continues without debugger.
