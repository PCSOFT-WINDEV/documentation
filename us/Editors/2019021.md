...
**Warning** It is not possible to simultaneously debug several executables or services on the same machine from several remote workstations (e.g. 2 developers wanting to debug two different applications at the same time, on the same remote workstation).
...
**Note** Code may therefore be out of phase between the current project code in the editor and the executable code: out-of-phase code is overlined in pink when debugging.
...
- On the "Code" tab, "Debugger" option, click "Pause" (![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=main_debog.gif)).
...
- 2nd method: In the editor, stop the debugger (icon ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=EtatIconeArretTest.gif) or Shift + F5).
