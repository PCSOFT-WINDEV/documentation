


## Remove patches
			



<a name="NOTE1"></a>
<a name="NOTE1_1"></a>


## Overview
<a name="overview_ELTTEXTE000086"></a>
To quickly change the behavior of an application, you can use [application patches](../Editeurs/2030031.md). When installing the application, you can erase existing patch libraries. 

<a name="NOTE2"></a>
<a name="NOTE2_1"></a>


## Remove patch libraries
<a name="remove_patch_libraries_ELTTEXTE000110"></a>
The patch libraries are configured in the setup editor WDInst. To start WDInst, go to the "Tools" tab, "Utilities" group and click "WDInst".

To uninstall the patch libraries:

1. Select "Setup settings .. Uninstall patches". 

2. Via the "Add" button, select the patch libraries that must be deleted from the client computer.  

3. Validate. 





## Voir aussi

[How to create a patch?](../Editeurs/2030008.md)

[Application patch](../Editeurs/2030031.md)

