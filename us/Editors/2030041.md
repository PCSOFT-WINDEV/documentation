...
6. If the WEBDEV project is linked to an analysis, you can:
	...
	- Share the analysis with the initial project. The same analysis will be used by the WEBDEV project and the new WINDEV project.

	- Copy the analysis to the new project directory. The analysis will be duplicated. Each project (WINDEV and WEBDEV) will evolve with its own analysis

	- Ignore the analysis. The analysis is not used.
...
9. Validate. The import starts automatically and the new WINDEV project is created.
...
To import WEBDEV elements (pages, etc.):
...
- the following controls are not imported: IFrame, Frameset, Breadcrumb, Pager, etc. Controls that are not available in WINDEV are replaced by the "&lt;Not imported&gt;" label

- thumbnails, clickable images and images are imported as images.

- cells are imported as images or captions.

- links are changed to buttons

- page opening actions are automatically replaced by the corresponding window opening code.

- internal pages are converted to internal windows.

- code not supported in WINDEV is commented out (JavaScript code, PHP code...)
