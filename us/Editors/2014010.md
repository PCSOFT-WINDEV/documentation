...
2. 1.00C is the reference version. This means that the applications that use the component will only run if the component version is greater than or equal to the reference version.
	...
	- ***First case***: Customer applications use the component (.WDK file) in a version lower than 1.00C. An error will occur when loading these applications.<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=AppC_CompoA_MarchePascopier.gif)
