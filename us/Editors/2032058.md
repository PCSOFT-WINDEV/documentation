...
6. Repeat the previous steps for each item into which you want to insert a value . 
	Note: the ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=ass_creation_req_insertion%20-%20HC%20N%B0004.gif) button automatically generates a parameter for each item not filled in.
...
- It is also possible to create a query from one or more data files described in the data model editor: simply drag and drop the data file(s) selected in the "Analysis" pane into the query editor.
