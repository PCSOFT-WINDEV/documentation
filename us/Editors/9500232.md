...
## Prerequisite
<a name="prerequisite_ELTTEXTE000177"></a>
...
- The contents of the MySQL server archive must be copied to the WEBDEV WordPress directory: 
	...
	2. Create a "mysql" folder in the [personal elements](../Editeurs/2020015.md) folder of WEBDEV. This folder will be: 
		...
		- "&lt;User&gt;\\AppData\\Roaming\\PC SOFT\\31.0\\Wordpress" if WEBDEV is installed following the Windows standard.
...
3. Extract the contents of the archive in the "\\WordPress\\mysql\\" directory. The "bin", "lib" (...) folders must be at the root of the "\\WordPress\\mysql\\" folder.
	...
	- if WEBDEV is installed in a unique directory:
						"C:\\&lt;WEBDEV 2026 folder&gt;\\Personal\\WordPress\\mysql\\bin\\mysqld.exe"

	- if WEBDEV is installed following the Windows standard:
						"C:\\Users\\&lt;User&gt;\\AppData\\Roaming\\PC SOFT\\31.0\\WordPress\\mysql\\bin\\mysqld.exe"

- Local TCP ports 3399 and 8099 are used. Firewall security devices must allow WEBDEV to communicate locally through these ports (localhost).
...
Note: Before importing a WordPress theme, please read its license to make sure you can import it.
...
3. The import wizard opens. 
	Note: If the prerequisites are not met, the wizard will not suggest importing the theme. 
