...
1. Import the data files (or tables) from the external database into the analysis. During this import, the data can be:

	- stored in its current format. No conversion is required.
			**Note**: You can detect any changes made to the database. Simply [synchronize the analysis with the external database](../Editeurs/2011024.md).
...
3. Handle the imported data files via the WLanguage functions. For more details, see [Database access](../WDLang4/3044202.md).
...
<a name="how_import_data_file_table_into_analysis_ELTTEXTE000228"></a>


### Prerequisites for converting a data file (or table) from an external database to an HFSQL data file
<a name="prerequisites_for_converting_data_file_table_from_external_database_hfsql_data_file_ELTPARAGRAPHE000052"></a>
...
<a name="importing_data_file_into_analysis_ELTPARAGRAPHE000110"></a>

To import a data file into an analysis: 

1. In the data model editor, open the analysis where the import must be performed.

2. On the "Analysis" tab, in the "Creation" group, expand "Import" and select "Import file/table descriptions...". The data file import wizard opens.
...
6. Confirm the import. The current analysis is automatically generated.
...
- To import an HFSQL classic data file into an analysis, drag the file from the Windows File Explorer and drop it onto the data model editor. The import wizard opens automatically.
