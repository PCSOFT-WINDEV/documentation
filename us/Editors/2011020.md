...
2. Define the parameters for generating the LDM from the CDM:
	...
	- In the "Generation" tab: 
		...
		- if the analysis logic model needs to be reset.




- In the "Data files" tab, the options for generating the data files created: 

	- the ***type of access to the data files***. Depending on the type chosen, data files will be handled by the WINDEV engine, by an OLEDB driver or by one of WINDEV, WEBDEV and WINDEV Mobile's native Connectors (SQL Server, Oracle, etc.).
...
1. **The existence of the entity in the LDM**:
	...
	- if the entity is new, the corresponding data file is created.
...
3. **The existence of relationships and associations**. When the association requires the creation of one or more relationship files, the data files and LDM fields are updated as described above. When a relationship has been modified, the link of the LDM is recreated or updated:
...
- The deletion of entity properties can be taken into account by checking the "Reinitialize LDM of analysis" option in the LDM MCD generation options. In this case, the files of the LDM only contain the items of the corresponding entity in the CDM after the conversion.
