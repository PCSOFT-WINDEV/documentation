...
- **If an application must create or modify HFSQL data files**, they must be located in the application directory (returned by [SysDir](../WDLang1/3073026.md) and the *srCommonAppData* constant).
...
- If the application was started in single-user mode, the directories used were automatically virtualized by Windows Vista/7 (if the user was not allowed access to these directories). The application data files are located in the "C:\\Users\\&lt;LOGIN&gt;AppData\\Local\\VirtualStore\\Windows\\" directory
