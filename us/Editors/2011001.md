...
To manage a parallel link, the identifier of the "Product" data file is copied to the "Information" data file. This identifier is also a unique key in the "Info" data file.
...
- This type of link is rare because the two data files can be combined into one.
...
An optional data binding is performed by copying the identifier of each of the data files into the other data file.
...
To manage a complement link, the key of the "Product" data file is copied to the "Detail" data file. To ensure the maximum cardinality of 1, it remains a unique key. Since this key is unique, it is not possible to insert more than one record in "Details" for a record in "Product".
...
To manage a shared link, the key of the "Supplier" data file is copied to the "Product" data file. It becomes a multiple key to optimize the speed of the integrity check.
...
**Note** This type of connection is common. WINDEV allows you to automatically create the windows used to manage the data files linked by a 0,N - 1,1 link.
<a name="NOTE5_4"></a>
