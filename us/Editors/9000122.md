...
### A function is not available under Linux: how can I adapt my existing code?
<a name="function_not_available_under_linux_how_can_adapt_existing_code_ELTPARAGRAPHE000020"></a>
...
Note: There are many execution targets. You can combine them in the same statement. 
...
Some fields are not yet available in the Linux framework, such as TreeView control, Hierarchical table, Bar code or Shape control. 
...
It is a "minor" control (the Shape control for example, used to identify areas in the UI). Depending on the case, you have the ability to: 
...
**The field cannot be deleted or moved**
...
**Caution**: This solution should be used with caution and for windows with low code and interface requirements.. Indeed, once the window has been doubled, all modifications, corrections, etc. must be made twice, with the not inconsiderable risk of oversights.. 
...
### I must modify the UI of a window. How to be sure to use controls that are available for all my configurations? 
<a name="must_modify_the_window_how_sure_use_controls_that_are_available_for_all_configurations_ELTPARAGRAPHE000092"></a>
...
- [fSep](../WDLang1/1000019647.md) returns the separator corresponding to the platform currently run. This function is used to build a path regardless of the current platform. 
	...
	```
**Note** The [CompleteDir](../WDLang1/3024018.md) function is used to add the correct separator to a tree structure. However, the code becomes less readable after 2 inclusions. 
...
- [fSeparator](../WDLang1/1000018861.md) is used to retrieve a path with normalized separators. This function is very useful if the application uses paths stored/built on a different platform. 
	...
	// Under Windows: ".\Temp\MyFile.txt"
	// Under Linux: "./Temp/MyFile.txt"
	```
...
[InLinuxMode (Function)](../Editeurs/1000019577.md)
