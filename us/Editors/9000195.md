...
<DIV class="specObsolete">
	...
	From version <B>26 (93)</B>, WINDEV Mobile supports Android 11. For more details, see [Android 11: Changing application behavior](../Editeurs/9000201.md). 
</DIV><a name="NOTE1"></a>
<a name="NOTE1_1"></a>
...
2. **On Android 10 with API level 29**: 
	...
	- A new permission has been introduced in Android 10 for functions that enable device localization: the ACCESS_BACKGROUND_LOCATION permission.. 
...
**Remark**: When publishing an application on the Play Store, the following error may be displayed: "The private key has not been properly encrypted, or we do not accept this type of key".. It is necessary to recreate the signature file of the application (this file can be recreated in the application generation wizard). 
**Attention**: If identifiers are based on the signature file (e.g. Google Map identifier), it is also necessary to re-generate these identifiers with the new key.. 


### Reminder: the "TargetSdkVersion" target API level
<a name="reminder_the_targetsdkversion_target_api_level_ELTPARAGRAPHE000084"></a>
...
As of WINDEV Mobile 25 Update 3, push notifications, location tracking, beacon detection, etc. are reinstated.. Additionally, the WLanguage security mechanism has been improved to trigger a fatal error if a function that opens a window is called when the application is in the background (see list of relevant functions). 
...
- check all processes that can be launched from the application in the background: these processes must not open a window. To optimize your code, you can: 
...
| [EmailRunApp](../WDLang3/1000019564.md) | Starts the native email application on the current computer: <br><br>- Computer running Windows,<br><br>- Mobile device (Android, iPhone, iPad). <br><br><br> |
...
| [Input](../WDLang1/3021016.md) | Displays a message allowing the user to enter data. |
...
[InBackgroundMode (Function)](../Editeurs/1000023457.md)
