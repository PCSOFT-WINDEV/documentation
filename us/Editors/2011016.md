...
For example, in a PRODUCT data file, the product reference must be unique. Each record must correspond to a different product, identified by its reference.
...
Several records of a data file can be associated with several records of another data file and vice versa.

Example: An order can contain several products and each product can be used in several orders.

In this case, a junction file must be created. This file contains the unique key of each data file that must be linked (as well as other information if necessary).
