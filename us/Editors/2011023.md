...
- **easily identify the records**. For example, in a "Product" data file, the reference identifies the product. This reference is unique for each product.
...
For example, in a "PRODUCT" data file, each product is identified by a unique reference. The "REFERENCE" item must be a unique key.
...
For example, in the "Customer" data file, you can define a key with duplicates on the "NAME" item. The content of this key can be the same in different records (e.g. for homonyms). This key enables you to perform a search by customer name.
