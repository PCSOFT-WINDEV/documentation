...
2. Create the setup procedure. Go to the "Setup procedure" tab, "Project" group, expand "Generation" and select "Create setup procedure". 
...
8. Follow the different steps of the wizard and confirm. For more details, see [Setup wizard: Steps for creating a WINDEV application setup program](../Editeurs/2028062.md). 
...
**When running the installation program**the wizard suggests:<br>![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=Install_IP_Assistant_Instcl.gif)
...
**Case 2: Remote deployment of the application on the HTTP server** server (at a hosting provider). In this case, an FTP connection must be used to send the elements to the server.

- Enter the URL of your site at the hosting provider: this information corresponds to the address that users will need to enter to access your application's installation page. The setup of the reference application will be found at this URL.
	In the setup options, if you have specified that: 
...
1. Create the application executable by clicking ![](https://doc.pcsoft.fr/en-US/images/image.awp?langid=3&name=ico_generer.gif) in the quick access buttons.

2. Create the setup procedure. Go to the "Project" tab, "Generation" group, expand "Setup procedure" and select "Create setup procedure".
