...
The second executable is the [robot controller (or Robot controller)](../RobotMonitor/3541102.md). **Its setup is optional**. The Robot controller is an additional security tool. It is used to check whether the monitoring robot operates properly. If it is installed, the Robot controller must be installed on a computer that will never be switched off, different from the setup computer of the monitoring robot.  
...
1. Run the installation package (WX2026PACKROBOT.EXE) on the computer where the monitoring robot is to be installed. Reminder: The installation package (WX2026PACKROBOT.EXE) is available in the "Install\\Robot" subdirectory of the product used.
...
4. Select the installation directory of the monitoring robot. Go to the next step.  
...
2. Specify whether a Robot controller will be deployed. The Robot controller is used to regularly check the activity of the monitoring robot. The setup of a Robot controller is not mandatory but it is an additional security.
	If a Robot controller must be installed, specify the name of the computer where the Robot controller will be installed.
...
3. Select the alert options of the monitoring robot. These options will be used by default for all the tests run by the monitoring robot. You can:
	...
	- use WINDEV, WEBDEV and WINDEV Mobile's internal messaging system: WDBal. This option assumes that the database of the Control Centers is accessible from the computer of the monitoring robot.
...
4. Go to the next plane. This plane depends on the options that were previously checked.
	...
	- **If the WDBal messaging must be used**, specify:
		...
		- the default recipients. The name specified in the table must correspond to the name of a contributor found in the database of Control Centers.
						Note: For each test, new recipients can be added.
...
5. Specify the Windows account (and the associated password) used to create the scheduled task that will start the monitoring robot. Please note: this account must have administrative rights to perform this operation..
...
6. Specify the name of the shared directory ("MyRobot" for example) that will allow the monitors (installed on other computers) and/or the Robot controller to access the test data. This directory will be created on the current computer and automatically shared.
...
- [install the Robot controller (optional)](../RobotMonitor/3541102.md).
