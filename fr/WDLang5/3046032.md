...
- [Imprimer un état sur une zone mémoire](../WDChamp/1011061.md).
	![iPhone/iPad](https://doc.pcsoft.fr/ext/images/fr/IPHONE.png) Cette option n'est pas disponible dans cette version.
...
- en lançant une impression (icône ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Ico_Imprimer.gif)).

- en testant l'état (icône ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Ico_Go_Fenetre_WD_bl.gif)).
...
## Exemple
<a class="notetitle" target="_blank" href="$DOC$=1000003046032&name=iimprimeetat_fonction">Voir des exemples supplémentaires</a>
...
<a name="principe_impression_etat_ELTPARAGRAPHE000189"></a>La fonction **iImprimeEtat** lit la source de données de l'état (fichiers de données, requête, champ Table, etc.). Pour chaque enregistrement, la fonction **iImprimeEtat** imprime le bloc *Corps*. Les autres blocs sont imprimés uniquement lorsque nécessaire :
...
<a name="etat_avec_borne_minimale_maximale_ELTPARAGRAPHE000210"></a>Si l'état a été créé sous l'éditeur d'états en spécifiant des bornes minimales et maximales, les deux premiers paramètres passés à l'état grâce à la fonction **iImprimeEtat** doivent correspondre respectivement à la borne minimale et à la borne maximale.
...
<a name="execution_une_requete_etat_base_sur_cette_requete_ELTPARAGRAPHE000232"></a>Lors de l'impression d'un état basé sur une requête, si la requête a déjà été exécutée, elle n'est pas ré-exécutée. Pour forcer la ré-exécution de la requête (ajout d'enregistrements par exemple), il suffit d'utiliser les fonctions [HExécuteRequête](../WDLang4/3044080.md) ou [iInitRequêteEtat](../WDLang5/3046021.md).
<a name="NOTE0_4"></a>


### Destination de l'impression
<a name="destination_impression_ELTPARAGRAPHE000247"></a>Il est possible de choisir la destination de l'impression de l'état grâce à la fonction [iDestination](../WDLang5/3046074.md).
...
<a name="passage_parametres_ELTPARAGRAPHE000271"></a>Les paramètres indiqués par la fonction **iImprimeEtat** correspondent aux paramètres définis dans le code d'ouverture de l'état à l'aide de la syntaxe suivante :
...
<a name="imprimer_etat_composant_ELTPARAGRAPHE000294"></a>Pour imprimer un état d'un composant, il suffit d'utiliser directement le nom de l'état du composant (le composant étant intégré dans le projet). Par exemple :
...
### Etats imbriqués
<a name="etats_imbriques_ELTPARAGRAPHE000325"></a>Il est possible d'appeler la fonction **iImprimeEtat** au sein même d'un état. Pour plus de détails, consultez l'aide sur les [Etats imbriqués](../WDChamp/1011001.md). 
...
[iInitEtatVerso (Fonction)](../WDLang5/1410090612.md)
