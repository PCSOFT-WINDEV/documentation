...
Identifiant associé à un élément spécifique du champ Arbre. Par défaut, ce paramètre correspond à une chaîne vide (""). Pour associer un identifiant à un élément déjà créé, utilisez la fonction [&lt;Arbre&gt;.Modifie](../WDLang1/1000023763.md).
...
<a name="branche_feuille_ELTPARAGRAPHE000302"></a>Si un des niveaux supérieurs à la feuille ajoutée n'existe pas et n'est pas associé à un identifiant (gestion des doublons), ce niveau est automatiquement créé. Les caractéristiques de ce nouveau niveau (ou branche de l'élément) sont les suivantes :
...
<a name="ordre_priorite_pour_tri_ELTPARAGRAPHE000318"></a>L'ordre de priorité des paramètres du mode d'ajout est le suivant :
...
### Doublons
<a name="doublons_ELTPARAGRAPHE000334"></a>L'acceptation des doublons est à manipuler avec précaution (constante *aAccepteDoublon*). En effet, il est nécessaire de gérer un identifiant spécifique pour identifier le niveau où l'ajout doit être effectué. Pour plus de détails, consultez [Gestion des doublons dans les champs Arbre](../WDChamp/1013325.md).

Si cette gestion n'est pas effectuée, quelques incohérences peuvent survenir.

Par exemple : Si deux "Niveau 2" existent, dans lequel faut-il rajouter "Niveau 3" ? Impossible de trouver quel est le "Niveau 2" à manipuler. Par défaut, l'ajout sera effectué dans le premier élément trouvé.<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=TVDoub.gif)
En cas d'ajout d'un élément déjà existant dans un niveau, la fonction **&lt;Arbre&gt;.Ajoute** retourne <u><u><u><u>Faux</u></u></u></u> si le mode d'ajout n'a pas été défini avec la constante *aAccepteDoublon*. L'élément n'est pas ajouté.
<a name="NOTE0_4"></a>


### Format des images enroulées/déroulées
<a name="format_des_images_enrouleesderoulees_ELTPARAGRAPHE000360"></a>![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png) Les formats d'images supportés sous Windows sont :
...
<a name="type_des_images_enrouleesderoulees_ELTPARAGRAPHE000409"></a>Les images utilisées peuvent être :
...
![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png) Les images utilisées peuvent uniquement être des fichiers image. Ces fichiers doivent être présents dans le répertoire "&lt;NomProjet&gt;_WEB". Ces images ne sont pas transformées lors de l'enregistrement de la page. Il n'est pas nécessaire d'utiliser la fonction [fRepWeb](../WDLang1/3036052.md) pour indiquer le chemin de l'image. Ce chemin est relatif au répertoire "&lt;NomProjet&gt;_WEB".**Remarque** : Il est conseillé d'utiliser des images 16 x 16 pixels (taille des petites icônes).
<a name="NOTE0_5"></a>


### Astuce concernant les images des branches
<a name="astuce_concernant_les_images_des_branches_ELTPARAGRAPHE000435"></a>Attention : Si le nom de l'image ne change pas entre deux appels à la fonction **&lt;Arbre&gt;.Ajoute**, l'image n'est pas rechargée en mémoire (même si son contenu a changé).
...
[&lt;Arbre&gt;.DéplaceElément (Fonction)](../WDLang1/1000023755.md)

[&lt;Arbre&gt;.Insère (Fonction)](../WDLang1/1000023761.md)

[&lt;Arbre&gt;.Modifie (Fonction)](../WDLang1/1000023763.md)
