...
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png) L'utilisation des procédures automatiques permet d'éviter l'utilisation de la fonction **TimerSys**. Pour plus de détails, consultez les [procédures automatiques](../Editeurs/9000072.md).
...
<a name="temps_execution_procedure_ELTPARAGRAPHE000262"></a>Si le temps de traitement de la procédure appelée par le timer est supérieur au temps demandé entre chaque appel de la procédure, les appels du timer ne s'empilent pas : il n'y aura qu'un seul appel en attente.
...
<a name="timer_fenetres_soeurs_ELTPARAGRAPHE000272"></a>Dans le cas de l'ouverture multiple d'une même fenêtre gérant un timer (fenêtres soeurs dans un MDI), il faut utiliser la fonction **TimerSys** sans préciser le numéro de timer. Ainsi, un numéro de timer sera automatiquement attribué à chaque fenêtre.
...
<a name="timer_service_windows_ELTPARAGRAPHE000303"></a>Pour utiliser la fonction **TimerSys** depuis un [Service Windows](../WDLang1/1000017114.md), il est nécessaire d'appeler cette fonction à partir d'une fenêtre ouverte par le code du service. 
