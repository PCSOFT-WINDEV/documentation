
## JSONVersVariant (Fonction)

***En anglais : JSONToVariant***



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Convertit une chaîne au format JSON en une variable de type Variant. 


<a name="Exemple1"></a>
<a name="sample_code"></a>

## Exemple
<a class="notetitle" target="_blank" href="$DOC$=1000021296&name=jsonversvariant_fonction">Voir des exemples supplémentaires</a>

```wl
sJZon est une chaîne ANSI = [
	{
	"glossary": {
	"title": "example glossary",
	"GlossDiv": {
	"title": "S",
	"GlossList": {
	"GlossEntry": {
	"ID": "SGML",
	"SortAs": "SGML",
	"GlossTerm": "Standard Generalized Markup Language",
	"Acronym": "SGML",
	"Abbrev": "ISO 8879:1986",
	"GlossDef": {
	"para": "A meta-markup language, used to create markup languages such as DocBook.",
	"GlossSeeAlso": ["GML", "XML"]
	},
	"GlossSee": "markup"
	}
	}
	}
	}
	}
]

soit jz = JSONVersVariant(sJZon)
```

<a name="XSYNTAXE"></a>

## Syntaxe
<a name="SYNTAXE1"></a>

`<Résultat> = JSONVersVariant(<Données JSON>)`
---

**`<Résultat> : (Variant)`**

Nom de la variable de type Variant contenant le résultat de la conversion. 

**`<Données JSON> : (Chaîne de caractères ANSI encodé en ASCII 7 bits + encodage JSON ou UTF-8)`**

Chaîne de caractères contenant les données au format JSON. 



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques


### Encodage
<a name="encodage_ELTPARAGRAPHE000061"></a>L'encodage utilisé correspond au standard JSON : 

- utilisation d'un encodage en ASCII 7 bits pour les 128 premiers caractères, les caractères non accentués, 

- utilisation d'un encodage JSON des autres caractères : "\\u" suivi du code du caractère en hexadécimal sur 4. Exemple : pour "é" (code ASCII 233, code hexadécimal E9), l'encodage correspond à "\\u00E9". 



<a name="NOTE0_2"></a>


### Contenu du variant
<a name="contenu_variant_ELTPARAGRAPHE000071"></a>Le contenu du variant dépend du JSON manipulé. Il peut correspondre à : 

- la valeur Null, 

- un type simple : entier, chaîne de caractères, booléen, ...

- un tableau,

- un objet complexe (type [MembreVariant](../Motscles/1000021047.md), correspondant à un tableau de membres dont chaque membre est un variant). 



<a name="NOTE0_3"></a>


### Tester NULL sur une valeur
<a name="tester_null_sur_une_valeur_ELTPARAGRAPHE000086"></a>La valeur "NULL" est souvent utilisée en JSON. 

Pour vérifier la valeur "NULL" d'un variant, il est possible : 

- d'utiliser la propriété **Valeur** du variant. Par exemple : 
	
	```wl
	v est un Variant = JSONVersVariant("{""m"":null}")
	SI v.m..Valeur = Null ALORS Trace("NULL")
	```


- de tester directement la valeur NULL. Par exemple : 
	
	```wl
	v est un Variant = JSONVersVariant("{""m"":null}")
	SI v.m = Null ALORS Trace("NULL")
	```









<a name="XComposante"></a>

## Composante :
wd310xml.dll
## Voir aussi

[VariantVersJSON (Fonction)](../WDLang1/1000021051.md)

[Variant (Type de variable)](../WDLang1/1514019.md)

[Membre (Propriété)](../WDLang1/1000021078.md)

[JSON (Type de variable)](../WDLang1/1000023611.md)

[JSONVersChaîne (Fonction)](../WDLang1/1000024530.md)

[ChaîneVersJSON (Fonction)](../WDLang1/1000024531.md)

[Fonctions de manipulation des variables JSON](../WDLang1/1410091296.md)

