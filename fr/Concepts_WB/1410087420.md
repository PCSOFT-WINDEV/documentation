...
- **2ème méthode** : Création du Modèle Conceptuel des Données (MCD), puis création automatique de l'analyse à partir du MCD. Pour plus de détails, consultez l'aide en ligne.
...
<a name="creation_mld_modele_logique_des_donnees_ELTTEXTE000285"></a>
...
<a name="creer_une_analyse_mld_ELTPARAGRAPHE000028"></a>**Pour créer un MLD** :

1. Cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P1_ICO_Cr%E9er_cpt.gif) parmi les boutons d'accès rapide. La fenêtre de création d'un nouvel élément s'affiche : cliquez sur "Données" puis sur "Analyse". L'assistant de création de l'analyse est lancé.
...
5. Validez l'assistant de création de l'analyse. La création du premier fichier de données est automatiquement proposée.
...
<a name="ajouter_fichier_donnees_dans_une_analyse_ELTPARAGRAPHE000037"></a>L'analyse permet de décrire la structure des fichiers de données utilisés par le projet. Un fichier de données présent dans l'analyse peut être :
...
3. Spécifiez :
 ...
 Selon le type choisi, ce fichier de données sera manipulé par le moteur HFSQL, par un pilote OLE DB ou par un des Connecteurs Natifs de WINDEV ou WEBDEV (SQL Server, Oracle, ...). 
...
Par exemple : <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P5_BaseDonn%E9esExterne1.gif&type=thumb)
Ici, une base de données Oracle, sur le serveur. Il suffit de faire glisser le nom de la base pour en transférer la description sous l'éditeur d'analyses. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P5_Base%20Oracle%20WB%20-%20HC%20N%B0001.gif&type=thumb)

<a name="NOTE2_4"></a>


### Créer une rubrique
<a name="creer_une_rubrique_ELTPARAGRAPHE000046"></a>**Pour créer une rubrique** :
...
**Remarque** : il est également possible de créer une rubrique à partir des méta-types proposés par WINDEV. Pour cela, il suffit de cliquer sur le bouton ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=MLD_Metatype_ico.gif). La liste des méta-types disponibles apparaît.
<a name="NOTE2_5"></a>


### Créer une liaison
<a name="creer_une_liaison_ELTPARAGRAPHE000055"></a>Il est possible de créer différents types de liaison entre les fichiers de données. Pour plus de détails sur les liaisons, consultez le paragraphe Caractéristiques des liaisons définies dans une analyse.
...
<a name="caracteristiques_des_liaisons_definies_dans_une_analyse_ELTTEXTE000517"></a>


### Fichier de données propriétaire et fichier de données membre
<a name="fichier_donnees_proprietaire_fichier_donnees_membre_ELTPARAGRAPHE000066"></a>Dans le cas de liaison entre deux fichiers de données, il existe un fichier de données ***propriétaire*** et un fichier de données ***membre*** :
...
La représentation d'une telle liaison est la suivante :<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_1N11.gif)

<a name="NOTE3_2"></a>


### Les cardinalités
<a name="les_cardinalites_ELTPARAGRAPHE000075"></a>Les cardinalités permettent de dénombrer les liens entre les fichiers de données.
...
Pour plus de clarté, ces deux questions peuvent être décomposées en quatre questions. Par exemple, pour décrire une liaison entre le fichier de données Club et le fichier de données Personne : <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=CardinaliteFenDescriptionWB.gif)
...
Cet exemple présente deux cardinalités différentes :<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=cardinaliteMLD.gif)
...
<a name="les_cardinalites_avancees_ELTPARAGRAPHE000084"></a>Il est possible de définir avec plus de précision les cardinalités. On parle alors de **cardinalités avancées**.
...
Cet exemple présente deux cardinalités différentes :<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=cardinaliteAvMLD.gif)
...
<a name="integrite_referentielle_ELTPARAGRAPHE000093"></a>L'intégrité référentielle d'une base de données correspond au respect des contraintes impliquées par les liaisons entre les fichiers de données.
...
<a name="les_differents_types_liaisons_ELTPARAGRAPHE000102"></a>Les liaisons entre fichiers de données peuvent être de plusieurs types :
...
Dans le cas d'une liaison parallèle, ***chaque enregistrement*** d'un fichier de données (Produit) est relié à ***un enregistrement*** dans un autre fichier de données (Information), et ***réciproquement***.<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonParallele.gif)
...
- ***Chaque enregistrement*** de l'autre fichier de données (Groupe) ***n'a aucun enregistrement ou un seul enregistrement*** associé dans le premier fichier de données (Catégorie).<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonFacultative.gif)
...
- ***Chaque enregistrement*** de l'autre fichier de données (Détail) a ***nécessairement un enregistrement*** associé dans le premier fichier de données (Produit).<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Liaisoncomplement.gif)
...
**Liaison partagée de cardinalité 0,N - 0,1**<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_0N01.gif)
...
**Liaison partagée de cardinalité 0,N - 1,1**<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_0N11.gif)
Avec ce type de liaison :
...
- un membre peut ne pas avoir de propriétaire (un produit peut avoir aucun fournisseur).<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_1N01.gif)
...
- chaque membre a un seul propriétaire (chaque produit a un seul fournisseur).<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_1N11.gif)
...
Les liaisons entre les fichiers de données sont les suivantes :<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=LiaisonPartagee_1N1N.gif)
...
<a name="manipulations_possibles_sur_une_analyse_ELTTEXTE000766"></a>
...
<a name="generation_analyse_mld_ELTTEXTE000790"></a>
...
<a name="gestion_des_versions_analyse_ELTTEXTE000842"></a>
...
### Restauration d'une analyse
<a name="restauration_une_analyse_ELTPARAGRAPHE000159"></a>Pour restaurer une analyse à partir du gestionnaire de versions :
...
### Annulation de la dernière génération
<a name="annulation_derniere_generation_ELTPARAGRAPHE000168"></a>L'annulation de la dernière génération de l'analyse permet de restaurer l'analyse dans son état avant la dernière génération. Les modifications effectuées depuis ne sont pas appliquées.
<a name="NOTE6_4"></a>


### Remettre à 1 le numéro de la version
<a name="remettre_1_numero_version_ELTPARAGRAPHE000177"></a>Il est possible de remettre le numéro de génération de l'analyse à "1" : sous le volet "Analyse", dans le groupe "Analyse", cliquez sur "Génération" et sélectionnez l'option "Remettre à 1 la version de l'analyse".
