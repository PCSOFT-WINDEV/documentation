...
### Créer un champ Table hiérarchique lié à un fichier de données
<a name="creer_champ_table_hierarchique_lie_fichier_donnees_ELTPARAGRAPHE000031"></a>Pour créer un champ de type Table hiérarchique lié à un fichier de données :
...
Cet onglet permet de définir les différentes sources du champ Table hiérarchique. En effet, un champ Table hiérarchique peut être lié à différentes sources, permettant d'afficher des enregistrements imbriqués.
	Sélectionnez le mode de remplissage "Fichier de données/Requête".
...
Lorsque un champ Table hiérarchique lié à un fichier de données comporte de nombreux éléments et sous-éléments, son remplissage complet peut être long. 
...
### Créer un champ de type Table hiérarchique lié à un fichier de données
<a name="creer_champ_type_table_hierarchique_lie_fichier_donnees_ELTPARAGRAPHE000084"></a>Pour créer un champ de type Table hiérarchique lié à un fichier de données :

1. Sous le volet "Création", dans le groupe "Données", déroulez "Table et Liste" et sélectionnez "Table hiérarchique".

2. L'assistant de création d'un champ Table hiérarchique se lance. 

3. Dans l'assistant, sélectionnez l'option "Afficher des données d'un fichier de données ou d'une requête existante". Passez à l'étape suivante. 

4. Définissez les différentes sources du champ Table hiérarchique. En effet, un champ Table hiérarchique peut être lié à différentes sources, permettant d'afficher des enregistrements imbriqués. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=WB_Table_Hierachique_ass.gif)
...
<a name="definir_les_sources_champ_table_hierarchique_fenetre_description_champ_ELTPARAGRAPHE000111"></a>Pour définir les sources du champ Table hiérarchique : 
...
4. Définissez les caractéristiques de cette source :
	...
	- Affichée : correspond à la rubrique affichée dans la première colonne du champ Table hiérarchique.
			![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png) Remarque : Pour chaque niveau, il est possible de personnaliser l'image enroulée et déroulée affichée. 
...
Un champ Table hiérarchique peut afficher des enregistrements dépendant de l'enregistrement précédent (cas classique d'une liaison récursive permettant par exemple de gérer les parents / enfants)<br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=TableH_Fichier_Recursive.gif)
...
<a name="definir_les_liaisons_des_differentes_colonnes_champ_table_hierarchique_lie_fichier_donnees_ELTPARAGRAPHE000143"></a>Par défaut, la première colonne du champ Table hiérarchique (colonne représentant la hiérarchie) affiche les éléments indiqués dans les sources (onglet "Contenu", valeur "Affiché" des différentes sources).
