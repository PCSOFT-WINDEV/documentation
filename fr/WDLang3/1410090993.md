...
**A partir de la version 2025, un serveur d'authentification OAuth est présent par défaut dans le Serveur d'Application WEBDEV** (voir [Les différents Serveurs d'application WEBDEV](../Presentation/8010005.md)).
...
1. [Lancer le site d'administration Baas et créer les identifiants OAuth](#NOTE3_1). 

2. [Utiliser un identifiant OAuth 2.0 en WLangage](#NOTE5_1).
...
<a name="prerequis_installation_ELTTEXTE000360"></a>


### Pré-requis : installation d'un serveur HFSQL
<a name="prerequis_installation_serveur_hfsql_ELTPARAGRAPHE000047"></a>Le programme d'installation du serveur HFSQL est disponible dans le package d'installation de WINDEV ou WINDEV Mobile. Si vous ne possédez pas ce package d'installation, l'installation du serveur HFSQL est également disponible sur le site de PC SOFT ([http://www.pcsoft.fr](https://www.pcsoft.fr)).
...
<a name="installation_serveur_application_webdev_ELTPARAGRAPHE000054"></a>Le serveur d'application WEBDEV permet d'installer les modules nécessaires au serveur d'Authentification OAuth. 
...
10. L'étape suivante permet de gérer l'hébergement de sites SaaS. L'activation du SaaS installe automatiquement un site et un Webservice pour administrer vos sites SaaS. Pour plus de détails, consultez [SaaS](../WDLang3/9500213.md). Validez cette étape.
...
13. Spécifiez les caractéristiques du serveur HFSQL utilisé pour le service BaaS. 
	Remarque : Ce serveur HFSQL a été installé précédemment (dans les pré-requis de cette page). <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Install_serveur_store%20-%20HC%20N%B0003.gif)
...
28. A la fin de l'installation, l'assistant affiche un récapitulatif de ce type : 
	...
	Compte HFSQL à utiliser pour que le site et le webservice WDBaas accèdent au serveur HFSQL : 
	- Serveur : xxx
	- Base de données : xxx
	...
	Le mot de passe de l'administrateur du site WDBaaS (Utilisateur : xxx) est : xxxx
	```
...
<a name="creer_identifiant_oauth_via_serveur_authentification_oauth_ELTTEXTE000413"></a>


### Lancement du site du Store Privé
<a name="lancement_site_store_prive_ELTPARAGRAPHE000152"></a>Pour configurer un identifiant OAuth, il est nécessaire d'utiliser le site du Store Privé :
...
<a name="creation_identifiant_oauth_20_ELTPARAGRAPHE000173"></a>Pour créer un identifiant OAuth 2.0 : 
...
<a name="creation_des_apis_pour_webservice_ELTPARAGRAPHE000200"></a>Il est possible d'associer un identifiant OAuth à une API REST. L'accès à une API REST sera autorisé en indiquant un token. Seul un token valide permettra d'accéder à l'API.
...
Ces scopes sont connus via la fonction [OpenIDLitIdentité](../WDLang3/1000023573.md). 

Il est possible d'ajouter des scopes. 

Il est possible de spécifier les scopes à utiliser via la propriété **Scope** des variables de type [OpenIDParamètres](../WDLang3/1000023561.md) et [OAuth2Paramètres](../WDLang3/1000022212.md). La valeur de ces scopes peut être connue via la propriété "Source" de la variable de type [OpenIDIdentité](../WDLang3/1000023567.md) : il suffit de rechercher la valeur dans le JSON correspondant. 
...
<a name="utiliser_identifiant_oauth_20_wlangage_ELTTEXTE000455"></a>
...
<a name="utilisation_token_type_application_ELTPARAGRAPHE000261"></a>Cet exemple permet d'utiliser un token de type "Application", sans saisie de login et / ou mot de passe d'authentification. Le code WLangage est le suivant : 
...
<a name="utilisation_token_type_utilisateur_ELTPARAGRAPHE000269"></a>Cet exemple permet d'utiliser un token de type "Utilisateur" avec une page de connexion et la saisie d'un login et mot de passe d'authentification. 
