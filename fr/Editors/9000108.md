...
## Présentation
<a name="presentation_ELTTEXTE000694"></a>
...
- [Notification d'inactivité de l'application](#NOTE6_1).

- [Fonctionnement du focus et de la sélection](#NOTE7_1).
...
- [Opérateurs d'indirection](#NOTE16_1).
...
<a name="elements_projets_disponibles_ELTTEXTE000718"></a>
<a name="NOTE1_1"></a>
...
### Champs disponibles
<a name="champs_disponibles_ELTPARAGRAPHE000105"></a>Les champs suivants sont disponibles pour le développement Android :
...
- [Champ Libellé avec masque d'affichage](../WDChamp/1013179.md)

- [Champ Modèle de champs](../WDChamp/9000095.md)

- [Champ Fenêtre interne](../WDChamp/1013255.md)

- [Superchamp](../WDChamp/1013091.md)

- [Champ HTML](../WDChamp/1013105.md)

- [Champ Caméra](../WDChamp/1013188.md)
...
## Bases de données
<a name="bases_donnees_ELTTEXTE000748"></a>
...
- Gestion des mémos avec HFSQL : Le mode Unicode ou ANSI d'utilisation des chaînes en exécution défini dans la [configuration du projet](../Editeurs/9000030.md) détermine l'alphabet utilisé pour le stockage. En ANSI, par défaut les chaînes affectées aux rubriques mémos HFSQL sont encodées en UTF8. La fonction [ChangeAlphabet](../WDLang1/3054001.md) peut être appelée afin de changer l'encodage.

- Base de données SQLite : Si votre projet Android partage son analyse avec un autre projet (non Android), vous pouvez utiliser la fonction [HChangeConnexion](../WDLang4/3044150.md) pour associer les fichiers de données de l'application à une connexion de type SQLite. Pour plus de détails, consultez [Application Android : Utilisation et partage de données](../Editeurs/9000123.md). 
...
- Il est également possible d'accéder à une base de données tierce (Oracle, SQL Server, ...) via un Webservice. Pour plus de détails, consultez [Accès à une base de données via un Webservice](../WDLang4/1000021063.md).
...
<a name="menus_application_ELTTEXTE000772"></a>
Dans une application Android, les options de l'application peuvent être proposées : 

- via le champ [Action Bar](../WDChamp/1000025001.md). Cette solution est actuellement recommandée. 
...
### Menus contextuels
<a name="menus_contextuels_ELTPARAGRAPHE000242"></a>Les menus contextuels sont supportés par WINDEV Mobile pour les applications Android.
...
## Fenêtres modales
<a name="fenetres_modales_ELTTEXTE000802"></a>
Les fenêtres modales ne sont pas supportées par le système d'exploitation Android. La fonction [Ouvre](../WDLang1/3038035.md) n'est pas autorisée.
...
- Il est possible de spécifier la valeur de retour grâce à la propriété [ValeurRenvoyée](../Proprietes/2510134.md) de la fenêtre fille et de récupérer cette valeur via l'événement "Fermeture d'une fenêtre fille de la fenêtre" de la fenêtre mère. 
...
### Bouton Home/Accueil
<a name="bouton_homeaccueil_ELTPARAGRAPHE000299"></a>L'appui sur le bouton "Accueil" (ou bouton "Home") ne ferme pas l'application en cours : l'application est mise en arrière-plan. L'événement "Fermeture" de la fenêtre n'est donc pas exécuté. 
...
<a name="notification_inactivite_application_ELTTEXTE000856"></a>
...
Pour éviter le déclenchement de ce mécanisme, les phases de calcul ne doivent pas être placées dans le thread principal de l'application. Utilisez des procédures lancées dans des threads secondaires (consultez la fonction [ThreadExécute](../WDLang1/3077024.md) pour plus de détails). 
...
<a name="fonctionnement_focus_selection_ELTTEXTE000880"></a>
...
<a name="ascenseurs_automatiques_dans_les_fenetres_ELTTEXTE000904"></a>
...
<a name="gestion_des_repertoires_ELTTEXTE000928"></a>
...
<a name="changement_orientation_ecran_ELTTEXTE000952"></a>
Pour détecter le changement d'orientation de l'écran, utilisez l'événement "Changement d'orientation" des fenêtres. 

Il est également possible d'utiliser les [fonctions CapteurXXXX](../WDLang1/1000019370.md) pour détecter les mouvements du téléphone. 

Remarque : La famille de [fonctions GPS](../WDLang3/1000019192.md) permet de gérer la géolocalisation du téléphone. 

<a name="NOTE14_1"></a>


## Divers
<a name="divers_ELTTEXTE000976"></a>
...
## Gestion des threads
<a name="gestion_des_threads_ELTTEXTE001000"></a>
La fonction [ThreadArrête](../WDLang1/3077019.md) n'est pas disponible sous Android.

Seul le thread lui-même peut s'arrêter en sortant de la procédure WLangage appelée par la fonction [ThreadExécute](../WDLang1/3077024.md).
...
<a name="operateurs_indirection_ELTTEXTE001024"></a>
Les [indirections](../Motscles/1512005.md) sont gérées uniquement sur les variables globales.
...
<a name="utilisation_des_polices_natives_android_ELTTEXTE001048"></a>
...
Pour plus de détails, consultez [Gestion des polices dans un projet iOS ou Android](../Editeurs/9500238.md). 

<a name="NOTE18_1"></a>


## Gestion des logs ADB
<a name="gestion_des_logs_adb_ELTTEXTE001072"></a>


### Débogage d'applications déployées via les logs ADB
<a name="debogage_applications_deployees_via_les_logs_adb_ELTPARAGRAPHE000487"></a>Lorsqu'une application est déployée, les éventuelles erreurs de l'application qui se produisent chez les utilisateurs finaux sont mises à disposition du concepteur de l'application dans le "log ADB".
...
- Messages des asserts (fonctions [dbgAssertion](../WDLang1/3014022.md), dbgVérifiexxx, [dbgErreur](../WDLang1/1000020526.md)(niveau "Assert"). 
...
<a name="impression_format_pdf_ELTTEXTE001096"></a>
...
## API level
<a name="api_level_ELTTEXTE001120"></a>
...
[Tableau récapitulatif : Gestion des répertoires de l'application selon la plateforme](../Editeurs/1000020167.md)

[Accès à une base de données via un Webservice](../Editeurs/1000021063.md)

[Tester et déboguer une application sur un appareil Android](../Editeurs/9000193.md)

[Android 10 : Modification de comportement des applications](../Editeurs/9000195.md)

[Android 11 : Modifications du comportement des applications](../Editeurs/9000201.md)
