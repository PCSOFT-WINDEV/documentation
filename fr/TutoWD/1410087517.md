...
| [Leçon précédente](../TutoWD/1410087516.md) | [Sommaire](../TutoWD/1410087560.md) | [Leçon suivante](../TutoWD/1410087580.md) |
| --- | --- | --- |
...
|   |   |   |
| --- | --- | --- |
| ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Avertissement.png) | A retenir | Une collection de procédures permet de regrouper toutes les procédures globales à un projet. Ces procédures pourront être appelées depuis n'importe quel code du projet. |





1. Dans le volet "Code" du ruban, dans le groupe "Procédures", déroulez "Nouveau" et sélectionnez "Nouvelle procédure globale". <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P2_Ma_premiere_procedure%20-%20HC%20N%B0001.jpg)


2. Dans la fenêtre de sauvegarde de l'élément qui s'affiche, un nom est proposé par défaut pour la collection de procédures globales. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P2_Ma_premiere_procedure%20-%20HC%20N%B0002.jpg)
Conservez ce nom et validez (bouton "OK"). 

3. Une nouvelle fenêtre s'affiche permettant de saisir le nom de la procédure et ses caractéristiques : <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P2_Ma_premiere_procedure%20-%20HC%20N%B0003.jpg)
...
5. La procédure globale "Bienvenue" est affichée sous l'éditeur de code. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P2_Ma_premiere_procedure%20-%20HC%20N%B0004.jpg)
Cette procédure est composée : 
	...
	- du code : 
		...
		```
			...
			|   |   |   |
| --- | --- | --- |
| ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Avertissement.png) | A retenir | Les éléments clé d'une procédure sont le mot-clé PROCEDURE suivi du nom de la procédure et de parenthèses. |
...
- Pour appeler une procédure, il suffit d'écrire son nom directement dans le code. Nous allons appeler notre procédure depuis le code d'initialisation du projet :  
	...
	5. Testons immédiatement ce code : cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=ICO_GO_Projet_WD_GAF.jpg) parmi les boutons d'accès rapide. 

	6. Une fenêtre d'information s'affiche. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=P2_Ma_premiere_procedure%20-%20HC%20N%B0005.jpg)
...
- Nous avons créé une procédure globale dans cet exemple. Le WLangage propose 3 types de procédures : 
	...
	- **Procédure locale** à une Fenêtre, Page ou Fenêtre mobile : ce type de procédure peut être appelé dans tous les événements et traitements dépendant de l'objet dans lequel cette procédure a été déclarée. Pour plus de détails, consultez [Procédure locale](../Motscles/1513009.md). 
			|   |   |   |
| --- | --- | --- |
| ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=note.png) | Note | Il est également possible de créer des procédures locales à un champ. Ces procédures pourront être utilisées dans tous les événements WLangage associés au champ. |
...
<a name="questce_quun_parametre_ELTPARAGRAPHE000265"></a>Un paramètre est une valeur envoyée à une procédure lors de l'appel de la procédure.
...
7. Testons immédiatement ce code : cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=ICO_GO_Projet_WD_GAF.jpg) parmi les boutons d'accès rapide. 
...
<a name="comment_utiliser_les_parametres_ELTPARAGRAPHE000341"></a>Par défaut, le WLangage utilise le **passage de paramètre par référence** (ou par adresse). Le paramètre dans la procédure représente (référence) directement la variable passée lors de l'appel.
...
- Testons tout de suite cette fonctionnalité sur notre exemple : 
	...
	4. Testons immédiatement ce code : cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=ICO_GO_Projet_WD_GAF.jpg) parmi les boutons d'accès rapide. 
...
|   |   |   |
| --- | --- | --- |
| ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=note.png) | Note | Dans une même procédure, certains paramètres peuvent être passés par référence et d'autres paramètres peuvent être passés par valeur. Il suffit d'utiliser le mot-clé LOCAL devant chaque paramètre passé par valeur. |
...
### Renvoyer une valeur
<a name="renvoyer_une_valeur_ELTPARAGRAPHE000395"></a>Une procédure peut renvoyer une valeur. Voici quelques exemples : 
...
<a name="parametres_obligatoires_optionnels_ELTPARAGRAPHE000450"></a>Les paramètres reçus dans la procédure peuvent être obligatoires ou optionnels. Un paramètre obligatoire doit être renseigné lors de l'appel de la procédure alors qu'un paramètre optionnel peut être omis : il prendra dans ce cas la valeur par défaut définie lors de la déclaration des paramètres de la procédure.|   |   |   |
| --- | --- | --- |
| ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=Avertissement.png) | A retenir | Dans la déclaration d'une procédure, les paramètres optionnels sont toujours les derniers paramètres spécifiés (après tous les paramètres obligatoires). |
...
- Nous allons maintenant rendre le second paramètre optionnel. En effet, le montant de la TVA peut correspondre à 20 par défaut (les autres taux possibles étant 10, 5.5 ou 2.1). 
	...
	6. Testons immédiatement ce code : cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=ICO_GO_Projet_WD_GAF.jpg) parmi les boutons d'accès rapide. 
