
## Allouer (Mot réservé)

***En anglais : New***
				



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
Le mot-clé **Allouer** permet de réserver de la place en mémoire pour contenir :

- [Un tableau dynamique](../Motscles/1514057.md)

- Un objet de classe dynamique

- [Un objet automation dynamique](../Motscles/1514015.md)

- [Une structure dynamique](../Motscles/1514007.md)







<a name="sample_code"></a>

## Exemple

<a name="Exemple1"></a>

```wl
// Syntaxe 1
TableauClient est un tableau dynamique
// Allocation de ce tableau dynamique
TableauClient = allouer un tableau dynamique de 4 par 7 entiers
```


<a name="Exemple2"></a>



```wl
// Syntaxe 2
MFichier est une classe 
	Nom est une chaîne
	Extension est une chaîne
	Répertoire est une chaîne
FIN
FichierSource est un objet MFichier dynamique
...
// Instanciation de l'objet
FichierSource = allouer un MFichier
// Traitement sur l'objet
```


<a name="Exemple3"></a>



```wl
// Syntaxe 3
// Création d'un objet automation dynamique
MonObjetDynamique = allouer un objet automation MonServeur
```


<a name="Exemple4"></a>



```wl
// Syntaxe 4
RefProduit est une structure
	CodeF est un entier
	CodePr est une chaîne fixe sur 10
FIN

Fauteuil est une RefProduit dynamique
Chaise est une RefProduit dynamique
...
Fauteuil = allouer une RefProduit
Fauteuil:CodeF = 7
Fauteuil:CodePr = "Meuble"
```



<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe

### Allouer un tableau dynamique

`<Nom du tableau dynamique> = allouer un tableau dynamique de <Dimension 1> [par <Dimension 2> ... [par <Dimension 10>]] <Type des éléments du tableau>
OU
 <Nom du tableau dynamique> = allouer un tableau dynamique de <Dimension 1> [, <Dimension 2> ... [, <Dimension 10>]] <Type des éléments du tableau>
`
---

**`<Nom du tableau dynamique> : ()`**

Nom du tableau dynamique à manipuler. Ce tableau a été préalablement déclaré.

**`<Dimension 1>...un et <u><u><u><u>dynamique</u></u></u></u> ne sont pas obligatoires : ce sont des mots d'agrément.

- Pour plus de détails sur les tableaux, consultez les [Tableaux](../Motscles/1514030.md).  




<a name="SYNTAXE2"></a>

### Instancier un objet de classe dynamique

`<Nom de l'objet> = allouer un <Nom de la classe> ([<Paramètres>])
`
---

**`<Nom de l'objet> : ()`**

Nom de l'instance de la classe.

**`<Nom de la classe> : ()`**

Nom de la classe à instancier. Ce nom a été défini lors de la création de la classe sous l'éditeur de code.

**`<Paramètres> : ()`**

Paramètres optionnels du constructeur.

Pour plus de détails sur les objets de classe dynamique, consultez [Instanciation dynamique d'un objet](../POO/6010003.md).  


<a name="SYNTAXE3"></a>

### Allouer un objet automation dynamique

`<Nom de l'objet automation dynamique> = allouer un objet automation <Nom du serveur automation>
`
---

**`<Nom de l'objet automation dynamique> : ()`**

Nom de l'objet automation dynamique à allouer. Cet objet a été préalablement déclaré.

**`<Nom du serveur automation> : ()`**

Nom du serveur de l'objet automation dynamique.

Pour plus de détails sur les objets automation dynamiques, consultez [Objet automation dynamique](../Motscles/1514015.md).  


<a name="SYNTAXE4"></a>

### Allouer une variable de type structure dynamique

`<Nom de la variable> = allouer une <Nom de la structure dynamique>
`
---

**`<Nom de la variable> : ()`**

Nom de la variable de type structure dynamique à allouer.

**`<Nom de la structure dynamique> : ()`**

Nom d'une structure précédemment déclarée.

Pour plus de détails sur les structures dynamiques, consultez [Structure dynamique](../Motscles/1514007.md).  



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques




### Cas particulier : les tableaux dynamiques
<a name="cas_particulier_les_tableaux_dynamiques_ELTPARAGRAPHE000135"></a>Si la déclaration et l'allocation du [tableau dynamique](../Motscles/1514057.md) sont réalisées en une seule ligne de code, il n'est pas nécessaire d'utiliser le mot-clé **Allouer** pour réserver de la place en mémoire.

Par exemple :


```wl
// Déclaration et allocation d'un tableau dynamique
TableauClient est un tableau dynamique de 4 par 7 entiers

// Est équivalent à

TableauClient est un tableau dynamique
TableauClient = allouer un tableau dynamique de 4 par 7 entiers
```


**Remarque** : Dans ce cas, il ne faut pas utiliser le mot-clé [Libérer](../Motscles/1511012.md) pour libérer explicitement le tableau dynamique. Si le mot-clé [Libérer](../Motscles/1511012.md) est utilisé, une erreur d'exécution sera générée.




## Voir aussi

[Tableau dynamique (Type de variable)](../Motscles/1514057.md)

[Objet Automation Dynamique](../Motscles/1514015.md)

[Libérer (Mot réservé)](../Motscles/1511012.md)

[Copie et clonage](../Motscles/6010018.md)

