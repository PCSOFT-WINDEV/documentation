...
Attention : Une modification du fichier de données dans l'analyse (ajout d'une rubrique clé par exemple) peut modifier la clé de parcours utilisée. 
 
POUR TOUT <Fichier de données> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN
 
2. Parcours complet selon la clé passée en paramètre
 
POUR TOUT <Fichier de données> SUR <Rubrique clé> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN`
---
...
POUR TOUT <Fichier de données> AVEC "<1ère Condition> 
...
POUR TOUT <Fichier de données> AVEC <Rubrique Clé> [ = / <= / >= ] <Valeur> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN
 
2. Filtre de comparaison par rapport à un intervalle de valeurs
 
POUR TOUT <Fichier de données> AVEC <Rubrique Clé> = <Valeur minimale> A <Valeur maximale> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN

POUR TOUT <Fichier de données> AVEC <Valeur minimale> <= <Rubrique Clé> <= <Valeur maximale> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN`
---
...
`POUR TOUT <Fichier de données> AVEC <Rubrique Clé> [= <Début de la valeur recherchée> [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN`
---
...
POUR TOUT <Fichier de données> AVEC <Clé composée> = [<Valeur composante 1>, ..., <Valeur composante N>] [[SANSSAUVEPOSITION,] [<Sens>]]
...
FIN
...
`POUR TOUT <Fichier de données> AVEC Fonction_WLangage(Fichier_de_données.RubriqueSpatiale, variable_spatiale)
...
FIN`
---
...
Nom d'une fonction WLangage de manipulation des géométries. Les fonctions disponibles pour une conditions sur un index spatial sont les suivantes : [GéométrieChevauche](../WDLang4/1410090308.md), [GéométrieContient](../WDLang4/1410090138.md), [GéométrieCouvre](../WDLang4/1410090140.md), [GéométrieCroise](../WDLang4/1410090341.md), [GéométrieDistance](../WDLang4/1410090297.md), [GéométrieEgale](../WDLang4/1410090295.md), [GéométrieEstContenu](../WDLang4/1410090139.md), [GéométrieEstCouvert](../WDLang4/1410090141.md), [GéométrieIntersecte](../WDLang4/1410090115.md). 
...
### 7 - Parcours d'un fichier de données avec une condition sur un index sémantique

`POUR TOUT <Fichier de données> [,<Similarité>] AVEC RechercheSémantique(Fichier_de_données.IndexSémantique, Expression recherchée, Nombre maximum de résultat)
...
FIN`
---
...
Nom du fichier de données, de la vue ou de la requête HFSQL à parcourir (contenant un index sémantique).

**`<Similarité> : (Entier optionnel)`**

Score de similarité entre le vecteur cherché et le vecteur résultat. Ce score est compris entre 0 et 1. Plus ce score est proche de 1, plus les vecteurs sont sémantiquement proches. 
Par défaut, ce paramètre a pour valeur 1. 
...
Nom de la fonction WLangage permettant de réaliser la recherche sémantique. 

**`<IndexSémantique> : ()`**

Rubrique de type Index sémantique du fichier de données. 

**`<Expression recherchée> : ()`**

Valeur recherchée. 

**`<Nombre maximum de résultat> : (Entier)`**
...
### Equivalence
<a name="equivalence_ELTPARAGRAPHE000350"></a>

- La syntaxe POUR TOUT &lt;Fichier de données&gt; AVEC &lt;Rubrique&gt; = &lt;Valeur&gt; est équivalente à un parcours du fichier de données avec la fonction [HLitRecherche](../WDLang4/3044050.md). Ce parcours tient compte des filtres précédemment effectués sur le fichier de données. 
...
### Filtre (syntaxe 2)
<a name="filtre_syntaxe_2_ELTPARAGRAPHE000370"></a>La syntaxe générale d'un filtre a la forme suivante :
...
<a name="sortir_une_boucle_type_pour_tout_ELTPARAGRAPHE000452"></a>Plusieurs instructions sont possibles :
...
<a name="influence_mode_sortie_boucle_sur_parcours_automatique_des_fichiers_donnees_ELTPARAGRAPHE000482"></a>
...
<a name="execution_iteration_suivante_ELTPARAGRAPHE000505"></a>Pour exécuter directement l'itération suivante **sans terminer le code de l'itération en cours**, utilisez l'instruction [Continuer](../Motscles/1510003.md) :
...
<a name="modification_cle_utilisee_pour_parcours_automatique_des_fichiers_donnees_des_requetes_ELTPARAGRAPHE000516"></a>Lors du parcours d'un fichier de données (ou d'une requête), si la valeur de la rubrique de parcours est modifiée, certains enregistrements peuvent être parcourus plusieurs fois.
...
[Comment faire un parcours de fichier de données avec un filtre ?](../Motscles/20011.md)

[Comment faire un parcours complet d'un fichier de données ?](../Motscles/20010.md)
...
[RechercheSémantique (Fonction)](../Motscles/1410091374.md)
