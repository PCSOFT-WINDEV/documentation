...
- <u><u><u><u>Vrai</u></u></u></u> si l'interface utilisateur du périphérique Twain utilisé doit être affichée. Lors de l'enregistrement du document, le &lt;Fichier PDF&gt; sera proposé par défaut.
...
<u><u><u><u>Vrai</u></u></u></u> pour charger automatiquement les pages dans le périphérique. Toutes les pages contenues dans le chargeur du scanner sont scannées dans un unique fichier PDF.

Remarque : Sans chargeur, pour scanner plusieurs pages dans un unique fichier PDF, il faut scanner chaque page dans un fichier PDF puis utiliser la fonction [PDFFusionne](../WDLang6/1000019712.md).
...
Cette chaîne d'octets peut être par la suite affectée à un champ Image, enregistrée dans un fichier (fonction [fSauveBuffer](../WDLang1/1000019412.md)), envoyée à travers un réseau (grâce aux fonctions Socket), ...
...
<a name="document_provenant_peripherique_twain_ELTPARAGRAPHE000197"></a>Le "document provenant du périphérique Twain" correspond par exemple :

- soit au document présent dans un scanneur au moment de l'appel de la fonction **TwainVersPDF**. Dans ce cas, les paramètres spécifiés grâce à la fonction [TwainZoneScan](../WDLang6/3078014.md) sont pris en compte.
...
<a name="gestion_type_image_ELTPARAGRAPHE000573"></a>Si plusieurs constantes sont combinées dans le paramètre &lt;Type d'image&gt;, le meilleur type d'image supporté par le périphérique sera sélectionné (selon l'ordre de gestion des couleurs).
...
<a name="scanneur_rectoverso_ELTPARAGRAPHE000588"></a>Si le scanneur gère le recto-verso, il est possible de réaliser un scan recto-verso. Le document généré contient le recto et le verso du document scanné.
...
<a name="fonctionnement_dans_des_applications_64_bits_ELTPARAGRAPHE000595"></a>Dans une application 64 bits, cette fonction utilisera automatiquement le driver Twain 64 bits. Si ce driver n'est pas installé, le périphérique TWAIN ne pourra pas être manipulé. 
