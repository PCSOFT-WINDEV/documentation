...
2. [Paramétrer la gestion des projets dans le Centre de Suivi de Projets](#NOTE3).

3. [Ajouter le composant Feedback dans votre projet (par l'intermédiaire du menu automatique '?')](#NOTE4).

4. [Paramétrer la gestion des spools dans le Centre de Suivi de Projets](#NOTE5).

5. [Récupérer et traiter les retours Client](#NOTE6).
...
## Etape 2 : Paramétrer la gestion des projets dans le Centre de Suivi de Projets
<a name="etape_2_parametrer_gestion_des_projets_dans_centre_suivi_projets_ELTTEXTE000437"></a>
Pour gérer les retours client sur une application, il est nécessaire de lancer et de paramétrer le Centre de Suivi de Projets.

**Remarque** : Dans cette partie, seule la configuration nécessaire à l'utilisation des retours Client est abordée. Nous considérons que la configuration des intervenants et de leurs horaires de travail a été effectuée. Pour plus de détails, consultez [Comment gérer un projet de développement (Chef de Projet)](../CCSuivi/3540722.md).

1. Lancez le Centre de Suivi de Projets depuis WINDEV : sous le volet "Outils", dans le groupe "Centres de Contrôle", cliquez sur "Suivi de Projets".
...
2. Vérifiez la présence du projet de développement dans le Centre de Suivi de Projets : déroulez ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=CC_Suivi_Configuration%20-%20HC%20N%B0001.gif)
 puis sélectionnez "Projets". Votre projet peut apparaître soit dans un groupe de projets, soit dans la liste "&lt;Non affecté à un groupe&gt;".
...
### Présentation
<a name="presentation_ELTPARAGRAPHE000087"></a>Le composant **Feedback** est un composant livré avec WINDEV. Ce composant permet aux utilisateurs finaux de vos applications de vous faire parvenir :
...
### Comment le faire ?
<a name="comment_faire_ELTPARAGRAPHE000104"></a>Le composant Feedback peut être intégré automatiquement ou manuellement dans votre application.
...
4. Validez. Le composant Feedback est automatiquement inséré dans votre projet et les éléments nécessaires à la gestion des retours client sont automatiquement créés dans le Centre de Suivi de Projets.
...
Lors du déploiement de votre application, les utilisateurs pourront utiliser le composant Feedback pour vous envoyer des suggestions. Pour plus de détails, consultez [Composant Feedback](../Editeurs/2014013.md).

<a name="NOTE5"></a>
<a name="NOTE5_1"></a>


## Etape 4 : Paramétrer la gestion des spools dans le Centre de Suivi de Projets
<a name="etape_4_parametrer_gestion_des_spools_dans_centre_suivi_projets_ELTTEXTE000491"></a>


### Qu'est-ce qu'un spool ?
<a name="questce_quun_spool_ELTPARAGRAPHE000158"></a>Un spool de demandes correspond à une liste de demandes. Il peut s'agir par exemple de dysfonctionnements signalés par les utilisateurs de l'application ou des suggestions d'amélioration demandées par les clients.
<a name="NOTE5_2"></a>


### Le spool des retours client
<a name="spool_des_retours_client_ELTPARAGRAPHE000165"></a>Les demandes envoyées par les utilisateurs de vos applications (via le composant Feedback) sont récupérées dans un spool. Ce spool est automatiquement créé :
...
<a name="configuration_des_retours_client_par_mails_ELTPARAGRAPHE000182"></a>Si les utilisateurs de vos applications n'ont pas accès à la base de données des Centres de Contrôle, les remarques vont être envoyées par mails. Le spool des Retours Client de votre application doit pouvoir récupérer ces mails.

1. Lancez le Centre de Suivi de Projets depuis WINDEV : sous le volet "Outils", dans le groupe "Centres de Contrôle", cliquez sur "Suivi de Projets".

2. Affichez ou créez un onglet de type "Demandes". Dans le ruban, dans le groupe "Gestion", cliquez sur "Spools".

3. Un spool "Retours_Client&lt;Nom du projet&gt;" a automatiquement été créé si vous utilisez le composant Feedback via le menu automatique. Si le spool correspondant à votre application n'existe pas, il est possible de l'ajouter (bouton "Nouveau"). Le nom de ce spool n'a pas d'importance mais il doit être lié à votre projet. Pour plus de détails, consultez [Création d'un spool](../CCSuivi/3540707.md).
...
### Gérer les accusés de réception des retours client
<a name="gerer_les_accuses_reception_des_retours_client_ELTPARAGRAPHE000223"></a>Lors de retours client par email, il est possible d'envoyer automatiquement un email pour confirmer la réception de la suggestion.
...
**Astuce** : Il est possible d'automatiser l'import des retours client afin de réaliser cette manipulation automatiquement tous les jours, toutes les semaines, tous les mois, ... Cette option est paramétrable dans les options du Centre de Suivi de Projets : déroulez ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=CC_Suivi_Configuration%20-%20HC%20N%B0001.gif)
 puis sélectionnez "Options". Cliquez sur "Options personnelles" et configurez les retours client.
...
<a name="recuperation_des_emails_mode_deconnecte_ELTPARAGRAPHE000273"></a>Pour récupérer les emails de suggestions et de bugs :

1. Lancez le Centre de Suivi de Projets depuis WINDEV : sous le volet "Outils", dans le groupe "Centres de Contrôle", cliquez sur "Suivi de Projets".

2. Déroulez ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=CC_Suivi_Configuration%20-%20HC%20N%B0001.gif)
 puis sélectionnez "Importer les retours client".
...
3. Sélectionnez la demande voulue et affichez son menu contextuel :
	...
	- L'option "***Créer une tâche depuis la demande***" permet de créer une nouvelle tâche. Les informations de la demande sont automatiquement insérée dans la nouvelle tâche. N'oubliez pas d'indiquer l'intervenant et le projet lié à la tâche. La nouvelle tâche pourra être visualisée dans le Centre de Suivi de Projets de l'intervenant spécifié (dans sa liste des tâches). La demande passe alors en demande planifiée.
	...
	- L'option "***Créer un incident depuis la demande***" permet de créer un nouvel incident. Les informations de la demande sont automatiquement insérée dans l'incident. N'oubliez pas d'indiquer l'intervenant et le projet lié à l'incident.  L'incident pourra être visualisé dans le Centre de Suivi de Projets de l'intervenant spécifié (dans la liste des incidents). La demande passe alors en demande planifiée.
...
Pour plus de détails sur la réalisation d'une tâche ou la correction d'un incident, consultez [Comment gérer les tâches et le planning](../CCSuivi/3540721.md).
