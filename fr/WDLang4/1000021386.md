...
<a name="lien_champ_avec_une_variable_wlangage_avec_une_rubrique_fichier_donnees_ELTPARAGRAPHE000271"></a>La liaison d'un champ avec une variable WLangage ou avec une rubrique d'un fichier de données peut être connue :
...
- Si plusieurs champs de la fenêtre sont reliés à une même rubrique du fichier de données, la rubrique reliée peut ne pas être mise à jour correctement par la fonction **&lt;Fenêtre&gt;.VersSource**. ***Il est déconseillé de relier plusieurs champs d'une fenêtre à une même rubrique d'un fichier de données.***

- La fonction **&lt;Fenêtre&gt;.VersSource** n'affecte pas les rubriques ni les variables (data binding) pour les champs de type image. Utilisez la fonction [&lt;Source&gt;.AttacheMémo](../WDLang4/1000024194.md).
