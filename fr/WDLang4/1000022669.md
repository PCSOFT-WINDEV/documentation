...
***En anglais : &lt;Connection variable&gt;.InfoViewRights***
...
sDroits = MaConnexion.InfoDroitvue("Dupont", hDroitLecture + hDroitAjout, ...
		"MaBase", "MaVueClient")
...
Nom de la base de données contenant la vue (SQL ou matérialisée) pour laquelle les droits sont recherchés. Si la base de données correspond à la base de données définie dans la connexion, ce paramètre peut correspondre à une chaîne vide (""). Si aucune base de données n'est définie dans la connexion et si ce paramètre correspond à une chaîne vide, une erreur WLangage est affichée.
