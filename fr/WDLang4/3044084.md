...
- le nom de la requête est déjà utilisée dans l'analyse en cours (nom de fichier de données ou nom défini par une des fonctions suivantes : [HDéclare](../WDLang4/3044203.md), [HDéclareExterne](../WDLang4/3044204.md), [HDécritFichier](../WDLang4/3044206.md)).
...
![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Widget Android](https://doc.pcsoft.fr/ext/images/fr/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png) Il n'est pas possible d'exécuter des requêtes dont le code SQL contient des fonctions WLangage. 
<a name="Exemple1"></a>
...
![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Widget Android](https://doc.pcsoft.fr/ext/images/fr/ANDROIDWIDGET.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png) Il n'est pas possible d'exécuter des requêtes dont le code SQL contient des fonctions WLangage. ![PHP](https://doc.pcsoft.fr/ext/images/fr/PHP.png) Ce paramètre peut prendre uniquement la valeur *hRequêteDéfaut*. Si une autre constante est utilisée, une erreur est générée.Ce paramètre n'est pas disponible si &lt;Nom de la requête&gt; correspond à une variable de type Requête SQL.
...
- soit à une connexion définie sous l'éditeur d'analyses ou par programmation avec la fonction [HDécritConnexion](../WDLang4/3044205.md) ou [HOuvreConnexion](../WDLang4/3044107.md). 
...
<a name="lecture_des_donnees_lors_initialisation_ELTPARAGRAPHE000493"></a>Selon la requête (tris, groupes, ...), les données peuvent ou non être lues au moment de l'initialisation de la requête.
...
<a name="texte_requete_sql_ELTPARAGRAPHE000523"></a>Si un nom de fichier de données ou un nom de rubrique contient des espaces, il est nécessaire d'entourer ces noms par des crochets dans le texte de la requête. Par exemple :
...
<a name="executer_une_requete_sql_parametree_ELTPARAGRAPHE000534"></a>Une requête SQL peut contenir des paramètres. Pour exécuter ce type de requête SQL avec la fonction **HExécuteRequêteSQL**, il faut : 
...
![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Widget Android](https://doc.pcsoft.fr/ext/images/fr/ANDROIDWIDGET.png) Les requêtes paramétrées ne sont pas disponibles. Néanmoins, les variables de type 'Requête SQL' sont disponibles. Ce type de variable peut correspondre à une requête paramétrée. Pour plus de détails, consultez [Variable de type Requête SQL](../Motscles/1514082.md). 
<a name="NOTE0_2B"></a>


### Injection SQL : Comment s'en prémunir ? 
<a name="injection_sql_comment_sen_premunir_ELTPARAGRAPHE000584"></a>L'injection SQL est une technique de piratage consistant à injecter du code SQL dans les paramètres des requêtes, forçant ainsi l'exécution de code SQL non désiré. 
...
<a name="insertion_donnees_binaires_ELTPARAGRAPHE000618"></a>Lors de l'insertion de données par INSERT ou UPDATE, si une rubrique mémo doit être affectée avec des données binaires pouvant contenir le caractère \\0 (code ASCII zéro), il faut placer les données dans un buffer hexadécimal. 
...
### Jointures
<a name="jointures_ELTPARAGRAPHE000641"></a>Les jointures du type (A join B on x=y) join C on y=z ... ne sont pas gérées automatiquement par les Accès Natifs : il faut utiliser dans ce cas la constante *hRequêteSansCorrection*pour gérer ce type de jointure.
...
<a name="pourquoi_utiliser_constante_hrequetesanscorrection_ELTPARAGRAPHE000654"></a>Par défaut, WINDEV et WEBDEV interprètent les requêtes SQL :
...
<a name="NOTE0_6"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/fr/ER.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/fr/IPHONE.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Langage Externe](https://doc.pcsoft.fr/ext/images/fr/LE.png)![PHP](https://doc.pcsoft.fr/ext/images/fr/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png)![HFSQL Classic](https://doc.pcsoft.fr/ext/images/fr/HF.png)![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/fr/HFCS.png)![Hyper File 5.5](https://doc.pcsoft.fr/ext/images/fr/HF55.png) 

### Requêtes sur des fichiers de données HFSQL
<a name="requetes_sur_des_fichiers_donnees_hfsql_ELTPARAGRAPHE000724"></a>

- Pour optimiser le temps d'exécution de la requête, utilisez la fonction [HOptimise](../WDLang4/3044005.md) sur les différents fichiers de données intervenant dans la requête ou la fonction [HOptimiseRequête](../WDLang4/3044003.md).

- Pour optimiser le temps de traitement des requêtes, utilisez la fonction [HStatCalcule](../WDLang4/3044134.md) ou la fonction [HRéindexe](../WDLang4/3044133.md) sur tous les fichiers de données intervenant dans la requête.
...
<a name="NOTE0_7"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/fr/ER.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/fr/PHP.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/fr/AN.png) 

### Optimisation pour exécuter plusieurs fois une même requête (Accès natifs Oracle et SQL Server uniquement)
<a name="optimisation_pour_executer_plusieurs_fois_une_meme_requete_acces_natifs_oracle_sql_server_uniquement_ELTPARAGRAPHE000769"></a>Pour optimiser l'exécution d'une requête plusieurs fois, vous pouvez :
...
<a name="NOTE0_8"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/fr/ER.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![PHP](https://doc.pcsoft.fr/ext/images/fr/PHP.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/fr/AN.png) 

### Spécifier et récupérer des valeurs lors de l'exécution d'une requête (cas des procédures stockées)
<a name="specifier_recuperer_des_valeurs_lors_execution_une_requete_cas_des_procedures_stockees_ELTPARAGRAPHE000809"></a>**Remarque** : Cette note ne s'applique qu'aux Connecteurs Natifs Oracle et SQL Server.
...
<a name="requetes_sql_fonction_docparampagetitleshort_execution_requetes_sql_creees_par_editeur_requetes_ELTPARAGRAPHE000865"></a>
...
<a name="parcours_des_requetes_fonction_hlitpremierwdlang43044051md_ELTPARAGRAPHE000905"></a>
