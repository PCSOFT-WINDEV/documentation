...
### Manipuler une vue
<a name="manipuler_une_vue_ELTPARAGRAPHE000048"></a>Pour manipuler une vue sur un fichier de données HFSQL, les opérations à effectuer sont les suivantes :
...
3. Opérations spécifiques aux vues possibles :
	...
	- **Modifier l'ordre de tri des enregistrements de la vue**. Par défaut, les enregistrements d'une vue sont triés dans le même ordre que celui utilisé dans le fichier de données HFSQL correspondant. Cependant, il est possible de modifier cet ordre grâce à la fonction [HTrieVue](../WDLang4/3044114.md). Si le tri est appliqué sur plusieurs rubriques de la vue, le tri est appliqué d'abord selon la première rubrique de tri, puis selon la seconde, etc.
