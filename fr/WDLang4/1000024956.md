...
SI PAS sdr.ExécuteRequête() ALORS
	Trace(HErreurInfo())
SINON
	SI PAS sdr.ExporteXML("FicTest.xml", hExpCréation) ALORS
		Trace(HErreurInfo())	
	FIN
FIN
```
...
<a name="exporter_contenu_une_requete_ELTPARAGRAPHE000280"></a>Pour exporter le contenu d'une requête, il est nécessaire d'initialiser la requête (fonction [&lt;Source&gt;.ExécuteRequête](../WDLang4/1000024944.md)) avant d'utiliser la fonction **&lt;Source&gt;.ExporteXML**.
<a name="NOTE0_2"></a>


### Exporter un seul enregistrement
<a name="exporter_seul_enregistrement_ELTPARAGRAPHE000298"></a>Pour exporter un seul enregistrement vers une chaîne XML, utilisez la fonction [&lt;Source&gt;.EnregistrementVersXML](../WDLang4/1000024610.md).
<a name="NOTE0_3"></a>


### Rubriques tableau
<a name="rubriques_tableau_ELTPARAGRAPHE000313"></a>Si une des rubriques exportées est une rubrique de type tableau, chaque élément de la rubrique tableau est exporté dans un nouveau niveau :
...
### Divers
<a name="divers_ELTPARAGRAPHE000326"></a>L'entité document du fichier XML généré se nomme HF_DOCUMENT.
