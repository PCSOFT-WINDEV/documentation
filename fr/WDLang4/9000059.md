...
<a name="utilisation_analyse_dans_une_connexion_une_base_donnees_hfsql_classic_ELTPARAGRAPHE000266"></a>Dans le cas d'une connexion à une base de données HFSQL Classic :
...
<a name="arborescence_des_donnees_ELTPARAGRAPHE000319"></a>L'emplacement désigné dans la chaîne de connexion par l'élément **Initial Catalog** peut contenir des fichiers de données placés dans des sous-dossiers. Ces fichiers de données sont automatiquement déclarés par le provider OLE DB pour HFSQL, en remplaçant les "\\" de l'arborescence par le caractère "_" (underscore).
