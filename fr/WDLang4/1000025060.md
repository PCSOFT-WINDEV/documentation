...
![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/fr/HFCS.png) Les fichiers de données HFSQL Client/Serveur peuvent être réindexés de manière asynchrone grâce à la fonction [&lt;Connexion HFSQL&gt;.RéindexeAsynchrone](../WDLang4/1410091221.md). 
...
![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png) Ce paramètre est ignoré.
...
<a name="conditions_utilisation_ELTPARAGRAPHE000392"></a>
...
<a name="reindexation_automatique_ELTPARAGRAPHE000416"></a>La ré-indexation d'un fichier de données peut être rendue nécessaire par la perte ou l'altération du fichier d'index. Une ré-indexation automatique des fichiers de données peut être mise en place.
...
- configurer la valeur de la variable [H.RéindexationAutomatiqueVisible](../WDLang4/3044070.md). 
	...
	Pendant la ré-indexation, les différentes applications manipulant les données du fichier de données (lecture ou écriture d'enregistrements) seront temporairement arrêtées jusqu'à la fin de la ré-indexation.
...
- Si une ré-indexation automatique est effectuée, les applications utilisant les fichiers de données en cours de ré-indexation sont temporairement bloquées. Un message est affiché indiquant que le fichier de données est en cours de maintenance.
...
<a name="reindexation_chaud_ELTPARAGRAPHE000469"></a>La ré-indexation d'un fichier de données peut être effectuée même si des applications accèdent à ce fichier de données.
...
<a name="calcul_des_statistiques_ELTPARAGRAPHE000498"></a>Le calcul des statistiques permet d'optimiser les requêtes et les filtres sur le fichier de données. Ce calcul prend en compte uniquement les enregistrements actifs. Ce calcul peut également être obtenu par la fonction [&lt;Source&gt;.StatCalcule](../WDLang4/1000025064.md).
...
<a name="repertoire_des_fichiers_temporaires_ELTPARAGRAPHE000515"></a>Pendant la ré-indexation, des fichiers temporaires peuvent être créés. Par défaut, ces fichiers sont créés dans le répertoire temporaire de Windows ou dans le répertoire courant si il n'y a pas assez de place.
...
<a name="gestion_des_alphabets_non_latins_ELTPARAGRAPHE000546"></a>Pour modifier l'alphabet utilisé pour les recherches, les tris :

1. Spécifiez l'alphabet à utiliser avec la fonction [ChangeAlphabet](../WDLang1/3054001.md).

2. Ré-indexez le fichier de données avec la fonction **&lt;Source&gt;.Réindexe**. Utilisez la constante *hChangeAlphabet* pour prendre en compte l'alphabet spécifié avec la fonction [ChangeAlphabet](../WDLang1/3054001.md).
...
<a name="droits_acces_fichier_donnees_ELTPARAGRAPHE000573"></a>La ré-indexation provoque la recréation du fichier de données sur disque. Dans un système d'allocation gérant les droits au niveau des fichiers (NTFS, etc.), le fichier de données après ré-indexation aura donc les droits du répertoire auquel il appartient.
