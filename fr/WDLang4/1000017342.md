...
<a name="lien_champ_avec_une_variable_wlangage_avec_une_rubrique_fichier_donnees_ELTPARAGRAPHE000133"></a>La liaison d'un champ avec une variable WLangage ou avec une rubrique d'un fichier de données peut être connue :
...
<a name="mise_jour_des_rubriques_cas_particuliers_ELTPARAGRAPHE000148"></a>
...
- Si plusieurs champs de la fenêtre sont reliés à une même rubrique du fichier de données, la rubrique reliée peut ne pas être mise à jour correctement par la fonction **EcranVersSource**. ***Il est déconseillé de relier plusieurs champs d'une fenêtre à une même rubrique d'un fichier de données.***

- La fonction **EcranVersSource** n'affecte pas les rubriques ni les variables (data binding) pour les champs de type image. Utilisez la fonction [HAttacheMémo](../WDLang4/3044173.md).
