...
## Présentation
<a name="presentation_ELTTEXTE000547"></a>
...
<a name="creer_une_procedure_stockee_une_requete_stockee_ELTTEXTE000571"></a>


### La création de procédures stockées est effectuée dans l'analyse.
<a name="creation_procedures_stockees_est_effectuee_dans_analyse_ELTPARAGRAPHE000039"></a>Pour créer une procédure stockée :

1. Créez si nécessaire la collection de procédures stockées dans l'analyse :

	- soit cliquez sur ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=ico_nouveau.gif) parmi les boutons d'accès rapide. 
...
<a name="importer_une_collection_procedures_existante_ELTPARAGRAPHE000071"></a>Une collection de procédures stockées peut correspondre à une collection de procédures existante (dans le projet en cours, ou dans un autre projet).  
...
<a name="utiliser_une_requete_dans_une_procedure_stockee_ELTPARAGRAPHE000090"></a>Vous pouvez utiliser une requête créée avec l'éditeur de requêtes dans une procédure stockée. Pour cela, la création de la requête doit avoir été effectuée depuis l'éditeur d'analyses.
...
- Pour savoir si une fonction est utilisable ou non dans une procédure stockée, il suffit de consulter l'aide de cette fonction : si l'image ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=proc_stock.gif) est affichée dans la barre de titre de la fonction, la fonction est utilisable dans le code des procédures stockées.
...
<a name="procedures_stockees_compilables_64_bits_ELTPARAGRAPHE000186"></a>Pour utiliser un exécutable 32 bits avec des procédures stockées s'exécutant sur un serveur 64 bits : 
...
<a name="tester_une_procedure_stockee_ELTPARAGRAPHE000201"></a>Pour tester une procédure stockée :
...
### Limite
<a name="limite_ELTPARAGRAPHE000247"></a>Dans cette version, il n'est pas possible d'importer une collection de procédures stockées existante sur un serveur.
...
<a name="deployer_une_procedure_stockee_les_requetes_stockees_associees_ELTTEXTE000686"></a>


### Présentation
<a name="presentation_ELTPARAGRAPHE000256"></a>Pour tester et/ou exécuter une procédure stockée, il est nécessaire de la déployer sur le serveur. Ce déploiement peut être effectué :
...
### Déployer pour test
<a name="deployer_pour_test_ELTPARAGRAPHE000268"></a>Pour tester rapidement les modifications effectuées dans une procédure stockée :
...
2. Dans la fenêtre qui s'affiche, sélectionnez :
	...
	- la connexion concernée par la mise à jour. Il est également possible de saisir directement les caractéristiques de la connexion. <br>![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=proc_stock_Test.gif&type=thumb)
...
### Déployer en réel
<a name="deployer_reel_ELTPARAGRAPHE000299"></a>Pour mettre à jour une application Client/Serveur utilisant des procédures stockées, il est nécessaire d'effectuer les manipulations suivantes :
...
<a name="executer_une_procedure_stockee_dans_vos_programmes_ELTTEXTE000728"></a>
...
<a name="execution_une_procedure_stockee_depuis_une_requete_sql_ELTPARAGRAPHE000393"></a>Pour exécuter une procédure stockée depuis une requête SQL, il suffit de préciser dans la requête le nom de la procédure à exécuter et les paramètres à lui passer.
...
<a name="codes_erreur_pouvant_etre_rencontres_dans_les_procedures_stockees_ELTPARAGRAPHE000460"></a>Les erreurs suivantes peuvent être rencontrées lors de l'exécution des procédures stockées :
