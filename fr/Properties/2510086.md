


## MotDePasse (Propriété)

***En anglais : Password***
	



<a name="XUtilisation"></a>
<a name="Utilisation"></a>
<a name="description"></a>
La propriété **MotDePasse** peut être utilisée : 

- **sur les champs de saisie** (ou les colonnes de table de type texte).
	Dans ce cas, la propriété **MotDePasse** permet : 

	- de savoir si un champ de saisie est de type "Mot de passe"

	- si nécessaire de rendre ou non un champ de saisie de type mot de passe. 


L'option "Mot de passe" permet à l'utilisateur de saisir un texte sans que ce texte soit visible à l'écran : tous les caractères saisis sont remplacés par des astérisques ("\*").
	**Remarque** : Le type "Mot de passe" est une option des champs de saisie. Cette option est située dans l'onglet "Général" de la description du champ.

- **sur les connexions (HFSQL Client/Serveur, OLE DB, ...).**
	Dans ce cas, la propriété **MotDePasse** permet de définir le mot de passe de l'utilisateur utilisant la connexion. Dans ce cas, si la connexion est actuellement ouverte, elle sera fermée, puis ré-ouverte avec les nouveaux paramètres lors d'un premier accès à un fichier utilisant cette connexion.
	**Remarque** : La connexion peut correspondre :

	- soit à une connexion décrite sous l'éditeur d'analyses.

	- soit à une connexion décrite avec les fonctions [HConnecte](../WDLang4/3044263.md), [HDécritConnexion](../WDLang4/3044205.md) ou [HOuvreConnexion](../WDLang4/3044107.md).

	- soit à une connexion décrite avec une variable de type [Connexion](../WDLang4/1514073.md).










<a name="Exemple1"></a>
<a name="sample_code"></a>

## Exemple

![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png) 
```wl
// Le champ "SAI_CpteBanque" est-il de type "Mot de passe" ?
ResMotDePasse = SAI_CpteBanque.MotDePasse
```


<a name="Exemple2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/fr/ER.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/fr/IPHONE.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png)![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/fr/HFCS.png)![OLE DB](https://doc.pcsoft.fr/ext/images/fr/OLEDB.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/fr/AN.png) 
```wl
MaConnexion est une Connexion
// Description de la connexion
MaConnexion.Utilisateur = "USER"
MaConnexion.MotDePasse = "PASSWORD"
MaConnexion.Serveur = "MONSERVEUR" 
MaConnexion.BaseDeDonnées = "Base de données"
MaConnexion.Provider = hAccèsHFClientServeur
MaConnexion.Accès = hOLectureEcriture
MaConnexion.InfosEtendues = "Infos étendues"
MaConnexion.OptionsCurseur = hCurseurClient
HOuvreConnexion(MaConnexion)
```

<a name="XSYNTAXE"></a>
<a name="SYNTAXE1"></a>

## Syntaxe
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png) 
### Savoir si un champ de saisie est de type "Mot de passe"

`<Résultat> = <Champ de saisie>.MotDePasse`
---

**`<Résultat> : (Booléen)`**



- <u><u><u><u>Vrai</u></u></u></u> si le champ spécifié est de type "Mot de passe", 

- <u><u><u><u>Faux</u></u></u></u> dans le cas contraire.




**`<Champ de saisie> : (Nom de champ)`**

Nom du champ de saisie (ou de la colonne de Table) à manipuler.  


<a name="SYNTAXE2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png) 
### Modifier le type "Mot de passe" d'un champ de saisie

`<Champ de saisie>.MotDePasse = <Nouveau type>`
---

**`<Champ de saisie> : (Nom de champ)`**

Nom du champ de saisie (ou de la colonne de table) à manipuler.

**`<Nouveau type> : (Booléen)`**



- <u><u><u><u>Vrai</u></u></u></u> si le champ spécifié doit être de type "Mot de passe", 

- <u><u><u><u>Faux</u></u></u></u> dans le cas contraire.





<a name="SYNTAXE3"></a>
![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Etats et Requêtes](https://doc.pcsoft.fr/ext/images/fr/ER.png)![Windows](https://doc.pcsoft.fr/ext/images/fr/WINDOWS.png)![Linux](https://doc.pcsoft.fr/ext/images/fr/LX.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/fr/IPHONE.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png)![HFSQL Client/Serveur](https://doc.pcsoft.fr/ext/images/fr/HFCS.png)![OLE DB](https://doc.pcsoft.fr/ext/images/fr/OLEDB.png)![Connecteurs Natifs (Accès Natifs)](https://doc.pcsoft.fr/ext/images/fr/AN.png) 
### Définir le mot de passe utilisé lors d'une connexion

`<Connexion>.MotDePasse = <Nouveau mot de passe>`
---

**`<Connexion> : (Chaîne de caractères)`**

Nom de la connexion manipulée. Ce nom a été défini :

- soit sous l'éditeur d'analyses,

- soit avec les fonctions [HDécritConnexion](../WDLang4/3044205.md) ou [HOuvreConnexion](../WDLang4/3044107.md),

- soit avec le type [Connexion](../WDLang4/1514073.md).




**`<Nouveau mot de passe> : (Chaîne de caractères ou Chaîne secrète)`**

Mot de passe de l'utilisateur en cours à utiliser pour la connexion spécifiée.
**Utilisation de chaînes secrètes** : Si vous utilisez le coffre-fort de chaînes secrètes, le type de la chaîne secrète utilisable pour ce paramètre doit être "Chaîne Ansi ou Unicode".
Pour plus de détails sur les chaînes secrètes et l'utilisation du coffre-fort, consultez [Coffre-fort de chaînes secrètes](../Editeurs/1410087353.md). 



<a name="NOTE0"></a>
<a name="NOTE0_1"></a>

## Remarques
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png) 

### Option "Mot de passe"
<a name="option_mot_passe_ELTPARAGRAPHE000152"></a>L'option "Mot de passe" concerne uniquement l'affichage du contenu du champ. La valeur du champ correspond aux caractères saisis par l'utilisateur (et non aux astérisques ("\*")).
<a name="NOTE0_2"></a>
![WINDEV](https://doc.pcsoft.fr/ext/images/fr/WD.png)![WEBDEV - Code Serveur](https://doc.pcsoft.fr/ext/images/fr/WBS.png)![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png)![iPhone/iPad](https://doc.pcsoft.fr/ext/images/fr/IPHONE.png)![Java](https://doc.pcsoft.fr/ext/images/fr/JAVA.png)![Code Utilisateur (MCU)](https://doc.pcsoft.fr/ext/images/fr/MCU.png)![Ajax](https://doc.pcsoft.fr/ext/images/fr/AJAX.png) 

### Limite
<a name="limite_ELTPARAGRAPHE000161"></a>La propriété **MotDePasse** s'applique uniquement : 

- aux champs de saisie.

- aux colonnes de table de type texte.


![Android](https://doc.pcsoft.fr/ext/images/fr/ANDROID.png) La propriété **MotDePasse** ne s'applique pas aux colonnes de table de type texte.


- ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=WDMotDePasse.gif) ***Exemples complets (WINDEV)*** : **WD MotDePasse** <br>L'exemple WD MotDePasse présente un "coffre" à mot de passe. Il peut être utilisé pour gérer les mots de passe créés lors de l'utilisation de sites Internet mais également dans des applications ou des mots de passe de la vie courante (digicodes, codes de cartes bancaires, etc.) .<br>La sécurité des données est garantie par l'utilisation du mode de cryptage renforcé de HFSQL, ainsi même si les fichiers contenant les précieuses informations sont dérobés, celles-ci sont inexploitables sans connaitre le mot de passe principal.<br>L'exemple peut également être utilisé comme un générateur de mot de passe pour produire des mots de passe particulièrement sûrs.
- ![](https://doc.pcsoft.fr/fr-FR/images/image.awp?langid=5&name=WMMotdepasse.gif) ***Exemples multiplateforme (WINDEV Mobile)*** : **WM Mot de passe** <br>Sites web, comptes bancaires, ... notre vie quotidienne est remplie de mots de passe. Avec WM Mot de passe, vous n'avez plus besoin de mémoriser tous les mots de passe de vos différents comptes. L'application conserve pour vous, de façon sécurisée tous vos mots de passe. WM Mot de passe est également capable de générer pour vous des mots de passe extrêmement sûrs.


## Voir aussi

[Propriétés associées aux fenêtres, aux pages et aux champs](../Proprietes/2510107.md)

[Message (Fonction)](../Proprietes/3021006.md)

[Note (Propriété)](../Proprietes/2510085.md)

